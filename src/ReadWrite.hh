...
class ReadWrite {
 public:
  ...
  void ReadTBME_OakRidge(std::string spname, std::string tbmename,
                         Operator& Hbare, std::string format);
  ...
  void ReadBareTBME_Darmstadt(std::string filename, Operator& Hbare, int E1max,
                              int E2max, int lmax);
  template <class T>
  void ReadBareTBME_Darmstadt_from_stream(T& infile, Operator& Hbare, int E1max,
                                          int E2max, int lmax);
  void Read_Darmstadt_3body(std::string filename, Operator& Hbare, int E1max,
                            int E2max, int E3max);
  size_t Count_Darmstadt_3body_to_read(Operator& Hbare, int E1max, int E2max,
                                       int E3max,
                                       std::vector<int>& orbits_remap,
                                       std::vector<size_t>& nread_list);
  template <class T>
  void Read_Darmstadt_3body_from_stream(T& infile, Operator& Hbare, int E1max,
                                        int E2max, int E3max);
  //   void Store_Darmstadt_3body( std::vector<float>& ThreeBME, Operator&
  //   Hbare, int E1max, int E2max, int E3max);
  void Store_Darmstadt_3body(const std::vector<float>& ThreeBME,
                             const std::vector<size_t>& nread_list,
                             const std::vector<int>& orbits_remap,
                             Operator& Hbare, int E1max, int E2max, int E3max);
#ifndef NO_HDF5
  void GetHDF5Basis(ModelSpace* modelspace, std::string filename,
  ...
  void Write_me2j(std::string filename, Operator& op, int emax, int e2max,
                  int lmax);
  void Write_me2jp(std::string filename, Operator& op, int emax, int e2max,
                   int lmax);
  void Write_me3j(std::string filename, Operator& op, int E1max, int E2max,
                  int E3max);
  ...
  void WriteNuShellX_intfile(Operator& op, std::string filename,
                             std::string mode);
  ...
  void WriteTensorOneBody(std::string filename, Operator& H,
                          std::string opname);
  void WriteTensorTwoBody(std::string filename, Operator& H,
                          std::string opname);
  void WriteDaggerOperator(Operator& op, std::string filename,
                           std::string opname = "");
  ...
  void ReadTensorOperator_Nathan(std::string filename1b, std::string filename2b,
                                 Operator& op);
  void ReadOperator_Nathan(std::string filename1b, std::string filename2b,
                           Operator& op);
  ...
  void ReadRelCMOpFromJavier(std::string statefile, std::string MEfile,
                             Operator& Op);
  ...
  void SetCoMCorr(bool b) {
    doCoM_corr = b;
    std::cout << "Setting com_corr to " << b << std::endl;
  };
  void SetScratchDir(std::string d) {
    scratch_dir = d;
    std::cout << "Setting scratch dir to " << d << std::endl;
  };
  ...
  void ReadJacobi3NFiles(Jacobi3BME& jacobi3bme, std::string poi_name,
                         std::string eig_name, std::string v3int_name);
  ...
  void WriteTokyoFull(Operator& op,
                      std::string filename);  // only for Hamiltonian
  void WriteTensorTokyo(std::string filename, Operator& op);
  Operator ReadOperator2b_Miyagi(
      std::string, ModelSpace&);  // general operator me2j-like format
  ...
  //   std::map<std::string,std::string> InputParameters; // I believe this is
...
/// Wrapper class so we can treat a std::vector of floats like a stream, using
/// the extraction operator >>. This is used for the binary version of
/// ReadWrite::Read_Darmstadt_3body_from_stream().
// template <typename T>
class VectorStream {
 public:
  ...
  //  VectorStream& operator>>(T& x) { x = vec[i++]; return
  //  (VectorStream&)(*this);}
  VectorStream& operator>>(float& x) {
    x = vec[i++];
    return (VectorStream&)(*this);
  }
  //  VectorStream& operator>>(double& x) { x = vec[i++]; return
  //  (VectorStream&)(*this);}
  bool good() { return i < vec.size(); };
  void getline(char[], int){};  // Don't do nuthin'.
  void read(char* buf, size_t len) {
    memcpy((void*)buf, (const void*)&vec[i], len);
  };  // Totally untested...
 private:
  //  std::vector<T>& vec;
