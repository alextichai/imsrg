...
/// This returns the matrix element times a factor \f$
/// \sqrt{(1+\delta_{ij})(1+\delta_{kl})} \f$
double TwoBodyME::GetTBME(int ch_bra, int ch_ket, int a, int b, int c,
                          int d) const {
  double norm = 1;
...
double TwoBodyME::GetTBME_norm(int ch_bra, int ch_ket, int a, int b, int c,
                               int d) const {
  ...
  if (bra_ind < 0 or ket_ind < 0 or bra_ind > tbc_bra.GetNumberKets() or
      ket_ind > tbc_ket.GetNumberKets())
    return 0;
  ...
  if (ch_bra > ch_ket) {
    return phase * modelspace->phase(tbc_bra.J - tbc_ket.J) *
           GetMatrix(ch_ket, ch_bra)(ket_ind, bra_ind);
  }
  return phase * GetMatrix(ch_bra, ch_ket)(bra_ind, ket_ind);
}

void TwoBodyME::SetTBME(int ch_bra, int ch_ket, int a, int b, int c, int d,
                        double tbme) {
...
void TwoBodyME::AddToTBME(int ch_bra, int ch_ket, int a, int b, int c, int d,
                          double tbme) {
  TwoBodyChannel& tbc_bra = modelspace->GetTwoBodyChannel(ch_bra);
  TwoBodyChannel& tbc_ket = modelspace->GetTwoBodyChannel(ch_ket);
  //   cout << "Tzbra = " << tbc_bra.Tz << "   Tzket = " << tbc_ket.Tz << "
  //   rank_T = " << rank_T << endl;
  int bra_ind = tbc_bra.GetLocalIndex(std::min(a, b), std::max(a, b));
  int ket_ind = tbc_ket.GetLocalIndex(std::min(c, d), std::max(c, d));
  //   cout << "bra_ind = " << bra_ind << " = local index of " << min(a,b) << "
  //   " << max(a,b) << " from channel " << ch_bra << endl; cout << "ket_ind = "
  //   << ket_ind << " = local index of " << min(c,d) << " " << max(c,d) << "
  //   from channel " << ch_ket << endl;
  double phase = 1;
  if (a > b) phase *= tbc_bra.GetKet(bra_ind).Phase(tbc_bra.J);
  if (c > d) phase *= tbc_ket.GetKet(ket_ind).Phase(tbc_ket.J);
  if (ch_bra > ch_ket) {
  ...
  //   cout << "Getting Matrix " << ch_bra << "," << ch_ket << "(" << bra_ind <<
  //   "," << ket_ind
  //        << "), dimension = " << GetMatrix(ch_bra,ch_ket).n_rows << "x" <<
  //        GetMatrix(ch_bra,ch_ket).n_cols << endl;
  GetMatrix(ch_bra, ch_ket)(bra_ind, ket_ind) += phase * tbme;
...
void TwoBodyME::SetTBME(int ch_bra, int ch_ket, Ket& bra, Ket& ket,
                        double tbme) {
  SetTBME(ch_bra, ch_ket, bra.p, bra.q, ket.p, ket.q, tbme);
}
void TwoBodyME::AddToTBME(int ch_bra, int ch_ket, Ket& bra, Ket& ket,
                          double tbme) {
  AddToTBME(ch_bra, ch_ket, bra.p, bra.q, ket.p, ket.q, tbme);
}

double TwoBodyME::GetTBME_norm(int ch_bra, int ch_ket, int ibra,
                               int iket) const {
  return GetMatrix(ch_bra, ch_ket)(ibra, iket);
}

void TwoBodyME::SetTBME(int ch_bra, int ch_ket, int ibra, int iket,
                        double tbme) {
...
void TwoBodyME::AddToTBME(int ch_bra, int ch_ket, int ibra, int iket,
                          double tbme) {
  if (ch_bra > ch_ket) {
    std::swap(ch_bra, ch_ket);
    std::swap(ibra, iket);
    tbme *= modelspace->phase(modelspace->GetTwoBodyChannel(ch_bra).J -
                              modelspace->GetTwoBodyChannel(ch_ket).J);
  }
  GetMatrix(ch_bra, ch_ket)(ibra, iket) += tbme;
...
double TwoBodyME::GetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                          int t_ket, int a, int b, int c, int d) const {
...
void TwoBodyME::SetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                        int t_ket, int a, int b, int c, int d, double tbme) {
  int ch_bra = modelspace->GetTwoBodyChannelIndex(j_bra, p_bra, t_bra);
  int ch_ket = modelspace->GetTwoBodyChannelIndex(j_ket, p_ket, t_ket);
  SetTBME(ch_bra, ch_ket, a, b, c, d, tbme);
}
void TwoBodyME::AddToTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                          int t_ket, int a, int b, int c, int d, double tbme) {
  int ch_bra = modelspace->GetTwoBodyChannelIndex(j_bra, p_bra, t_bra);
  int ch_ket = modelspace->GetTwoBodyChannelIndex(j_ket, p_ket, t_ket);
...
double TwoBodyME::GetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                          int t_ket, Ket& bra, Ket& ket) const {
  int ch_bra = modelspace->GetTwoBodyChannelIndex(j_bra, p_bra, t_bra);
  int ch_ket = modelspace->GetTwoBodyChannelIndex(j_ket, p_ket, t_ket);
  return GetTBME(ch_bra, ch_ket, bra, ket);
}
void TwoBodyME::SetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                        int t_ket, Ket& bra, Ket& ket, double tbme) {
  int ch_bra = modelspace->GetTwoBodyChannelIndex(j_bra, p_bra, t_bra);
  int ch_ket = modelspace->GetTwoBodyChannelIndex(j_ket, p_ket, t_ket);
...
double TwoBodyME::GetTBME_J(int j_bra, int j_ket, int a, int b, int c,
                            int d) const {
...
void TwoBodyME::SetTBME_J(int j_bra, int j_ket, int a, int b, int c, int d,
                          double tbme) {
  Orbit& oa = modelspace->GetOrbit(a);
  Orbit& ob = modelspace->GetOrbit(b);
  Orbit& oc = modelspace->GetOrbit(c);
  Orbit& od = modelspace->GetOrbit(d);
  int ch_bra = modelspace->GetTwoBodyChannelIndex(j_bra, (oa.l + ob.l) % 2,
                                                  (oa.tz2 + ob.tz2) / 2);
  int ch_ket = modelspace->GetTwoBodyChannelIndex(j_ket, (oc.l + od.l) % 2,
                                                  (oc.tz2 + od.tz2) / 2);
  SetTBME(ch_bra, ch_ket, a, b, c, d, tbme);
}
void TwoBodyME::AddToTBME_J(int j_bra, int j_ket, int a, int b, int c, int d,
                            double tbme) {
  Orbit& oa = modelspace->GetOrbit(a);
  Orbit& ob = modelspace->GetOrbit(b);
  Orbit& oc = modelspace->GetOrbit(c);
...
double TwoBodyME::GetTBME_J_norm(int j_bra, int j_ket, int a, int b, int c,
                                 int d) const {
  Orbit& oa = modelspace->GetOrbit(a);
  Orbit& ob = modelspace->GetOrbit(b);
  Orbit& oc = modelspace->GetOrbit(c);
  ...
  //   if (ch_bra <= ch_ket)
  //     return GetTBME_norm(ch_bra,ch_ket,a,b,c,d);
  //   return modelspace->phase(j_bra - j_ket) *
  //   GetTBME_norm(ch_ket,ch_bra,c,d,a,b);
}
...
double TwoBodyME::GetTBME(int j, int p, int t, int a, int b, int c,
                          int d) const {
  int ch = modelspace->GetTwoBodyChannelIndex(j, p, t);
  return GetTBME(ch, ch, a, b, c, d);
}

double TwoBodyME::GetTBME_norm(int j, int p, int t, int a, int b, int c,
                               int d) const {
  int ch = modelspace->GetTwoBodyChannelIndex(j, p, t);
  return GetTBME_norm(ch, ch, a, b, c, d);
}

void TwoBodyME::SetTBME(int j, int p, int t, int a, int b, int c, int d,
                        double tbme) {
  int ch = modelspace->GetTwoBodyChannelIndex(j, p, t);
  SetTBME(ch, ch, a, b, c, d, tbme);
}

void TwoBodyME::AddToTBME(int j, int p, int t, int a, int b, int c, int d,
                          double tbme) {
  int ch = modelspace->GetTwoBodyChannelIndex(j, p, t);
...
void TwoBodyME::AddToTBME(int j, int p, int t, Ket& bra, Ket& ket,
                          double tbme) {
  int ch = modelspace->GetTwoBodyChannelIndex(j, p, t);
...
/// \f[
/// \left\langle ab | V | cd \right\rangle_{pnpn} =
/// \frac{\sqrt{(1+\delta_{ab})(1+\delta_{cd})}}{2}
///       \left( \left\langle ab | V | cd \right\rangle_{10} + \left\langle ab |
///       V | cd \right\rangle_{00}   \right)
/// \f]
/// \f[
/// \left\langle ab | V | cd \right\rangle_{pnnp} =
/// \frac{\sqrt{(1+\delta_{ab})(1+\delta_{cd})}}{2}
///       \left( \left\langle ab | V | cd \right\rangle_{10} - \left\langle ab |
///       V | cd \right\rangle_{00}   \right)
/// \f]
///
void TwoBodyME::Set_pn_TBME_from_iso(int j, int T, int tz, int a, int b, int c,
                                     int d, double tbme) {
  // convert everyting to proton labels. Incrementing by 1 gets the neutron
  // label
  a -= a % 2;
  b -= b % 2;
  c -= c % 2;
  ...
  if (a != b and c != d) AddToTBME(j, parity, tz, a + 1, b, c + 1, d, tbme);
  if (a != b and (a != c or b != d))
    AddToTBME(j, parity, tz, a + 1, b, c, d + 1, tbme * isospin_phase);
}

double TwoBodyME::Get_iso_TBME_from_pn(int j, int T, int tz, int a, int b,
                                       int c, int d) {
  // convert everyting to proton labels. Incrementing by 1 gets the neutron
  // label
  a -= a % 2;
  ...
  int parity = (modelspace->GetOrbit(a).l + modelspace->GetOrbit(b).l) % 2;
  int isospin_phase = 2 * T - 1;
  double tbme = GetTBME(j, parity, tz, a, b + 1, c, d + 1) +
                GetTBME(j, parity, tz, a + 1, b, c + 1, d) +
                isospin_phase * (GetTBME(j, parity, tz, a, b + 1, c + 1, d) +
                                 GetTBME(j, parity, tz, a + 1, b, c, d + 1));
  tbme *= 0.5;
  if (a == b) tbme /= PhysConst::SQRT2;
  if (c == d) tbme /= PhysConst::SQRT2;
  return tbme;
...
/// \f[ \bar{V}_{ijkl} = \sqrt{(1+\delta_{ij})(1+\delta_{kl})}
/// \frac{\sum_{J}(2J+1) V_{ijkl}^J}{(2j_i+1)(2j_j+1)} \f]
...
double TwoBodyME::Norm() const {
  ...
  for (auto& itmat : MatEl) {
    ...
    int degeneracy = (2 * Jket + 1) * (std::min(Jbra, Jket + rank_J) -
                                       std::max(-Jbra, Jket - rank_J) + 1);
    //      int degeneracy = 1;//(2*Jket+1) * (std::min(Jbra,Jket+rank_J) -
    //      std::max(-Jbra,Jket-rank_J)+1); double n2 =
    //      arma::norm(matrix,"fro");
...
int TwoBodyME::size() {
  int size = 0;
  for (auto& itmat : MatEl) size += itmat.second.size();
...
void TwoBodyME::WriteBinary(std::ofstream& of) {
  ...
  for (auto& itmat : MatEl)
    of.write((char*)itmat.second.memptr(),
             itmat.second.size() * sizeof(double));
}
