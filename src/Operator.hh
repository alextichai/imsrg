...
/// The Operator class provides a generic operator up to three-body, scalar or
/// tensor. The class contains lots of methods and overloaded operators so that
/// the resulting code that uses the operators can look as close as possible to
/// the math that is written down.
class Operator {
 public:
  // Fields
  ModelSpace* modelspace;  ///< Pointer to the associated modelspace
  double ZeroBody;         ///< The zero body piece of the operator.
  arma::mat OneBody;       ///< The one body piece of the operator, stored in a
                      ///< single NxN armadillo matrix, where N is the number of
  ...
  ThreeLegME
      ThreeLeg;  ///< Three-legged operators, used if this is a
                 ///< particle-number-changing operator, i.e. if legs is odd
  ...
  int legs;           ///< The maximum number of particle legs in a diagrammatic
                      ///< representation, e.g. a 2-body operator has 4 legs.

  int E2max;  ///< For two-body matrix elements, \f$ e_i + e_j \leq \f$ E2max
  int E3max;  ///< For three-body matrix elements, \f$ e_i + e_j + e_k \leq \f$
              ///< E3max

  bool hermitian;
  bool antihermitian;
  int nChannels;  ///< Number of two-body channels \f$ J,\pi,T_z \f$ associated
                  ///< with the model space

  std::map<std::array<int, 3>, std::set<index_t> >
      OneBodyChannels;  // a set makes more sense for this, because it only
                        // contains unique entries
  //  std::map<std::array<int,3>,std::vector<index_t> > OneBodyChannels;
  index_t Q_space_orbit;  // Orbit with the same quantum numbers as this dagger
                          // operator. -1 if it's not a dagger operator.
  ...
  size_t GetTwoBodyDimension(size_t ch_bra, size_t ch_ket) {
    return TwoBody.GetMatrix(ch_bra, ch_ket).n_cols;
  };
  double GetTwoBody(size_t ch_bra, size_t ch_ket, size_t i, size_t j);
  void SetTwoBody(int J1, int p1, int T1, int J2, int p2, int T2, int i, int j,
  ...
  ModelSpace* GetModelSpace()
      const;  // making this const isn't strictly kosher, but hopefully we
              // shouldn't be in the business of tweaking the modelspace...
  ...
  void Symmetrize();      ///< Copy the upper-half triangle to the lower-half
                          ///< triangle for each matrix
  void AntiSymmetrize();  ///< Copy the upper-half triangle to the lower-half
                          ///< triangle with a minus sign.
  ...
  // Undoing normal ordering is equivalent to doing normal ordering with
  // negative occupations. So the occupations na,nb etc are all multiplied by
  // the sign passed to the methods.
  Operator DoNormalOrdering()
      const;  ///< Calls DoNormalOrdering2() or DoNormalOrdering3(), depending
              ///< on the rank of the operator.
  Operator DoNormalOrdering2(int sign, std::set<index_t> occupied)
      const;  ///< Returns the normal ordered two-body operator
  Operator DoNormalOrdering3(int sign, std::set<index_t> occupied)
      const;  ///< Returns the normal ordered three-body operator
  Operator DoNormalOrderingCore() const;  ///< Normal order with respect to core
  //  Operator DoNormalOrdering2(int sign=+1) const; ///< Returns the normal
  //  ordered two-body operator Operator DoNormalOrdering3(int sign=+1) const;
  //  ///< Returns the normal ordered three-body operator
  Operator DoNormalOrderingDagger(int sign, std::set<index_t> occupied)
      const;  ///< Returns the normal ordered dagger operator
  Operator UndoNormalOrdering()
      const;  ///< Returns the operator normal-ordered wrt the vacuum
  //  Operator UndoNormalOrdering2() const; ///< Returns the operator
  //  normal-ordered wrt the vacuum Operator UndoNormalOrdering2() const;  ///<
  //  Returns the operator normal-ordered wrt the vacuum Operator
  //  UndoNormalOrdering3() const;  ///< Returns the operator normal-ordered wrt
  //  the vacuum Operator UndoNormalOrdering2() const {return
  //  this->DoNormalOrdering2(-1);}; ///< Returns the operator normal-ordered
  //  wrt the vacuum Operator UndoNormalOrdering3() const {return
  //  this->DoNormalOrdering3(-1);}; ///< Returns the operator normal-ordered
  //  wrt the vacuum Operator UndoNormalOrderingDagger() const; ///< Returns the
  //  operator normal-ordered wrt the vacuum Operator UndoNormalOrderingDagger()
  //  const {return this->DoNormalOrderingDagger(-1);}; ///< Returns the
  //  operator normal-ordered wrt the vacuum

  Operator Truncate(ModelSpace& ms_new);  ///< Returns the operator trunacted to
                                          ///< the new model space

  // In principle, these methods should probably be factorized out, but I don't
