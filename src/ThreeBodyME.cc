...
ThreeBodyME::ThreeBodyME()
    : threebody_storage(new ThreeBodyStorage_iso()),
      modelspace(NULL),
      E3max(0),
      herm(1) {}
...
ThreeBodyME::ThreeBodyME(ModelSpace* ms, int e3max)
    : threebody_storage(new ThreeBodyStorage_iso(ms, e3max)),
      modelspace(ms),
      E3max(e3max),
      emax(ms->Emax),
      herm(1) {}
...
ThreeBodyME::ThreeBodyME(const ThreeBodyME& Tbme)
    : threebody_storage(Tbme.threebody_storage->Clone()),
      modelspace(Tbme.modelspace),
      //   storage_mode(Tbme.storage_mode), E3max(Tbme.E3max), emax(Tbme.emax),
      //   herm(Tbme.herm),
      E3max(Tbme.E3max),
      emax(Tbme.emax),
      herm(Tbme.herm),
...
//*******************************************************************
/// Get three body matrix element in proton-neutron formalism, regardless of how
/// it is stored. \f[
///  V_{abcdef}^{(pn)} = \sum_{t_{ab} t_{de} T} <t_a t_b | t_{ab}> <t_d t_e |
///  t_{de}> <t_{ab} t_c | T> <t_{de} t_f| T> V_{abcdef}^{t_{ab} t_{de} T}
/// \f]
//*******************************************************************
ThreeBME_type ThreeBodyME::GetME_pn(int Jab_in, int Jde_in, int twoJ, int a,
                                    int b, int c, int d, int e, int f) const {
  return threebody_storage->GetME_pn(Jab_in, Jde_in, twoJ, a, b, c, d, e, f);
}

ThreeBME_type ThreeBodyME::GetME_pn_ch(size_t ch_bra, size_t ch_ket,
                                       size_t ibra, size_t iket) const {
  return threebody_storage->GetME_pn_ch(ch_bra, ch_ket, ibra, iket);
}

// Get a 3-body matrix element in isospin formalism, regardless of how it is
// stored.
ThreeBME_type ThreeBodyME::GetME_iso(int Jab_in, int Jde_in, int twoJ,
                                     int tab_in, int tde_in, int twoTabc,
                                     int twoTdef, int a, int b, int c, int d,
                                     int e, int f) const {
  return threebody_storage->GetME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in,
                                      twoTabc, twoTdef, a, b, c, d, e, f);
}

// If we only provide one value of total isospin, assume it's the same T for bra
// and ket.
ThreeBME_type ThreeBodyME::GetME_iso(int Jab_in, int Jde_in, int twoJ,
                                     int tab_in, int tde_in, int twoT, int a,
                                     int b, int c, int d, int e, int f) const {
  if (rank_T > 0) {
    std::cout << "TROUBLE  !!! " << __FILE__ << "  line " << __LINE__
              << std::endl;
  }
  return threebody_storage->GetME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in,
                                      twoT, twoT, a, b, c, d, e, f);
}

void ThreeBodyME::SetME_pn(int Jab_in, int Jde_in, int twoJ, int a, int b,
                           int c, int d, int e, int f, ThreeBME_type V) {
  threebody_storage->SetME_pn(Jab_in, Jde_in, twoJ, a, b, c, d, e, f, V);
}

void ThreeBodyME::SetME_pn_ch(size_t ch_bra, size_t ch_ket, size_t ibra,
                              size_t iket, ThreeBME_type V) {
  threebody_storage->SetME_pn_ch(ch_bra, ch_ket, ibra, iket, V);
}

void ThreeBodyME::SetME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in,
                            int tde_in, int twoTabc, int twoTdef, int a, int b,
                            int c, int d, int e, int f, ThreeBME_type V) {
  threebody_storage->SetME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoTabc,
                               twoTdef, a, b, c, d, e, f, V);
}

// If we only provide one value of total isospin, assume it's the same T for bra
// and ket.
void ThreeBodyME::SetME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in,
                            int tde_in, int twoT, int a, int b, int c, int d,
                            int e, int f, ThreeBME_type V) {
  if (rank_T > 0) {
    std::cout << "TROUBLE  !!! " << __FILE__ << "  line " << __LINE__
              << std::endl;
  }
  threebody_storage->SetME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoT, twoT,
                               a, b, c, d, e, f, V);
}

void ThreeBodyME::AddToME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in,
                              int tde_in, int twoTabc, int twoTdef, int a,
                              int b, int c, int d, int e, int f,
                              ThreeBME_type V) {
  threebody_storage->AddToME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoTabc,
                                 twoTdef, a, b, c, d, e, f, V);
}

void ThreeBodyME::AddToME_pn_ch(size_t ch_bra, size_t ch_ket, size_t ibra,
                                size_t iket, ThreeBME_type V) {
...
void ThreeBodyME::AddToME_iso(int Jab_in, int Jde_in, int twoJ, int tab_in,
  ...
  threebody_storage->AddToME_iso(Jab_in, Jde_in, twoJ, tab_in, tde_in, twoT,
                                 twoT, a, b, c, d, e, f, V);
}

void ThreeBodyME::AddToME_pn(int Jab_in, int Jde_in, int twoJ, int a, int b,
                             int c, int d, int e, int f, ThreeBME_type V) {
  threebody_storage->AddToME_pn(Jab_in, Jde_in, twoJ, a, b, c, d, e, f, V);
}

std::vector<double> ThreeBodyME::GetME_pn_TwoOps(int Jab, int Jde, int twoJ,
                                                 int a, int b, int c, int d,
                                                 int e, int f,
                                                 const ThreeBodyME& X,
                                                 const ThreeBodyME& Y) const {
  return threebody_storage->GetME_pn_TwoOps(Jab, Jde, twoJ, a, b, c, d, e, f,
                                            *(X.threebody_storage),
                                            *(Y.threebody_storage));
}

ThreeBME_type ThreeBodyME::GetME_pn_no2b(int a, int b, int c, int d, int e,
                                         int f, int J2b) const {
  return threebody_storage->GetME_pn_no2b(a, b, c, d, e, f, J2b);
}

ThreeBME_type ThreeBodyME::GetME_pn_mono(int a, int b, int c, int d, int e,
                                         int f) const {
  return threebody_storage->GetME_pn_mono(a, b, c, d, e, f);
}

////////////////////////////////////////////////////////////////////////////////////////////////
/// Begin under-the-hood implementation of the matrix-element
/// setter-getter-adders
//////////////////////////////////////////////////////////////////////////////////////////////
...
// transform matrix elements that will be used later, or impose some E3max-type
// cuts.
//*****************************************************************************
void ThreeBodyME::TransformToPN() {
  //  ThreeBodyStorage_pn TBS_pn( modelspace, E3max, rank_J, rank_T, parity  );
  std::shared_ptr<ThreeBodyStorage> TBS_pn(
      new ThreeBodyStorage_pn(modelspace, E3max, rank_J, rank_T, parity));
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ich = 0; ich < nch; ich++) {
    ...
    for (size_t ibra = 0; ibra < nbras; ibra++) {
      ...
      //      std::cout << "   ibra = " << ibra << "  iketmax = " << iketmax <<
      //      std::endl; for (size_t iket=0; iket<=ibra; iket++)
      for (size_t iket = iketmin; iket < nkets; iket++) {
        ...
        //                  << bra.Jpq << " " << ket.Jpq << " " << twoJ << " abc
        //                  def "
        //                  << bra.p << " " << bra.q << " " << bra.r << "   "
        //                  << ket.p << " " << ket.q << " " << ket.r
        //                  << std::endl;
        double me_pn = threebody_storage->GetME_pn(
            bra.Jpq, ket.Jpq, twoJ, bra.p, bra.q, bra.r, ket.p, ket.q, ket.r);
        //        std::cout << "Call SetME_pn_ch with pn mode " << std::endl;
        TBS_pn->SetME_pn_ch(ch_bra, ch_ket, ibra, iket, me_pn);
        //        std::cout << "Set " << TBS_pn->GetME_pn_ch( ch_bra, ch_ket,
        //        ibra, iket ) << "  and it should be " << me_pn << std::endl;
      }
    }
  }
...
void ThreeBodyME::SwitchToPN_and_discard() {
  ...
  threebody_storage = std::shared_ptr<ThreeBodyStorage>(
      new ThreeBodyStorage_pn(modelspace, E3max, rank_J, rank_T, parity));
...
void ThreeBodyME::SetMode(std::string mode) {
  ...
  if (mode == "isospin") {
    threebody_storage = std::shared_ptr<ThreeBodyStorage>(
        new ThreeBodyStorage_iso(modelspace, E3max, rank_J, rank_T, parity));
  } else if (mode == "pn") {
    threebody_storage = std::shared_ptr<ThreeBodyStorage>(
        new ThreeBodyStorage_pn(modelspace, E3max, rank_J, rank_T, parity));
  } else if (mode == "no2b") {
    threebody_storage = std::shared_ptr<ThreeBodyStorage>(
        new ThreeBodyStorage_no2b<ME_single_type>(modelspace, E3max, rank_J,
                                                  rank_T, parity));
  } else if (mode == "no2bhalf") {
    threebody_storage = std::shared_ptr<ThreeBodyStorage>(
        new ThreeBodyStorage_no2b<ME_half_type>(modelspace, E3max, rank_J,
                                                rank_T, parity));
  } else if (mode == "mono") {
    threebody_storage = std::shared_ptr<ThreeBodyStorage>(
        new ThreeBodyStorage_mono<ME_single_type>(modelspace, E3max, rank_J,
                                                  rank_T, parity));
  }
  threebody_storage->Allocate();
  //  storage_mode = pn;
...
size_t ThreeBodyME::GetKetIndex_withRecoupling(
    int Jab_in, int twoJ, size_t a_in, size_t b_in, size_t c_in,
    std::vector<size_t>& iket, std::vector<double>& recouple) const {
  return threebody_storage->GetKetIndex_withRecoupling(Jab_in, twoJ, a_in, b_in,
                                                       c_in, iket, recouple);
}
...
/// permutation of fermionic operators. That is handled in
/// ThreeBodyME::AccessME. Here, we just only with the angular momentum /
/// isospin recoupling factors
//*******************************************************************
double ThreeBodyME::RecouplingCoefficient(
    ThreeBodyStorage::Permutation recoupling_case, double ja, double jb,
    double jc, int Jab_in, int Jab, int twoJ) const {
  return threebody_storage->RecouplingCoefficient(recoupling_case, ja, jb, jc,
                                                  Jab_in, Jab, twoJ);
}

int ThreeBodyME::PermutationPhase(
    ThreeBodyStorage::Permutation recoupling_case) const {
  return threebody_storage->PermutationPhase(recoupling_case);
}

void ThreeBodyME::Permute(ThreeBodyStorage::Permutation perm, size_t a_in,
                          size_t b_in, size_t c_in, size_t& a_out,
                          size_t& b_out, size_t& c_out) {
  threebody_storage->Permute(perm, a_in, b_in, c_in, a_out, b_out, c_out);
}

std::vector<ThreeBodyStorage::Permutation> ThreeBodyME::UniquePermutations(
    size_t a, size_t b, size_t c) const {
...
double ThreeBodyME::Norm() const { return threebody_storage->Norm(); }

void ThreeBodyME::Erase() { threebody_storage->Erase(); }

void ThreeBodyME::Deallocate() { threebody_storage->Deallocate(); }

size_t ThreeBodyME::size() { return threebody_storage->size(); }
...
void ThreeBodyME::WriteFile(std::vector<std::string> StringInputs,
                            std::vector<int> IntInputs) const {
  threebody_storage->WriteFile(StringInputs, IntInputs);
}

void ThreeBodyME::ReadFile(std::vector<std::string> StringInputs,
