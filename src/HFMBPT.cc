...
HFMBPT::HFMBPT(Operator& hbare)
    : HartreeFock(hbare),
      C_HO2NAT(modelspace->GetNumberOrbits(), modelspace->GetNumberOrbits(),
               arma::fill::eye),
      C_HF2NAT(modelspace->GetNumberOrbits(), modelspace->GetNumberOrbits(),
               arma::fill::eye),
      use_NAT_occupations(false),
      NAT_order("occupation") {}

//*********************************************************************
// post Hartree-Fock method
// This method assumes that we've already performed a Hartree-Fock calculation,
// so that the matrix C maps the HO basis to the HF basis. The first step is to
// compute the 1b density matrix rho up to the MBPT2 level, using the NO2B
// Hamiltonian in the HF basis. The second step is to diagonalize the density
// matrix. An optional third step is to change the occupations to match the
// eigenvalues of the density matrix. This requires some thinking, since if all
// orbits are fractionally occupied, then count as both hole and particle
// states, and routines that benefit from only acting on hh or ph blocks now act
// on all orbits, and things slow down. As a compromise, we place a threshold on
// the occupations, and all orbits with occupations below that threshold (which
// will be most of them) are set to zero and labeled as "particle". To maintain
// the right particle number, we "back-fill" the hole states using a somewhat
// arbitrary but not totally outlandish procedure.
//*********************************************************************
void HFMBPT::GetNaturalOrbitals() {
  ...
  Occ =
      arma::vec(norbits, arma::fill::zeros);  // Occupations are the eigenvalues
                                              // of the density matrix
  GetDensityMatrix();  // Include 2nd order MBPT corrections to rho
  DiagonalizeRho();    // Find the 1b transformation that diagonalizes rho, but
                       // don't apply it to anything yet.
  ...
  if (std::abs(AfromTr - A) > 1e-8) {
    std::cout << "Warning: Mass != Tr(rho)   " << A << " != " << AfromTr
              << std::endl;
    exit(0);
  }
  C_HO2NAT = C * C_HF2NAT;
  ...
  for (auto i : modelspace->all_orbits) {
    ...
    oi.occ_nat = std::abs(Occ(i));  // it's possible that Occ(i) is negative,
                                    // and for occ_nat, we don't want that.
  }

  if (use_NAT_occupations)  // use fractional occupation
  {
    double keep_occ_threshold =
        0.02;  // anything with occupation less than keep_occ_threshold gets set
               // to zero, otherwise everything is a hole.

    double NfromTr = 0;
    double ZfromTr = 0;
    std::cout << "Switching to occupation numbers obtained from 2nd order 1b "
                 "density matrix."
              << std::endl;
    std::vector<index_t> holeorbs_tmp;
    std::vector<double> hole_occ_tmp;
    // Figure out how many particles are living in orbits with occupations above
    // our threshold. We do this separately for protons and neutrons.
    for (auto& i : modelspace->all_orbits) {
      Orbit& oi = HartreeFock::modelspace->GetOrbit(i);
    ...
    int N = A - Z;
    double aloquot = 0.005;  // This is how much we increase an occupation in
                             // each back-filling pass.
    ...
    while ((Z - ZfromTr) > ModelSpace::OCC_CUT) {
      for (size_t i = 0; i < holeorbs_tmp.size(); i++) {
        ...
        double occ_increase = std::min(
            {aloquot, (Z - ZfromTr) / (oi.j2 + 1.), 1.0 - hole_occ_tmp[i]});
    ...
    while ((N - NfromTr) > ModelSpace::OCC_CUT) {
      for (size_t i = 0; i < holeorbs_tmp.size(); i++) {
        ...
        if (oi.tz2 < 0) continue;
        double occ_increase = std::min(
            {aloquot, (N - NfromTr) / (oi.j2 + 1.), 1.0 - hole_occ_tmp[i]});
        hole_occ_tmp[i] += occ_increase;
    ...
    // Now we tell modelspace about the new occupations, and any needed
    // reclassification of 'holes' and 'particles'
    UpdateReference();

    // Calling UpdateReference screws up the occ_nat values, so we need to set
    // them again here.
  ...
  // In principle, this could be iterated until self-consistency. But I don't do
  // that for now. This would only have an impact if the energy ordering of
  // occupied and unoccupied levels gets flipped, which would be a fairly
  // pathological case.
  arma::mat tmp = C_HO2NAT.cols(holeorbs);
  rho = (tmp.each_row() % hole_occ) *
        tmp.t();  // now rho is in the HO basis, with our prescribed occupations
                  // in the NAT basis
  UpdateF();  // Now F is in the HO basis, but with rho from filling in the NAT
              // basis.
...
void HFMBPT::DiagonalizeRho() {
  for (auto& it : Hbare.OneBodyChannels) {
    ...
    bool success = false;
    success =
        arma::eig_sym(eig, vec, rho_ch);  // eigenvalues of rho (i.e.
                                          // occupations) are in ascending order
    if (not success) {
    ...
    //    Occ(orbvec_d) = eig; // assigning with descending indexes produces NAT
    //    orbits ordered by descending occupation. C_HF2NAT.submat(orbvec,
    //    orbvec_d) = vec; // NAT orbits ordered by descending occupation

    Occ(orbvec) = arma::reverse(
        eig);  // reverse so NAT orbits ordered by descending occupation.
    C_HF2NAT.submat(orbvec, orbvec) =
        arma::reverse(vec, 1);  // "1" means reverse elements in each row
                                //    std::cout << "1-body channel ";
    //    for ( auto x : it.first ) std::cout << x << " ";
    //    std::cout << "  : " << std::endl << Occ(orbvec) << std::endl;
    //    std::cout << " C submat: " << std::endl << C_HF2NAT.submat(orbvec,
    //    orbvec ) << std::endl;
  }
  // Choose ordering and phases so that C_HF2NAT looks as close to the identity
  // as possible NB: Rather than C_HF2NAT being close to the identity, we really
  // want the orbits for a given (l,j,tz) ordered according to decreasing
  // occupation, so that a later emax_imsrg truncation is reasonable. -SRS Jan
  // 2021.
  //  ReorderHFMBPTCoefficients();
  //  std::cout << " line " << __LINE__ << "   Occ = " << std::endl << Occ <<
  //  std::endl;
}
  ...
  for (auto& it : OpHF.TwoBody.MatEl) {
    ...
    for (int i = 0; i < nkets; ++i) {
      ...
      for (int j = 0; j < nkets; ++j) {
        ...
        Dket(i, j) =
            C_HF2NAT(ket_hf.p, ket_nat.p) * C_HF2NAT(ket_hf.q, ket_nat.q);
        if (ket_hf.p != ket_hf.q) {
          Dket(i, j) += C_HF2NAT(ket_hf.q, ket_nat.p) *
                        C_HF2NAT(ket_hf.p, ket_nat.q) * ket_hf.Phase(tbc_ket.J);
        }
    ...
    if (ch_bra == ch_ket) {
      ...
      for (int i = 0; i < nbras; ++i) {
        ...
        for (int j = 0; j < nbras; ++j) {
          ...
          Dbra(i, j) =
              C_HF2NAT(bra_hf.p, bra_nat.p) * C_HF2NAT(bra_hf.q, bra_nat.q);
          if (bra_hf.p != bra_hf.q) {
            Dbra(i, j) += C_HF2NAT(bra_hf.q, bra_nat.p) *
                          C_HF2NAT(bra_hf.p, bra_nat.q) *
  ...
  for (auto& it : OpHO.TwoBody.MatEl) {
    ...
    for (int i = 0; i < nkets; ++i) {
      ...
      for (int j = 0; j < nkets; ++j) {
        Ket& ket_nat = tbc_ket.GetKet(j);
        Dket(i, j) =
            C_HO2NAT(ket_ho.p, ket_nat.p) * C_HO2NAT(ket_ho.q, ket_nat.q);
        if (ket_ho.p != ket_ho.q) {
          Dket(i, j) += C_HO2NAT(ket_ho.q, ket_nat.p) *
                        C_HO2NAT(ket_ho.p, ket_nat.q) * ket_ho.Phase(tbc_ket.J);
        }
    ...
    if (ch_bra == ch_ket) {
      ...
      for (int i = 0; i < nbras; ++i) {
        ...
        for (int j = 0; j < nbras; ++j) {
          ...
          Dbra(i, j) =
              C_HO2NAT(bra_ho.p, bra_nat.p) * C_HO2NAT(bra_ho.q, bra_nat.q);
          if (bra_ho.p != bra_ho.q) {
            Dbra(i, j) += C_HO2NAT(bra_ho.q, bra_nat.p) *
                          C_HO2NAT(bra_ho.p, bra_nat.q) *
...
Operator HFMBPT::GetNormalOrderedHNAT(int particle_rank) {
  ...
  //  UpdateF();  // Now F is in the HO basis, but with rho from filling in the
  //  NAT basis.
  CalcEHF();
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int i = 0; i < npq; ++i) {
      ...
      for (int j = 0; j < npq; ++j) {
        ...
        if (bra.p != bra.q) {
          D(i, j) +=
              C_HO2NAT(bra.q, ket.p) * C_HO2NAT(bra.p, ket.q) * bra.Phase(J);
        }
        ...
        for (auto a : modelspace->all_orbits) {
          ...
          for (int b : Hbare.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
            ...
            //              V3NO(i,j) += rho(a,b) * GetVNO2B(bra.p, bra.q, a,
            //              ket.p, ket.q, b, J);
...
void HFMBPT::GetDensityMatrix() {
  ...
  // After the HF step, rho is the density of harmonic oscillator states for a
  // filled HF reference
  //  i.e. <a|rho|b> = sum_i <a|i> <b|i>  where i is an occupied HF state and a
  //  and b are HO basis states.
  // Now we switch to the HF basis, so rho should be diagonal before adding in
  // the perturbative corrections.
  rho.zeros();  // This and the following line fixes bug found by Baishan Dec
                // 2020.
  for (auto& i : HartreeFock::modelspace->holes)
    rho(i, i) =
        HartreeFock::modelspace->GetOrbit(i).occ;  // Set hole occupations to 1.

  //  std::cout << std::endl << "before perturbative correction, rho is" <<
  //  std::endl << rho << std::endl;
  // compute second order corrections to the density matrix
  DensityMatrixPP(H);
  DensityMatrixHH(H);
  DensityMatrixPH(H);
  //  std::cout << std::endl << "after perturbative correction, rho is" <<
  //  std::endl << rho << std::endl;
...
//
//        *~~~~~~~*      <a|rho|b> = 1/2 sum_{cijJ} (2J+1)/(2j_a+1)
//        <ac|V|ij><ij|V|bc> / Delta
// R0--- / \     / \a
...
//
// When using fraction occupations, including the factor (1-na)(1-nb) [ni
// nj(1-nc)]^2 and an equivalent one in the HH term produces a density matrix
// with the correct particle number, encoded in the 2j+1 weighted trace of rho.
//*********************************************************************
      ...
      //      for(auto& c : HartreeFock::modelspace->all_orbits)
      {
        ...
        for (auto& i : HartreeFock::modelspace->holes) {
          ...
          for (auto& j : HartreeFock::modelspace->holes) {
            ...
            int Jmin = std::max(std::abs(oa.j2 - oc.j2),
                                std::max(std::abs(oi.j2 - oj.j2),
                                         std::abs(ob.j2 - oc.j2))) /
                       2;
            int Jmax = std::min(oa.j2 + oc.j2,
                                std::min(oi.j2 + oj.j2, ob.j2 + oc.j2)) /
                       2;
            ...
            tbme *= (1 - oa.occ) * (1 - ob.occ) * (1 - oc.occ) * (1 - oc.occ) *
                    oi.occ * oi.occ * oj.occ * oj.occ;
            double epsilon = 0.5 * sqrt(std::abs(e_acij * e_bcij));
            r += 0.5 * (sqrt(tbme + epsilon * epsilon) - epsilon) /
                 sqrt(tbme + epsilon * epsilon);
          }
        }
      }
...
//
//        *~~~~~~~*      <i|rho|j> = -1/2 sum_{abkJ} (2J+1)/(2j_i+1)
//        <ab|V|ik><jk|V|ab> / Delta
// R0--- / \     / \j
...
void HFMBPT::DensityMatrixHH(Operator& H) {
  ...
  for (auto& i : HartreeFock::modelspace->holes) {
    ...
    for (auto& j :
         HartreeFock::modelspace->OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
      if (j > i) continue;
      double ej = H.OneBody(j, j);
      Orbit& oj = HartreeFock::modelspace->GetOrbit(j);
      if (oj.occ < ModelSpace::OCC_CUT) continue;
      ...
      //      for(auto& a : HartreeFock::modelspace->all_orbits)
      {
        ...
        //        for(auto& b : HartreeFock::modelspace->all_orbits)
        {
          ...
          for (auto& k : HartreeFock::modelspace->holes) {
            ...
            int Jmin = std::max(std::abs(oa.j2 - ob.j2),
                                std::max(std::abs(oi.j2 - ok.j2),
                                         std::abs(oj.j2 - ok.j2))) /
                       2;
            int Jmax = std::min(oa.j2 + ob.j2,
                                std::min(oi.j2 + ok.j2, oj.j2 + ok.j2)) /
                       2;
            ...
            tbme *= (1 - oa.occ) * (1 - oa.occ) * (1 - ob.occ) * (1 - ob.occ) *
                    ok.occ * ok.occ * oi.occ * oj.occ;
            if (true) {
...
//
//      (RHO)           <i|rho|a> = 1/2 sum_{bcjJ} (2J+1)/(2j_i+1)
//      <aj|V|bc><bc|V|ij> / Delta
// R0--- / \a
...
//      (RHO)           <i|rho|a> = -1/2 sum_{bkjJ} (2J+1)/(2j_i+1)
//      <kj|V|ib><ab|V|kj> / Delta
// R0--- / \i
//     a(   )~~~~*       with Delta = (ea-ei)(ea+eb-ej-ek)
// R0--- \ /j  b( )k
//        *~~~~~~*
//
// Equivalent diagrams can be drawn with rho on the bottom, corresponding to
// <2|rho|0>, and the formulas are the same. In (limited) tests, a small gap
// between particle and hole levels did not appear to be a problem for these
// diagrams, so the MBPT2 expression is used directly. This may need to be
// revisited.
//*********************************************************************
      ...
      for (auto& b : HartreeFock::modelspace->particles) {
        ...
        for (auto& c : HartreeFock::modelspace->particles) {
          ...
          for (auto& j : HartreeFock::modelspace->holes) {
            ...
            int Jmin = std::max(std::abs(oa.j2 - oj.j2),
                                std::max(std::abs(ob.j2 - oc.j2),
                                         std::abs(oi.j2 - oj.j2))) /
                       2;
            int Jmax = std::min(oa.j2 + oj.j2,
                                std::min(ob.j2 + oc.j2, oi.j2 + oj.j2)) /
                       2;
            ...
            tbme *=
                (1 - oa.occ) * (1 - ob.occ) * (1 - oc.occ) * oi.occ * oj.occ;
            r += tbme / (e_ai * e_bcij);
          }
        }
      }
      ...
      for (auto& b : HartreeFock::modelspace->particles) {
        ...
        for (auto& j : HartreeFock::modelspace->holes) {
          ...
          for (auto& k : HartreeFock::modelspace->holes) {
            ...
            int Jmin = std::max(std::abs(ok.j2 - oj.j2),
                                std::max(std::abs(oi.j2 - ob.j2),
                                         std::abs(oa.j2 - ob.j2))) /
                       2;
            int Jmax = std::min(ok.j2 + oj.j2,
                                std::min(oi.j2 + ob.j2, oa.j2 + ob.j2)) /
                       2;
...
void HFMBPT::PrintSPEandWF() {
  ...
  for (auto i : modelspace->all_orbits) {
    ...
    std::cout << std::fixed << std::setw(3) << i << ": " << std::setw(3) << oi.n
              ...
              << std::setprecision(6) << F_natbasis(i, i) << " "
              << std::setw(12) << oi.occ << " " << std::setw(12) << oi.occ_nat
              << "   | ";
...
void HFMBPT::ReorderHFMBPTCoefficients() {
  for (index_t i = 0; i < C_HF2NAT.n_rows;
  ...
  if (NAT_order == "energy") {
    // note that we do this in just one pass, and that in the unlikely case that
    // one of the occupied orbits gets swapped with an unoccupied orbit, we
    // would in principle need to recompute the energies and orderings but let's
    // just hope that this doesn't happen.
    std::cout << "Ordering NAT orbits according to increasing energy..."
              << std::endl;
    for (auto& it : Hbare.OneBodyChannels) {
      arma::uvec orbvec(
          std::vector<index_t>(it.second.begin(), it.second.end()));
      ...
      arma::mat fNAT_chan =
          CNAT_chan.t() * CHF_chan.t() * fHO_chan * CHF_chan * CNAT_chan;
      ...
      C_HF2NAT.submat(orbvec, orbvec) = C_HF2NAT(
          orbvec,
          orbvec_sorted);  // sort the column indices, <row|col> = <HF|NAT>
    ...
    std::cout
        << "Ordering NAT orbits according to second order energy impact..."
        << std::endl;
    Operator H_temp = GetNormalOrderedHNAT(
        2);  // particle rank 2 - we don't care about threebody here
...
// double HartreeFock::GetHF3bme( int Jab, int Jde, int J2,  size_t a, size_t b,
// size_t c, size_t d, size_t e, size_t f) double HFMBPT::GetTransformed3bme(
// int Jab, int Jde, int J2,  size_t a, size_t b, size_t c, size_t d, size_t e,
// size_t f)
double HFMBPT::GetTransformed3bme(Operator& OpIn, int Jab, int Jde, int J2,
        ...
        for (auto delta : OpIn.OneBodyChannels.at({od.l, od.j2, od.tz2})) {
          ...
          for (auto epsilon : OpIn.OneBodyChannels.at({oe.l, oe.j2, oe.tz2})) {
            ...
            for (auto phi : OpIn.OneBodyChannels.at({of.l, of.j2, of.tz2})) {
              ...
              //         double V_ho = OpIn.ThreeBody.GetME( Jab,  Jde,  J2,
              //         tab,  tde,  T2,  alpha,  beta,  gamma,  delta, epsilon,
              //         phi);
              V_nat += V_ho * C_HO2NAT(alpha, a) * C_HO2NAT(beta, b) *
                       C_HO2NAT(gamma, c) * C_HO2NAT(delta, d) *
                       C_HO2NAT(epsilon, e) * C_HO2NAT(phi, f);
            }  // for phi
          }    // for epsilon
        }      // for delta
      }        // for gamma
...
// Modified version of GetMP2_Energy. Determines each orbital's impact on the
// total MP2 energy (i.e., by how much would EMP2 change if this single orbital
// were removed)
arma::vec HFMBPT::GetMP2_Impacts(Operator& OpIn) const {
  //   std::cout << "  a    b    i    j    J    na     nb    tbme    denom dE"
  //   << std::endl;
  double t_start = omp_get_wtime();
  double de = 0;

  int nparticles = OpIn.modelspace->particles.size();
  arma::vec orbit_impacts(OpIn.modelspace->all_orbits.size(),
                          arma::fill::zeros);

  std::vector<index_t> particles_vec(
      OpIn.modelspace->particles.begin(),
      OpIn.modelspace->particles
        ...
        for (auto& b : OpIn.modelspace->holes) {
          ...
          for (int J = Jmin; J <= Jmax; J += dJ) {
            ...
            if (std::abs(tbme) > 1e-6) {
              de = (2 * J + 1) * oa.occ * ob.occ * tbme * tbme /
                   denom;  // no factor 1/4 because of the restricted sum
