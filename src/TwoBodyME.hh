...
/// The two-body piece of the operator, stored in a vector of maps of of
/// armadillo matrices. The index of the vector indicates the J-coupled two-body
/// channel of the ket state, while the map key is the two-body channel of the
/// bra state. This is done to allow for tensor operators which connect
/// different two-body channels without having to store all possible
/// combinations. In the case of a scalar operator, there is only one map key
/// for the bra state, corresponding to that of the ket state. The normalized
/// J-coupled TBME's are stored in the matrices. However, when the TBME's are
/// accessed by GetTBME(), they are returned as
/// \f$ \tilde{\Gamma}_{ijkl} \equiv \sqrt{(1+\delta_{ij})(1+\delta_{kl})}
/// \Gamma_{ijkl} \f$ because the flow equations are derived in terms of \f$
/// \tilde{\Gamma} \f$. For efficiency, only matrix elements with \f$ i\leq j
/// \f$ and \f$ k\leq l \f$ are stored. When performing sums that can be cast as
/// matrix multiplication, we have something of the form \f[ \tilde{Z}_{ijkl}
/// \sim \frac{1}{2} \sum_{ab}\tilde{X}_{ijab} \tilde{Y}_{abkl} \f] which may be
/// rewritten as a restricted sum, or matrix multiplication \f[ Z_{ijkl} \sim
/// \sum_{a\leq b} X_{ijab} Y_{abkl} = \left( X\cdot Y \right)_{ijkl} \f]
class TwoBodyME {
 public:
  ...
  void AddToTBME(int ch_bra, int ch_ket, int a, int b, int c, int d,
                 double tbme);
  ...
  double GetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                 int t_ket, Ket& bra, Ket& ket) const;
  void SetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket, int t_ket,
               Ket& bra, Ket& ket, double tbme);
  void AddToTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                 int t_ket, Ket& bra, Ket& ket, double tbme);
  double GetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
                 int t_ket, int a, int b, int c, int d) const;
  void SetTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket, int t_ket,
               int a, int b, int c, int d, double tbme);
  void AddToTBME(int j_bra, int p_bra, int t_bra, int j_ket, int p_ket,
  ...
  void AddToTBME_J(int j_bra, int j_ket, int a, int b, int c, int d,
                   double tbme);
  ...
  void Set_pn_TBME_from_iso(int j, int T, int tz, int a, int b, int c, int d,
                            double tbme);
  ...
  //  void   AddToTBME_RelCM(int n1, int l1, int n2, int l2, int L12, int S12,
  //  int J12, int T12, int Tz12, int n3, int l3, int n4, int l4, int L34, int
  //  S34, int J34, int T34, int Tz34, double Vrel, double Vcm);
  //  vector<pair<int,double>> GetLabFrameKets(int n, int lam, int N, int LAM,
  //  int L, int S, int J, int T, int Tz);
