...
std::function<double(double, double)> Generator::wegner_func =
    [](double Hod, double denom) { return Hod * denom; };
std::function<double(double, double)> Generator::white_func =
    [](double Hod, double denom) { return Hod / denom; };
std::function<double(double, double)> Generator::atan_func =
    [](double Hod, double denom) { return 0.5 * atan(2 * Hod / denom); };
std::function<double(double, double)> Generator::imaginarytime_func =
    [](double Hod, double denom) {
      return Hod * ((denom > 0) - (denom < 0));
    };  // signum function

std::function<double(double, double)> Generator::qtransferatan1_func =
    [](double Hod, double denom) {
      return pow(std::abs(denom) * M_NUCLEON / HBARC / HBARC, 0.5 * 1) *
             atan_func(Hod, denom);
    };
...
void Generator::AddToEta(Operator& H_s, Operator& Eta_s) {
  ...
  if (generator_type == "wegner")
    ConstructGenerator_SingleRef(
        wegner_func);  // never tested, probably doesn't work.
    ...
    int n;
    std::istringstream(generator_type.substr(generator_type.find("_") + 1)) >>
        n;
    std::function<double(double, double)> qtransferatanN_func =
        [n](double Hod, double denom) {
          return pow(std::abs(denom) * M_NUCLEON / HBARC / HBARC, 0.5 * n) *
                 atan_func(Hod, denom);
        };
  ...
  //   else if (generator_type == "rspace") ConstructGenerator_Rspace();
    ...
    std::cout << "Error. Unkown generator_type: " << generator_type
              << std::endl;
  }
...
void Generator::SetDenominatorDeltaOrbit(std::string orb) {
  if (orb == "all")
    ...
    std::cout << "Setting denominator delta orbit " << orb << " => "
              << modelspace->GetOrbitIndex(orb) << std::endl;
  }
}
...
double Generator::Get1bDenominator(int i, int j) {
  ...
  if (denominator_delta_index == -12345 or i == denominator_delta_index or
      j == denominator_delta_index)
...
double Generator::Get2bDenominator(int ch, int ibra, int iket) {
  ...
  if (denominator_partitioning == Epstein_Nesbet) {
    ...
    denominator +=
        (ni - nl) * H->TwoBody.GetTBMEmonopole(i, l, i, l);  // ph'ph'
    denominator +=
        (nj - nk) * H->TwoBody.GetTBMEmonopole(j, k, j, k);  // p'hp'h
    denominator +=
...
double Generator::Get3bDenominator(int i, int j, int k, int l, int m, int n) {
  auto h1 = H->OneBody;
  double denominator =
      h1(i, i) + h1(j, j) + h1(k, k) - h1(l, l) - h1(m, m) - h1(n, n);
  if (denominator_partitioning ==
      Epstein_Nesbet) {  // This  is unpleasant and almost certainly not
                         // necessary, so not implementing it yet...
  }
  return denominator;
}
...
double Generator::Get2bDenominator_Jdep(int ch, int ibra, int iket) {
  ...
  denominator +=
      (1 - ni - nj) * H->TwoBody.GetTBME(tbc.J, i, j, i, j);  // pp'pp'
  denominator -=
      (1 - nk - nl) * H->TwoBody.GetTBME(tbc.J, k, l, k, l);          // hh'hh'
  denominator += (ni - nk) * H->TwoBody.GetTBMEmonopole(i, k, i, k);  // phph
...
void Generator::ConstructGenerator_SingleRef(
  ...
  for (auto& a : modelspace->core) {
    for (auto& i :
         imsrg_util::VectorUnion(modelspace->valence, modelspace->qspace)) {
  ...
  for (int ch = 0; ch < Eta->nChannels; ++ch) {
    ...
    for (auto& iket : tbc.GetKetIndex_cc())  // cc means core-core ('holes'
                                             // refer to the reference state)
    {
      for (auto& ibra :
           imsrg_util::VectorUnion(tbc.GetKetIndex_qq(), tbc.GetKetIndex_vv(),
                                   tbc.GetKetIndex_qv())) {
...
void Generator::ConstructGenerator_SingleRef_3body(
  ...
  std::cout << __func__ << "  looping in generator 3-body part .  Size of H3 = "
            << H->ThreeBodyNorm() << std::endl;
  //    for (auto a : modelspace->core )
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) * occnat_c *
           (1 - occnat_c)) < modelspace->GetOccNat3Cut())
        continue;
      size_t a = bra.p;
      size_t b = bra.q;
      size_t c = bra.r;
      ...
      for (size_t iket = 0; iket < nkets3; iket++) {
        ...
        if (not((ket.op->cvq == 0) and (ket.oq->cvq == 0) and
                (ket.oR->cvq == 0)))
          continue;  // cvq==0 means core orbit
        double d_ei =
        ...
        double d_ek =
            std::abs(2 * ket.oR->n + ket.oR->l - e_fermi[ket.oR->tz2]);
        ...
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_k *
             (1 - occnat_k)) < modelspace->GetOccNat3Cut())
          continue;
        size_t i = ket.p;
        size_t j = ket.q;
        size_t k = ket.r;
        ...
        Eta->ThreeBody.AddToME_pn_ch(
            ch3, ch3, ibra, iket,
            eta);  // hermitian conjugate automatically gets added
  ...
  std::cout << "Norm of Eta3 = " << std::setprecision(8) << Eta->ThreeBodyNorm()
            << std::endl;
...
/*
void Generator::ConstructGenerator_ShellModel(std::function<double
(double,double)>& eta_func)
{


//   // One body piece -- make sure the valence one-body part is diagonal
//   for ( auto& a : imsrg_util::VectorUnion(modelspace->core,
modelspace->valence))
//   {
//      for (auto& i : imsrg_util::VectorUnion( modelspace->valence,
modelspace->qspace ) )
//      {
//         if (i==a) continue;
//         double denominator = Get1bDenominator(i,a);
...
//   }
   ...
   for (int ch=0;ch<nchan;++ch)
   {
      ...
      // Decouple the core
//      for ( auto& iket : imsrg_util::VectorUnion( tbc.GetKetIndex_cc(),
tbc.GetKetIndex_vc() ) ) for ( auto& iket : imsrg_util::VectorUnion(
tbc.GetKetIndex_vc() ) )
      {
         for ( auto& ibra : imsrg_util::VectorUnion( tbc.GetKetIndex_vv(),
tbc.GetKetIndex_qv(), tbc.GetKetIndex_qq() ) )
//         for ( auto& ibra : imsrg_util::VectorUnion( tbc.GetKetIndex_vv(),
tbc.GetKetIndex_qv() ) )
         {
            double denominator = Get2bDenominator(ch,ibra,iket);
//            ETA2(ibra,iket) = 0.5*atan(2*H2(ibra,iket) / denominator);
            ...
            ETA2(iket,ibra) = - ETA2(ibra,iket) ; // Eta needs to be
antisymmetric
         }
...
//         for ( auto& ibra : imsrg_util::VectorUnion( tbc.GetKetIndex_qv(),
tbc.GetKetIndex_qq() ) )
//         {
...
void Generator::ConstructGenerator_ShellModel(
    std::function<double(double, double)>& eta_func) {
  // One body piece -- make sure the valence one-body part is diagonal
  for (auto& a :
       imsrg_util::VectorUnion(modelspace->core, modelspace->valence)) {
    for (auto& i :
         imsrg_util::VectorUnion(modelspace->valence, modelspace->qspace)) {
      if (i == a) continue;
      double denominator = Get1bDenominator(i, a);
  ...
  for (int ch = 0; ch < nchan; ++ch) {
    ...
    // Decouple the core
    for (auto& iket :
         imsrg_util::VectorUnion(tbc.GetKetIndex_cc(), tbc.GetKetIndex_vc())) {
      for (auto& ibra :
           imsrg_util::VectorUnion(tbc.GetKetIndex_vv(), tbc.GetKetIndex_qv(),
                                   tbc.GetKetIndex_qq())) {
        double denominator = Get2bDenominator(ch, ibra, iket);
        //            ETA2(ibra,iket) = 0.5*atan(2*H2(ibra,iket) / denominator);
        ETA2(ibra, iket) = eta_func(H2(ibra, iket), denominator);
...
// off diagonal pieces are  ppp|ccc  ppp|ccv ppp|cvv  qpp|vvv   where p is
// either v or q
// void Generator::ConstructGenerator_SingleRef_3body(std::function<double
// (double,double)>& etafunc )
void Generator::ConstructGenerator_ShellModel_3body(
    std::function<double(double, double)>& etafunc) {
  double t_start = omp_get_wtime();
  //     size_t ncore = modelspace->core.size();
  std::vector<size_t> corevec;
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      for (size_t iket = 0; iket < nkets3; iket++) {
        ...
        if ((ket.op->cvq == 2) or (ket.oq->cvq == 2) or (ket.oR->cvq == 2))
          continue;  // cvq==2 means q, i.e. not core or valence. we want all c
                     // or v in ket.
        if ((bra.op->cvq == 1) and (bra.oq->cvq == 1) and (bra.oR->cvq == 1) and
            (ket.op->cvq == 1) and (ket.oq->cvq == 1) and (ket.oR->cvq == 1))
          continue;  // no vvvvvv
        double d_ea =
            std::abs(2 * ket.op->n + ket.op->l - e_fermi[ket.op->tz2]);
        double d_eb =
...
void Generator::ConstructGenerator_ShellModel_3body(std::function<double
(double,double)>& eta_func)
{
    // Three body piece
//    int E3cut = 99;
    ...
    std::map<int,double> e_fermi = modelspace->GetEFermi();
     ...
     std::cout << "looping in generator 3-body part .  Size of H3 = " <<
H->ThreeBodyNorm() << std::endl; std::vector<size_t> alist; for (auto a :
imsrg_util::VectorUnion(modelspace->valence,modelspace->qspace) )
alist.push_back(a); size_t a_size = alist.size();

//    for (auto a :
imsrg_util::VectorUnion(modelspace->valence,modelspace->qspace) ) for (size_t
ind_a=0; ind_a<a_size; ind_a++ )
    {
     auto a = alist[ind_a];
...
//         // if any bra indices match any ket indices, then the normal ordering
...
//            if ( k==a or k==b or k==c) continue;
            // check if everything is valence, becase we don't want vvv|vvv to
be off-diagonal if (   (std::find( modelspace->valence.begin(),
modelspace->valence.end(), a) != modelspace->valence.end()) // a is valence and
(std::find( modelspace->valence.begin(), modelspace->valence.end(), b) !=
modelspace->valence.end()) // b is valence and  (std::find(
modelspace->valence.begin(), modelspace->valence.end(), c) !=
modelspace->valence.end()) // c is valence and  (std::find(
modelspace->valence.begin(), modelspace->valence.end(), i) !=
modelspace->valence.end()) // i is valence and  (std::find(
modelspace->valence.begin(), modelspace->valence.end(), j) !=
modelspace->valence.end()) // j is valence and  (std::find(
modelspace->valence.begin(), modelspace->valence.end(), k) !=
modelspace->valence.end()) // k is valence ) continue;
...
std::abs(2*Jij-ok.j2) ); int twoJ_max = std::min( 2*Jab+oc.j2, 2*Jij+ok.j2 );
            for (int twoJ=twoJ_min; twoJ<=twoJ_max; twoJ+=2)
            {
              double ME_od = H->ThreeBody.GetME_pn( Jab, Jij, twoJ,
a,b,c,i,j,k); double eta = 0.5*atan(2*ME_od / denominator);
//              double eta = (ME_od / denominator);
//              if (std::abs(eta)>1e-6)
//              if (std::abs(eta)>1e-3)
//              {
//                std::cout << "big eta !  <" << a << " " << b << " " << c << ",
" << Jab << " | " << i << " " << j << " " << k << ", " << Jij << " > _ " << twoJ
<< "  => " << ME_od << " / " << denominator << " = " << eta << std::endl;
//              }
...
void Generator::ConstructGenerator_ShellModel_NpNh(
    std::function<double(double, double)>& eta_func) {
  ...
  for (auto& c : modelspace->core) {
    for (auto& cprime : modelspace->core) {
      ...
      //     Eta->OneBody(c,cprime) = 0.5*atan(2*H->OneBody(c,cprime) /
      //     denominator );
      Eta->OneBody(c, cprime) = eta_func(H->OneBody(c, cprime), denominator);
      //     std::cout << "c,cprime = " << c << " " << cprime << "  etacc' = "
      //     << Eta->OneBody(c,cprime) << std::endl;
  ...
  for (int ch = 0; ch < nchan; ++ch) {
    TwoBodyChannel& tbc = modelspace->GetTwoBodyChannel(ch);
    //     std::cout << "ch = " << ch << "  vc size = " <<
    //     tbc.GetKetIndex_vc().size() << "   qc size = " <<
    //     tbc.GetKetIndex_qc().size() << std::endl;
    arma::mat& ETA2 = Eta->TwoBody.GetMatrix(ch);
    arma::mat& H2 = H->TwoBody.GetMatrix(ch);
    // decouple Gamma_qcvc'
    for (auto& iket : tbc.GetKetIndex_vc()) {
      ...
      for (auto& ibra : tbc.GetKetIndex_qc()) {
        Ket& bra = tbc.GetKet(ibra);
        //         std::cout << bra.p << " " << bra.q << " " << ket.p << " " <<
        //         ket.q << std::endl;
        if ((ket.p == bra.p) or (ket.p == bra.q) or (ket.q == bra.p) or
            (ket.q == bra.q))
          continue;
        double denominator = Get2bDenominator(ch, ibra, iket);
        //         ETA2(ibra,iket) = 0.5*atan( 2*H2(ibra,iket) / denominator );
        ...
        //         std::cout << "   qcvc': " << ket.p << " " << ket.q << " " <<
        //         bra.p << " " << bra.q << "    " << ETA2(ibra,iket) <<
        //         std::endl;
      }
    }
    ...
    for (auto& iket : tbc.GetKetIndex_cc()) {
      Ket& ket = tbc.GetKet(iket);
      for (auto& ibra : imsrg_util::VectorUnion(tbc.GetKetIndex_vc(),
                                                tbc.GetKetIndex_qc())) {
        Ket& bra = tbc.GetKet(ibra);
        if ((ket.p == bra.p) or (ket.p == bra.q) or (ket.q == bra.p) or
            (ket.q == bra.q))
...
imsrg_util::VectorUnion(modelspace->valence,modelspace->qspace) )
      {
         double denominator = Get1bDenominator(i,a);
         Eta->OneBody(i,a) =  RspaceRegulator.OneBody(i,a) *
0.5*atan(2*H->OneBody(i,a)/denominator); Eta->OneBody(a,i) = -
Eta->OneBody(i,a);
      }
   }
   ...
   for (int ch=0;ch<Eta->nChannels;++ch)
   {
      ...
      for ( auto& iket : tbc.GetKetIndex_cc() )
      {
         for ( auto& ibra : imsrg_util::VectorUnion(tbc.GetKetIndex_qq(),
tbc.GetKetIndex_vv(), tbc.GetKetIndex_qv() ) )
         {
            double denominator = Get2bDenominator(ch,ibra,iket);
//            double denominator = Get2bDenominator_Jdep(ch,ibra,iket);
            ETA2(ibra,iket) =  R2(ibra,iket)  *  0.5*atan(2*H2(ibra,iket) /
denominator); ETA2(iket,ibra) = - ETA2(ibra,iket) ; // Eta needs to be
antisymmetric
         }
      }
    }
...
//  std::cout << "Succeeded in creating RspaceRegulator" << std::endl;
  double oscillator_b_squared = HBARC*HBARC / M_NUCLEON /
modelspace->GetHbarOmega(); double x0 = r0*r0/oscillator_b_squared; // express
length in units of oscillator length: r0^2 = x0 * b^2
...
//  for (int a=0; a<norb; a++)
  for (auto a : modelspace->all_orbits )
  {
    ...
    for ( int b : RspaceRegulator.OneBodyChannels.at({oa.l, oa.j2, oa.tz2}) )
    {
      ...
      {
        double I_p = gsl_sf_gamma_inc_P(p+1.5,x0);  // Talmi integral for a step
function Theta(r0 -r ); double B_p = AngMom::TalmiB(oa.n, oa.l, ob.n, ob.l, p);
        matel += I_p * B_p;
      }
...
//  #pragma omp parallel for schedule(dynamic,1)  // In order to make this
parallel, need to precompute the Moshinsky brackets. for (int ch=0; ch<nchan;
++ch)
  {
...
tranformation for (int N_ab=0; N_ab<=fab/2; ++N_ab)  // N_ab = CoM n for a,b
               {
                 for (int Lam_ab=0; Lam_ab<= fab-2*N_ab; ++Lam_ab) // Lam_ab =
CoM l for a,b
                 {
                   int Lam_cd = Lam_ab; // the projector depends only on the
relative distance r, so center of mass bit must be the same. int N_cd = N_ab;
                   for (int lam_ab=(fab-2*N_ab-Lam_ab)%2; lam_ab<=
(fab-2*N_ab-Lam_ab); lam_ab+=2) // lam_ab = relative l for a,b
                   {
...
std::abs(bra.op->tz2+ket.oq->tz2)*modelspace->phase( lam_ab + Sab ))/ 2; if (
asymm_factor ==0 ) continue;

                      int lam_cd = lam_ab; // conserve lam and Lam
                      int n_ab = (fab - 2*N_ab-Lam_ab-lam_ab)/2; // n_ab is
determined by energy conservation int n_cd = (fcd - 2*N_cd-Lam_cd-lam_cd)/2; //
n_cd is determined by energy conservation
...
modelspace->GetMoshinsky(N_ab,Lam_ab,n_ab,lam_ab,na,la,nb,lb,Lab);
                      ...
                      double mosh_cd =
modelspace->GetMoshinsky(N_cd,Lam_cd,n_cd,lam_cd,nc,lc,nd,ld,Lcd); if
(std::abs(mosh_cd)<1e-8) continue;
//                      std::cout << "n_ab, lam_ab = " << n_ab << " " << lam_ab
<< "    n_cd, lam_cd = " << n_cd << " " << lam_cd << std::endl;

                      double me_rel = 0;
                      for (int p=oa.l; p<= oa.n+ob.n+oa.l; p++)
                      {
                        double I_p = gsl_sf_gamma_inc_P(p+1.5,x0);  // Talmi
integral for a step function Theta(r0 -r ); double B_p = AngMom::TalmiB(oa.n,
oa.l, ob.n, ob.l, p); me_rel += I_p * B_p;
                      }


//                      int i_equiv = modelspace->GetOrbitIndex( n_ab, lam_ab,
2*lam_ab+1, 1 );  // the matrix element is independent of spin/isospin, so just
pick one.
//                      int j_equiv = modelspace->GetOrbitIndex( n_cd, lam_cd,
2*lam_cd+1, 1 );  // the matrix element is independent of spin/isospin, so just
pick one.

                      double prefactor = njab * njcd * mosh_ab * mosh_cd *
asymm_factor; matel +=  me_rel * prefactor ; // don't need to do the whole Talmi
integral dance. We already did that.
//                      matel +=  RspaceRegulator.OneBody(i_equiv,j_equiv) *
prefactor ; // don't need to do the whole Talmi integral dance. We already did
that.
...
//  std::cout << "Done with the 2b part. Norm is " <<
RspaceRegulator.OneBodyNorm() << "  " << RspaceRegulator.TwoBodyNorm() <<
std::endl;
