...
class HFMBPT : public HartreeFock {
 public:
  arma::mat C_HO2NAT;  // transforamtion coefficients, 1st index ho, 2nd index
                       // NAT basis
  arma::mat C_HF2NAT;  // transforamtion coefficients, 1st index hf, 2nd index
                       // NAT basis
  arma::vec Occ;       // Occupation number

  bool use_NAT_occupations;  // Option to use occupations from (if true) density
                             // matrix, or (if false) use naive filling.
  ...
  void PrintSPEandWF();  // Function override, since we want to express the SPWF
                         // in terms of HO states
  void ReorderHFMBPTCoefficients();
  void UseNATOccupations(bool tf = true) {
    use_NAT_occupations = tf;
  };  // Choose whether to use occupations from rho.
  void OrderNATBy(std::string order) {
    NAT_order = order;
  };  // Choose how to label orbits ("occupation", "energy", "mp2")
