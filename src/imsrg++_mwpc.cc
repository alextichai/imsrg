...
int main(int argc, char **argv) {
  ...
  bool nucleon_mass_correction =
      parameters.s("nucleon_mass_correction") == "true";
  bool relativistic_correction =
      parameters.s("relativistic_correction") == "true";
  ...
  bool use_NAT_occupations =
      (parameters.s("use_NAT_occupations") == "true") ? true : false;
  bool order_NAT_by_energy =
      (parameters.s("order_NAT_by_energy") == "true") ? true : false;
  ...
  bool write_HO_ops =
      parameters.s("write_HO_ops") == "true"; // added by Antoine Belley
  bool write_HF_ops =
      parameters.s("write_HF_ops") == "true"; // added by Antoine Belley

  int eMax = parameters.i("emax");
  int lmax =
      parameters.i("lmax"); // so far I only use this with atomic systems.
  ...
  ////  if (e3Max_imsrg==-1 and eMax_imsrg != -1) e3Max_imsrg = std::min(E3max,
  /// 3*eMax_imsrg);
  ...
  // unpack the awkward input format for reading an operator from file, and put
  // it into a struct. the format should look like
  // OpName^j_t_p_r^/path/to/2bfile^/path/to/3bfile  if particle rank of Op is
  // 2-body, then 3bfile is not needed.
  ...
  for (auto &tag : opsfromfile) {
    ...
    std::cout << "Parsed tag. opname = " << opff.opname << "  " << opff.j << " "
              << opff.t << " " << opff.p << " " << opff.r
              << "   file2 = " << opff.file2name
              << "    file3 = " << opff.file3name << std::endl;

    // now make sure the files exist before we add them to the list.

    //     if( not std::ifstream(f2name).good() )
    if (opff.file2name != "") {
      if (!std::ifstream(opff.file2name).good()) {
        //       std::cout << "trouble reading " << f2name << " exiting. " <<
        //       std::endl;
        std::cout << "trouble reading " << opff.file2name << " exiting. "
                  << std::endl;
        return 1;
      }
    }

    // is there a 3-body file too?
    if (opff.file3name != "") {
      //       getline(ss,f3name,'^');
      //       if( not std::ifstream(f3name).good() )
      if (!std::ifstream(opff.file3name).good()) {
        std::cout << "trouble reading " << opff.file3name << " exiting. "
                  << std::endl;
        //         std::cout << "trouble reading " << f3name << " exiting. " <<
        //         std::endl;
        return 1;
      }
    }
  ...
  if (((method == "magnus") || (method == "magnus_backoff")) &&
      ((opnames.size() + opsfromfile.size()) > 0)) {
    if ((scratch == "/dev/null") || (scratch == "/dev/null/")) {
      ...
      testin >> checkpassed;
      if ((checkpassed != "PASSED") or (not testout.good()) or
          (not testin.good())) {
  ...
  if (custom_valence_space != "") // if a custom space is defined, the input
  ...
  // For both dagger operators and single particle wave functions, it's
  // convenient to just get every orbit in the valence space. So if
  // SPWF="valence" ,  we append all valence orbits
  if (std::find(spwf.begin(), spwf.end(), "valence") != spwf.end()) {
    // this erase/remove idiom is needed because remove just shuffles things
    // around rather than actually removing it.
    spwf.erase(std::remove(spwf.begin(), spwf.end(), "valence"),
               std::end(spwf));
  ...
  if (std::find(opnames.begin(), opnames.end(), "rhop_all") != opnames.end()) {
    opnames.erase(std::remove(opnames.begin(), opnames.end(), "rhop_all"),
                  std::end(opnames));
  ...
  if (std::find(opnames.begin(), opnames.end(), "rhon_all") != opnames.end()) {
    opnames.erase(std::remove(opnames.begin(), opnames.end(), "rhon_all"),
                  std::end(opnames));
    for (double r = 0.0; r <= 10.0; r += 0.2) {
      std::ostringstream opn;
      opn << "rhon_" << r;
  ...
  if (inputtbme == "none" and physical_system == "atomic") {
    ...
    Hbare -= Z * imsrg_util::VCentralCoulomb_Op(modelspace, lmax) *
             sqrt((M_ELECTRON * 1e6) / M_NUCLEON);
    Hbare += imsrg_util::VCoulomb_Op(modelspace, lmax) *
             sqrt((M_ELECTRON * 1e6) /
                  M_NUCLEON); // convert oscillator length from fm with nucleon
                              // mass to nm with electon mass (in eV).
    Hbare += imsrg_util::KineticEnergy_Op(
        modelspace); // Don't need to rescale this, because it's related to the
                     // oscillator frequency, which we input.
    Hbare /= PhysConst::HARTREE; // Convert to Hartree
  }

  if (fmt2 != "nushellx" and physical_system != "atomic" and
      hw_trap < 0) // Don't need to add kinetic energy if we read a shell model
                   // interaction
  {
  ...
  if (hw_trap > 0) {
    Hbare += 0.5 * (PhysConst::M_NUCLEON * hw_trap * hw_trap) /
             (PhysConst::HBARC * PhysConst::HBARC) *
             imsrg_util::RSquaredOp(modelspace);
    Hbare += imsrg_util::KineticEnergy_Op(
        modelspace); // use lab-frame kinetic energy
  }
  ...
  if ((IMSRG3) and (Hbare.ThreeBodyNorm() > 1e-5))
    hno_particle_rank = 3;
  ...
  if (basis == "HF" and method != "HF") {
    ...
    if ((IMSRG3 or perturbative_triples) and OccNat3Cut > 0)
  ...
  if (perturbative_triples) {
    ...
    std::cout << "We will compute perturbative triples corrections"
              << std::endl;
    std::cout << "Truncations: dE3max = " << dE3max
              << "   OccNat3Cut = " << std::scientific << OccNat3Cut
  ...
  if (IMSRG3) {
    ...
    if (hno_particle_rank <
        3) // if we're doing IMSRG3, we need a 3 body operator
    {
  ...
  HNO -= BetaCM * 1.5 * hwBetaCM; // This is just the zero-body piece. The other
                                  // stuff was added earlier.
  ...
  // Calculate all the desired operators. If we're using magnus, we'll do this
  // after the flow is over
  // TODO(mheinz): revert this logic to standard magnus logic and shift backoff
  // details into solver.
  if (!((method == "magnus") || (method == "magnus_backoff"))) {
    ...
    // Calculate first order perturbative correction to some operators, if
    // that's what we asked for. Strictly speaking, it doesn't make much sense
    // to do this and then proceed with the IMSRG calculation, but I'm not here
    // to tell people what to do...
    for (auto &opnamept1 : opnamesPT1) {
      ops.emplace_back(imsrg_util::FirstOrderCorr_1b(
          imsrg_util::OperatorFromString(modelspace, opnamept1), HNO));
      opnames.push_back(opnamept1 + "PT1");
    }
    for (auto &opnametda :
         opnamesTDA) { // passing the argument "TDA" just sets the phhp and hpph
                       // blocks to zero in the RPA calculation
      ops.emplace_back(imsrg_util::RPA_resummed_1b(
          imsrg_util::OperatorFromString(modelspace, opnametda), HNO, "TDA"));
      opnames.push_back(opnametda + "TDA");
    }
    for (auto &opnamerpa : opnamesRPA) {
      ops.emplace_back(imsrg_util::RPA_resummed_1b(
          imsrg_util::OperatorFromString(modelspace, opnamerpa), HNO, "RPA"));
    ...
    for (auto &opff : opsfromfile_unpacked) {
      ...
      if (input_op_fmt == "navratil") {
        ...
        if (opff.r > 2 and opff.file3name != "")
          rw.Read_Darmstadt_3body(opff.file3name, op, file3e1max, file3e2max,
                                  file3e3max);
        ...
        if (opff.r > 2 and opff.file3name != "") {
          ...
          op.ThreeBody.ReadFile({opff.file3name}, {file3e1max, file3e2max,
                                                   file3e3max, file3e1max});
          // rw.Read_Darmstadt_3body( opff.file3name, op,
    ...
    for (size_t i = 0; i < ops.size(); ++i) {
      //     std::cout << "Before transforming  " << opnames[i] << " has 3b norm
      //     " << ops[i].ThreeBodyNorm() << std::endl;
      // We don't transform a DaggerHF, because we want the a^dagger to already
      // refer to the HF basis.
      if ((basis == "HF") and
          (opnames[i].find("DaggerHF") == std::string::npos)) {
        ops[i] = hf.TransformToHFBasis(ops[i]);
      } else if ((basis == "NAT") and
                 (opnames[i].find("DaggerHF") == std::string::npos)) {
        ops[i] = hf.TransformHOToNATBasis(ops[i]);
      }
      //     std::cout << "After transforming  " << opnames[i] << " has 3b norm
      //     " << ops[i].ThreeBodyNorm() << std::endl;
      ops[i] = ops[i].DoNormalOrdering();
      //     std::cout << "Before normal ordering  " << opnames[i] << " has 3b
      //     norm " << ops[i].ThreeBodyNorm() << std::endl;
      std::cout << basis << " expectation value  " << opnames[i] << "  "
                << ops[i].ZeroBody << std::endl;
      if (method == "MP3") {
        double dop = ops[i].MP1_Eval(HNO);
        std::cout << "Operator 1st order correction  " << dop << "  ->  "
                  << ops[i].ZeroBody + dop << std::endl;
      }
      if (opnames[i] == "Rp2") {
        ...
        std::cout << " HF charge radius = "
                  << (abs(Rp2) < 1e-6
                          ? 0.0
                          : sqrt(Rp2 + PROTON_RCH2 +
  ...
  if (method == "FCI") {
    if (valence_file_format == "tokyo") {
      ...
      // Haven't yet implemented FCI operators for Tokyo format. I should do
      // this...
      for (size_t i = 0; i < ops.size(); i++) {
    ...
    } else // Write in NuShellX Format
    {
      ...
      std::cout << "HNO had zero body = " << HNO.ZeroBody << "  and I add "
                << previous_zero_body << " to it. " << std::endl;
      HNO.ZeroBody += previous_zero_body;

      rw.WriteNuShellX_int(HNO, intfile + ".int");
      rw.WriteNuShellX_sps(HNO, intfile + ".sp");

      std::cout << "NO wrt vacuum. One Body term is hopfully still diagonal?"
                << std::endl
                << HNO.OneBody << std::endl;
      ...
      for (index_t i = 0; i < ops.size(); ++i) {
        ...
        if ((ops[i].GetJRank() + ops[i].GetTRank() + ops[i].GetParity()) < 1) {
          ...
          rw.WriteTensorOneBody(intfile + opnames[i] + "_1b.op", ops[i],
                                opnames[i]);
          rw.WriteTensorTwoBody(intfile + opnames[i] + "_2b.op", ops[i],
                                opnames[i]);
        }
      }
    }
  ...
  if (only_2b_omega) {
    std::cout << " Restricting the Magnus operator Omega to be 2b."
              << std::endl;
  ...
  if ((eMax_imsrg != -1) or (e2Max_imsrg != -1) or (e3Max_imsrg != -1) or
      (eMax_3body_imsrg != -1)) {
    ...
    //     if (physical_system == "atomic")
    //     modelspace_imsrg.InitSingleSpecies(eMax_imsrg, eMax_imsrg,
    //     e3Max_imsrg, reference, valence_space); if (occ_file != "none" and
    //     occ_file != "" )
    //     modelspace_imsrg.Init_occ_from_file(eMax_imsrg,e2Max_imsrg,e3Max_imsrg,valence_space,occ_file);
    ...
    //     modelspace = modelspace_imsrg;  // this could cause some confusion
    //     later on...
    //    hf.PrintSPEandWF();
  } else {
  ...
  if (threebody_threshold > 1e-12) {
    std::cout << "skipping IMSRG(3) commutator terms if norm of either "
                 "operator is below "
              << threebody_threshold << std::endl;
  }
  ...
  if (method == "flow" or method == "flow_RK4") {
    ...
    std::cout << " Added ops. FlowingOps.size = "
              << imsrgsolver.FlowingOps.size() << std::endl;
  }

  imsrgsolver.SetGenerator(core_generator);
  if (core_generator.find("imaginary") != std::string::npos or
  ...
  if (perturbative_triples and
      ((method == "magnus") || (method == "magnus_backoff"))) {
    //    modelspace.SetdE3max(dE3max);
    //    modelspace.SetOccNat3Cut(OccNat3Cut);
    //    size_t nstates_kept = modelspace.CountThreeBodyStatesInsideCut();
    //    std::array<size_t,2> nstates =
    //    modelspace.CountThreeBodyStatesInsideCut(); std::cout << "Truncations:
    //    dE3max = " << dE3max << "   OccNat3Cut = " << std::scientific <<
    //    OccNat3Cut << "  ->  number of 3-body states kept:  " << nstates[0] <<
    //    " out of " << nstates[1] << std::endl << std::fixed;
    double dE_triples = imsrgsolver.GetPerturbativeTriples();
    std::cout << "Perturbative triples:  " << std::setw(16)
              << std::setprecision(8) << dE_triples << " -> "
              << imsrgsolver.GetH_s().ZeroBody + dE_triples << std::endl;
  }
  ...
  if (nsteps > 1 and
      valence_space != reference) // two-step decoupling, do core first
  {
    if ((method == "magnus") || (method == "magnus_backoff"))
      smax *= 2;
    ...
    if (valence_generator.find("imaginary") != std::string::npos or
  ...
  if (imsrg3_at_end) {
    if (method.find("magnus") != std::string::npos) {
      std::cout << "Performing final BCH transformation at the IMSRG(3) level"
                << std::endl;

      //      modelspace.SetdE3max(dE3max);
      //      modelspace.SetOccNat3Cut(OccNat3Cut);
      //      int new_E3max = std::min(modelspace.GetE3max(), int(
      //      std::ceil(3*std::max( modelspace.GetEFermi()[-1],
      //      modelspace.GetEFermi()[+1])+dE3max)));
      modelspace_imsrg.SetdE3max(dE3max);
      modelspace_imsrg.SetOccNat3Cut(OccNat3Cut);
      int new_E3max = std::min(
          modelspace_imsrg.GetE3max(),
          int(std::ceil(3 * std::max(modelspace_imsrg.GetEFermi()[-1],
                                     modelspace_imsrg.GetEFermi()[+1]) +
                        dE3max)));
      ...
      // Now throw away the residual 3-body so we don't need to keep it after
      // re-normal ordering
      H_with_3.SetNumberLegs(4);
      H_with_3.SetParticleRank(2);
      imsrgsolver.FlowingOps[0] = H_with_3;
    } else {
      std::cout << "selected imsrg3_at_end, but method != magnus, so I don't "
                   "know what to do. Ignoring."
                << std::endl;
    }
  }
  ...
  //      rw.WriteOperatorHuman(ops[i],intfile+opnames[i]+"_step2.op");
    ...
    if (method == "flow" or method == "flow_RK4" )
    {
      for (size_t i=0;i<ops.size();++i)
      {
        ops[i] = imsrgsolver.GetOperator(i+1);  // the zero-th operator is the
  Hamiltonian
      }
    }
  */
  ...
  //  if (modelspace.valence.size() > 0 or basis=="NAT")
  {
    ...
    if (not renormal_order) {
      ...
      for (auto c : modelspace_imsrg.core) {
        //         if ( (find( modelspace.holes.begin(), modelspace.holes.end(),
        //         c) == modelspace.holes.end()) or
        //         (std::abs(1-modelspace.GetOrbit(c).occ)>1e-6))
        if ((find(modelspace_imsrg.holes.begin(), modelspace_imsrg.holes.end(),
                  c) == modelspace_imsrg.holes.end()) or
  ...
  if (renormal_order) {
    ...
    //    imsrgsolver.GetNOmegaWritten(); std::cout << "Undoing NO wrt A=" <<
    //    modelspace.GetAref() << " Z=" << modelspace.GetZref() << std::endl;
    ...
    //    for ( auto i : modelspace.all_orbits ) std::cout << "  " << i << " : "
    //    << HNO.OneBody(i,i) << std::endl;
    ...
    //    imsrgsolver.Solve();
    // Change operators to the new basis, then apply the rest of the
    // transformation
  ...
  if (modelspace_imsrg.valence.size() > 0) {
    if (valence_file_format == "antoine") // this is still being tested...
    {
      ...
      rw.WriteAntoine_input(imsrgsolver.GetH_s(), intfile + ".inp",
                            modelspace_imsrg.GetAref(),
                            modelspace_imsrg.GetZref());
    }
    ...
    //       {
    //          if (
    //          ((ops[i].GetJRank()+ops[i].GetTRank()+ops[i].GetParity())<1) and
    //          (ops[i].GetNumberLegs()%2==0) )
    //          {
    //            if (valence_file_format == "tokyo")
    //            {
    ...
    //            }
    //          }
    //          else if ( ops[i].GetNumberLegs()%2==1) // odd number of legs ->
    //          this is a dagger operator
    //          {
    ////            rw.WriteNuShellX_op(ops[i],intfile+opnames[i]+".int"); // do
    /// this for now. later make a *.dag format.
    //            rw.WriteDaggerOperator( ops[i],
    //            intfile+opnames[i]+".dag",opnames[i]);
    //          }
    //          else
    //          {
    //            if (valence_file_format == "tokyo")
    ...
    //       }
    //    }
  } else // single ref. just print the zero body pieces out. (maybe check if its
         // magnus?)
  {
    ...
    if (!((method == "magnus") || (method == "magnus_backoff"))) {
      for (index_t i = 0; i < ops.size(); ++i) {
        ...
        Operator &op =
            imsrgsolver
                .FlowingOps[i + 1]; // the first operator is the Hamiltonian
        ...
        if (opnames[i] == "Rp2") {
          ...
          std::cout << " IMSRG charge radius = "
                    << sqrt(op.ZeroBody + PROTON_RCH2 +
                            NEUTRON_RCH2 * (A - Z) / Z + DARWIN_FOLDY)
                    << std::endl;
        }
        if ((op.GetJRank() > 0) or
            (op.GetTRank() >
             0)) // if it's a tensor, you probably want the full operator
        {
          std::cout << "Writing operator to " << intfile + opnames[i] + ".op"
                    << std::endl;
  ...
  if ((method == "magnus") || (method == "magnus_backoff")) {
    ...
    for (size_t i = 0; i < opnames.size(); ++i) {
      ...
      if (opname.find("_FROMFILE") != std::string::npos) {
        ...
        std::cout << "reading " << opff.opname << " with " << opff.j << " "
                  << opff.t << " " << opff.p << " " << opff.r << "  from file "
                  << opff.file2name << std::endl;
        ...
        if (input_op_fmt == "navratil") {
          ...
          if (opff.file2name != "") {
            Operator optmp =
                rw.ReadOperator2b_Miyagi(opff.file2name, modelspace);
            op.TwoBody = optmp.TwoBody;
          }
          if (opff.r > 2 and opff.file3name != "")
            rw.Read_Darmstadt_3body(opff.file3name, op, file3e1max, file3e2max,
      ...
      if (write_HO_ops) {
        ...
        if (valence_file_format == "tokyo") {
          ...
          rw.WriteTensorOneBody(intfile + opnames[i] + "_HO_1b.op", op,
                                opnames[i]);
          rw.WriteTensorTwoBody(intfile + opnames[i] + "_HO_2b.op", op,
                                opnames[i]);
        }
      }
      ...
      if (write_HF_ops) {
        ...
        if (valence_file_format == "tokyo") {
          ...
          rw.WriteTensorOneBody(intfile + opnames[i] + "_HF_1b.op", op,
                                opnames[i]);
          rw.WriteTensorTwoBody(intfile + opnames[i] + "_HF_2b.op", op,
                                opnames[i]);
        }
      }
      ...
      //      rw.WriteOperatorHuman(ops[i],intfile+opnames[i]+"_step2.op");
      //      std::cout << "After renormal ordering Op(5,4) is " <<
      //      std::setprecision(10) << op.OneBody(5,4) << std::endl;

      std::cout << "      " << op.GetJRank() << " " << op.GetTRank() << " "
                << op.GetParity() << "   " << op.GetNumberLegs() << std::endl;
      if (((op.GetJRank() + op.GetTRank() + op.GetParity()) < 1) and
      ...
      } else if (op.GetNumberLegs() % 2 ==
                 1) // odd number of legs -> this is a dagger operator
      {
        //      rw.WriteNuShellX_op(ops[i],intfile+opnames[i]+".int"); // do
        //      this for now. later make a *.dag format.
  ...
  if (method == "flow" or method == "flow_RK4") {
    for (size_t i = 0; i < ops.size(); ++i) {
      auto op = imsrgsolver.GetOperator(
          i + 1); // the zero-th operator is the Hamiltonian
      auto opname = opnames[i];
  ...
  //  std::cout << "Made it here and write_omega is " << write_omega <<
  //  std::endl;
  if (write_omega) {
