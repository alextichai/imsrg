...
ThreeBodyChannelNO2B::ThreeBodyChannelNO2B(int J2, int P2, int J1, int P1,
                                           int T3, ThreeBodySpaceNO2B& thr)
...
void ThreeBodyChannelNO2B::UnHash_abct(int key, int& a, int& b, int& c,
                                       int& Tab) const {
...
ThreeBodySpaceNO2B::ThreeBodySpaceNO2B() : ThreeBodyChannels(), NChannels(-1) {}
...
int ThreeBodySpaceNO2B::Hash_Channel(int J2, int P2, int J1, int P1,
                                     int T3) const {
  return (J2 + (J1 << 8) + (P2 << 16) + (P1 << 17) + (T3 << 18));
}

void ThreeBodySpaceNO2B::UnHash_Channel(int key, int& J2, int& P2, int& J1,
                                        int& P1, int& T3) const {
  J2 = ((key >> 0) & 255);
  J1 = ((key >> 8) & 255);
  P2 = ((key >> 16) & 1);
...
int ThreeBodySpaceNO2B::GetChannelIndex(int Jab, int Pab, int Jc, int Pc,
                                        int T) const {
...
//////////////////////////////////////////////////////////////////////////////////////////
//////  Begin implementation of ThreeBodyStorage_no2b methods. This is the main
///event.
////////////////////////////////////////////////////////////////////////////////////////

template <class StoreType>
ThreeBodyStorage_no2b<StoreType>::ThreeBodyStorage_no2b(
    const ThreeBodyStorage_no2b<StoreType>& TBS_in)
...
template <class StoreType>
std::shared_ptr<ThreeBodyStorage> ThreeBodyStorage_no2b<StoreType>::Clone()
    const {
  return std::shared_ptr<ThreeBodyStorage>(
      new ThreeBodyStorage_no2b<StoreType>(*this));
};
...
template <class StoreType>
void ThreeBodyStorage_no2b<StoreType>::SetME_iso_no2b(
    int a, int b, int c, int Tab, int d, int e, int f, int Tde, int J2,
    int twoT, ThreeBodyStorage::ME_type V) {
...
// with a weight 2J+1
template <class StoreType>
ThreeBodyStorage::ME_type ThreeBodyStorage_no2b<StoreType>::GetME_iso_no2b(
    int a, int b, int c, int Tab, int d, int e, int f, int Tde, int J2,
    int twoT) const {
  ...
  int key_bra = ch_no2b.GetIndex(a, b, c, Tab);
  int key_ket = ch_no2b.GetIndex(d, e, f, Tde);
  if (ch_no2b.iphase.find(key_bra) == ch_no2b.iphase.end())
    return vout;  // return 0.
  if (ch_no2b.iphase.find(key_ket) == ch_no2b.iphase.end())
    return vout;  // return 0.
  int ph = ch_no2b.GetPhase(key_bra) * ch_no2b.GetPhase(key_ket);
  int ibra = ch_no2b.abct2n.at(key_bra);
  int iket = ch_no2b.abct2n.at(key_ket);
...
template <class StoreType>
ThreeBodyStorage::ME_type ThreeBodyStorage_no2b<StoreType>::GetME_pn_no2b(
    int a, int b, int c, int d, int e, int f, int J2) const {
  ...
  for (int T12 : {0, 1}) {
    ...
    for (int T45 : {0, 1}) {
      ...
      for (int twoT = 1; twoT <= std::min(2 * T12 + 1, 2 * T45 + 1);
           twoT += 2) {
        vout += GetME_iso_no2b(i1, i2, i3, T12, i4, i5, i6, T45, J2, twoT) *
                AngMom::CG(0.5, oa.tz2 * 0.5, 0.5, ob.tz2 * 0.5, T12,
                           (oa.tz2 + ob.tz2) * 0.5) *
                AngMom::CG(0.5, od.tz2 * 0.5, 0.5, oe.tz2 * 0.5, T45,
                           (od.tz2 + oe.tz2) * 0.5) *
                AngMom::CG(T12, (oa.tz2 + ob.tz2) * 0.5, 0.5, oc.tz2 * 0.5,
                           twoT * 0.5, Z3 * 0.5) *
                AngMom::CG(T45, (od.tz2 + oe.tz2) * 0.5, 0.5, of.tz2 * 0.5,
                           twoT * 0.5, Z3 * 0.5);
      }
    }
  }
...
template <class StoreType>
size_t ThreeBodyStorage_no2b<StoreType>::CountME(
        ...
        for (int i4 = 0; i4 <= i1; i4++) {
          ...
          for (int i5 = 0; i5 <= i4; i5++) {
            ...
            for (int i6 = 0; i6 < Norbs; i6++) {
              ...
              size_t JT_block_size =
                  (Jmax + 1 - Jmin) *
                  ISOSPIN_BLOCK_DIMENSION;  // ISOSPIN_BLOCK_DIMENSION inherited
                                            // form base class
...
template <class StoreType>
void ThreeBodyStorage_no2b<StoreType>::ReadFile(
          ...
          for (int iF4 = 0; iF4 <= iF1; iF4++) {
            ...
            for (int iF5 = 0; iF5 <= iF4; iF5++) {
              ...
              for (int iF6 = 0; iF6 < Norbs_file; iF6++) {
                ...
                int Jmin = std::max(std::abs(j1 - j2), std::abs(j4 - j5)) / 2;
                int Jmax = std::min(j1 + j2, j4 + j5) / 2;
                //                 int JT_block_size = (Jmax+1-Jmin) * 5; // 5
                //                 comes from the 5 possible isospin
                //                 combinations 001 011 101 111 113 int
...
#pragma omp barrier
                          if (this_thread == 0) {
                            ...
                            if (filemode == "bin")
                              infile.read((char*)&vbuf[0],
                                          n_this_pass * nwords);
                            else if (filemode == "gz")
                              for (size_t iread = 0; iread < n_this_pass;
                                   iread++)
...
#pragma omp barrier
                        ...
                        if ((i1 == i2 and (J + T12) % 2 == 0) or
                            (i4 == i5 and (J + T45) % 2 == 0)) {
                          if (abs(vset) > 1.e-6) {
                            std::cout
                                << "Warning: something wrong, this three-body "
                                   "matrix element has to be zero"
                                << std::endl;
                            std::cout
                                << std::setw(4) << i1 << std::setw(4) << i2
                                << std::setw(4) << i3 << std::setw(4) << T12
                                << std::setw(4) << i4 << std::setw(4) << i5
                                << std::setw(4) << i6 << std::setw(4) << T45
                                << std::setw(4) << J << std::setw(4) << T3
                                << std::setw(16) << counter << std::setw(12)
                                << std::setprecision(6) << vset << std::endl;
                          }
                        }

                        if (std::abs(vset) > 1e-8) {
                          SetME_iso_no2b(i1, i2, i3, T12, i4, i5, i6, T45, J,
                                         T3, vset);
                        }
  ...
  IMSRGProfiler::timer["ThreeBodyStorage_no2b::ReadFile"] +=
      omp_get_wtime() - t_start;
}

// Make our concrete instantiations of the ThreeBodyStorage_no2b class. We use
