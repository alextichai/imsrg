...
/// Transform operators and write them
  ...
  if ((method == "magnus") || (method == "magnus_backoff"))
  {
    ...
    // Evaluating Sum Rules for the monopole operator
    if(sum_rule)
    {
      // fetch Hamiltonian
      ...
      Operator Monopole = imsrg_util::RSquaredOp(modelspace);
      //Operator M0 = imsrg_util::mom0(modelspace);

      Operator M0 = imsrg_util::QdotQ(modelspace, 0, "isoscalar", 2);
      Operator Quadrupole = imsrg_util::QdotQ(modelspace, 2, "isoscalar");
      Operator Octupole   = imsrg_util::QdotQ(modelspace, 1, "isoscalar");

      // PRINT MONOPOLE VALUES FOR POLARISABILITY SOMEWHERE

      // DON'T REPEAT CALCULATIONS FIFTY TIMES, IF POL!=0 THEN NO OTHER MOMENTS

      if (basis == "oscillator")
      {
        Monopole = Monopole.DoNormalOrdering();
        M0 = M0.DoNormalOrdering();
        Quadrupole = Quadrupole.DoNormalOrdering();
        Octupole = Octupole.DoNormalOrdering();
      }
      else if (basis == "HF")
      {
        Monopole = hf.TransformToHFBasis(Monopole).DoNormalOrdering();
        M0 = hf.TransformToHFBasis(M0).DoNormalOrdering();
        Quadrupole = hf.TransformToHFBasis(Quadrupole).DoNormalOrdering();
        Octupole = hf.TransformToHFBasis(Octupole).DoNormalOrdering();
      }
      else if ( basis == "NAT")
      {
        Monopole = hf.TransformHOToNATBasis(Monopole).DoNormalOrdering();
        M0 = hf.TransformHOToNATBasis(M0).DoNormalOrdering();
        Quadrupole = hf.TransformHOToNATBasis(Quadrupole).DoNormalOrdering();
        Octupole = hf.TransformHOToNATBasis(Octupole).DoNormalOrdering();
      }

      Operator op_s = imsrgsolver.Transform(Monopole);
      Operator qu_s = imsrgsolver.Transform(Quadrupole);
      Operator m0_s = imsrgsolver.Transform(M0);

      std::cout << Quadrupole.ZeroBody << std::endl;
      std::cout << Octupole.ZeroBody << std::endl;
      std::cout << qu_s.ZeroBody << std::endl;
      std::cout << M0.ZeroBody << std::endl;
      std::cout << m0_s.ZeroBody << std::endl;
      ...
      if(Monopole.rank_J == 0)
      {
        ...
        Operator S0 = Commutator::EvaluateCommutatorSumRule_op(Monopole, H_HF, 1); // Using both unevolved H and operator
        ...
        Operator Sn = imsrgsolver.Transform(S0);			  // Using the unevolved S0 and evolve it as a whole
      ...
      Monopole = Monopole.UndoNormalOrdering();
      // One-body part
      rw.Write_me1j(intfile + "_monopole.me1j", Monopole, emax_imsrg, emax_imsrg);
      // Two-body part
      if(Monopole.rank_J == 0)
        rw.Write_me2jp(intfile + "_monopole.me2jp", Monopole, emax_imsrg, 2* emax_imsrg, emax_imsrg);

      // H(s): generate vacuum represetation and write to file (HF basis)
      H_HF = H_HF.UndoNormalOrdering();
      Hs   = Hs.UndoNormalOrdering();
      // One-body part
      rw.Write_me1j(intfile + "_Hs.me1j", Hs, emax_imsrg, emax_imsrg);
      // Two-body part
      rw.Write_me2jp(intfile + "_Hs.me2jp", Hs, emax_imsrg, 2 * emax_imsrg, emax_imsrg);

      arma::mat CHO  = hf.C;
      arma::mat CHOt = CHO.t(); // Transformation from HF to HO basis

      // Transform H(0) and H(s) from the HF to the HO basis and write to file
      Operator H0_HO = H_HF.Transform(CHOt);
      Operator Hs_HO = Hs.Transform(CHOt);
      // One-body part
      rw.Write_me1j(intfile + "_H0.me1j_HO", H0_HO, emax_imsrg, emax_imsrg);
      rw.Write_me1j(intfile + "_Hs.me1j_HO", Hs_HO, emax_imsrg, emax_imsrg);
      // Two-body part
      rw.Write_me2jp(intfile + "_H0.me2jp_HO", H0_HO, emax_imsrg, 2 * emax_imsrg, emax_imsrg);
      rw.Write_me2jp(intfile + "_Hs.me2jp_HO", Hs_HO, emax_imsrg, 2 * emax_imsrg, emax_imsrg);
    }
