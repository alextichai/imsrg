...
void ThreeLegME::Allocate() {
  ...
  for (size_t ch = 0; ch < nChannels; ++ch) {
    ...
    MatEl[{ch}] = arma::mat(tbc.GetNumberKets(), modelspace->GetNumberOrbits(),
                            arma::fill::zeros);
  }
}

void ThreeLegME::Deallocate() { MatEl.clear(); }
...
double ThreeLegME::GetME_norm(size_t ch, size_t a, size_t b, size_t c) const {
  ...
  if (bra_ind < 0 or bra_ind > tbc.GetNumberKets()) return 0;
