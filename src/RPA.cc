...
void RPA::ConstructAMatrix_byIndex(size_t ich_CC) {
  ...
  for (auto iket_ai : tbc_CC.GetKetIndex_ph()) {
    ...
    index_t a = ket_ai.p;
    index_t i = ket_ai.q;
    std::cout << " a i = " << a << " " << i << "   spe: " << H.OneBody(a, a)
              << "   " << H.OneBody(i, i) << std::endl;
    ...
    if (ket_ai.op->occ >
        ket_ai.oq
            ->occ)  // if orbit a is more occupied than orbit i, switch them
    {
      std::swap(a, i);
      std::swap(ja, ji);
    ...
    for (auto iket_bj : tbc_CC.GetKetIndex_ph()) {
      ...
      //         double Delta_ijab = OneBody(i,i) + OneBody(j,j) - OneBody(a,a)
      //         - OneBody(b,b);
      double H1b =
          (iket_ai == iket_bj) ? H.OneBody(a, a) - H.OneBody(i, i) : 0.;
      ...
      if (AngMom::Triangle(jj, jb, Jph) and AngMom::Triangle(ji, ja, Jph)) {
        ...
        {
          //            V_aibj -= modelspace->GetSixJ(ja,ji,Jph,jb,jj,J1)  *
          //            (2*J1 + 1) *  H.TwoBody.GetTBME_J_norm(J1,a,j,b,i);
          V_aibj -= modelspace->GetSixJ(ja, ji, Jph, jb, jj, J1) *
                    (2 * J1 + 1) * H.TwoBody.GetTBME_J(J1, a, j, b, i);
        }
      }
      A(I_ph, II_ph) = H1b + V_aibj * phase_ai * phase_bj;
      //         Vbar_iabj(I_ph,II_ph) = tbme_iabj * phase_ia * phase_bj /
      //         Delta_ijab;
      II_ph++;
    }
    I_ph++;
...
/// Suhonen (11.70)
//  B_aibj(J) = (-1)^{jb+jb+J} sqrt(1+delta_ab)(1+delta_ij)} sum_J' (-1)^J'
//  (2J'+1)
//                 * {ja ji J } <ab; J' | V | ij; J'>
//                   {jb jj J'}
...
void RPA::SolveRPA() {
  ...
  arma::mat AB =
      arma::join_vert(arma::join_horiz(A, B), arma::join_horiz(-B, -A));
  arma::eig_gen(eigvals, eigvecs, AB);

  // now deal with the fact that we get spurious copies of each eigenvector with
  // a negative eigenvalue
  double normimag = arma::norm(arma::imag(eigvals), "fro");
  if (normimag > 1e-3) {
    std::cout << "WARNING: non-zero imaginary part of eigenvalues! " << __FILE__
              << " " << __func__ << " " << __LINE__ << std::endl;
  }
  arma::uvec positive_indices =
      arma::find(arma::real(eigvals) + arma::imag(eigvals) > 0);
  ...
  for (size_t mu = 0; mu < len; mu++) {
    ...
    //      double nxy = x2 - y2;
    std::cout << " mu = " << mu << "  norm = " << nxy << "   Xmin/max " << xmin
              << " " << xmax << "   Ymin/max " << ymin << " " << ymax
              << "  E = " << Energies(mu) << "  vs  " << Etmp(mu) << std::endl;
...
double RPA::TransitionToGroundState(Operator& OpIn, size_t mu) {
  ...
  size_t ich_CC = modelspace->GetTwoBodyChannelIndex(lambda, OpIn.GetParity(),
                                                     OpIn.GetTRank());
  TwoBodyChannel_CC& tbc_CC = modelspace->GetTwoBodyChannel_CC(ich_CC);
  //   int Jph = tbc_CC.J;
  ...
  for (auto iket_mi : tbc_CC.GetKetIndex_ph()) {
    ...
    mat_el += OpIn.OneBody(m, i) *
              (AngMom::phase(lambda) * X(I_mi, mu) + Y(I_mi, mu));
    //      std::cout << __func__ << "mu = " << mu << "   mi " << m << " " << i
    //      << "  X,Y,M " << X(I_mi,mu) << " " << Y(I_mi,mu) << " " <<
    //      OpIn.OneBody(m,i) << std::endl;
...
double RPA::PVCouplingEffectiveCharge(Operator& OpIn, size_t k, size_t l) {
  ...
  size_t ich_CC = modelspace->GetTwoBodyChannelIndex(
      OpIn.GetJRank(), OpIn.GetParity(), OpIn.GetTRank());
  TwoBodyChannel_CC& tbc_CC = modelspace->GetTwoBodyChannel_CC(ich_CC);
  size_t nkets_ph = tbc_CC.GetKetIndex_ph().size();

  //      std::cout << "A is a " << A.n_rows << "x" << A.n_cols << " matrix.   "
  //                << "X is a " << X.n_rows << "x" << X.n_cols << " matrix.   "
  //                << "E is a " << Energies.n_rows << "x" << Energies.n_cols <<
  //                " matrix.   " << std::endl;

  //  size_t mu_collective = Energies.index_min();

  if (nkets_ph != nbosons) {
    std::cout << "YIKES! TROUBLE " << __FILE__ << " " << __func__ << " "
              << __LINE__ << "  " << nkets_ph << " != " << nbosons << std::endl;
  }
  int Jph = tbc_CC.J;
  ...
  for (size_t mu = 0; mu < nbosons; mu++) {
    ...
    for (auto iket_mi : tbc_CC.GetKetIndex_ph()) {
      ...
      if (AngMom::Triangle(jk, jl, Jph) and AngMom::Triangle(ji, jm, Jph)) {
        for (int J1 = J1min; J1 <= J1max; ++J1)  // Pandya 1: <ai`| V |bj`>_Jtot
        {
          //            VA_milk -= modelspace->GetSixJ(jm,ji,Jph,jl,jk,J1)  *
          //            (2*J1 + 1) *  H.TwoBody.GetTBME_J_norm(J1,m,k,l,i);
          VA_milk -= modelspace->GetSixJ(jm, ji, Jph, jl, jk, J1) *
                     (2 * J1 + 1) * H.TwoBody.GetTBME_J(J1, m, k, l, i);
          //            if ( m==6 and i==0 and k==2 and l==2)
          //            {
          //              std::cout << " line " << __LINE__ << " mu = " << mu <<
          //              "  J1=" << J1 << " :  " << AngMom::phase(J1) << " * "
          //              << (2*J1 + 1)
          //                        << " * " <<
          //                        modelspace->GetSixJ(jm,ji,Jph,jl,jk,J1)<< "
          //                        * "  << H.TwoBody.GetTBME_J_norm(J1,m,k,l,i)
          //                        << "  => " << VA_milk << std::endl;
          //            }
        }
      }
      ...
      if (AngMom::Triangle(jl, jk, Jph) and AngMom::Triangle(ji, jm, Jph)) {
        for (int J1 = J1min; J1 <= J1max; ++J1)  // Pandya 1: <ai`| V |bj`>_Jtot
        {
          //            VA_mikl -= modelspace->GetSixJ(jm,ji,Jph,jk,jl,J1)  *
          //            (2*J1 + 1) *  H.TwoBody.GetTBME_J_norm(J1,m,l,k,i);
          VA_mikl -= modelspace->GetSixJ(jm, ji, Jph, jk, jl, J1) *
                     (2 * J1 + 1) * H.TwoBody.GetTBME_J(J1, m, l, k, i);
        }
      }
      VA_mikl *= phase_mi;
      ...
      if (AngMom::Triangle(jk, jl, Jph) and AngMom::Triangle(ji, jm, Jph)) {
        for (int J1 = J1min; J1 <= J1max; ++J1)  // Pandya 1: <ai`| V |bj`>_Jtot
        {
          VB_milk += AngMom::phase(J1) * (2 * J1 + 1) *
                     modelspace->GetSixJ(jm, ji, Jph, jk, jl, J1) *
                     H.TwoBody.GetTBME_J(J1, m, l, i, k);
        }
      }
      ...
      if (AngMom::Triangle(jl, jk, Jph) and AngMom::Triangle(ji, jm, Jph)) {
        for (int J1 = J1min; J1 <= J1max; ++J1)  // Pandya 1: <ai`| V |bj`>_Jtot
        {
          VB_mikl += AngMom::phase(J1) * (2 * J1 + 1) *
                     modelspace->GetSixJ(jm, ji, Jph, jl, jk, J1) *
                     H.TwoBody.GetTBME_J(J1, m, k, i, l);
        }
      }
      ...
      ////         B(ai,bj)
      ////	     int phase_ib = AngMom::phase( ji + jb + Jph);
      ////         if ( AngMom::Triangle(jj,jb,Jph) and
      ///AngMom::Triangle(ji,ja,Jph)) /         { /          for (int
      ///J1=J1min;J1<=J1max;++J1)  //Pandya 1: <ai`| V |bj`>_Jtot /          {
      ////            V_aibj += AngMom::phase(J1) * (2*J1 + 1) *
      ///modelspace->GetSixJ(ja,ji,Jph,jj,jb,J1)  *
      ///H.TwoBody.GetTBME_J(J1,a,b,i,j); /          } /         } /
      ///B(I_ph,II_ph) =  V_aibj *phase_ib *  phase_ai *phase_bj;
      ...
      //         T_mu += OpIn.OneBody(m,i) * X(I_mi,mu) + OpIn.OneBody(i,m)*
      //         Y(I_mi,mu);
      ...
      ////            std::cout << "mu=" << mu << " -> mi = " << m << " " << i
      ///<< std::endl; /            std::cout << " Tmu = " << Omi << " * " <<
      ///X(I_mi,mu) << "  flipped is " << Oim << std::endl; / std::cout << "
      ///gamma_mu_kl = " << X(I_mi,mu) << " * " << VA_milk << std::endl; /
      ///std::cout << " gamma_mu_lk = " << X(I_mi,mu) << " * " << VA_mikl <<
      ///std::endl;
      //         }
      I_mi++;
    }  // for iket_mi
    ...
    //     std::cout << "    Teff += " << gamma_mu_kl << " * " << T_mu << "  / (
    //     " << el << " - " << ek << " - " << Omega_mu << " ) " << std::endl;
    //     std::cout << "    Teff += " << gamma_mu_kl << " * " << T_mu << "  / (
    //     " << ek << " - " << el << " - " << Omega_mu << " ) " << std::endl;
