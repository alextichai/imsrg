...
Operator::Operator(ModelSpace& ms, int Jrank, int Trank, int p, int part_rank)
    : modelspace(&ms),
      ...
      //    TwoBody(&ms,Jrank,Trank,p),  ThreeBody(&ms,Jrank,Trank,p),
      //    ThreeLeg(&ms), ThreeBodyNO2B(), TwoBody(&ms,Jrank,Trank,p),
      //    ThreeBody(&ms,Jrank,Trank,p), ThreeLeg(&ms),
      TwoBody(),
      ThreeBody(&ms, Jrank, Trank, p),
      ThreeLeg(&ms),
      rank_J(Jrank),
      rank_T(Trank),
      parity(p),
      particle_rank(part_rank),
      legs(2 * part_rank),
      E3max(ms.GetE3max()),
      hermitian(true),
      antihermitian(false),
  ...
  //  if (particle_rank >=3) ThreeBody.Allocate();   // Don't allocate
  //  automatically. Wait until we're sure we want it.
  IMSRGProfiler::counter["N_Operators"]++;
}
...
Operator::Operator(const Operator& op)
    : modelspace(op.modelspace),
      ...
      ThreeLeg(op.ThreeLeg),
      //  OneBody(op.OneBody), TwoBody(op.TwoBody) ,ThreeBody(op.ThreeBody),
      //  ThreeLeg(op.ThreeLeg), ThreeBodyNO2B(op.ThreeBodyNO2B),
      rank_J(op.rank_J),
      rank_T(op.rank_T),
      parity(op.parity),
      ...
      Q_space_orbit(op.Q_space_orbit) {
  IMSRGProfiler::counter["N_Operators"]++;
}
...
Operator::Operator(Operator&& op)
    : modelspace(op.modelspace),
      ZeroBody(op.ZeroBody),
      OneBody(std::move(op.OneBody)),
      TwoBody(std::move(op.TwoBody)),
      ThreeBody(std::move(op.ThreeBody)),
      ThreeLeg(std::move(op.ThreeLeg)),
...
Operator& Operator::operator+=(const Operator& rhs) {
  ...
  OneBody += rhs.OneBody;
  if (rhs.GetParticleRank() > 1) TwoBody += rhs.TwoBody;
  if (rhs.GetParticleRank() > 2) ThreeBody += rhs.ThreeBody;
  //   if (rhs.GetParticleRank() >2 )
  //     ThreeBodyNO2B += rhs.ThreeBodyNO2B;
  if (rhs.GetNumberLegs() % 2 == 1) ThreeLeg += rhs.ThreeLeg;
  return *this;
}
...
size_t Operator::Size() {
  return sizeof(ZeroBody) + OneBody.size() * sizeof(double) + TwoBody.size() +
         ThreeBody.size();
}
...
void Operator::SetOneBody(int i, int j, double val) {
  ...
  if (rank_J > 0)
    flip_phase *= modelspace->phase(
        (modelspace->GetOrbit(i).j2 - modelspace->GetOrbit(j).j2) / 2);
  OneBody(j, i) = flip_phase * val;
}

void Operator::SetTwoBody(int J1, int p1, int T1, int J2, int p2, int T2, int i,
                          int j, int k, int l, double v) {
  TwoBody.SetTBME(J1, p1, T1, J2, p2, T2, i, j, k, l, v);
}

double Operator::GetTwoBody(size_t ch_bra, size_t ch_ket, size_t ibra,
                            size_t iket) {
  if (ch_bra <= ch_ket or IsNonHermitian()) {
    ...
    int flipphase =
        modelspace->phase(tbc_bra.J - tbc_ket.J) * (IsHermitian() ? 1 : -1);
...
void Operator::WriteBinary(std::ofstream& ofs) {
  ...
  //  if (particle_rank > 1)
  if (legs > 3) TwoBody.WriteBinary(ofs);
  //  if (particle_rank > 2)
  if (legs > 5) ThreeBody.WriteBinary(ofs);
...
void Operator::ReadBinary(std::ifstream& ifs) {
  ...
  //  if (particle_rank > 1)
  if (legs > 3) TwoBody.ReadBinary(ifs);
  //  if (particle_rank > 2)
  if (legs > 5) ThreeBody.ReadBinary(ifs);
...
Operator Operator::DoNormalOrdering() const {
  if (legs % 2 > 0) return DoNormalOrderingDagger(+1, modelspace->holes);
  if (legs > 5)
...
Operator Operator::DoNormalOrderingCore() const {
  if (legs % 2 > 0) return DoNormalOrderingDagger(+1, modelspace->core);
  if (legs > 5)
...
Operator Operator::DoNormalOrdering2(int sign,
                                     std::set<index_t> occupied) const {
      ...
      if (particle_rank > 1) {
        for (auto& l : occupied) {
          ...
          for (int J = Jmin; J <= Jmax; J++) {
            opNO.ZeroBody += (2 * J + 1) * ok.occ * ol.occ *
                             TwoBody.GetTBME_J_norm(J, J, k, l, k, l);
          }
        }
      }
    }
  ...
  if (TwoBody.Norm() > 1e-7) {
    for (auto& itmat : TwoBody.MatEl) {
      ...
      for (index_t a = 0; a < norbits; ++a) {
        ...
        double ja = oa.j2 / 2.0;
        index_t bstart = (IsNonHermitian() or ch_bra != ch_ket)
                             ? 0
                             : a;  // If it's neither hermitian or anti, we need
                                   // to do the full sum
        for (auto& b : opNO.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
          ...
          for (auto& h : occupied)  // C++11 syntax
          {
            ...
            if (opNO.rank_J == 0) {
              opNO.OneBody(a, b) += hatfactor / (2 * ja + 1.0) * sign * oh.occ *
                                    TwoBody.GetTBME(ch_bra, ch_ket, a, h, b, h);
            } else {
              double jh = oh.j2 * 0.5;
              if ((ja + jh < J_bra) or (abs(ja - jh) > J_bra) or
                  (jb + jh < J_ket) or (abs(jb - jh) > J_ket))
                continue;
              ...
              double ME =
                  hatfactor * sign * oh.occ *
                  modelspace->phase(ja + jh - J_ket - opNO.rank_J) *
                  modelspace->GetSixJ(J_bra, J_ket, opNO.rank_J, jb, ja, jh) *
                  TwoBody.GetTBME(ch_bra, ch_ket, a, h, b, h);
              if (a > b) {
...
//*******************************************************************************
///   Normal ordering of a three body operator. Start by generating the normal
///   ordered two body piece, then use DoNormalOrdering2() to get the rest.
///   (Note that there are some numerical factors). The normal ordered two body
///   piece is \f[ \Gamma^J_{ijkl} = V^J_{ijkl} + \sum_a n_a  \sum_K
///   \frac{2K+1}{2J+1} V^{(3)JJK}_{ijakla} \f] Right now, this is only set up
///   for scalar operators, but I don't anticipate handling 3body tensor
///   operators in the near future.
//*******************************************************************************
// Operator Operator::DoNormalOrdering3()
Operator Operator::DoNormalOrdering3(int sign,
                                     std::set<index_t> occupied) const {
  double t_start = omp_get_wtime();
  std::cout << "begin " << __func__ << "   norm of 3b is " << ThreeBodyNorm()
            << std::endl;
  //   Operator opNO3 = Operator(*modelspace);
  if (rank_J > 0) {
    std::cout << " Uh oh. Trying to call " << __func__
              << "  on an operator with rank_J = " << rank_J
              << "   you should probably implement that first..." << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    for (size_t ibra = 0; ibra < tbc_bra.GetNumberKets(); ++ibra) {
      ...
      for (size_t iket = iket_min; iket < tbc_ket.GetNumberKets(); ++iket) {
        ...
        for (auto& a : occupied) {
          ...
          //               {
          //                  Gamma(ibra,iket) += (K2+1) * sign*oa.occ *
          //                  ThreeBody.GetME_pn(tbc_bra.J,tbc_ket.J,K2,i,j,a,k,l,a);
          //                  // This is unnormalized.
          //                   std::cout << " accessing 3bme   "<< tbc_bra.J <<
          //                   " " << tbc_ket.J << " " << K2 << "    " << i << "
          //                   " << j << " " << a << "  " << k << " "  << l << "
          //                   " << a << "       " <<
          //                   ThreeBody.GetME_pn(tbc_bra.J,tbc_ket.J,K2,i,j,a,k,l,a)
          //                   << "  ->  " << Gamma(ibra,iket) << std::endl;
          //                                                   }

          Gamma(ibra, iket) +=
              sign * oa.occ *
              ThreeBody.GetME_pn_no2b(i, j, a, k, l, a, tbc_bra.J);
        }
        Gamma(ibra, iket) /= (2 * tbc_bra.J + 1) *
                             sqrt((1 + bra.delta_pq()) * (1 + ket.delta_pq()));
        if (opNO3.GetTRank() != 0 or opNO3.GetParity() != 0) {
          Gamma(ibra, iket) *=
              sqrt(2 * tbc_bra.J + 1);  // reduced matrix element
        }
      }
    }
  }
  ...
  //   std::cout << "IN " << __func__ << "  line " << __LINE__ << "   norms of
  //   NO 3b pieces are " << opNO2.ZeroBody << "   " << opNO2.OneBodyNorm() << "
  //   " << opNO2.TwoBodyNorm() << "  and thie original 3b norm was  " <<
  //   ThreeBody.Norm() << "  which produced a no2b with norm " <<
  //   opNO3.TwoBodyNorm() << std::endl; std::cout << " opNO2 has storage mode "
  //   << opNO2.ThreeBody.GetStorageMode() << "  and this has storage mode " <<
  //   ThreeBody.GetStorageMode() << "  and opNO3 has " <<
  //   opNO3.ThreeBody.GetStorageMode() << std::endl; std::cout << "Are they
  //   allocated? " << opNO2.ThreeBody.IsAllocated() << "  " <<
  //   ThreeBody.IsAllocated() << "  " << opNO3.ThreeBody.IsAllocated() <<
  //   std::endl;
  std::cout << __func__ << "  contributed " << opNO2.ZeroBody
            << "  to the zero body part" << std::endl;
...
///  dagger operator.
///
Operator Operator::DoNormalOrderingDagger(int sign,
                                          std::set<index_t> occupied) const {
  Operator opNO(*this);
  ...
  for (auto& itmat : ThreeLeg.MatEl) {
    ...
    for (auto a : OneBodyChannels.at({oQ.l, oQ.j2, oQ.tz2})) {
      ...
      for (auto& h : occupied)  // C++11 syntax
      {
        ...
        opNO.OneBody(a, 0) -=
            hatfactor / (2 * ja + 1) * sign * oh.occ *
            ThreeLeg.GetME(
                ch_bra, h, a,
                h);  // The GetTBME returns an unnormalized matrix element.
        //          opNO.OneBody(a,Q) -= hatfactor/(2*ja+1) * sign*oh.occ *
        //          ThreeLeg.GetME(ch_bra,h,a,h);  // The GetTBME returns an
        //          unnormalized matrix element. opNO.OneBody(a,Q) -=
        //          hatfactor/(2*ja+1) * sign*oh.occ *
        //          TwoBody.GetTBME(ch_bra,ch_ket,h,a,h,Q);  // The GetTBME
        //          returns an unnormalized matrix element.
      }
    }
  }  // loop over channels
...
Operator Operator::Truncate(ModelSpace& ms_new) {
  ...
  if (new_emax > modelspace->GetEmax()) {
    std::cout << "Error: Cannot truncate an operator with emax = "
              << modelspace->GetEmax() << " to one with emax = " << new_emax
              << std::endl;
    return OpNew;
  }
  ...
  //  std::cout << "Done truncating one body " << std::endl << OneBody <<
  //  std::endl << std::endl << OpNew.OneBody << std::endl;
  ...
  for (auto& itmat : OpNew.TwoBody.MatEl) {
    ...
    int ch_bra_old = modelspace->GetTwoBodyChannelIndex(
        tbc_bra_new.J, tbc_bra_new.parity, tbc_bra_new.Tz);
    int ch_ket_old = modelspace->GetTwoBodyChannelIndex(
        tbc_ket_new.J, tbc_ket_new.parity, tbc_ket_new.Tz);
    ...
    for (int ibra = 0; ibra < nbras_new; ++ibra) {
      ...
      ibra_old(ibra) =
          tbc_bra_old.GetLocalIndex(old_orbs(bra_new.p), old_orbs(bra_new.q));
    }

    for (int iket = 0; iket < nkets_new; ++iket) {
      auto ket_new = tbc_ket_new.GetKet(iket);
      iket_old(iket) =
          tbc_ket_old.GetLocalIndex(old_orbs(ket_new.p), old_orbs(ket_new.q));
    }
  ...
  if (particle_rank >= 3) {
    ...
    for (size_t ch3 = 0; ch3 < nch; ch3++) {
      ...
      size_t ch3_old = modelspace->GetThreeBodyChannelIndex(
          Tbc_new.twoJ, Tbc_new.parity, Tbc_new.twoTz);
      ...
      for (size_t ibra = 0; ibra < nkets; ibra++) {
        ...
        for (size_t iket = ibra; iket < nkets; iket++) {
          ...
          if (storage_mode == "pn") {
            auto matel =
                ThreeBody.GetME_pn_ch(ch3_old, ch3_old, ibra_old, iket_old);
            ...
            for (int tab = 0; tab <= 1; tab++) {
              for (int tde = 0; tde <= 1; tde++) {
                for (int twoT = std::abs(Tbc_new.twoTz); twoT <= 3; twoT += 2) {
                  auto matel = ThreeBody.GetME_iso(
                      bra.Jpq, ket.Jpq, Tbc_new.twoJ, tab, tde, twoT, bra.p,
                      bra.q, bra.r, ket.p, ket.q, ket.r);
                  OpNew.ThreeBody.SetME_iso(bra.Jpq, ket.Jpq, Tbc_new.twoJ, tab,
                                            tde, twoT, bra.p, bra.q, bra.r,
                                            ket.p, ket.q, ket.r, matel);
                }
              }
            }
          }
...
void Operator::Erase() {
  ...
  TwoBody.Erase();
  //  if (particle_rank >=3)
  if (legs >= 6) ThreeBody.Erase();
  if ((legs % 2) > 0) ThreeLeg.Erase();
}
...
void Operator::SetNumberLegs(int l) {
  ...
  if (legs % 2 == 0) {
    ...
    OneBody.zeros(modelspace->GetNumberOrbits(),
                  1);  // reduce it to a single column
    ThreeLeg.Allocate();
...
void Operator::MakeReduced() {
  if (rank_J > 0) {
    std::cout << "Trying to reduce an operator with J rank = " << rank_J
              << ". Not good!!!" << std::endl;
    return;
  }
...
void Operator::MakeNotReduced() {
  if (rank_J > 0) {
    std::cout << "Trying to un-reduce an operator with J rank = " << rank_J
              << ". Not good!!!" << std::endl;
    return;
  }
  for (size_t a = 0; a < modelspace->GetNumberOrbits(); ++a) {
...
//// this routine then multiplies the TBME <ab|Op|cd> by coeff if a==b, and
///again if c==d
...
/// +  \sum_{\substack{i\leq j // a\leq b}}\sum_{J}
/// (2J+1)\frac{|\Gamma_{ijab}^{J}|^2}{f_{aa}+f_{bb}-f_{ii}-f_{jj}} \f]
///
double Operator::GetMP2_Energy() {
  ...
  std::vector<index_t> particles_vec(
      modelspace->particles.begin(),
      modelspace->particles.end());  // convert set to vector for OMP looping
  ...
  for (int ii = 0; ii < nparticles; ++ii) {
    ...
    for (auto& a : modelspace->holes) {
      ...
      if (abs(OneBody(i, a)) > 1e-6)
        Emp2 += (oa.j2 + 1) * oa.occ * OneBody(i, a) * OneBody(i, a) /
                (OneBody(a, a) - OneBody(i, i));
        ...
        for (auto& b : modelspace->holes) {
          ...
          for (int J = Jmin; J <= Jmax; J += dJ) {
            ...
            if (std::abs(tbme) > 1e-6) {
              Emp2 += (2 * J + 1) * oa.occ * ob.occ * tbme * tbme /
                      denom;  // no factor 1/4 because of the restricted sum
              //              std::cout << "MBPT2 " << a << " " << b << " " << i
              //              << " " << j << "    " << J << "  " << oa.occ << "
              //              " << ob.occ << "  "
...
/// \f[
/// \frac{1}{8}\sum_{abijpq}\sum_J
/// (2J+1)\frac{\Gamma_{abij}^J\Gamma_{ijpq}^J\Gamma_{pqab}^J}{(f_a+f_b-f_p-f_q)(f_a+f_b-f_i-f_j)}
/// +\sum_{abcijk}\sum_J (2J+1)^2
/// \frac{\bar{\Gamma}_{a\bar{i}j\bar{b}}^J\bar{\Gamma}_{j\bar{b}k\bar{c}}^J\bar{\Gamma}_{k\bar{c}a\bar{i}}^J}{(f_a+f_b-f_i-f_j)(f_a+f_c-f_i-f_k)}
/// \f]
///
//*************************************************************
...
std::array<double, 3> Operator::GetMP3_Energy() {
  ...
  double Ehh = 0;
  double Eph = 0;
  // This can certainly be optimized, but I'll wait until this is the
  // bottleneck.
  ...
  for (int ich = 0; ich < nch; ++ich) {
    ...
    for (auto iket_ij : tbc.GetKetIndex_hh()) {
      ...
      for (auto iket_ab : tbc.GetKetIndex_pp()) {
        ...
        index_t b = ket_ab.q;
        double Delta_ijab =
            OneBody(i, i) + OneBody(j, j) - OneBody(a, a) - OneBody(b, b);
    ...
    for (auto iket_ai : tbc_CC.GetKetIndex_ph()) {
      ...
      for (auto iket_bj : tbc_CC.GetKetIndex_ph()) {
        ...
        if (AngMom::Triangle(jj, jb, Jph) and AngMom::Triangle(ji, ja, Jph)) {
          for (int J1 = J1min; J1 <= J1max;
               ++J1)  // Pandya 1: <ai`| V |jb`>_Jtot
          {
            tbme_iabj -= modelspace->GetSixJ(ja, ji, Jph, jj, jb, J1) *
                         (2 * J1 + 1) * TwoBody.GetTBME_J(J1, i, j, b, a);
          }
        }
        ...
        if (AngMom::Triangle(jj, jb, Jph) and AngMom::Triangle(ji, ja, Jph)) {
          for (int J1 = J1min; J1 <= J1max;
               ++J1)  // Pandya 1: <ai`| V |jb`>_Jtot
          {
            tbme_bjck -= modelspace->GetSixJ(jb, jj, Jph, ja, ji, J1) *
                         (2 * J1 + 1) * TwoBody.GetTBME_J(J1, b, i, a, j);
          }
        }
  ...
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets; ibra++) {
      ...
      for (size_t iket = 0; iket < nkets; iket++) {
        ...
        double unocc_ket =
            (1 - ket.op->occ) * (1 - ket.oq->occ) * (1 - ket.oR->occ);
        ...
        Emp2 += 1. / 36 * symm_ijk * symm_abc * (twoJ + 1) * occ_bra *
                unocc_ket * V * V / (Eijk - Eabc);
      }  // for iket
    }    // for ibra
  }      // for ch3
...
std::array<double, 2> Operator::GetPPHH_Ladders() {
  ...
  for (int ich = 0; ich < nch; ++ich) {
    ...
    for (auto iket_ij : tbc.GetKetIndex_hh()) {
      ...
      int J = tbc.J;
      Epp += (2 * J + 1) *
             arma::trace(M_ijpp * (arma::inv(ONE - M_pppp) - ONE) * M_ppij);
    ...
    for (auto iket_ab : tbc.GetKetIndex_pp()) {
      ...
      int J = tbc.J;
      Ehh += (2 * J + 1) *
             arma::trace(M_abhh * (arma::inv(ONE - M_hhhh) - ONE) * M_hhab);
...
/// \f[
///  \mathcal{O}^{(1)} = 2\sum_{abij}
///  \frac{H_{abij}\mathcal{O}_{ijab}}{\Delta_{abij}}
/// \f]
///
//*************************************************************
    ...
    for (int ich = 0; ich < nch; ++ich) {
      ...
      for (auto ibra : tbc.GetKetIndex_hh()) {
        ...
        for (auto iket : tbc.GetKetIndex_pp()) {
          ...
          double Delta_abij = H.OneBody(bra.p, bra.p) +
                              H.OneBody(bra.q, bra.q) -
                              H.OneBody(ket.p, ket.p) - H.OneBody(ket.q, ket.q);
          opval += 2 * (2 * J + 1) * Hmat(ibra, iket) * Opmat(iket, ibra) /
                   Delta_abij;
        }
      }
    }
...
double Operator::OneBodyNorm() const {
  ...
  for (auto p : modelspace->all_orbits) {
    ...
    for (auto q : OneBodyChannels.at({op.l, op.j2, op.tz2})) {
      Orbit& oq = modelspace->GetOrbit(q);
      int degeneracy_factor =
          (op.j2 + 1) * ((std::min(oq.j2, op.j2 + rank_J) -
                          std::max(-oq.j2, op.j2 - rank_J)) /
                             2 +
                         1);
...
double Operator::OneLegNorm() const {
  ...
  for (auto p : modelspace->all_orbits) {
    ...
    nrm +=
        OneBody(p, 0) * OneBody(p, 0) * degeneracy_factor * degeneracy_factor;
  }
  return sqrt(nrm);
}
...
double Operator::Trace(int Atrace, int Ztrace) const {
  ...
  double M =
      ...
      3;  // number of m-scheme orbits for the proton or neutron partitions
  ...
  for (size_t ch = 0; ch < modelspace->GetNumberTwoBodyChannels(); ++ch) {
    ...
    switch (tbc.Tz) {
      ...
      case (1):
        ...
        break;
      default:
        std::cout << "AAAHHH blew the switch statement. tbc.Tz = " << tbc.Tz
                  << std::endl;
    }
  }
  ...
  for (auto itmat : TwoBody.MatEl) {
    ...
    for (size_t ibra = 0; ibra < tbc.GetNumberKets(); ++ibra) {
      ...
      for (size_t iket = 0; iket < tbc.GetNumberKets(); ++iket) {
        ...
        itmat.second.row(ibra) *=
            1. / (1 + exp((ei + ej + ek + el - 4 * Efermi) / (2 * T)));
      }
    }
  }
}
    ...
    for (auto i : modelspace->all_orbits) {
      ...
      for (size_t j : OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
        ...
        if (rank_J == 0)
          ...
          Orbit& oj = modelspace->GetOrbit(j);
          OneBody(j, i) =
              -modelspace->phase((oi.j2 - oj.j2) / 2) * OneBody(i, j);
        }
      }
    }
  }
...
{
//   std::cout << "  a    b    i    j    J    na     nb    tbme    denom     dE"
<< std::endl; double t_start = omp_get_wtime(); double de = 0;

   int nparticles = modelspace->particles.size();
   arma::vec orbit_impacts(modelspace->all_orbits.size(), arma::fill::zeros);

   std::vector<index_t>
particles_vec(modelspace->particles.begin(),modelspace->particles.end()); //
convert set to vector for OMP looping
//   for ( auto& i : modelspace->particles)
//   #pragma omp parallel for reduction(+:Emp2)
   for ( int ii=0;ii<nparticles;++ii)
   {
//     index_t i = modelspace->particles[ii];
     ...
     for (auto& a : modelspace->holes)
     {
       ...
       if (abs(OneBody(i,a))>1e-6)
        de = (oa.j2+1) * oa.occ *
OneBody(i,a)*OneBody(i,a)/(OneBody(a,a)-OneBody(i,i)); orbit_impacts(a) += de;
        orbit_impacts(i) += de;
       ...
       for (index_t j : modelspace->particles)
       {
         ...
         for ( auto& b: modelspace->holes)
         {
           ...
           for (int J=Jmin; J<=Jmax; J+=dJ)
           {
             ...
             if (std::abs(tbme)>1e-6)
             {
              de = (2*J+1)* oa.occ * ob.occ * tbme*tbme/denom; // no factor 1/4
because of the restricted sum
