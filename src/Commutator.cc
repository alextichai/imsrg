...
std::map<std::string, bool> comm_term_on = {
    {"comm330ss", true},        {"comm331ss", true},
    {"comm231ss", true},        {"comm132ss", true},
    {"comm232ss", true},        {"comm332_ppph_hhhpss", true},
    {"comm332_pphhss", true},   {"comm133ss", true},
    {"comm223ss", true},        {"comm233_pp_hhss", true},
    {"comm233_phss", true},     {"comm333_ppp_hhhss", true},
...
void Set_BCH_Transform_Threshold(double x) { bch_transform_threshold = x; }

void Set_BCH_Product_Threshold(double x) { bch_product_threshold = x; }

void SetThreebodyThreshold(double x) { threebody_threshold = x; }

void SetUseBruecknerBCH(bool tf) { use_brueckner_bch = tf; }

void SetUseGooseTank(bool tf) { use_goose_tank_correction = tf; }

void SetUseIMSRG3(bool tf) { use_imsrg3 = tf; }

void SetUseIMSRG3N7(bool tf) { use_imsrg3_n7 = tf; }

void SetOnly2bOmega(bool tf) { only_2b_omega = tf; }

void SetBCHSkipiEq1(bool tf) { bch_skip_ieq1 = tf; }

void SetIMSRG3Noqqq(bool tf) { imsrg3_no_qqq = tf; }

void SetIMSRG3valence2b(bool tf) { imsrg3_valence_2b = tf; }
...
Operator Commutator(const Operator& X, const Operator& Y) {
  ...
  if (xrank == 0) {
    ...
    if ((xlegs % 2 == 1) and (ylegs % 2 == 1)) {
      std::cout << "TROUBLE!!!! Called commutator with two Dagger operators. "
                   "This isn't implemented. Dying..."
                << std::endl;
    ...
    std::cout << "In Tensor-Tensor because X.rank_J = " << X.rank_J
              << "  X.rank_T = " << X.rank_T << "  X.parity = " << X.parity
              << "   ";
    std::cout << "  Y.rank_J = " << Y.rank_J << "  Y.rank_T = " << Y.rank_T
              << "  Y.parity = " << Y.parity << std::endl;
...
Operator CommutatorScalarScalar(const Operator& X, const Operator& Y) {
  ...
  //   std::cout << " " << __FILE__ << "  line " << __LINE__ << "  constructing
  //   Z " << std::endl;
  Operator Z(ms, z_Jrank, z_Trank, z_parity, z_particlerank);

  //   Operator Z( *(Y.GetModelSpace()), std::max(X.GetJRank(),Y.GetJRank()),
  //   std::max(X.GetTRank(),Y.GetTRank()), (X.GetParity()+Y.GetParity())%2,
  //   std::max(X.GetParticleRank(),Y.GetParticleRank()) );

  if ((X.IsHermitian() and Y.IsHermitian()) or
      (X.IsAntiHermitian() and Y.IsAntiHermitian()))
    Z.SetAntiHermitian();
  else if ((X.IsHermitian() and Y.IsAntiHermitian()) or
           (X.IsAntiHermitian() and Y.IsHermitian()))
    Z.SetHermitian();
  else
    Z.SetNonHermitian();

  if (Z.GetParticleRank() > 2) {
    std::cout << " " << __FILE__ << "  line " << __LINE__ << "  SwitchToPN "
              << std::endl;
  ...
  if (not Z.IsAntiHermitian()) {
    comm110ss(X, Y, Z);
    if (X.particle_rank > 1 and Y.particle_rank > 1) comm220ss(X, Y, Z);
  }

  //   std::cout << "In " << __func__ << "                   X. Is the 3body
  //   hermitian " << X.ThreeBody.IsHermitian() << std::endl;

  //   std::cout << __FILE__ << __LINE__ << "  DONT FORGET TO FIX THIS!" <<
  //   std::endl;
  //     std::cout << " " << __FILE__ << "  line " << __LINE__ << "  start
  //     commutator terms " << std::endl;
  ...
  if (use_imsrg3 and (X.Norm() > threebody_threshold) and
      (Y.Norm() > threebody_threshold)) {
    ...
    if (comm_term_on["comm223ss"]) {
      // This one is essential. If it's not here, then there are no induced 3
      // body terms
    ...
    // after going through once, we've stored all the 6js (and maybe 9js), so we
    // can run in OMP loops from now on
  ...
  if (discard_2b_from_3b or discard_1b_from_3b or discard_0b_from_3b) {
    ...
    Zcopy.EraseOneBody();
    Zcopy.EraseTwoBody();
    Zcopy = Zcopy.DoNormalOrdering();  // Now just have the NO 0,1,2b parts of
                                       // the 3b piece
...
/// Commutator \f$[X,Y]\f$ where \f$ X \f$ is a scalar operator and \f$Y\f$ is a
/// tensor operator. Should be called through Commutator()
Operator CommutatorScalarTensor(const Operator& X, const Operator& Y) {
  ...
  if (use_imsrg3 and X.GetJRank() == 0 and Y.GetJRank() == 0 and
      Z.GetJRank() == 0) {
...
/// Commutator where \f$ X \f$ is a scalar and \f$Y\f$ is a dagger (i.e. it
/// creates an additional particle). Should be called through Commutator()
Operator CommutatorScalarDagger(const Operator& X, const Operator& Y) {
  ...
  //   if ( (X.IsHermitian() and Y.IsHermitian()) or (X.IsAntiHermitian() and
  //   Y.IsAntiHermitian()) ) Z.SetAntiHermitian(); else if ( (X.IsHermitian()
  //   and Y.IsAntiHermitian()) or (X.IsAntiHermitian() and Y.IsHermitian()) )
  //   Z.SetHermitian(); else Z.SetNonHermitian();
...
/// formula](http://en.wikipedia.org/wiki/Baker-Campbell-Hausdorff_formula) \f[
/// Z = X + [Y,X] + \frac{1}{2!}[Y,[Y,X]] + \frac{1}{3!}[Y,[Y,[Y,X]]] + \ldots
/// \f] with all commutators truncated at the two-body level.
Operator BCH_Transform(const Operator& OpIn, const Operator& Omega) {
  return use_brueckner_bch ? Brueckner_BCH_Transform(OpIn, Omega)
                           : Standard_BCH_Transform(OpIn, Omega);
}
...
Operator Standard_BCH_Transform(const Operator& OpIn, const Operator& Omega) {
  //   std::cout << "!!! " << __func__ << " !!!   particles ranks are " <<
  //   OpIn.GetParticleRank() << "  and  " << Omega.GetParticleRank()
  //             << "  PN mode is " << OpIn.ThreeBody.GetStorageMode() << " and
  //             " << Omega.ThreeBody.GetStorageMode()  << std::endl;
  ...
  //   Only call SetMode and SetParticleRank if ThreeBody is not already
  //   allocated. Otherwise, we are erasing the 3N inherited from OpIn. if
  //   (use_imsrg3  )
  if (use_imsrg3 and not OpOut.ThreeBody.IsAllocated()) {
    std::cout << __func__ << "  Allocating Three Body with pn mode"
              << std::endl;
    OpOut.ThreeBody.SetMode("pn");
    OpOut.SetParticleRank(3);
  }
  double factorial_denom = 1.0;
  Operator goosetank_chi;  // auxiliary one-body operator used to recover
                           // 4th-order quadruples.
  ...
  if (nx > bch_transform_threshold) {
    ...
    //     }
    double epsilon =
        nx * exp(-2 * ny) * bch_transform_threshold /
        (2 * ny);  // this should probably be explained somewhere...
    for (int i = 1; i <= max_iter; ++i) {
      // Specifically for the perturbative triples, we need 1/(i+1)! rather than
      // 1/i! This is because we have Wbar = [Omega,H]_3b +
      // 1/2![Omega,[Omega,H]]_3b + 1/3![Omega,[Omega,[Omega,H]]]_3b + ...
      //                              = [Omega, Htilde]_3b,  where Htilde = H +
      //                              1/2![Omega,H] + 1/3![Omega,[Omega,H]] +
      //                              ...
      ...
      if (use_goose_tank_correction) {
        ...
        OpNested.OneBody +=
            chi_last;  // add the chi from the previous step to OpNested.
      }
      ...
      if (OpOut.rank_J > 0) {
        std::cout << "Tensor BCH, i=" << i << "  Norm = " << std::setw(12)
                  << std::setprecision(8) << std::fixed
                  << OpNested.OneBodyNorm() << " " << std::setw(12)
                  << std::setprecision(8) << std::fixed
                  << OpNested.TwoBodyNorm() << " " << std::setw(12)
                  << std::setprecision(8) << std::fixed << OpNested.Norm()
                  << std::endl;
      }
      epsilon *= i + 1;
      if (OpNested.Norm() < epsilon) break;
      if (i == warn_iter)
        std::cout << "Warning: BCH_Transform not converged after " << warn_iter
                  << " nested commutators" << std::endl;
      else if (i == max_iter)
        std::cout << "Warning: BCH_Transform didn't coverge after " << max_iter
                  << " nested commutators" << std::endl;
    }
  }
  //   std::cout << "Done with BCH_Transform, 3-body norm of OpOut = " <<
  //   OpOut.ThreeBodyNorm() << std::endl;
  OpIn.profiler.timer["BCH_Transform"] += omp_get_wtime() - t_start;
  return OpOut;
}

//  Update the auxiliary one-body operator chi, using Omega and the ith nested
//  commutator This has not been tested for tensor commutators, but it *should*
//  work.
//
Operator GooseTankUpdate(const Operator& Omega, const Operator& OpNested) {
  double t_start = omp_get_wtime();
  Operator goosetank_chi = Operator(*(OpNested.modelspace), OpNested.rank_J,
                                    OpNested.rank_T, OpNested.parity, 1);
  ...
  goosetank_chi.Symmetrize();  // the commutator call only does half the matrix,
                               // so we symmetrize
  //   int norbits = OpNested.modelspace->GetNumberOrbits();
  //   for (int i=0;i<norbits;++i)  // enforce n_in_j + nbar_i nbar_j
  for (auto i :
       OpNested.modelspace->all_orbits)  // enforce n_in_j + nbar_i nbar_j
  {
    ...
    for (auto j : OpNested.modelspace->all_orbits) {
      ...
      goosetank_chi.OneBody(i, j) *=
          oi.occ * oj.occ + (1.0 - oi.occ) * (1.0 - oj.occ);
    }
  }
...
/// by employing the [Baker-Campbell-Hausdorff
/// formula](http://en.wikipedia.org/wiki/Baker-Campbell-Hausdorff_formula) \f[
...
Operator BCH_Product(Operator& X, Operator& Y) {
  ...
  double nx = X.Norm();
  double ny = Y.Norm();
  std::vector<double> bernoulli = {1.0, -0.5,    1. / 6, 0.0,     -1. / 30,
                                   0.0, 1. / 42, 0,      -1. / 30};
  std::vector<double> factorial = {1.0,  1.0,  2.0,   6.0,   24.,
                                   120., 720., 5040., 40320.};
  ...
  // We assume X is small, but just in case, we check if we should include the
  // [X,[X,Y]] term.
  ...
  int k = 1;
  // k=1 adds 1/2[X,Y],  k=2 adds 1/12 [Y,[Y,X]], k=4 adds -1/720
  // [Y,[Y,[Y,[Y,X]]]], and so on.
...
double EstimateBCHError(Operator& Omega, Operator H) {
  ...
  //  H223.ThreeBody.SwitchToPN_and_discard();
  ...
  double est_err = 2. / 3 * normOmega * normOmega * Norm223 +
                   1. / 6 * normOmega * normOmega * Norm2223 +
                   exp(2. * normOmega) / 24 * Norm4nested;
  std::cout << "Contributions to err "
            << 2. / 3 * normOmega * normOmega * Norm223 << "  "
            << 1. / 6 * normOmega * normOmega * Norm2223 << "  "
            << exp(2. * normOmega) / 24 * Norm4nested << "    220 = " << Norm220
            << "  ||H2|| ,||Omega2|| = " << normH2 << " " << normOmega2
            << std::endl;
...
/// \f[
///  [X_{1)},Y_{(1)}]_{(0)} = \sum_{a} n_a (2j_a+1)
///  \left(X_{(1)}Y_{(1)}-Y_{(1)}X_{(1)}\right)_{aa}
/// \f]
...
//           X           Y
//
//  [ X^(2), Y^(2) ]^(0) = 1/2 Sum_abcd  Sum_J (2J+1) x_abcd y_cdab (n_a n_b
//  nbar_c nbar_d)
//                       = 1/2 Sum_J (2J+1) Sum_abcd x_abcd y_cdab (n_a n_b
//                       nbar_c nbar_d) = 1/2 Sum_J (2J+1) Sum_ab
//                       (X*P_pp*Y)_abab  P_hh
//
//  -- AGREES WITH NATHAN'S RESULTS (within < 1%)
/// \f[
/// [X_{(2)},Y_{(2)}]_{(0)} = \frac{1}{2} \sum_{J} (2J+1) \sum_{abcd} (n_a n_b
/// \bar{n}_c \bar{n}_d) \tilde{X}_{abcd}^{J} \tilde{Y}_{cdab}^{J} \f] may be
/// rewritten as \f[ [X_{(2)},Y_{(2)}]_{(0)} = 2 \sum_{J} (2J+1)
/// Tr(X_{hh'pp'}^{J} Y_{pp'hh'}^{J}) \f] where we obtain a factor of four from
/// converting two unrestricted sums to restricted sums, i.e. \f$\sum_{ab}
/// \rightarrow \sum_{a\leq b} \f$, and using the normalized TBME.
...
void comm220ss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  for (int ch = 0; ch < Y.nChannels; ++ch) {
    ...
    Z.ZeroBody +=
        2 * (2 * tbc.J + 1) *
        arma::sum(arma::diagvec(X2 * Y2) %
                  nn);  // This could be made more efficient, but who cares?
  }
}
...
//      i |              i |             |
//        |    ___.Y       |__X__        |
//        |___(_)    _     |   (_)__.    |  [X2,Y1](1)  =  1/(2j_i+1)
//        sum_ab(n_a-n_b)y_ab
//      j | X            j |        Y    |        * sum_J (2J+1) x_biaj^(J)
//                                       |
//---------------------------------------*        = 1/(2j+1) sum_a n_a sum_J
//(2J+1)
//                                                  * sum_b y_ab x_biaj - yba
//                                                  x_aibj
//
//                     (note: I think this should actually be)
//                                                = sum_ab (n_a nbar_b) sum_J
//                                                (2J+1)/(2j_i+1)
//                                                      * y_ab xbiaj - yba
//                                                      x_aibj
//
...
/// \f[
/// [X_{(1)},Y_{(2)}]_{ij} = \frac{1}{2j_i+1}\sum_{ab} (n_a \bar{n}_b) \sum_{J}
/// (2J+1) (X_{ab} Y^J_{biaj} - X_{ba} Y^J_{aibj}) \f]
      ...
      for (auto& a : Z.modelspace->holes)  // C++11 syntax
      {
        ...
        if (Y.particle_rank > 1) {
          for (auto b : X.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
            ...
            Z.OneBody(i, j) += (ob.j2 + 1) * nanb * X.OneBody(a, b) *
                               Y.TwoBody.GetTBMEmonopole(b, i, a, j);
            Z.OneBody(i, j) -= (oa.j2 + 1) * nanb * X.OneBody(b, a) *
                               Y.TwoBody.GetTBMEmonopole(a, i, b, j);
          }
        }
        //             for (index_t b=0; b<norbits; ++b)
        //             for (auto b : Z.modelspace->all_orbits)
        if (X.particle_rank > 1) {
          for (auto b : Y.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
            ...
            double nanb = oa.occ * (1 - ob.occ);
            if (std::abs(nanb) < ModelSpace::OCC_CUT) continue;
            Z.OneBody(i, j) -= (ob.j2 + 1) * nanb * Y.OneBody(a, b) *
                               X.TwoBody.GetTBMEmonopole(b, i, a, j);
            Z.OneBody(i, j) += (oa.j2 + 1) * nanb * Y.OneBody(b, a) *
                               X.TwoBody.GetTBMEmonopole(a, i, b, j);
          }
        }
      }
    }
...
//*****************************************************************************************
//
//      i |              i |            [X2,Y2](1)  =  1/(2(2j_i+1)) sum_J
//      (2J+1)
//        |__Y__           |__X__           * sum_abc (nbar_a*nbar_b*n_c +
//        n_a*n_b*nbar_c) |    /\          |    /\          * (x_ciab y_abcj -
//        y_ciab xabcj) |   (  )   _     |   (  )
//        |____\/          |____\/       = 1/(2(2j+1)) sum_J (2J+1)
//      j | X            j |  Y            *  sum_c ( Pp*X*Phh*Y*Pp -
//      Pp*Y*Phh*X*Pp)  - (Ph*X*Ppp*Y*Ph - Ph*Y*Ppp*X*Ph)_cicj
//
//
// -- AGREES WITH NATHAN'S RESULTS
//   No factor of 1/2 because the matrix multiplication corresponds to a
//   restricted sum (a<=b)
// \f[
...
// \f]
/// \f[
/// [X_{(2)},Y_{(2)}]_{ij} = \frac{1}{2(2j_i+1)}\sum_{J}(2J+1)\sum_{abc}
/// (\bar{n}_a\bar{n}_bn_c + n_an_b\bar{n}_c)
///  (X^{J}_{ciab} Y^{J}_{abcj} - Y^{J}_{ciab}X^{J}_{abcj})
/// \f]
/// This may be rewritten as
/// \f[
/// [X_{(2)},Y_{(2)}]_{ij} = \frac{1}{2j_i+1} \sum_{c} \sum_{J} (2J+1) \left(
/// n_c \mathcal{M}^{J}_{pp,icjc} + \bar{n}_c\mathcal{M}^{J}_{hh,icjc} \right)
/// \f]
/// With the intermediate matrix \f[ \mathcal{M}^{J}_{pp} \equiv \frac{1}{2}
/// (X^{J}\mathcal{P}_{pp} Y^{J} - Y^{J}\mathcal{P}_{pp}X^{J}) \f] and likewise
/// for \f$ \mathcal{M}^{J}_{hh} \f$
// void Operator::comm221ss( const Operator& X, const Operator& Y)
void comm221ss(const Operator& X, const Operator& Y, Operator& Z) {
  double t_start = omp_get_wtime();

  //   static TwoBodyME Mpp = Y.TwoBody; // SRS: Is there a good reason to make
  //   these static? static TwoBodyME Mhh = Y.TwoBody;
  ...
  for (int indexi = 0; indexi < norbits; ++indexi) {
    ...
    for (int j : Z.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
      ...
      for (auto& c : Z.modelspace->all_orbits) {
        ...
        double nbarc = 1.0 - nc;
        int Jmin =
            std::max(std::abs(oc.j2 - oi.j2), std::abs(oc.j2 - oj.j2)) / 2;
...
//*****************************************************************************************
//
//    |     |               |      |           [X2,Y1](2) = sum_a ( Y_ia X_ajkl
//    + Y_ja X_iakl - Y_ak X_ijal - Y_al X_ijka ) |     |___.Y          |__X___|
//    |     |         _     |      |
//    |_____|               |      |_____.Y
//    |  X  |               |      |
//
...
/// \f[
/// [X_{(1)},Y_{(2)}]^{J}_{ijkl} = \sum_{a} ( X_{ia}Y^{J}_{ajkl} +
/// X_{ja}Y^{J}_{iakl} - X_{ak} Y^{J}_{ijal} - X_{al} Y^{J}_{ijka} ) \f] here,
/// all TBME are unnormalized, i.e. they should have a tilde.
  ...
  for (int ich = 0; ich < n_nonzero; ++ich) {
    ...
    for (int indx_ij = 0; indx_ij < npq; ++indx_ij) {
      ...
      for (int a : Z.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
        ...
        factor_ia.push_back(a > j ? flipphaseij
                                  : (a == j ? PhysConst::SQRT2 : 1));
      }
      if (i != j) {
        for (int a : Z.OneBodyChannels.at({oj.l, oj.j2, oj.tz2})) {
          ...
          factor_ja.push_back(i > a ? flipphaseij
                                    : (i == a ? PhysConst::SQRT2 : 1));
        }
      }
      ...
      if (X.particle_rank > 1 and Y.particle_rank > 1) {
        W2.col(indx_ij) =
            join_horiz(Y2.cols(join_vert(u_ind2_aj, u_ind2_ai)),
                       X2.cols(join_vert(u_ind2_aj, u_ind2_ai))) *
            join_vert(
                join_vert(X1.unsafe_col(i).rows(u_ind1_ia) % v_factor_ia,
                          X1.unsafe_col(j).rows(u_ind1_ja) % v_factor_ja),
                -join_vert(Y1.unsafe_col(i).rows(u_ind1_ia) % v_factor_ia,
                           Y1.unsafe_col(j).rows(u_ind1_ja) % v_factor_ja));
      } else if (X.particle_rank < 2 and Y.particle_rank > 1) {
        W2.col(indx_ij) =
            Y2.cols(join_vert(u_ind2_aj, u_ind2_ai)) *
            join_vert(X1.unsafe_col(i).rows(u_ind1_ia) % v_factor_ia,
                      X1.unsafe_col(j).rows(u_ind1_ja) % v_factor_ja);
...
//
//  |     |      |     |
//  |__Y__|      |__x__|   [X2,Y2](2)_pp(hh) = 1/2 sum_ab (X_ijab Y_abkl -
//  Y_ijab X_abkl)(1 - n_a - n_b) |     |  _   |     |                = 1/2 [
//  X*(P_pp-P_hh)*Y - Y*(P_pp-P_hh)*X ]
//  |__X__|      |__Y__|
//  |     |      |     |
//
// -- AGREES WITH NATHAN'S RESULTS
//   No factor of 1/2 because the matrix multiplication corresponds to a
//   restricted sum (a<=b)
/// Calculates the part of the commutator \f$ [X_{(2)},Y_{(2)}]_{(2)} \f$ which
/// involves particle-particle or hole-hole intermediate states. \f[
/// [X_{(2)},Y_{(2)}]^{J}_{ijkl} = \frac{1}{2} \sum_{ab} (\bar{n}_a\bar{n}_b -
/// n_an_b) (X^{J}_{ijab}Y^{J}_{ablk} - Y^{J}_{ijab}X^{J}_{abkl}) \f] This may
/// be written as \f[ [X_{(2)},Y_{(2)}]^{J} = \mathcal{M}^{J}_{pp} -
/// \mathcal{M}^{J}_{hh} \f] With the intermediate matrices \f[
/// \mathcal{M}^{J}_{pp} \equiv \frac{1}{2}(X^{J} \mathcal{P}_{pp} Y^{J} - Y^{J}
/// \mathcal{P}_{pp} X^{J}) \f] and likewise for \f$ \mathcal{M}^{J}_{hh} \f$.
...
void ConstructScalarMpp_Mhh(const Operator& X, const Operator& Y,
                            const Operator& Z, TwoBodyME& Mpp, TwoBodyME& Mhh) {
  double t_start = omp_get_wtime();
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  for (int ich = 0; ich < nch; ++ich) {
    ...
    Matrixhh =
        X2_bra.cols(bras_hh) * arma::diagmat(nanb_bra) * Y2.rows(bras_hh);
    if (kets_hh.size() > 0)
      Matrixpp += X2_bra.cols(bras_hh) * arma::diagmat(nbarnbar_hh_bra) *
                  Y2.rows(bras_hh);
    if (kets_ph.size() > 0)
      Matrixpp += X2_bra.cols(bras_ph) * arma::diagmat(nbarnbar_ph_bra) *
                  Y2.rows(bras_ph);

    if (Z.IsHermitian() and ch_bra == ch_ket) {
      Matrixpp += Matrixpp.t();
      Matrixhh += Matrixhh.t();
    } else if (Z.IsAntiHermitian() and
               ch_bra == ch_ket)  // i.e. LHS and RHS are both hermitian or
                                  // ant-hermitian
    {
      Matrixpp -= Matrixpp.t();
      Matrixhh -= Matrixhh.t();
      ...
      Matrixhh -=
          Y2.cols(kets_hh) * arma::diagmat(nanb_ket) * X2_ket.rows(kets_hh);
      Matrixpp -= Y2.cols(kets_hh) * arma::diagmat(nbarnbar_hh_ket) *
                  X2_ket.rows(kets_hh);
      if (kets_ph.size() > 0)
        Matrixpp -= Y2.cols(kets_ph) * arma::diagmat(nbarnbar_ph_ket) *
                    X2_ket.rows(kets_ph);
    }
...
/// the intermediate matrices \f$\mathcal{M}_{pp} \f$ and \f$ \mathcal{M}_{hh}
/// \f$, we can combine them and only calculate the intermediates once.
...
// int ch_cc, std::string orientation="normal") const
void DoPandyaTransformation_SingleChannel(const Operator& Z,
                                          arma::mat& TwoBody_CC_ph, int ch_cc,
                                          std::string orientation = "normal") {
  int herm = Z.IsHermitian() ? 1 : -1;
  TwoBodyChannel& tbc_cc = Z.modelspace->GetTwoBodyChannel_CC(ch_cc);
  int nKets_cc = tbc_cc.GetNumberKets();
  arma::uvec kets_ph =
      arma::join_cols(tbc_cc.GetKetIndex_hh(), tbc_cc.GetKetIndex_ph());
  ...
  if (orientation == "normal")
    ...
    std::cout << __PRETTY_FUNCTION__ << " =>  Unknown orientation input  "
              << orientation << ". Don't know what to do with this."
              << std::endl;
    return;
  }
  ...
  for (int ibra = 0; ibra < nph_kets; ++ibra) {
    ...
    // we want to evaluate a<=b and a>=b, so to avoid code duplication, we turn
    // this into a loop over the two orderings
    std::vector<size_t> ab_switcheroo = {bra_cc.p, bra_cc.q};
    for (int ab_case = 0; ab_case <= 1; ab_case++) {
      int a = ab_switcheroo[ab_case];  // this little bit gives us a,b if
                                       // ab_case=0 and b,a if ab_case=1
      int b = ab_switcheroo[1 - ab_case];
      size_t bra_shift =
          ab_case *
          nph_kets;  // if we switch a<->b, we offset the bra index by nph_kets
      ...
      for (int iket_cc = 0; iket_cc < nKets_cc; ++iket_cc) {
        ...
        {
          TwoBody_CC_ph(iket_cc, ibra + bra_shift) =
              herm * Xbar *
              na_nb_factor;  // we slap the (na-nb) on the transposed one.
        }
      }
    }
  }
...
/*
void DoPandyaTransformation_SingleChannel_XandY(const Operator& X, const
Operator& Y, arma::mat& X2_CC_ph, arma::mat& Y2_CC_ph, int ch_cc)
{
//   int hX = X.IsHermitian() ? 1 : -1;
//   int hY = X.IsHermitian() ? 1 : -1;
   TwoBodyChannel& tbc_cc = X.modelspace->GetTwoBodyChannel_CC(ch_cc);
   int nKets_cc = tbc_cc.GetNumberKets();
   arma::uvec kets_ph = arma::join_cols(tbc_cc.GetKetIndex_hh(),
tbc_cc.GetKetIndex_ph() ); int nph_kets = kets_ph.n_rows; int J_cc = tbc_cc.J;

   X2_CC_ph.zeros( nKets_cc, 2*nph_kets );
   Y2_CC_ph.zeros( 2*nph_kets, nKets_cc);

   // TODO: For a set of one-body channels a,b the J coupling stuff should all
be the same, so
   //       there is probably some gain to be had by looping over radial quantum
numbers na,nb
   ...
   {
      Ket & bra_cc = tbc_cc.GetKet( kets_ph[ibra] );
      // we want to evaluate a<=b and a>=b, so to avoid code duplication, we
turn this into a loop over the two orderings std::vector<size_t> ab_switcheroo =
{ bra_cc.p, bra_cc.q }; for ( int ab_case=0; ab_case<=1; ab_case++)
      {
        int a = ab_switcheroo[ab_case];   // this little bit gives us a,b if
ab_case=0 and b,a if ab_case=1 int b = ab_switcheroo[1-ab_case]; size_t
bra_shift = ab_case*nph_kets;  // if we switch a<->b, we offset the bra index by
nph_kets
...
//           int d = iket_cc < nKets_cc ? ket_cc.q : ket_cc.p;
           ...
           // Check the isospin projection. If this isn't conserved in the usual
channel,
           // then all the xcbad and yadcb will be zero and we don't need to
bother computing SixJs. if ( ( std::abs(oa.tz2+od.tz2  - ob.tz2-oc.tz2) !=
2*X.GetTRank() ) and ( std::abs(oa.tz2+od.tz2  - ob.tz2-oc.tz2) !=
2*Y.GetTRank() ) )    continue;

           double jc = oc.j2*0.5;
           double jd = od.j2*0.5;

//           if (oc.n>0 or od.n>0) continue; // There is probably a more robust
way to do this.

           std::set<size_t> cvals;
           std::set<size_t> dvals;
           for ( auto cc : Y.OneBodyChannels.at({oc.l,oc.j2,oc.tz2}) )
cvals.insert(cc); for ( auto dd : Y.OneBodyChannels.at({od.l,od.j2,od.tz2}) )
dvals.insert(dd);
...
//           {
//               std::cout << " oops " << __func__ << " " << __LINE__ << " abcd
<< " << a << " " << b << " " << c << " " << d << "   Xbar,Ybar = " << Xbar << "
" << Ybar << std::endl;
//           }
...
void DoPandyaTransformation_SingleChannel_XandY(const Operator& X,
                                                const Operator& Y,
                                                arma::mat& X2_CC_ph,
                                                arma::mat& Y2_CC_ph,
                                                int ch_cc) {
  ...
  // TODO: For a set of one-body channels a,b the J coupling stuff should all be
  // the same, so
  //       there is probably some gain to be had by looping over radial quantum
  //       numbers na,nb and nc,nd  inside the Jph loop.
  ...
  for (int ibra = 0; ibra < nph_kets; ++ibra) {
    ...
    for (int ab_case = 0; ab_case <= 1; ab_case++) {
      ...
      for (int iket_cc = 0; iket_cc < nKets_cc; ++iket_cc) {
        ...
        // Check the isospin projection. If this isn't conserved in the usual
        // channel, then all the xcbad and yadcb will be zero and we don't need
        // to bother computing SixJs.
        ...
        //           if ( (oa.tz2+od.tz2) != (ob.tz2+oc.tz2))
        //           {
        //               std::cout << " oops " << __func__ << " " << __LINE__ <<
        //               "   abcd << " << a << " " << b << " " << c << " " << d
        //               << "   Xbar,Ybar = " << Xbar << " " << Ybar <<
        //               std::endl;
        //           }
...
// void Operator::DoPandyaTransformation(deque<arma::mat>& TwoBody_CC_ph,
// std::string orientation="normal") const
void DoPandyaTransformation(const Operator& Z,
                            std::deque<arma::mat>& TwoBody_CC_ph,
                            std::string orientation = "normal") {
  // loop over cross-coupled channels
  int n_nonzero = Z.modelspace->SortedTwoBodyChannels_CC.size();
#pragma omp parallel for schedule( \
    dynamic, 1) if (not Z.modelspace->scalar_transform_first_pass)
  for (int ich = 0; ich < n_nonzero; ++ich) {
    int ch_cc = Z.modelspace->SortedTwoBodyChannels_CC[ich];
    DoPandyaTransformation_SingleChannel(Z, TwoBody_CC_ph[ch_cc], ch_cc,
                                         orientation);
  }
}

// currently, this is not used anywhere...
void AddInversePandyaTransformation_SingleChannel(Operator& Z, arma::mat& Zbar,
                                                  int ch_cc) {
  // Only go parallel if we've previously calculated the SixJ's. Otherwise, it's
  // not thread safe.
  ...
  //   #pragma omp parallel for schedule(dynamic,1) if (not
  //   Z.modelspace->scalar_transform_first_pass)
  for (int ich = 0; ich < n_nonzeroChannels; ++ich) {
    ...
    for (int ibra = 0; ibra < nKets; ++ibra) {
      ...
      for (int iket = ketmin; iket < nKets; ++iket) {
        ...
        //            if ( ((oi.l+ol.l)%2==parity_cc)  and
        //            (std::abs(oi.tz2+ol.tz2)==Tz_cc*2) and (Jcc>=jmin) and
        //            (Jcc<=jmax) )
        if (((oi.l + ol.l) % 2 == parity_cc) and
            (std::abs(oi.tz2 - ol.tz2) == Tz_cc * 2) and (Jcc >= jmin) and
            (Jcc <= jmax)) {
        ...
        if (k == l) {
          ...
          //              if ( (oi.l+ok.l)%2==parity_cc  and
          //              std::abs(oi.tz2+ok.tz2)==Tz_cc*2 and Jcc>=jmin and
          //              Jcc<=jmax)
          if ((oi.l + ok.l) % 2 == parity_cc and
              std::abs(oi.tz2 - ok.tz2) == Tz_cc * 2 and Jcc >= jmin and
              Jcc <= jmax) {
            ...
            int indx_lj = tbc_cc.GetLocalIndex(std::min(l, j), std::max(l, j)) +
                          (l > j ? nkets_cc : 0);
            // we always have i<=k so we should always flip Z_jlki =
            // (-1)^{i+j+k+l} Z_iklj the phase we get from that flip combines
            // with the phase from Pij, to give the phase included below
            commji -= Jhat2 * sixj * Zbar(indx_ik, indx_lj);
          }
        }
        double norm = bra.delta_pq() == ket.delta_pq() ? 1 + bra.delta_pq()
                                                       : PhysConst::SQRT2;
#pragma omp atomic
        Zmat(ibra, iket) -=
            (commij - Z.modelspace->phase(jk + jl - J) * commji) / norm;
      }
    }
  }
}
...
void AddInversePandyaTransformation(const std::deque<arma::mat>& Zbar,
                                    Operator& Z) {
  ...
  for (int ich = 0; ich < n_nonzeroChannels; ++ich) {
    ...
    for (int ibra = 0; ibra < nKets; ++ibra) {
      ...
      for (int iket = ketmin; iket < nKets; ++iket) {
        ...
        double norm = bra.delta_pq() == ket.delta_pq() ? 1 + bra.delta_pq()
                                                       : PhysConst::SQRT2;
        Z.TwoBody.GetMatrix(ch, ch)(ibra, iket) -=
            (commij - Z.modelspace->phase(jk + jl - J) * commji) / norm;
      }
    }
  }
...
///*************************************
// deque<arma::mat> Operator::InitializePandya(size_t nch, std::string
// orientation="normal")
std::deque<arma::mat> InitializePandya(Operator& Z, size_t nch,
                                       std::string orientation = "normal") {
  ...
  for (int ich = 0; ich < n_nonzero; ++ich) {
    ...
    int nKets_cc = tbc_cc.GetNumberKets();
    int nph_kets =
        tbc_cc.GetKetIndex_hh().size() + tbc_cc.GetKetIndex_ph().size();
    if (orientation == "normal")
...
//
/// Calculates the part of \f$ [X_{(2)},Y_{(2)}]_{ijkl} \f$ which involves ph
/// intermediate states, here indicated by \f$ Z^{J}_{ijkl} \f$ \f[ Z^{J}_{ijkl}
/// = \sum_{ab}(n_a\bar{n}_b-\bar{n}_an_b)\sum_{J'} (2J'+1) \left[
///  \left\{ \begin{array}{lll}
///  j_i  &  j_j  &  J \\
///  j_k  &  j_l  &  J' \\
///  \end{array} \right\}
...
///    \bar{Y}^{J'}_{i\bar{l}b\bar{a}}\bar{X}^{J'}_{b\bar{a}k\bar{j}}
///    \right)\right]
/// \f]
/// The Pandya-transformed matrix elements are obtained with
/// DoPandyaTransformation(). The matrices \f$
/// \bar{X}^{J'}_{i\bar{l}a\bar{b}}\bar{Y}^{J'}_{a\bar{b}k\bar{j}} \f$ and \f$
/// \bar{Y}^{J'}_{i\bar{l}a\bar{b}}\bar{X}^{J'}_{a\bar{b}k\bar{j}} \f$ are
/// related by a Hermitian conjugation, which saves two matrix multiplications.
...
void comm222_phss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  //   const auto& pandya_lookup = Z.modelspace->GetPandyaLookup(Z.GetJRank(),
  //   Z.GetTRank(), Z.GetParity()); t_start = omp_get_wtime();
  ...
  //      index_t nKets_cc =
  //      Z.modelspace->GetTwoBodyChannel_CC(ch).GetNumberKets();
  //      Z_bar[ch].zeros( nKets_cc, 2*nKets_cc );
  ////      if ( pandya_lookup.at({ch,ch})[0].size()>0 ) lookup_empty[ich] =
  ///false;
  //      if ( pandya_lookup.at({ch,ch})[0].size()>0 )
  //      non_empty_channels.push_back(ch);
  //   }
  //
...
#pragma omp parallel for schedule( \
  ...
  for (size_t ch = 0; ch < nch; ++ch) {
    ...
    // This function gives  <ij`| X |ab`> *na(1-nb)  and   <ab`| Y |ij`>,  for
    // both orderings ab` and ba`
    ...
    //                                           [      |     ]
    //     create full Y matrix from the half:   [  Yhp | Y'ph]   where the
    //     prime indicates multiplication by (-1)^(i+j+k+l) h_y
    //                                           [      |     ]   Flipping hp
    //                                           <-> ph and multiplying by the
    //                                           phase is equivalent to [  Yph |
    //                                           Y'hp]   having kets |kj> with
    //                                           k>j.
    //
    //
    //      so <il|Zbar|kj> =  <il|Xbar|hp><hp|Ybar|kj> +
    //      <il|Xbar|ph><ph|Ybar|kj>
    //
    arma::mat Y_bar_ph_flip =
        arma::join_vert(Y_bar_ph.tail_rows(nph_kets) % PhaseMatY,
                        Y_bar_ph.head_rows(nph_kets) % PhaseMatY);
    Zbar_ch = Xt_bar_ph * arma::join_horiz(Y_bar_ph, Y_bar_ph_flip);

    // If Z is hermitian, then XY is anti-hermitian, and so XY - YX = XY +
    // (XY)^T
    if (Z.IsHermitian()) {
    ...
    // By taking the transpose, we get <il|Zbar|kj> with i>l and k<j, and we
    // want the opposite By the symmetries of the Pandya-transformed matrix
    // element, that means we pick up a factor hZ * phase(i+j+k+l). The hZ
    // cancels the hXhY we have for the "head" part of the matrix so we end up
  ...
  // and the ph commutator has an overall minus sign, so we're technically
  // subtracting the inverse Pandya transformation. Also, the inverse Pandya
  // transformation is just the regular Pandya transformation. The distinction
  // in the code is because some other commutator-specific things are done at
  // the same time.
...
)
{
   ...
   for (size_t ch_CC=0; ch_CC<nch_CC; ch_CC++)
   {
     ...
     /// Fill Xbar and Ybar    Xbar^Jcc_ijab  =  - sum_J  (2J+1) { i  j  Jcc }
X^J_ibaj
     ///                                                         { a  b  J   }
     for ( size_t ibra_CC=0; ibra_CC<nkets_CC; ibra_CC++)
     {
       ...
       for ( size_t index_ab=0; index_ab<nkets_ab; index_ab++)
       {
         ...
         double xbar_ijab = 0;
         double ybar_abij = 0;
         int Jmin = std::max( std::abs( oi.j2-ob.j2), std::abs( oj.j2-oa.j2)
)/2; int Jmax = std::max( ( oi.j2+ob.j2), ( oj.j2+oa.j2) )/2; for (int J=Jmin;
J<=Jmin; J++)
         {
         ...
         double xbar_ijba = 0;
         double ybar_baij = 0;
         int Jmin = std::max( std::abs( oi.j2-oa.j2), std::abs( oj.j2-ob.j2)
)/2; int Jmax = std::max( ( oi.j2+oa.j2), ( oj.j2+ob.j2) )/2; for (int J=Jmin;
J<=Jmin; J++)
         {
...
//  For the three-body operators, we often encounter formulas involving the
//  J-coupled permutation operator P(ij/k)^{J1,J}, which is defined as
//
//  P(ij/k)^{J1,J} = 1 - sum_J2 sqrt{(2J1+1)(2J2+1)} (-1)^2(ji+jj+jk)   {ji jk
//  J1} P_ik
//                                                                      {jk J
//                                                                      J2}
//
//                     - sum_J2 sqrt{(2J1+1)(2J2+1)} (-1)^{jj+jk+J1+J2) {jk ji
//                     J1} P_jk
//                                                                      {jk J
//                                                                      J2}
//
...
//   /\      /\      /\         Z_0 = 1/36 sum_abcdef (nanbnc n`dn`en`f)
//   (X_abcdef Y_defabc - Y_abcdef X_defabc)
// a(  )d  b(  )e  c(  )f
//   \/      \/      \/      Coupled expression:
//    ~~~~~~~~~~~~~~~~        Z_0 = 1/36 sum_abcdef sum_J1,J2,J  (nanbnc
//    n`dn`en`f)  (2J+1)
//                                    (X^{J1J2J}_abcdef Y^{J1J2J}_defabc -
//                                    Y^{J1J2J}_abcdef X^{J1J2J}_defabc)
//    Verified with UnitTest
//
/// Three body commutator expression
/// \f{equation}{
/// Z_0 = \frac{1}{36} \sum_{abcdef} \sum_{J_{1}J_{2}J} n_a n_b n_c
/// \bar{n}_d\bar{n}_e\bar{n}_f (2J+1)
///        ( X^{J_{1}J_{2}J}_{abcdef} Y^{J_1J_2J}_{defabc} -
///        Y^{J_{1}J_{2}J}_{abcdef} X^{J_1J_2J}_{defabc})
/// \f}
///
void comm330ss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  //  std::cout << " in  " << __func__ << "  ||X3|| = " << X3.Norm() << " ||Y3||
  //  = " << Y3.Norm()
  //            << "   hermitian? " << X.IsHermitian() << "  " <<
  //            Y.IsHermitian()
  //            << "   3body herm: " << X3.IsHermitian() << "  " <<
  //            Y3.IsHermitian()
  //            << "  Storage mode " << X3.GetStorageMode() << "   " <<
  //            Y3.GetStorageMode() << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1) reduction(+ : z0)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets; ibra++) {
      ...
      if (bra.p == bra.q and bra.q == bra.r)
        ...
        abc_symm = 3;
      if ((std::abs(ea - e_fermi[tza]) + std::abs(eb - e_fermi[tzb]) +
           std::abs(ec - e_fermi[tzc])) > Z.modelspace->GetdE3max())
        continue;
      if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) * occnat_c *
           (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
        continue;
      //      for (size_t iket=0;iket<nkets; iket++)
      //      for (size_t iket=ibra;iket<nkets; iket++)
      for (size_t iket = 0; iket < ibra;
           iket++)  // dont need iket=ibra because the commutator will be zero
      {
        ...
        int tze = ket.oq->tz2;
        int tzf = ket.oR->tz2;
        if ((std::abs(ed - e_fermi[tzd]) + std::abs(ee - e_fermi[tze]) +
             std::abs(ef - e_fermi[tzf])) > Z.modelspace->GetdE3max())
          continue;
        if ((occnat_d * (1 - occnat_d) * occnat_e * (1 - occnat_e) * occnat_f *
             (1 - occnat_f)) < Z.modelspace->GetOccNat3Cut())
          continue;
        // account for the iket>ibra case, which we don't do explicitly
        double occfactor = na * nb * nc * (1 - nd) * (1 - ne) * (1 - nf) -
                           (1 - na) * (1 - nb) * (1 - nc) * nd * ne * nf;
        ...
        z0 += 1. / 36 * occfactor * abc_symm * def_symm * (twoJ + 1) *
              (xabcdef * ydefabc - yabcdef * xdefabc);
        //        std::cout << "    abcdef " << bra.p << " " << bra.q << " " <<
        //        bra.r << " " << ket.p << " " << ket.q << " " << ket.r <<
        //        std::endl; std::cout << "     z0 += " << 1./36 << " * " <<
        //        occfactor << " * " << abc_symm << " * " << def_symm << " * "
        //                  << (twoJ+1) << " *  ( " << xabcdef << " * " <<
        //                  ydefabc << " - " << yabcdef << " * " << xdefabc << "
        //                  )  => " << z0 << std::endl;
  ...
  std::cout << "Adding " << std::setprecision(9) << z0 << "  to Zero Body"
            << std::endl;
  Z.ZeroBody += z0;
...
//   / \      / \    |          Z_ij = 1/12 sum_abcde (nanb n`c n`dn`e)
//   (X_abicde Y_cdeabj - Y_abicde X_cdeabj)
// a(   )c  b(   )d  |e
//   \ /      \ /    |       Coupled expression:
//    *~~~~~~~[Y]~~~~*        Z_ij = 1/12 sum_abcde sum_J1,J2,J  (nanb n`c
//    n`dn`e)  (2J+1)/(2ji+1)
//                   |j               (X^{J1J2J}_abicde Y^{J1J2J}_cdeabj -
//                   Y^{J1J2J}_abicde X^{J1J2J}_cdeabj)
//
...
#pragma omp parallel for schedule( \
    dynamic, 1) if (not Z.modelspace->scalar3b_transform_first_pass)
    ...
    for (auto j : Z.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
      ...
      for (size_t ch_ab = 0; ch_ab < nch2; ch_ab++) {
        ...
        for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
          ...
          //      double occnat_c = bra.oR->occ_nat;
          if ((std::abs(ea - e_fermi[tza]) + std::abs(eb - e_fermi[tzb]) +
               std::abs(ei - e_fermi[tzi])) > Z.modelspace->GetdE3max())
            continue;
          if ((std::abs(ea - e_fermi[tza]) + std::abs(eb - e_fermi[tzb]) +
               std::abs(ej - e_fermi[tzj])) > Z.modelspace->GetdE3max())
            continue;
          if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
               occnat_i * (1 - occnat_i)) < Z.modelspace->GetOccNat3Cut())
            continue;
          if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
               occnat_j * (1 - occnat_j)) < Z.modelspace->GetOccNat3Cut())
            continue;

          for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
            size_t ch_abi = Z.modelspace->GetThreeBodyChannelIndex(
                twoJ, (tbc_ab.parity + oi.l) % 2, tbc_ab.Tz * 2 + oi.tz2);
            if (ch_abi == size_t(-1))
            ...
            //             std::cout << "index_abi = " << index_abi << "
            //             index_abj = " << index_abj << std::endl; if
            //             ((index_abi==-1) or (index_abj==-1)) continue;
            ...
            for (size_t iket_cde = 0; iket_cde < nkets3; iket_cde++) {
              ...
              double occfactor = (ket_ab.op->occ * ket_ab.oq->occ) *
                                 (1 - ket_cde.op->occ) * (1 - ket_cde.oq->occ) *
                                 (1 - ket_cde.oR->occ);
              if (std::abs(occfactor) < 1e-6) continue;
              size_t c = ket_cde.p;
              size_t d = ket_cde.q;
              ...
              if ((std::abs(ec - e_fermi[tzc]) + std::abs(ed - e_fermi[tzd]) +
                   std::abs(ee - e_fermi[tze])) > Z.modelspace->GetdE3max())
                continue;
              if ((occnat_c * (1 - occnat_c) * occnat_d * (1 - occnat_d) *
                   occnat_e * (1 - occnat_e)) < Z.modelspace->GetOccNat3Cut())
                continue;
              ...
              zij += 1. / 12 * ab_symmetry_factor * cde_symmetry_factor *
                     occfactor * Jfactor *
                     (X3.GetME_pn(Jab, Jcd, twoJ, a, b, i, c, d, e) *
                          Y3.GetME_pn(Jcd, Jab, twoJ, c, d, e, a, b, j) -
                      Y3.GetME_pn(Jab, Jcd, twoJ, a, b, i, c, d, e) *
                          X3.GetME_pn(Jcd, Jab, twoJ, c, d, e, a, b, j));
            }  // for iket_cde
          }    // for twoJ
        }      // for iket_ab
      }        // for ch_ab
...
//                   |i
//    *~~[X]~~*      |        Uncoupled expression:
//   / \      / \    |          Z_ij = 1/4 sum_abcd (nanb n`cn`d) (X_abcd
//   Y_cdiabj - Y_abicdj X_cdab)
// a(   )c  b(   )d  |
//   \ /      \ /    |       Coupled expression:
//   *~~~~~~~[Y]~~~~~*        Z_ij = 1/4sum_abcd sum_J1J  (nanb n`c n`d)
//   (2J+1)/(2ji+1)
//                   |j               (X^{J1}_abcd Y^{J1J1J}_cdiabj -
//                   Y^{J1J1J}_abicdj X^{J1}_cdab)
//                   |
//                              We only sum a<=b and c<=d, so we do not
//                              explicitly include the factor 1/4, except for
//                              the a==b, or c==d case, where there is no double
//                              counting.
//  Verfied with UnitTest
//
      ...
      for (int ch = 0; ch < nch; ch++) {
        ...
        for (size_t ibra = 0; ibra < nkets; ibra++) {
          ...
          if ((d_ea + d_eb + std::min(d_ei, d_ej)) > Z.modelspace->dE3max)
            continue;
          if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
               std::max(occnat_i * (1 - occnat_i), occnat_j * (1 - occnat_j))) <
              Z.modelspace->GetOccNat3Cut())
            continue;
          double na = bra.op->occ;
          double nb = bra.oq->occ;
          if ((std::abs(na * nb) + std::abs((1 - na) * (1 - nb))) < 1e-6)
            continue;
          ...
          for (size_t iket = 0; iket < ibra; iket++) {
            ...
            double prefactor =
                na * nb * (1 - nc) * (1 - nd) - (1 - na) * (1 - nb) * nc * nd;
            ...
            for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
              ...
              double xcdiabj = 0;
              double ycdiabj = 0;
              if ((std::max(ea + eb + ej, ec + ed + ei) <=
                   Z.modelspace->E3max) and
                  (std::max(d_ea + d_eb + d_ej, d_ec + d_ed + d_ei) <=
                   Z.modelspace->dE3max) and
                  ((occnat_c * (1 - occnat_c) * occnat_d * (1 - occnat_d) *
                    occnat_i * (1 - occnat_i)) >
                   Z.modelspace->GetOccNat3Cut()) and
                  ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                    occnat_j * (1 - occnat_j)) >
                   Z.modelspace->GetOccNat3Cut())) {
                //                xcdiabj = X3.GetME_pn(J,J,twoJ,c,d,i,a,b,j);
                xcdiabj = x_particle_rank > 2
                              ? X3.GetME_pn(J, J, twoJ, c, d, i, a, b, j)
                              : 0;
                ycdiabj = Y3.GetME_pn(J, J, twoJ, c, d, i, a, b, j);
              }
              if ((std::max(ea + eb + ei, ec + ed + ej) <=
                   Z.modelspace->E3max) and
                  (std::max(d_ea + d_eb + d_ei, d_ec + d_ed + d_ej) <=
                   Z.modelspace->dE3max) and
                  ((occnat_c * (1 - occnat_c) * occnat_d * (1 - occnat_d) *
                    occnat_j * (1 - occnat_j)) >
                   Z.modelspace->GetOccNat3Cut()) and
                ...
                xabicdj = x_particle_rank > 2
                              ? X3.GetME_pn(J, J, twoJ, a, b, i, c, d, j)
                              : 0;
                yabicdj = Y3.GetME_pn(J, J, twoJ, a, b, i, c, d, j);
              }
              zij += prefactor * (twoJ + 1) *
                     ((Xabcd * ycdiabj - yabicdj * Xcdab) -
        ...
        for (size_t ibra = 0; ibra < nbras; ibra++) {
          ...
          for (size_t iket = 0; iket < nkets; iket++) {
            ...
            for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
              ...
              zij += prefactor * (twoJ + 1) *
                     ((Xabcd * ycdiabj - yabicdj * Xcdab));
              //              zij += prefactor * (twoJ+1) * ( (Xabcd * ycdiabj -
              //              yabicdj * Xcdab)
              //                                           -  (Yabcd * xcdiabj -
              //                                           xabicdj * Ycdab) );
...
//  |   |              Z_{ijkl}^{J} = sum_ab (nan`b-n`anb) sum_J' (2J'+1)/(2J+1)
//  ( X_ab * Y_{ijbkla}^{J,J,J'} )
// k|  l|
//
//
//                 When we call Symmetrize, we copy the upper triangle
//                 <ibra|iket> with ibra<=iket onto the lower triangle. Of
//                 course, calling AddToTBME updates both according to the
//                 symmetry, so we don't need to worry so much...
//
//  Verfied with UnitTest
//
void comm132ss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  Z.modelspace
      ->PreCalculateSixJ();  // Presumably this has already been called. If so,
                             // this does nothing. But we want to make sure.

  int norb = Z.modelspace->GetNumberOrbits();
  size_t nch = Z.modelspace->GetNumberTwoBodyChannels();
  // The only potentially thread-unsafe part of this loop is the access to 3b
  // matrix elements which might require recoupling, leading to a 6j. If these
  // are precomputed, there is no thread safety issue, so no need to check
  // first_pass
  //  #pragma omp parallel for schedule(dynamic,1) if (not
  //  Z.modelspace->scalar3b_transform_first_pass)
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch = 0; ch < nch; ch++) {
    auto& tbc = Z.modelspace->GetTwoBodyChannel(ch);
    int J = tbc.J;
    ...
    for (int ibra = 0; ibra < nkets; ibra++)  // <ij| states
    {
      ...
      for (int iket = ibra; iket < nkets; iket++)  // |kl> states
      {
        ...
        double zijkl = 0;
        // TODO loop over 1-body channels to do the 3-body recoupling in the
        // outer loop Then loop over a and b within that channel.
        for (int a = 0; a < norb; a++) {
          ...
          if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) *
               occnat_a * (1 - occnat_a)) < Z.modelspace->GetOccNat3Cut())
            continue;

          int twoJ_min = std::abs(oa.j2 - 2 * J);
          int twoJ_max = oa.j2 + 2 * J;
          // this is less efficient than doing the loop twice, but less code
          // and easily lets us treat the case of Y having nonzero Tz or odd
          // parity
          std::set<size_t> blist;
          //          std::cout << "HERE AT LINE " << __LINE__ <<  " and a is "
          //          << oa.l << " " << oa.j2 << " " << oa.tz2 << std::endl;
          for (auto b : X.OneBodyChannels.at({oa.l, oa.j2, oa.tz2}))
            blist.insert(b);
          for (auto b : Y.OneBodyChannels.at({oa.l, oa.j2, oa.tz2}))
            blist.insert(b);
          //          for ( auto b : Z.OneBodyChannels.at({oa.l,oa.j2,oa.tz2}) )
          //          // TODO: We can make this a<=b or a>=b, I think. Just need
          //          to mind some factors of 2
          for (auto b : blist) {
            Orbit& ob = Z.modelspace->GetOrbit(b);
            int eb = 2 * ob.n + ob.l;
            ...
            if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) *
                 occnat_b * (1 - occnat_b)) < Z.modelspace->GetOccNat3Cut())
              continue;
            double occfactor = oa.occ - ob.occ;
            if (std::abs(occfactor) < 1e-6) continue;
            for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
              //              double xijbkla =
              //              X3.GetME_pn(J,J,twoJ,i,j,b,k,l,a);
              //
              auto xandy =
                  Y3.GetME_pn_TwoOps(J, J, twoJ, i, j, b, k, l, a, X3, Y3);
              //              double xijbkla = x_particle_rank > 2 ?
              //              X3.GetME_pn(J,J,twoJ,i,j,b,k,l,a) : 0; double
              //              yijbkla = Y3.GetME_pn(J,J,twoJ,i,j,b,k,l,a);
              double xijbkla = xandy[0];
              double yijbkla = xandy[1];

              zijkl += occfactor * (twoJ + 1.) / (2 * J + 1) *
                       (X1(a, b) * yijbkla - Y1(a, b) * xijbkla);
            }
          }
        }
        // normalize the tbme
...
//  *~~[X]~*  |         Z_ijkl = -1/2 sum_abc (nanbn`c+n`an`bnc) ( (1-Pij)
//  X_icab * Y_abjklc - (1-Pkl) Yijcabl * Xabkc )
// a|    b/c\ |
//  |    /   \|
//  *~~[Y]~~~~*      Coupled expression:
//  |   |              Z_{ijkl}^{J} = -1/2 sum_abc (nanbn`c+n`an`bnc) sum_J'J"
//  (2J'+1)(2J"+1)/sqrt(2J+1) (-1)^{2J"+J'-J}
// k|  l|                           *  [   (1 - (-1)^{i+j-J}Pij) (-1)^{j-c} { j
// J" J' } X_icab^{J'} * Y_{abjklc}^{J'JJ"}
//                                                                          { c
//                                                                          i  J
//                                                                          }
//
//                                       -(1 - (-1)^{k+l-J}Pkl)  (-1)^{l-c} { l
//                                       J" J' } Y_{ijcabl}^{JJ'J"} *
//                                       X_{abkc}^{J'}  ]
//                                                                          { c
//                                                                          k  J
//                                                                          }
//
//   The factor 1/2 out front is absorbed by the fact that we only do the
//   ordering a<=b  (need to deal carefully with a==b)
//
...
void comm232ss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  std::map<std::array<int, 3>, std::vector<size_t>>
      local_one_body_channels;  //  maps {j,parity,tz} => vector of index_j
  std::map<std::array<int, 3>, std::vector<size_t>>
      external_local_one_body_channels;  //  maps {j,parity,tz} => vector of
                                         //  index_j
  for (auto j : Z.modelspace->all_orbits) {
    ...
    if (external_local_one_body_channels.find(obc) ==
        external_local_one_body_channels.end())
  ...
  //  {
  //    std::cout << lobc.first[0] << " " << lobc.first[1] << " " <<
  //    lobc.first[2] << " : "; for ( auto& i : lobc.second )  std::cout << i <<
  //    "  "; std::cout << std::endl;
  //  }

  // next, figure out which three-body states |klj`> and |abc`> exist, make a
  // list, and give them an index for where they'll sit in the matrix
  std::map<std::array<int, 3>, std::vector<std::array<size_t, 4>>>
      klj_list;  //  maps {j,parity,tz} => {kljJ}
  std::map<std::array<int, 3>, arma::mat>
      ZMAT_list;  //  maps {j,parity,tz} => matrix <i|Z|klj`>
  ...
  for (size_t ikey = 0; ikey < nkeys; ikey++) {
    ...
    for (auto& iter_j : local_one_body_channels) {
      ...
      for (int Jkl = Jkl_min; Jkl <= Jkl_max; Jkl++) {
        size_t ch_kl =
            Z.modelspace->GetTwoBodyChannelIndex(Jkl, paritykl, Tzkl);
        ...
        for (size_t iket_kl = 0; iket_kl < nkets_kl; iket_kl++) {
          ...
          //          double de_k = std::abs( 2*ket_kl.op->n + ket_kl.op->l -
          //          e_fermi[ket_kl.op->tz2]); double de_l = std::abs(
          //          2*ket_kl.oq->n + ket_kl.oq->l - e_fermi[ket_kl.oq->tz2]);
          ...
          for (size_t j : iter_j.second) {
            Orbit& oj = Z.modelspace->GetOrbit(j);
            //            double de_j = std::abs( 2*oj.n + oj.l -
            //            e_fermi[oj.tz2]);
            double occnat_j = oj.occ_nat;
            if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) *
                 occnat_j * (1 - occnat_j)) < Z.modelspace->GetOccNat3Cut())
              continue;
            //            if ( (de_k + de_l + de_j) > Z.modelspace->GetdE3max()
            //            ) continue; double nj = oj.occ;

            klj_list_i.push_back({ket_kl.p, ket_kl.q, j, (size_t)tbc_kl.J});
            //            std::cout <<"   ikey = " << ikey << " : " << j2i << "
            //            " << parityi << " " << tz2i << "   "
            //                      << "  adding " << ket_kl.p << " " <<ket_kl.q
            //                      << " " << j << "  J=" << tbc_kl.J <<
            //                      std::endl;
    ...
    size_t dim_i =
        obc_orbits.size();  // how many sp states are in this jpt channel
    size_t dim_klj = klj_list[obc_key].size();
    ZMAT_list[obc_key] = arma::mat(dim_i, dim_klj);
  }  // for ikey

  Z.modelspace->PreCalculateSixJ();  // if this has already been done, this does
                                     // nothing.
  // This loop is what takes all the time.
  // Outer loop over one-body quantum numbers, which also label the three-body
  // pph states |klj`> and |abc`>
  //  #pragma omp parallel for schedule(dynamic,1) if (not
  //  Z.modelspace->scalar3b_transform_first_pass)
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ikey = 0; ikey < nkeys; ikey++) {
    ...
    int j2i = obc_key[0];
    //    std::vector<size_t>& obc_orbits = local_one_body_channels[obc_key]; //
    //    the orbits that have quantum numbers {j2,parity,tz2}
    std::vector<size_t>& obc_orbits = external_local_one_body_channels
        [obc_key];  // the orbits that have quantum numbers {j2,parity,tz2}
    auto& klj_list_i =
        klj_list[obc_key];  // list of 3-body pph stats |klj`> with quantum
                            // numbers {j2,parity,tz2}

    std::vector<size_t>
        abc_list;  // keep track of which klj states should go in the abc loop
    ...
    for (size_t i_kljJ = 0; i_kljJ < klj_list[obc_key].size(); i_kljJ++) {
      ...
      if (a == b)
        occupation_factor *= 0.5;  // we sum a<=b, and drop the 1/2, but we
                                   // still need the 1/2 for a==b
      abc_list.push_back(i_kljJ);
      abc_occ_list.push_back(occupation_factor);
      //      std::cout << "   || ikey = " << ikey << "   keeping abc,J " << a
      //      << " " << b << " " << c << " " << kljJ[3] << "  with index " <<
      //      i_kljJ << std::endl;
    }
    ...
    size_t dim_klj =
        klj_list[obc_key]
            .size();  // how many 3-body pph states in this jpt channel
    size_t dim_abc = abc_list.size();  // how many 3-body states which
                                       // contribute to the |abc`> sum
    ...
    std::set<std::array<size_t, 5>>
        kljJJ_needed;  // we use a set to avoid repeats
    for (size_t ind_abc = 0; ind_abc < dim_abc; ind_abc++) {
      ...
      for (size_t ind_klj = 0; ind_klj < dim_klj; ind_klj++) {
        ...
        if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) * occnat_c *
             (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
          continue;
        if (imsrg3_no_qqq and
            ((ok.cvq + ol.cvq + oc.cvq) > 5 or (oa.cvq + ob.cvq + oj.cvq) > 5))
          continue;
        if (imsrg3_valence_2b and (ok.cvq != 1 or ol.cvq != 1 or oj.cvq != 1))
          continue;
        int twoJp_min =
            std::max(std::abs(2 * Jab - oj.j2), std::abs(2 * Jkl - j2c));
    ...
    std::vector<double>
        recoupling_cache;  // a vector storing all the relevant recoupling info
    std::unordered_map<size_t, size_t>
        recoupling_cache_lookup;  // a lookup table so we can find the info
                                  // easily
    ...
    for (auto kljJJ : kljJJ_needed) {
      ...
      size_t ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
          Jkl, twoJp, k, l, j, iketlist, recouplelist);
      ...
      for (size_t ik : iketlist)
        recoupling_cache.push_back(
            ik);  // Store the size_t (an unsigned int) as a double. I hope this
                  // doesn't cause problems...
      for (double rec : recouplelist)
        recoupling_cache.push_back(
            rec);  // Point to where this element lives in the vector
    }
    ...
    for (size_t ind_abc = 0; ind_abc < dim_abc; ind_abc++)  // rows of X2MAT
    {
      ...
      double jc = 0.5 * j2c;
      double occ_abc =
          abc_occ_list[ind_abc];  // TODO: we can probably incorporate that hat
                                  // factor with the occupation factor
      ...
      for (size_t ind_i = 0; ind_i < dim_i;
           ind_i++)  // ind_i indexes the list of sp states in this one body
                     // channel. columns of X2MAT
      {
        size_t i = obc_orbits[ind_i];  // i is the orbit index as per ModelSpace

        X2MAT(ind_i, ind_abc) = -sqrt((2 * Jab + 1.)) * occ_abc *
                                X.TwoBody.GetTBME_J(Jab, c, i, a, b);
        Y2MAT(ind_i, ind_abc) = -sqrt((2 * Jab + 1.)) * occ_abc *
                                Y.TwoBody.GetTBME_J(Jab, c, i, a, b);
      }  // for ind_i
      ...
      for (size_t ind_klj = 0; ind_klj < dim_klj; ind_klj++) {
        ...
        if ((de_a + de_b + de_j) > Z.modelspace->GetdE3max()) continue;
        if ((de_k + de_l + de_c) > Z.modelspace->GetdE3max()) continue;
        if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) * occnat_j *
             (1 - occnat_j)) < Z.modelspace->GetOccNat3Cut())
          continue;
        if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) * occnat_c *
             (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
          continue;
        ...
        for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
          //           double sixj = X.modelspace->GetSixJ(Jkl,jj,ji,  Jab, jc,
          //           0.5*twoJp );
          double sixj =
              X.modelspace->GetSixJ(jj, ji, Jkl, jc, 0.5 * twoJp, Jab);
          ...
          for (size_t ilist_abj = 0; ilist_abj < listsize_abj; ilist_abj++) {
            size_t ibra_abj =
                (size_t)recoupling_cache[pointer_abj + 2 + ilist_abj];
            double recouple_bra =
                recoupling_cache[pointer_abj + 2 + listsize_abj + ilist_abj];
            for (size_t ilist_klc = 0; ilist_klc < listsize_klc; ilist_klc++) {
              size_t iket_klc =
                  (size_t)recoupling_cache[pointer_klc + 2 + ilist_klc];
              double recouple_ket =
                  recoupling_cache[pointer_klc + 2 + listsize_klc + ilist_klc];

              //               xabjklc += recouple_bra*recouple_ket *
              //               X3.GetME_pn_ch(ch_check,ch_check, ibra_abj,
              //               iket_klc );
              if (x_has_3)
                xabjklc +=
                    recouple_bra * recouple_ket *
                    X3.GetME_pn_ch(ch_check, ch_check, ibra_abj, iket_klc);
              if (y_has_3)
                yabjklc +=
                    recouple_bra * recouple_ket *
                    Y3.GetME_pn_ch(ch_check, ch_check, ibra_abj, iket_klc);
            }
          }

          X3MAT(ind_abc, ind_klj) +=
              (twoJp + 1.) / sqrt(2 * Jkl + 1.) * sixj * xabjklc;
          Y3MAT(ind_abc, ind_klj) +=
              (twoJp + 1.) / sqrt(2 * Jkl + 1.) * sixj * yabjklc;

        }  // for twoJp
      }    // for ind_klj
    }      // for ind_abc

    //    std::cout << "ikey = " << ikey << "   " << obc_key[0] << " " <<
    //    obc_key[1] << " " << obc_key[2] << std::endl; if  (  (obc_key[1] != 1)
    //    or (obc_key[2]!=-1)  or (obc_key[0]>3 ) )
    //    {
    ...
    //    else
    //    {
    //       std::cout << "X2 looks like this: " << std::endl << X2MAT <<
    //       std::endl; std::cout << "In this channel, abclist is  "; for (auto&
    //       iabc : abc_list ) std::cout << iabc << " "; std::cout << std::endl;
    //    }
  ...
  // It looks like nothing dangerous happens inside the loop, so we don't need
  // to check for first_pass
  //  #pragma omp parallel for schedule(dynamic,1) if (not
  //  Z.modelspace->scalar3b_transform_first_pass)
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch = 0; ch < nch; ch++) {
    ...
    for (size_t ibra = 0; ibra < nkets; ibra++) {
      ...
      auto& lobc_i =
          external_local_one_body_channels.at({oi.j2, oi.l % 2, oi.tz2});
      auto& lobc_j =
          external_local_one_body_channels.at({oj.j2, oj.l % 2, oj.tz2});
      size_t ind_i = std::distance(lobc_i.begin(),
                                   std::find(lobc_i.begin(), lobc_i.end(), i));
      size_t ind_j = std::distance(lobc_j.begin(),
                                   std::find(lobc_j.begin(), lobc_j.end(), j));
      ...
      for (size_t iket = ibra; iket < nkets; iket++) {
        ...
        //        auto& lobc_k =
        //        local_one_body_channels.at({ok.j2,ok.l%2,ok.tz2}); auto&
        //        lobc_l = local_one_body_channels.at({ol.j2,ol.l%2,ol.tz2});
        auto& lobc_k =
            external_local_one_body_channels.at({ok.j2, ok.l % 2, ok.tz2});
        auto& lobc_l =
            external_local_one_body_channels.at({ol.j2, ol.l % 2, ol.tz2});
        size_t ind_k = std::distance(
            lobc_k.begin(), std::find(lobc_k.begin(), lobc_k.end(), k));
        size_t ind_l = std::distance(
            lobc_l.begin(), std::find(lobc_l.begin(), lobc_l.end(), l));
        ...
        size_t ind_klj = std::distance(
            list_i.begin(), std::find(list_i.begin(), list_i.end(), key_klj));
        size_t ind_kli = std::distance(
            list_j.begin(), std::find(list_j.begin(), list_j.end(), key_kli));
        size_t ind_ijl = std::distance(
            list_k.begin(), std::find(list_k.begin(), list_k.end(), key_ijl));
        size_t ind_ijk = std::distance(
            list_l.begin(), std::find(list_l.begin(), list_l.end(), key_ijk));

        if (ind_klj == list_i.size() or ind_kli == list_j.size() or
            ind_ijl == list_k.size() or ind_ijk == list_l.size())
          continue;

        //        std::cout << "   made it. ijkl = " << i << " " << j << " " <<
        //        k << " " << l << " J = " << J <<  "  with corresponding
        //        indices "
        //                  << ind_i << " , " << ind_klj << "  " << ind_j << " ,
        //                  " << ind_kli << "  "
        //                  << ind_k << " , " << ind_ijl << "  " << ind_l << " ,
        //                  " << ind_ijk << "   "
        //                  << ZMat_i(ind_i, ind_klj) << " " <<
        //                  ZMat_j(ind_j,ind_kli) << " " << ZMat_k(ind_k,
        //                  ind_ijl) << " " << ZMat_l(ind_l,ind_ijk)
        //                  << " the obc keys we used are : (" << oi.j2<< " " <<
        //                  oi.l%2 << " " << oi.tz2 << ")"
        //                  << "  ("                           << oj.j2<< " " <<
        //                  oj.l%2 << " " << oj.tz2 << ")"
        //                  << "  ("                           << ok.j2<< " " <<
        ...
        double zijkl =
            ZMat_j(ind_j, ind_kli) - phase_ij * ZMat_i(ind_i, ind_klj) -
            hermX * hermY *
                (ZMat_l(ind_l, ind_ijk) - phase_kl * ZMat_k(ind_k, ind_ijl));
        ...
        Z2.AddToTBME(ch, ch, ibra, iket, zijkl);
         ...
         //  Z.profiler.timer["comm232_first_pass"] += omp_get_wtime() - tstart;
  Z.profiler.timer[__func__] += omp_get_wtime() - tstart;
}
...
void comm232ss_debug(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  for (int ch = 0; ch < nch; ch++) {
    ...
    //    double tstart = omp_get_wtime();
    // The strategy used here is the following. We reorganize <ic|X|ab> as
    // <i|X|abc'>  and  <abj|Y|klc> as <abc'|Y|klj'> where the prime indicates
    // time reversal. Then we can cast things as a matrix multiplication
    //  <i|Z|klj'> = <i|X|abc'><abc'|Y|klj'>   and then we need to transform Z
    //  back to <ij|Z|kl>
    //
    //   i|   c|      i|                a| b| j|       a| b|   /c'
    ...
    //
    // The matrices for X and Z will clearly not be square. The left side is
    // just a single particle orbit, while the right has 3 orbits. Here, I
    // determine which single orbits will be needed in this two-body channel.
    ...
    for (auto i : ij_orbits_set) {
      ij_orbits.push_back(i);  // list of all the indices an orbit (either i or
                               // j ) in bra could have
      ij_orbits_lookup[i] = ij_orbits.size() - 1;
    }
    for (auto j : j_jvals_set) {
      j_jvals.push_back(j);  // list of possible j values (angular momentum) an
                             // orbit in bra could have
      jvals_lookup[j] = j_jvals.size() - 1;
    }
    size_t nij = ij_orbits.size();
    size_t njvals = j_jvals.size();
    //    std::cout << "   ch = " << ch << "   njvals = " << njvals <<
    //    std::endl;

    Z.profiler.timer["_comm232_block1"] += omp_get_wtime() - tstart;
    tstart = omp_get_wtime();

    // next, we make a list of the abc' combinations that will inter into the
    // sum
    ...
    for (int ch_ab = 0; ch_ab < nch; ch_ab++) {
      ...
      for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
        ...
        for (auto c : Z.modelspace->all_orbits) {
          ...
          //          double jc = 0.5*oc.j2;
          //          int ec = 2*oc.n + oc.l;
          //          if ( (std::abs( ea-e_fermi[oa.tz2]) +
          //          std::abs(eb-e_fermi[ob.tz2]) +
          //          std::abs(ec-e_fermi[oc.tz2])) > Z.modelspace->GetdE3max()
          //          ) continue;
          ...
          if (a == b)
            occfactor *= 0.5;  // we sum a<=b, and drop the 1/2, but we still
                               // need the 1/2 for a==b
          //          if (  (std::abs( 2*Jab -oc.j2)>oj.j2)  or
          //          ((2*Jab+oc.j2)<oj.j2) ) continue; if (  (std::abs( 2*Jab
          //          -oc.j2)>std::max(oi.j2,oj.j2))  or
          //          ((2*Jab+oc.j2)<std::min(oi.j2,oj.j2)) ) continue; if (
          //          tbc_ab.parity + oj.l + oc.l
...
#pragma omp parallel for schedule( \
    ...
    for (size_t ind_abc = 0; ind_abc < n_abc; ind_abc++) {
      ...
      for (size_t ind_i = 0; ind_i < nij; ind_i++) {
        ...
        //      if (ch==0) std::cout << "  ind_i = " << ind_i << "   i = " << i
        //      << std::endl;
        ...
        X2MAT(ind_i, ind_abc) = -sqrt((2 * Jab + 1.)) * occ_abc_list[ind_abc] *
                                X.TwoBody.GetTBME_J(Jab, c, i, a, b);
        Y2MAT(ind_i, ind_abc) = -sqrt((2 * Jab + 1.)) * occ_abc_list[ind_abc] *
                                Y.TwoBody.GetTBME_J(Jab, c, i, a, b);

        if ((oa.l + ob.l + oi.l + tbc.parity + oc.l) % 2 != Y.parity)
          continue;  // TODO: this will cause problems if we try something with
                     // Y.parity =1
        if (std::abs((oa.tz2 + ob.tz2 + oi.tz2) - (tbc.Tz * 2 + oc.tz2)) >
            2 * Y.rank_T)
          continue;

        int twoJp_min =
            std::max(std::abs(2 * Jab - oi.j2), std::abs(2 * J - oc.j2));
        int twoJp_max = std::min(2 * Jab + oi.j2, 2 * J + oc.j2);

        // Why is this part structured like this?
        // In tests, the time for this entire commutator routine was dominated
        // by the time to access 3-body matrix elements. That's because I'm
        // asking for them in an order different from the one in which they're
        // stored, which means recoupling. Fortunately, both the X and Y block
        // need exactly the same recoupling, and we can pull the recoupling for
        // the bra side a few loops out.
        for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
          std::vector<size_t> ibra_list;
          std::vector<double> recouple_bra_list;
          size_t ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
              Jab, twoJp, a, b, i, ibra_list, recouple_bra_list);

          for (size_t ind_jj = 0; ind_jj < njvals; ind_jj++) {
            int jj2 = j_jvals[ind_jj];
            double jj = 0.5 * jj2;
            double sixj =
                X.modelspace->GetSixJ(J, ji, jj, Jab, jc, 0.5 * twoJp);
            ...
            for (int iket = 0; iket < nkets; iket++) {
              ...
              int k = ket.p;
              int l = ket.q;
              //               if (i==0 and k==2 and l==2) std::cout <<
              //               "i==0,l==2,k==2 => index kli = " <<index_kli <<
              //               std::endl;
              double d_ek =
                  std::abs(2 * ket.op->n + ket.op->l - e_fermi[ket.op->tz2]);
              double d_el =
                  std::abs(2 * ket.oq->n + ket.oq->l - e_fermi[ket.oq->tz2]);
              double occnat_k = ket.op->occ_nat;
              double occnat_l = ket.oq->occ_nat;
              if ((d_ek + d_el + d_ec) > Z.modelspace->GetdE3max()) continue;
              if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) *
                   occnat_c * (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
                continue;
              std::vector<size_t> iket_list;
              std::vector<double> recouple_ket_list;
              ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
                  J, twoJp, k, l, c, iket_list, recouple_ket_list);

              double xabiklc = 0;
              double yabiklc = 0;
              for (size_t I = 0; I < ibra_list.size(); I++) {
                for (size_t J = 0; J < iket_list.size(); J++) {
                  // I explicitly check if x and y have 3-body components
                  // because the call GetME_pn_ch goes straight to the data
                  // array without a safety net. If the 3-body structure isn't
                  // allocated, then bad things will happen.
                  if (x_has_3)
                    xabiklc += recouple_bra_list[I] * recouple_ket_list[J] *
                               X3.GetME_pn_ch(ch_check, ch_check, ibra_list[I],
                                              iket_list[J]);
                  if (y_has_3)
                    yabiklc += recouple_bra_list[I] * recouple_ket_list[J] *
                               Y3.GetME_pn_ch(ch_check, ch_check, ibra_list[I],
                                              iket_list[J]);
                }
              }

              //               X3MAT(ind_abc, index_kli) -= (twoJp+1) * sixj *
              //               xabiklc; Y3MAT(ind_abc, index_kli) -= (twoJp+1) *
              //               sixj * yabiklc;
              X3MAT(ind_abc, index_kli) +=
                  (twoJp + 1) / sqrt(2 * J + 1) * sixj * xabiklc;
              Y3MAT(ind_abc, index_kli) +=
                  (twoJp + 1) / sqrt(2 * J + 1) * sixj * yabiklc;
              //               if (ch==8 and ind_abc ==9 and index_kli==20)
              //               {
              //          std::cout << "ind_abc = " << ind_abc << "   ind_kli =
              //          " << index_kli << "    abiklc = " << a << " " << b <<
              //          " " << i << " " << k << " " << l << " " << c << " Jkl
              //          Jab twoJp = " << J << " " << Jab << " " << twoJp << "
              //          read y " << Y3.GetME_pn(Jab,J,twoJp,a,b,i,k,l,c) << "
              //          yabiklc = " << yabiklc << " , " << Y3MAT(ind_abc,
              //          index_kli)  << std::endl;
              //               }
    ...
    //    ) ; std::cout << "Y3MAT size : " << Y3MAT.n_cols << " x " <<
    //    Y3MAT.n_rows << std::endl;
    arma::mat ZMat = (X2MAT * Y3MAT - Y2MAT * X3MAT);

    //    if (ch==8)
    //    {
    //      std::cout << "ch = " << ch << "   and X2MAT = " << std::endl <<
    //      X2MAT.row(5) << std::endl; std::cout << "ch = " << ch << "   and
    //      Y3MAT = " << std::endl << Y3MAT.col(20) << std::endl; std::cout << "
    //      product = " << X2MAT.row(5) * Y3MAT.col(20) << std::endl;
    //     double xysum =0;
    //     for (size_t ii=0; ii<X2MAT.n_cols; ii++)
    //     {
    //       double z = X2MAT(5,ii) * Y3MAT(ii,20);
    //       xysum += z;
    //       std::cout << X2MAT(5,ii) << " " << Y3MAT(ii,20) << "  " << z << "
    //       " << xysum << std::endl;
    //     }
    //      std::cout << "ch = " << ch << "   and Y2MAT = " << std::endl <<
    //      Y2MAT.row(5) << std::endl; std::cout << "ch = " << ch << "   and
    //      X3MAT = " << std::endl << X3MAT.col(20) << std::endl; std::cout << "
    //      product = " << Y2MAT.row(5) * X3MAT.col(20) << std::endl;
    //    }
...
#pragma omp parallel for schedule( \
    ...
    for (int ibra = 0; ibra < nkets; ibra++) {
      ...
      for (int iket = ibra; iket < nkets; iket++) {
        ...
        zijkl -=
            phase_ij * ZMat(ind_i, (ind_ji + ind_j * njvals) * nkets + iket);

        zijkl -= hermX * hermY *
                 ZMat(ind_l, (ind_jl + ind_k * njvals) * nkets + ibra);
        zijkl += hermX * hermY * phase_kl *
                 ZMat(ind_k, (ind_jk + ind_l * njvals) * nkets + ibra);

        ////       if (ch==0 and ibra<5 and iket<5)
        //       if (ch==8 and i==0 and j==1 and ( (k==2 and l==5) or (k==5 and
        //       l==2) ) )
        //       {
        //         std::cout << " ibra,iket " << ibra << " " << iket << "   ijkl
        //         " << i << " " << j << " " << k << " " << l << std::endl;
        //         std::cout << "    " << ZMat(ind_j,
        //         (ind_jj+ind_i*njvals)*nkets + iket) << "  -  " <<  phase_ij *
        //         ZMat(ind_i,  (ind_ji+ind_j*njvals)*nkets + iket)
        //                   << "  -  " << hermX*hermY* ZMat(ind_l,
        //                   (ind_jl+ind_k*njvals)*nkets + ibra) << "  +   " <<
        //                   hermX*hermY*phase_kl * ZMat(ind_k,
        //                   (ind_jk+ind_l*njvals)*nkets + ibra)
        //                   << "  third term obtained from Z(" << ind_l << " ,
        //                   " << (ind_jl+ind_k*njvals)*nkets + ibra  <<
        //                   std::endl;
        //       }
        ...
        for (auto c : Z.modelspace->all_orbits) {
          ...
          for (int ch_ab = 0; ch_ab < nch; ch_ab++) {
            ...
            for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
              ...
              if (std::abs(occfactor) < 1e-6) continue;
              if (a == b)
                occfactor *= 0.5;  // we sum a<=b, and drop the 1/2, but we
                                   // still need the 1/2 for a==b

              // Xicab term
              //              if ( ( (oi.l+oc.l+tbc_ab.parity)%2==0) and
              //              ((oi.tz2+oc.tz2)==2*tbc_ab.Tz)
              //                  and (std::abs(oi.j2-oc.j2)<=2*Jab)  and
              //                  (oi.j2+oc.j2>=2*Jab) )
              if (((((oi.l + oc.l + tbc_ab.parity) % 2 == X.parity) and
                    (std::abs(oi.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * X.rank_T)) or
                   (((oi.l + oc.l + tbc_ab.parity) % 2 == Y.parity) and
                    (std::abs(oi.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * Y.rank_T))) and
                  (std::abs(oi.j2 - oc.j2) <= 2 * Jab) and
                  (oi.j2 + oc.j2 >= 2 * Jab)) {
                if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) *
                     occnat_c * (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                     occnat_j * (1 - occnat_j)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                int twoJ_min = std::max(std::abs(oc.j2 - 2 * J),
                                        std::abs(oj.j2 - 2 * Jab));
                ...
                for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
                  ...
                  double hatfactor =
                      (twoJ + 1) * sqrt((2 * Jab + 1.) / (2 * J + 1));
                  double xabjklc = X3.GetME_pn(Jab, J, twoJ, a, b, j, k, l, c);
                  double yabjklc = Y3.GetME_pn(Jab, J, twoJ, a, b, j, k, l, c);
                  zijkl += occfactor * hatfactor * phasefactor * sixj *
                           (xciab * yabjklc - yciab * xabjklc);
                }
              }

              // Xjcab term
              //              if ( ( (oj.l+oc.l+tbc_ab.parity)%2==0) and
              //              ((oj.tz2+oc.tz2)==2*tbc_ab.Tz)
              if (((((oj.l + oc.l + tbc_ab.parity) % 2 == X.parity) and
                    (std::abs(oj.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * X.rank_T)) or
                   (((oj.l + oc.l + tbc_ab.parity) % 2 == Y.parity) and
                    (std::abs(oj.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * Y.rank_T))) and
                  (std::abs(oj.j2 - oc.j2) <= 2 * Jab) and
                  (oj.j2 + oc.j2 >= 2 * Jab)) {
                if ((occnat_k * (1 - occnat_k) * occnat_l * (1 - occnat_l) *
                     occnat_c * (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                ...
                for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
                  ...
                  zijkl -= occfactor * hatfactor * phasefactor * sixj *
                           (xcjab * yabiklc - ycjab * xabiklc);
                }
              }

              // Xabkc term
              //              if ( ( (ok.l+oc.l+tbc_ab.parity)%2==0) and
              //              ((ok.tz2+oc.tz2)==2*tbc_ab.Tz)
              if (((((ok.l + oc.l + tbc_ab.parity) % 2 == X.parity) and
                    (std::abs(ok.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * X.rank_T)) or
                   (((ok.l + oc.l + tbc_ab.parity) % 2 == Y.parity) and
                    (std::abs(ok.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * Y.rank_T))) and
                  (std::abs(ok.j2 - oc.j2) <= 2 * Jab) and
                  (ok.j2 + oc.j2 >= 2 * Jab)) {
                if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) *
                     occnat_c * (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                     occnat_l * (1 - occnat_l)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                int twoJ_min = std::max(std::abs(oc.j2 - 2 * J),
                                        std::abs(ol.j2 - 2 * Jab));
                ...
                for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
                  ...
                  zijkl -= occfactor * hatfactor * phasefactor * sixj *
                           (yijcabl * xabck - xijcabl * yabck);
                }
              }

              // Xablc term
              //              if ( ( (ol.l+oc.l+tbc_ab.parity)%2==0) and
              //              ((ol.tz2+oc.tz2)==2*tbc_ab.Tz)
              if (((((ol.l + oc.l + tbc_ab.parity) % 2 == X.parity) and
                    (std::abs(ol.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * X.rank_T)) or
                   (((ol.l + oc.l + tbc_ab.parity) % 2 == Y.parity) and
                    (std::abs(ol.tz2 + oc.tz2 - 2 * tbc_ab.Tz) ==
                     2 * Y.rank_T))) and
                  (std::abs(ol.j2 - oc.j2) <= 2 * Jab) and
                  (ol.j2 + oc.j2 >= 2 * Jab)) {
                if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) *
                     occnat_c * (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                     occnat_k * (1 - occnat_k)) < Z.modelspace->GetOccNat3Cut())
                  continue;
                int twoJ_min = std::max(std::abs(oc.j2 - 2 * J),
                                        std::abs(ok.j2 - 2 * Jab));
...
// i|  j|
//  |   |           Uncoupled expression:
//  *~~[X]~~~~*        Z_ijkl = 1/6 sum_abcd (nanbncn`d -n`an`bn`cnd)(X_ijdabc
//  Y_abckld - Y_ijdabc Xabckld)
//  |   |     /\                                                                                                    
// a|  b|   c(  )d
//  |   |     \/
//  *~~[Y]~~~~*      Coupled expression:
//  |   |              Z_{ijkl}^{J} = 1/6 sum_abcd (nanbncn`d-n`an`bn`cnd)
//  1/(2J+1) sum_J1J' (2J'+1)(X_{ijdabc}^{J J1 J'} Y_{abckld}^{J1 J J'} - X<->Y
//  )
// k|  l|
//
//
...
void comm332_ppph_hhhpss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  size_t nch3 = Z.modelspace->GetNumberThreeBodyChannels();
  Z.modelspace
      ->PreCalculateSixJ();  // if this is already done, then this does nothing.
  //  #pragma omp parallel for schedule(dynamic,1) if (not
  //  Z.modelspace->scalar3b_transform_first_pass)
#pragma omp parallel for schedule(dynamic, 1)
  for (int ch = 0; ch < nch; ch++) {
    ...
    // figure out how big the abcd side of the matrices should be, and store
    // some stuff for later lookup
    for (size_t ch_abc = 0; ch_abc < nch3; ch_abc++) {
      ...
      for (size_t iket_abc = 0; iket_abc < nkets_abc; iket_abc++) {
        ...
        double occ_abc_bar = (1 - ket_abc.op->occ) * (1 - ket_abc.oq->occ) *
                             (1 - ket_abc.oR->occ);
        double d_ea = std::abs(2 * ket_abc.op->n + ket_abc.op->l -
                               e_fermi[ket_abc.op->tz2]);
        double d_eb = std::abs(2 * ket_abc.oq->n + ket_abc.oq->l -
                               e_fermi[ket_abc.oq->tz2]);
        double d_ec = std::abs(2 * ket_abc.oR->n + ket_abc.oR->l -
                               e_fermi[ket_abc.oR->tz2]);
        ...
        double symm_factor =
            6;  // 6 possible orderings of abc. If a==b, then only 3 orderings,
                // and if a==b==c, then only 1 ordering.
        if (a == b and b == c)
          symm_factor = 1;
        ...
        for (auto d : Z.modelspace->all_orbits) {
          ...
          d_list.push_back(d);
          factor_list.push_back((twoJ + 1.) / (2 * J + 1) * occfactor *
                                symm_factor / 6.);
        }  // for d
      }    // for iket_abc
    }      // for ch_abc
    ...
    // fill the matrices  TODO: I think we can make this faster by first doing
    // the recoupling lookup for ijd
    for (size_t index_abcd = 0; index_abcd < dim_abcd; index_abcd++) {
      ...
      for (int ibra = 0; ibra < nkets; ibra++) {
        ...
        int j = bra.q;
        double d_ei =
            std::abs(2 * bra.op->n + bra.op->l - e_fermi[bra.op->tz2]);
        double d_ej =
            std::abs(2 * bra.oq->n + bra.oq->l - e_fermi[bra.oq->tz2]);
        double occnat_i = bra.op->occ_nat;
        double occnat_j = bra.oq->occ_nat;
        if ((d_ei + d_ej + d_ed) > Z.modelspace->dE3max) continue;
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_d *
             (1 - occnat_d)) < Z.modelspace->GetOccNat3Cut())
          continue;
        double norm2b = (i == j) ? 1. / sqrt(2) : 1;

        XMAT(ibra, index_abcd) =
            norm2b * X3.GetME_pn(J, Jab, twoJ, i, j, d, a, b, c);
        YMAT(ibra, index_abcd) =
            norm2b * factor * Y3.GetME_pn(J, Jab, twoJ, i, j, d, a, b, c);
...
void comm332_ppph_hhhpss( const Operator& X, const Operator& Y, Operator& Z )
{
  ...
  size_t nch3 = Z.modelspace->GetNumberThreeBodyChannels();
  #pragma omp parallel for schedule(dynamic,1) if (not
Z.modelspace->scalar3b_transform_first_pass) for (int ch=0; ch<nch; ch++)
  {
    TwoBodyChannel& tbc = Z.modelspace->GetTwoBodyChannel(ch);
    int J = tbc.J;
    int nkets = tbc.GetNumberKets();
      ...
      for (int iket=ibra; iket<nkets; iket++)
      {
        ...
        for (size_t ch_abc=0; ch_abc<nch3; ch_abc++)
        {
          ...
          for (size_t iket_abc=0; iket_abc<nkets_abc; iket_abc++)
          {
            ...
            size_t b = ket_abc.q;
            size_t c = ket_abc.r;
            double occ_abc = ket_abc.op->occ * ket_abc.oq->occ *
ket_abc.oR->occ; double occ_abc_bar = (1-ket_abc.op->occ) * (1-ket_abc.oq->occ)
* (1-ket_abc.oR->occ); if ( (std::abs(occ_abc)==0) and
(std::abs(occ_abc_bar)==0) ) continue; int Jab = ket_abc.Jpq;

              double symm_factor = 6;  // 6 possible orderings of abc. If a==b,
then only 3 orderings, and if a==b==c, then only 1 ordering. if ( a==b and b==c
) symm_factor = 1; else if (a==b or a==c or b==c ) symm_factor = 3;
              ...
              for ( auto d : Z.modelspace->all_orbits )
              {
                ...
                if (std::abs(occfactor)<1e-6) continue;
                if ( (std::abs( 2*J-od.j2) > twoJ) or (2*J+od.j2)<twoJ )
continue; if ( (od.l+tbc.parity + Tbc_abc.parity)%2>0) continue; if (
(Tbc_abc.twoTz)!=(od.tz2 + 2*tbc.Tz) ) continue; zijkl += symm_factor * 1./6  *
(twoJ+1.)/(2*J+1) * occfactor
                         *(  X3.GetME_pn(J,Jab,twoJ, i,j,d,a,b,c) *
Y3.GetME_pn(Jab, J, twoJ, a,b,c,k,l,d)
                           - Y3.GetME_pn(J,Jab,twoJ, i,j,d,a,b,c) *
X3.GetME_pn(Jab, J, twoJ, a,b,c,k,l,d) );
...
//           i| j|
//            |  |  Uncoupled expression:
//   *~~~[X]~~*  |            Z_ijkl = 1/6 sum_abcd (nanbncn`d
//   -n`an`bn`cnd)(X_ijdabc Y_abckld - Y_ijdabc Xabckld)
//  / \  / \  |  |                  + 1/4 (1-Pij)(1-Pkl) sum_{abcd} nanbn`cn`d (
//  X_abicdk Y_cdjabl - Y_abicdk X_cdjabl )
// (a c)(b d) |  |
//  \ /  \ /  |  |
//   *~~~[Y]~~|~~*
//            |  |   Coupled expression:
//           k| l|     Z_{ijkl}^{J} = 1/6 sum_abcd (nanbncn`d-n`an`bn`cnd)
//           1/(2J+1) sum_J1J' (2J'+1)(X_{ijdabc}^{J J1 J'} Y_{abckld}^{J1 J J'}
//           - X<->Y )
//                                    + 1/4  (1-Pij^J)(1-Pkl^J) sum_{abcd}
//                                    nanbn`cn`d sum_{J1 J2 J' J"}
//                                    (2J'+1)(2J"+1)  (-1)^{2J+J2+J'+J"+2j-i-k}
//                                      { p  q  J }
//                                    * { J1 J" s } ( X_{abicdk}^{J1 J2 J'}
//                                    Y_{cdjabl}^{J2 J1 J"} - X<->Y )
//                                      { J' J2 r }
//
//  We recouple the expression so that it factorizes and can be cast as matrix
//  multiplication
//
//            i| /k`
//             |/
...
void comm332_pphhss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  auto& Z2 = Z.TwoBody;
  std::map<int, double> e_fermi = Z.modelspace->GetEFermi();
  //  std::cout << "  fermi levels " << e_fermi[-1] << " " << e_fermi[+1] <<
  //  std::endl;
  ...
  // before entering the parallel loop, lets allocate the Pandya-transformed
  // matrices Zbar. we keep 4 because we want i<j and i>j and k<l and k>l
  ...
  Z.modelspace->PreCalculateSixJ();  // if it's been done, this won't do it
                                     // again. But make sure for thread safety.
  // Loop through Pandya-transformed channels and compute the matrix Zbar by mat
  // mult
  //  #pragma omp parallel for schedule(dynamic,1)
  //  There are some SixJs in this loop which are not automatically computed
  //  with PreCalculateSixJ, so we need to be careful the first time we call
  //  this.
#pragma omp parallel for schedule( \
    dynamic, 1) if (not Z.modelspace->scalar3b_transform_first_pass)
  for (size_t ch = 0; ch < nch_CC; ++ch) {
    ...
    for (size_t ch_ab = 0; ch_ab < nch; ch_ab++) {
      ...
      for (size_t ch_cd = 0; ch_cd < nch; ch_cd++) {
        ...
        for (int iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
          ...
          double occnat_a = ket_ab.op->occ_nat;
          double occnat_b = ket_ab.oq->occ_nat;
          double d_ea = std::abs(2 * ket_ab.op->n + ket_ab.op->l -
                                 e_fermi[ket_ab.op->tz2]);
          double d_eb = std::abs(2 * ket_ab.oq->n + ket_ab.oq->l -
                                 e_fermi[ket_ab.oq->tz2]);
          if (na * nb < 1e-8 and (1 - na) * (1 - nb) < 1e-8) continue;
          if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
               occnat_factor_max) < Z.modelspace->GetOccNat3Cut())
            continue;
          ...
          for (int iket_cd = 0; iket_cd < nkets_cd; iket_cd++) {
            ...
            double d_ec = std::abs(2 * ket_cd.op->n + ket_cd.op->l -
                                   e_fermi[ket_cd.op->tz2]);
            double d_ed = std::abs(2 * ket_cd.oq->n + ket_cd.oq->l -
                                   e_fermi[ket_cd.oq->tz2]);
            double occupation_factor =
    ...
    // Fill the matrices, which are basically <ij`| X | cd a`b`> type things
    // (the ticks ` mean time-reversed state)
    for (size_t ibra_CC = 0; ibra_CC < nKets_cc; ibra_CC++) {
      Ket& bra_CC = tbc_CC.GetKet(ibra_CC);
      int i = bra_CC.p;
      int j = bra_CC.q;
      ...
      for (size_t ch_ab = 0; ch_ab < nch; ch_ab++) {
        ...
        for (size_t ch_cd = 0; ch_cd < nch; ch_cd++) {
          ...
          // I erroneously thought I'd fixed a bug here in commit 7ffbe... but
          // it actually gave wrong answers. This seems to have corrected the
          // oopsie. --SRS
          int twoJp_min = std::min(
              std::max(std::abs(2 * Jab - j2i), std::abs(2 * Jcd - j2j)),
              std::max(std::abs(2 * Jab - j2j), std::abs(2 * Jcd - j2i)));
          int twoJp_max = std::max(std::min((2 * Jab + j2i), (2 * Jcd + j2j)),
                                   std::min((2 * Jab + j2j), (2 * Jcd + j2i)));
          //            int twoJp_min = std::max( {std::abs(2*Jab-j2i),
          //            std::abs(2*Jcd-j2j), std::abs(2*Jab-j2j),
          //            std::abs(2*Jcd-j2i) }); int twoJp_max = std::min(
          //            {(2*Jab+j2i), (2*Jcd+j2j), (2*Jab+j2j), (2*Jcd+j2i) });
          std::vector<double> sixj_ij_list;
          std::vector<double> sixj_ji_list;
          for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
            //              sixj_ij_list.push_back( (twoJp+1) *
            //              Z.modelspace->GetSixJ(ji,jj,J_ph, Jcd,Jab,0.5*twoJp)
            //              * Z.modelspace->phase((j2i+twoJp)/2) );
            //              sixj_ji_list.push_back( (twoJp+1) *
            //              Z.modelspace->GetSixJ(jj,ji,J_ph, Jcd,Jab,0.5*twoJp)
            //              * Z.modelspace->phase((j2j+twoJp)/2) );
            sixj_ij_list.push_back(
                (twoJp + 1) *
                Z.modelspace->GetSixJ(Jcd, Jab, J_ph, ji, jj, 0.5 * twoJp) *
                Z.modelspace->phase((j2i + twoJp) / 2));
            sixj_ji_list.push_back(
                (twoJp + 1) *
                Z.modelspace->GetSixJ(Jcd, Jab, J_ph, jj, ji, 0.5 * twoJp) *
                Z.modelspace->phase((j2j + twoJp) / 2));
          }

          bool abi_cdj_Tz_ok =
              ((2 * tbc_ab.Tz + oi.tz2) == (2 * tbc_cd.Tz + oj.tz2));
          bool abj_cdi_Tz_ok =
              ((2 * tbc_ab.Tz + oj.tz2) == (2 * tbc_cd.Tz + oi.tz2));
          ...
          for (int iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
            ...
            bool keep_abj =
                ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                  occnat_j * (1 - occnat_j)) >= Z.modelspace->GetOccNat3Cut());
            keep_abi =
                keep_abi and (d_ea + d_eb + d_ei <= Z.modelspace->dE3max);
            keep_abj =
                keep_abj and (d_ea + d_eb + d_ej <= Z.modelspace->dE3max);
            ...
            if (keep_abi or keep_abj) {
              for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
                ...
                size_t ch_check = -1;
                if (keep_abi and abi_cdj_Tz_ok and
                    (std::abs(2 * Jab - oi.j2) <= twoJp) and
                    (2 * Jab + oi.j2 >= twoJp)) {
                  ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
                      Jab, twoJp, a, b, i, ketlist, reclist);
                }
                ...
                reclist.clear();
                ch_check = -1;
                if (keep_abj and abj_cdi_Tz_ok and
                    (std::abs(2 * Jab - oj.j2) <= twoJp) and
                    (2 * Jab + oj.j2 >= twoJp)) {
                  ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
                      Jab, twoJp, a, b, j, ketlist, reclist);
                }
            ...
            for (int iket_cd = 0; iket_cd < nkets_cd; iket_cd++) {
              ...
              double symmetry_factor =
                  1;  // we only sum a<=b and c<=d, so we undercount by a factor
                      // of 4, canceling the 1/4 in the formula
              if (a == b)
                symmetry_factor *=
                    0.5;  // if a==b or c==d, then the permutation doesn't give
                          // a new state, so there's less undercounting
              if (c == d) symmetry_factor *= 0.5;

              ind_abcd++;  // increment the matrix index.

              if (not(keep_abi or keep_abj))
                continue;  // need this after the ind_abcd++... maybe be more
                           // clever?

              bool keep_cdi = ((occnat_c * (1 - occnat_c) * occnat_d *
                                (1 - occnat_d) * occnat_i * (1 - occnat_i)) >=
                               Z.modelspace->GetOccNat3Cut());
              bool keep_cdj = ((occnat_c * (1 - occnat_c) * occnat_d *
                                (1 - occnat_d) * occnat_j * (1 - occnat_j)) >=
                               Z.modelspace->GetOccNat3Cut());
              keep_cdi =
                  keep_cdi and (d_ec + d_ed + d_ei <= Z.modelspace->dE3max);
              keep_cdj =
                  keep_cdj and (d_ec + d_ed + d_ej <= Z.modelspace->dE3max);
              ...
              for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
                ...
                std::vector<double> reclist;
                size_t ch_check = -1;
                if (keep_abj and keep_cdi and abj_cdi_Tz_ok and
                    (std::abs(2 * Jcd - oi.j2) <= twoJp) and
                    (2 * Jcd + oi.j2 >= twoJp)) {
                  ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
                      Jcd, twoJp, c, d, i, ketlist, reclist);
                }
                ...
                reclist.clear();
                ch_check = -1;
                if (keep_abi and keep_cdj and abi_cdj_Tz_ok and
                    (std::abs(2 * Jcd - oj.j2) <= twoJp) and
                    (2 * Jcd + oj.j2 >= twoJp)) {
                  ch_check = Y.ThreeBody.GetKetIndex_withRecoupling(
                      Jcd, twoJp, c, d, j, ketlist, reclist);
                }
              ...
              for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
                double sixj_ij = sixj_ij_list[(twoJp - twoJp_min) /
                                              2];  // <-- these also include a
                                                   // phase and a (twoJp+1)
                ...
                if (keep_abi and keep_cdj and abi_cdj_Tz_ok) {
                  ...
                  auto& recouple_abi =
                      recouple_abi_list[(twoJp - twoJp_min) / 2];
                  auto& recouple_cdj =
                      recouple_cdj_list[(twoJp - twoJp_min) / 2];
                  double xabicdj = 0;
                  double ycdjabi = 0;
                  for (size_t Iabi = 0; Iabi < kets_abi.size(); Iabi++) {
                    for (size_t Icdj = 0; Icdj < kets_cdj.size(); Icdj++) {
                      xabicdj += recouple_abi[Iabi] * recouple_cdj[Icdj] *
                                 X3.GetME_pn_ch(ch_abi, ch_abi, kets_abi[Iabi],
                                                kets_cdj[Icdj]);
                      ycdjabi += recouple_abi[Iabi] * recouple_cdj[Icdj] *
                                 Y3.GetME_pn_ch(ch_abi, ch_abi, kets_cdj[Icdj],
                                                kets_abi[Iabi]);
                    }
                  }
                  X3_ij(ibra_CC, ind_abcd) += sixj_ij * xabicdj;
                  Y3_ij(ind_abcd, ibra_CC) +=
                      sixj_ij * ycdjabi * occupation_factor * symmetry_factor;
                }
                ...
                if (keep_abj and keep_cdi and abj_cdi_Tz_ok) {
                  ...
                  auto& recouple_abj =
                      recouple_abj_list[(twoJp - twoJp_min) / 2];
                  auto& recouple_cdi =
                      recouple_cdi_list[(twoJp - twoJp_min) / 2];
                  double xabjcdi = 0;
                  double ycdiabj = 0;
                  for (size_t Iabj = 0; Iabj < kets_abj.size(); Iabj++) {
                    for (size_t Icdi = 0; Icdi < kets_cdi.size(); Icdi++) {
                      xabjcdi += recouple_abj[Iabj] * recouple_cdi[Icdi] *
                                 X3.GetME_pn_ch(ch_abj, ch_abj, kets_abj[Iabj],
                                                kets_cdi[Icdi]);
                      ycdiabj += recouple_abj[Iabj] * recouple_cdi[Icdi] *
                                 Y3.GetME_pn_ch(ch_abj, ch_abj, kets_cdi[Icdi],
                                                kets_abj[Iabj]);
                    }
                  }
                  X3_ji(ibra_CC, ind_abcd) += sixj_ji * xabjcdi;
                  Y3_ji(ind_abcd, ibra_CC) +=
                      sixj_ji * ycdiabj * occupation_factor * symmetry_factor;
                }
    ...
    for (size_t ibra = 0; ibra < nkets; ibra++) {
      ...
      for (size_t iket = ibra; iket < nkets; iket++) {
        ...
        for (int J_ph = J_ph_min; J_ph <= J_ph_max; J_ph++) {
          int ch_cc =
              Z.modelspace->GetTwoBodyChannelIndex(J_ph, parity_cc, Tz_cc);
          TwoBodyChannel_CC& tbc_cc = Z.modelspace->GetTwoBodyChannel_CC(ch_cc);
          int indx_il = tbc_cc.GetLocalIndex(std::min(i, l), std::max(i, l));
          ...
          zijkl += (2 * J_ph + 1) * sixj *
                   (zbar_ilkj + phase_ij * phase_kl * zbar_jkli);
        }

        parity_cc = (oj.l + ol.l) % 2;
        Tz_cc = std::abs(oj.tz2 - ol.tz2) / 2;
        J_ph_min =
            std::max(std::abs(oj.j2 - ol.j2), std::abs(ok.j2 - oi.j2)) / 2;
        ...
        for (int J_ph = J_ph_min; J_ph <= J_ph_max; J_ph++) {
          ...
          zijkl -= (2 * J_ph + 1) * sixj *
                   (phase_ij * zbar_jlki + phase_kl * zbar_iklj);
        }

        // make it a normalized TBME
        zijkl /= sqrt((1. + bra.delta_pq()) * (1. + ket.delta_pq()));
        // the AddToTBME routine automatically takes care of the hermitian
        // conjugate as well
        Z2.AddToTBME(ch, ch, ibra, iket, zijkl);
  ...
  for (int ch = 0; ch < nch; ch++) {
    ...
    for (int ibra = 0; ibra < nkets; ibra++) {
      ...
      for (int iket = ibra; iket < nkets; iket++) {
        ...
        int J_ph_min = std::min({std::abs(ji2 - jl2), std::abs(jj2 - jk2),
                                 std::abs(jj2 - jl2), std::abs(ji2 - jk2)}) /
                       2;
        int J_ph_max =
            std::max({ji2 + jl2, jj2 + jk2, jj2 + jl2, ji2 + jk2}) / 2;
        //        int J_ph_min = std::max( std::abs(ji2 - jl2),
        //        std::abs(jj2-jk2) )/2; int J_ph_max = std::min( ji2+jl2,
        //        jj2+jk2 )/2;
        ...
        for (int J_ph = J_ph_min; J_ph <= J_ph_max; J_ph++) {
          ...
          for (int ch_ab = 0; ch_ab < nch; ch_ab++) {
            ...
            for (int ch_cd = 0; ch_cd < nch; ch_cd++) {
              ...
              for (int iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
                ...
                double occnat_a = ket_ab.op->occ_nat;
                double occnat_b = ket_ab.oq->occ_nat;
                bool keep_abi = ((occnat_a * (1 - occnat_a) * occnat_b *
                                  (1 - occnat_b) * occnat_i * (1 - occnat_i)) >=
                                 Z.modelspace->GetOccNat3Cut());
                bool keep_abj = ((occnat_a * (1 - occnat_a) * occnat_b *
                                  (1 - occnat_b) * occnat_j * (1 - occnat_j)) >=
                                 Z.modelspace->GetOccNat3Cut());
                bool keep_abk = ((occnat_a * (1 - occnat_a) * occnat_b *
                                  (1 - occnat_b) * occnat_k * (1 - occnat_k)) >=
                                 Z.modelspace->GetOccNat3Cut());
                bool keep_abl = ((occnat_a * (1 - occnat_a) * occnat_b *
                                  (1 - occnat_b) * occnat_l * (1 - occnat_l)) >=
                                 Z.modelspace->GetOccNat3Cut());
                keep_abi =
                    keep_abi and (d_ea + d_eb + d_ei <= Z.modelspace->dE3max);
                keep_abj =
                    keep_abj and (d_ea + d_eb + d_ej <= Z.modelspace->dE3max);
                keep_abk =
                    keep_abk and (d_ea + d_eb + d_ek <= Z.modelspace->dE3max);
                keep_abl =
                    keep_abl and (d_ea + d_eb + d_el <= Z.modelspace->dE3max);
                ...
                for (int iket_cd = 0; iket_cd < nkets_cd; iket_cd++) {
                  ...
                  double occupation_factor = (1 - na) * (1 - nb) * nc * nd -
                                             na * nb * (1 - nc) * (1 - nd);
                  if (std::abs(occupation_factor) < 1e-6) continue;
                  ...
                  keep_cdj =
                      keep_cdj and (d_ec + d_ed + d_ej <= Z.modelspace->dE3max);
                  keep_cdk =
                      keep_cdk and (d_ec + d_ed + d_ek <= Z.modelspace->dE3max);
                  keep_cdl =
                      keep_cdl and (d_ec + d_ed + d_el <= Z.modelspace->dE3max);
                  if (not(keep_cdk or keep_cdl)) continue;
                  if (not(keep_cdi or keep_cdj)) continue;

                  double symmetry_factor =
                      1;  // we only sum a<=b and c<=d, so we undercount by a
                          // factor of 4, canceling the 1/4 in the formula
                  if (a == b)
                    symmetry_factor *=
                        0.5;  // if a==b or c==d, then the permutation doesn't
                              // give a new state, so there's less undercounting
                  if (c == d) symmetry_factor *= 0.5;
                  ...
                  if (keep_abi and keep_cdl and keep_cdj and keep_abk and
                      ((tbc_ab.parity + tbc_cd.parity + oi.l + ol.l) % 2 ==
                       X.parity) and
                      (std::abs(2 * tbc_ab.Tz + oi.tz2 - 2 * tbc_cd.Tz -
                                ol.tz2) == 2 * X.rank_T) and
                      (std::abs(ji2 - jl2) <= 2 * J_ph) and
                      ((ji2 + jl2) >= 2 * J_ph) and
                      (std::abs(jj2 - jk2) <= 2 * J_ph) and
                      ((jj2 + jk2) >= 2 * J_ph)) {
                    int twoJp_min = std::max(std::abs(2 * Jab - ji2),
                                             std::abs(2 * Jcd - jl2));
                    int twoJp_max = std::min((2 * Jab + ji2), (2 * Jcd + jl2));
                    //                    if (i==0 and j==1 and k==0 and l==1)
                    //                    {
                    ////                      std::cout << "Getting a
                    ///contribution from Xabicdl = " << a << " " << b << " " <<
                    ///i << " " << c << " " << d << " " << l << std::endl;
                    //                      std::cout << "Getting a contribution
                    //                      from X = " << a << " " << b << " "
                    //                      << i << " " << c << " " << d << " "
                    //                      << l << "   " << na << " " << nb <<
                    //                      " " << nc << " " << nd << " -> " <<
                    //                      occupation_factor << std::endl;
                    //                    }
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(ji, jl, J_ph, Jcd,
                                                            Jab, 0.5 * twoJp);
                      X3_il += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((ji2 + twoJp) / 2) *
                               X3.GetME_pn(Jab, Jcd, twoJp, a, b, i, c, d, l);
                    }

                    twoJp_min = std::max(std::abs(2 * Jcd - jj2),
                                         std::abs(2 * Jab - jk2));
                    twoJp_max = std::min((2 * Jcd + jj2), (2 * Jab + jk2));
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(jj, jk, J_ph, Jab,
                                                            Jcd, 0.5 * twoJp);
                      Y3_jk += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((jk2 + twoJp) / 2) *
                               Y3.GetME_pn(Jcd, Jab, twoJp, c, d, j, a, b, k);
                    }
                  }

                  if (keep_abj and keep_cdl and keep_cdi and keep_abk and
                      ((tbc_ab.parity + tbc_cd.parity + oj.l + ol.l) % 2 ==
                       X.parity) and
                      (std::abs(2 * tbc_ab.Tz + oj.tz2 - 2 * tbc_cd.Tz -
                                ol.tz2) == 2 * X.rank_T) and
                      (std::abs(jj2 - jl2) <= 2 * J_ph) and
                      ((jj2 + jl2) >= 2 * J_ph) and
                      (std::abs(ji2 - jk2) <= 2 * J_ph) and
                      ((ji2 + jk2) >= 2 * J_ph)) {
                    int twoJp_min = std::max(std::abs(2 * Jab - jj2),
                                             std::abs(2 * Jcd - jl2));
                    int twoJp_max = std::min((2 * Jab + jj2), (2 * Jcd + jl2));

                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(jj, jl, J_ph, Jcd,
                                                            Jab, 0.5 * twoJp);
                      X3_jl += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((jj2 + twoJp) / 2) *
                               X3.GetME_pn(Jab, Jcd, twoJp, a, b, j, c, d, l);
                    }
                    twoJp_min = std::max(std::abs(2 * Jcd - ji2),
                                         std::abs(2 * Jab - jk2));
                    twoJp_max = std::min((2 * Jcd + ji2), (2 * Jab + jk2));
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(ji, jk, J_ph, Jab,
                                                            Jcd, 0.5 * twoJp);
                      Y3_ik += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((jk2 + twoJp) / 2) *
                               Y3.GetME_pn(Jcd, Jab, twoJp, c, d, i, a, b, k);
                    }
                    //                    if (i==0 and j==1 and k==0 and l==1)
                    //                    {
                    ////                      std::cout << "Getting a
                    ///contribution from Xabjcdl = " << a << " " << b << " " <<
                    ///j << " " << c << " " << d << " " << l << std::endl;
                    //                      std::cout << "Getting a contribution
                    //                      from X = " << a << " " << b << " "
                    //                      << j << " " << c << " " << d << " "
                    //                      << l  << "   "  << na << " " << nb
                    //                      << " " << nc << " " << nd << " ->
                    //                      "<< occupation_factor << " X3_jl = "
                    //                      << X3_jl << "  Y3_ik = " << Y3_ik <<
                    //                      std::endl;
                    //                    }
                  }

                  if (keep_abi and keep_cdk and keep_cdj and keep_abl and
                      ((tbc_ab.parity + tbc_cd.parity + oi.l + ok.l) % 2 ==
                       X.parity) and
                      (std::abs(2 * tbc_ab.Tz + oi.tz2 - 2 * tbc_cd.Tz -
                                ok.tz2) == 2 * X.rank_T) and
                      (std::abs(ji2 - jk2) <= 2 * J_ph) and
                      ((ji2 + jk2) >= 2 * J_ph) and
                      (std::abs(jj2 - jl2) <= 2 * J_ph) and
                      ((jj2 + jl2) >= 2 * J_ph)) {
                    ...
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(ji, jk, J_ph, Jcd,
                                                            Jab, 0.5 * twoJp);
                      X3_ik += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((ji2 + twoJp) / 2) *
                               X3.GetME_pn(Jab, Jcd, twoJp, a, b, i, c, d, k);
                    }
                    twoJp_min = std::max(std::abs(2 * Jcd - jj2),
                                         std::abs(2 * Jab - jl2));
                    ...
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(jj, jl, J_ph, Jab,
                                                            Jcd, 0.5 * twoJp);
                      Y3_jl += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((jl2 + twoJp) / 2) *
                               Y3.GetME_pn(Jcd, Jab, twoJp, c, d, j, a, b, l);
                    }
                  }

                  if (keep_abj and keep_cdk and keep_cdi and keep_abl and
                      ((tbc_ab.parity + tbc_cd.parity + oj.l + ok.l) % 2 ==
                       X.parity) and
                      (std::abs(2 * tbc_ab.Tz + oj.tz2 - 2 * tbc_cd.Tz -
                                ok.tz2) == 2 * X.rank_T) and
                      (std::abs(jj2 - jk2) <= 2 * J_ph) and
                    ...
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(jj, jk, J_ph, Jcd,
                                                            Jab, 0.5 * twoJp);
                      X3_jk += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((jj2 + twoJp) / 2) *
                               X3.GetME_pn(Jab, Jcd, twoJp, a, b, j, c, d, k);
                    }
                    twoJp_min = std::max(std::abs(2 * Jcd - ji2),
                                         std::abs(2 * Jab - jl2));
                    ...
                    for (int twoJp = twoJp_min; twoJp <= twoJp_max;
                         twoJp += 2) {
                      double sixj_x = Z.modelspace->GetSixJ(ji, jl, J_ph, Jab,
                                                            Jcd, 0.5 * twoJp);
                      Y3_il += (twoJp + 1) * sixj_x *
                               Z.modelspace->phase((jl2 + twoJp) / 2) *
                               Y3.GetME_pn(Jcd, Jab, twoJp, c, d, i, a, b, l);
                    }
                  }

                  zbar_ilkj +=
                      occupation_factor * symmetry_factor * X3_il * Y3_jk;
                  zbar_jlki +=
                      occupation_factor * symmetry_factor * X3_jl * Y3_ik;
                  zbar_iklj +=
                      occupation_factor * symmetry_factor * X3_ik * Y3_jl;
                  zbar_jkli +=
                      occupation_factor * symmetry_factor * X3_jk * Y3_il;
                  //                  zbar_ilkj += occupation_factor *
                  //                  symmetry_factor * ( X3_il * Y3_jk +
                  //                  phase_ij*phase_kl*X3_jk * Y3_il);
                  //                  zbar_jlki += occupation_factor *
                  //                  symmetry_factor * ( X3_jl * Y3_ik +
                  //                  phase_ij*phase_kl*X3_ik * Y3_jl);
          ...
          //        int phase_kl = Z.modelspace->phase( (jk2+jl2)/2-J);
          zijkl += (2 * J_ph + 1) *
                   (sixj1 * (zbar_ilkj + phase_ij * phase_kl * zbar_jkli) -
                    sixj1_flip * (phase_ij * zbar_jlki + phase_kl * zbar_iklj));
          //        zijkl += (2*J_ph+1) * (  sixj1 * zbar_ilkj   - sixj1_flip*
          //        phase_ij*zbar_jlki ); if ( i==0 and j==1 and k==0 and l==1)
          //        {
          //           std::cout << " Jph = " << J_ph << " zbar_ilkj " <<
          //           zbar_ilkj << "   zbar_jkli " << zbar_jkli << " zbar_jlki
          //           " << zbar_jlki << "   zbar_iklj " << zbar_iklj << "  ->
          //           " << zijkl << std::endl;
          //        }
      ...
      for (int iket=ibra; iket<nkets; iket++)
      {
        ...
        for (int ch_ab=0; ch_ab<nch; ch_ab++)
        {
          ...
          for (int iket_ab=0; iket_ab<nkets; iket_ab++)
          {
            ...
            double nb = ket_ab.oq->occ;
            double d_ea = std::abs(2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2]); double d_eb = std::abs(2*ket_ab.oq->n + ket_ab.oq->l -
e_fermi[ket_ab.oq->tz2]); double occnat_a = ket_ab.op->occ_nat; double occnat_b
= ket_ab.oq->occ_nat; bool keep_abi = ( (occnat_a*(1-occnat_a) *
occnat_b*(1-occnat_b) * occnat_i*(1-occnat_i) ) >= Z.modelspace->GetOccNat3Cut()
) ; bool keep_abj = ( (occnat_a*(1-occnat_a) * occnat_b*(1-occnat_b) *
occnat_j*(1-occnat_j) ) >= Z.modelspace->GetOccNat3Cut() ) ; bool keep_abk = (
(occnat_a*(1-occnat_a) * occnat_b*(1-occnat_b) * occnat_k*(1-occnat_k) ) >=
Z.modelspace->GetOccNat3Cut() ) ; bool keep_abl = ( (occnat_a*(1-occnat_a) *
occnat_b*(1-occnat_b) * occnat_l*(1-occnat_l) ) >= Z.modelspace->GetOccNat3Cut()
) ; keep_abi = keep_abi and (d_ea+d_eb+d_ei <= Z.modelspace->dE3max ); keep_abj
= keep_abj and (d_ea+d_eb+d_ej <= Z.modelspace->dE3max ); keep_abk = keep_abk
and (d_ea+d_eb+d_ek <= Z.modelspace->dE3max ); keep_abl = keep_abl and
(d_ea+d_eb+d_el <= Z.modelspace->dE3max ); if ( not ( keep_abi or keep_abj) )
continue; if ( not ( keep_abk or keep_abl) ) continue;
            ...
            for (int ch_cd=0; ch_cd<nch; ch_cd++)
            {
              ...
              for (int iket_cd=0; iket_cd<nkets; iket_cd++)
              {
                ...
                double nd = ket_cd.oq->occ;
                double d_ec = std::abs(2*ket_cd.op->n + ket_cd.op->l -
e_fermi[ket_cd.op->tz2]); double d_ed = std::abs(2*ket_cd.oq->n + ket_cd.oq->l -
e_fermi[ket_cd.oq->tz2]); double occnat_c = ket_cd.op->occ_nat; double occnat_d
= ket_cd.oq->occ_nat;
//                double jc = 0.5*ket_cd.op->j2;
//                double jd = 0.5*ket_cd.oq->j2;
                double occupation_factor = (1-na)*(1-nb)*nc*nd -
na*nb*(1-nc)*(1-nd); if (std::abs(occupation_factor)<1e-6) continue;


                bool keep_cdi = ( (occnat_c*(1-occnat_c) * occnat_d*(1-occnat_d)
* occnat_i*(1-occnat_i) ) >= Z.modelspace->GetOccNat3Cut() ) ; bool keep_cdj = (
(occnat_c*(1-occnat_c) * occnat_d*(1-occnat_d) * occnat_j*(1-occnat_j) ) >=
Z.modelspace->GetOccNat3Cut() ) ; bool keep_cdk = ( (occnat_c*(1-occnat_c) *
occnat_d*(1-occnat_d) * occnat_k*(1-occnat_k) ) >= Z.modelspace->GetOccNat3Cut()
) ; bool keep_cdl = ( (occnat_c*(1-occnat_c) * occnat_d*(1-occnat_d) *
occnat_l*(1-occnat_l) ) >= Z.modelspace->GetOccNat3Cut() ) ; keep_cdi = keep_cdi
and (d_ec+d_ed+d_ei <= Z.modelspace->dE3max ); keep_cdj = keep_cdj and
(d_ec+d_ed+d_ej <= Z.modelspace->dE3max ); keep_cdk = keep_cdk and
(d_ec+d_ed+d_ek <= Z.modelspace->dE3max ); keep_cdl = keep_cdl and
(d_ec+d_ed+d_el <= Z.modelspace->dE3max ); if ( not ( keep_cdk or keep_cdl) )
continue; if ( not ( keep_cdi or keep_cdj) ) continue;

                double symmetry_factor = 1;  // we only sum a<=b and c<=d, so we
undercount by a factor of 4, canceling the 1/4 in the formula if (a==b)
symmetry_factor *= 0.5; // if a==b or c==d, then the permutation doesn't give a
new state, so there's less undercounting if (c==d) symmetry_factor *= 0.5;

                // Figure out which range of twoJp and twoJpp we will need
                int twoJp_min  = std::max(  std::min(std::abs(2*Jab -
ji2),std::abs(2*Jab-jj2)), std::min(std::abs(2*Jcd - jl2),std::abs(2*Jcd-jk2))
); int twoJpp_min = std::max(  std::min(std::abs(2*Jcd -
jj2),std::abs(2*Jcd-ji2)), std::min(std::abs(2*Jab - jk2),std::abs(2*Jab-jl2))
); int twoJp_max  = std::min(  2*Jab + std::max(ji2,jj2),  2*Jcd +
std::max(jk2,jl2) ); int twoJpp_max = std::min(  2*Jcd + std::max(ji2,jj2),
2*Jab + std::max(jk2,jl2) );

                if (twoJpp_max<twoJpp_min) continue;
                for (int twoJp=twoJp_min; twoJp<=twoJp_max; twoJp+=2)
                {

                    double xabicdl = (keep_abi and keep_cdl) ?
X3.GetME_pn(Jab,Jcd,twoJp, a,b,i,c,d,l) : 0; double xabicdk = (keep_abi and
keep_cdk) ? X3.GetME_pn(Jab,Jcd,twoJp, a,b,i,c,d,k) : 0; double xabjcdl =
(keep_abj and keep_cdl) ? X3.GetME_pn(Jab,Jcd,twoJp, a,b,j,c,d,l) : 0; double
xabjcdk = (keep_abj and keep_cdk) ? X3.GetME_pn(Jab,Jcd,twoJp, a,b,j,c,d,k) : 0;
                  ...
                  for (int twoJpp=twoJpp_min; twoJpp<=twoJpp_max; twoJpp+=2)
                  {
                    ...
                    // I think having these in the inner loop may be disastrous
for performance double ninej1 = (keep_abi and keep_cdl and keep_cdj and
keep_abk) ? Z.modelspace->GetNineJ(Jab,jk,Jpp, ji,J,jj, Jp,jl,Jcd) : 0; double
ninej2 = (keep_abj and keep_cdl and keep_cdi and keep_abk) ?
Z.modelspace->GetNineJ(Jab,jk,Jpp, jj,J,ji, Jp,jl,Jcd) : 0; // permute i<->j
                    double ninej3 = (keep_abi and keep_cdk and keep_cdj and
keep_abl) ? Z.modelspace->GetNineJ(Jab,jl,Jpp, ji,J,jj, Jp,jk,Jcd) : 0; //
permute k<->l double ninej4 = (keep_abj and keep_cdk and keep_cdi and keep_abl)
? Z.modelspace->GetNineJ(Jab,jl,Jpp, jj,J,ji, Jp,jk,Jcd) : 0; // permute i<->j
and k<->l

                    // These phase factors account for the minus signs
associated with the permutations
                    // so that all the permuted terms should just be added with
their phase (no extra minus sign). int phase1 = Z.modelspace->phase(
(ji2+jk2+twoJp+twoJpp)/2 ); int phase2 = Z.modelspace->phase(
(ji2+jk2+twoJp+twoJpp)/2 -J ); // from permuting i<->j int phase3 =
Z.modelspace->phase( (ji2+jk2+twoJp+twoJpp)/2 -J ); // from permuting k<->l int
phase4 = Z.modelspace->phase( (ji2+jk2+twoJp+twoJpp)/2 ); // from permuting
i<->j and k<->l

                    double ycdjabk = (keep_cdj and keep_abk) ?
Y3.GetME_pn(Jcd,Jab,twoJpp,c,d,j,a,b,k) : 0; double ycdjabl = (keep_cdj and
keep_abl) ? Y3.GetME_pn(Jcd,Jab,twoJpp,c,d,j,a,b,l) : 0; double ycdiabk =
(keep_cdi and keep_abk) ? Y3.GetME_pn(Jcd,Jab,twoJpp,c,d,i,a,b,k) : 0; double
ycdiabl = (keep_cdi and keep_abl) ? Y3.GetME_pn(Jcd,Jab,twoJpp,c,d,i,a,b,l) : 0;
                    zijkl += symmetry_factor * hatfactor * occupation_factor * (
                                      1*phase1 * ninej1 * xabicdl * ycdjabk
                                    + 1*phase2 * ninej2 * xabjcdl * ycdiabk //
i<->j
                                    + 1*phase3 * ninej3 * xabicdk * ycdjabl //
k<->l
                                    + 1*phase4 * ninej4 * xabjcdk * ycdiabl //
i<->j and k<->l
                                   );
        ...
        zijkl /=  sqrt((1.+bra.delta_pq())*(1.+ket.delta_pq()));
        // the AddToTBME routine automatically takes care of the hermitian
conjugate as well Z2.AddToTBME(ch,ch,ibra,iket,zijkl);
      }// for iket
    }// for ibra
  }// for ch
  Z.profiler.timer[__func__] += omp_get_wtime() - tstart;
...
//  |   |   *--[X]     Z_ijklmn = sum_a P(ij/k) (X_ka * Y_ijalmn) - P(lm/n)
//  (Y_ijklma * X_an) |   |   |a
//  *~~[Y]~~*       Coupled expression:
//  |   |   |          Z_{ijklmn}^{J1,J2,J} = sum_a P(ij/k)^{J1,J} (X_ka *
//  Y_{ijalmn}^{J1,J2,J})
// l|  m|  n|                                      -P(lm/n)^{J2,J}
// (Y_{ijklma}^{J1,J2,J} * X_an)
//
//  Checked with UnitTest and passed
//
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t iket = 0; iket < nkets; iket++) {
      Ket3& ket = Tbc.GetKet(iket);
      double d_ei =
          std::abs(2 * ket.op->n + ket.op->l - e_fermi.at(ket.op->tz2));
      double d_ej =
          std::abs(2 * ket.oq->n + ket.oq->l - e_fermi.at(ket.oq->tz2));
      double d_ek =
          std::abs(2 * ket.oR->n + ket.oR->l - e_fermi.at(ket.oR->tz2));
      ...
      if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_k *
           (1 - occnat_k)) < Z.modelspace->GetOccNat3Cut())
        continue;
    ...
    for (size_t index_bra = 0; index_bra < nkets_kept; index_bra++) {
      ...
      for (auto a : X.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
        ...
        if ((occnat_a * (1 - occnat_a) * occnat_j * (1 - occnat_j) * occnat_k *
             (1 - occnat_k)) < Z.modelspace->GetOccNat3Cut())
          continue;
        std::vector<size_t> ket_list;
        std::vector<double> recouple_list;

        //        size_t ch_check = Z3.GetKetIndex_withRecoupling( Jij, twoJ, a,
        //        j, k,  ket_list,  recouple_list );
        Z3.GetKetIndex_withRecoupling(Jij, twoJ, a, j, k, ket_list,
                                      recouple_list);
      ...
      for (auto a : X.OneBodyChannels.at({oj.l, oj.j2, oj.tz2})) {
        ...
        if ((occnat_i * (1 - occnat_i) * occnat_a * (1 - occnat_a) * occnat_k *
             (1 - occnat_k)) < Z.modelspace->GetOccNat3Cut())
          continue;
        std::vector<size_t> ket_list;
        std::vector<double> recouple_list;
        //        size_t ch_check = Z3.GetKetIndex_withRecoupling( Jij, twoJ, i,
        //        a, k,  ket_list,  recouple_list );
        Z3.GetKetIndex_withRecoupling(Jij, twoJ, i, a, k, ket_list,
                                      recouple_list);
        for (size_t ilist = 0; ilist < ket_list.size(); ilist++) {
          auto iter_find = kept_lookup.find(ket_list[ilist]);
          if (iter_find == kept_lookup.end()) continue;
      ...
      for (auto a : X.OneBodyChannels.at({ok.l, ok.j2, ok.tz2})) {
        ...
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_a *
             (1 - occnat_a)) < Z.modelspace->GetOccNat3Cut())
          continue;
        std::vector<size_t> ket_list;
        std::vector<double> recouple_list;
        //        size_t ch_check = Z3.GetKetIndex_withRecoupling( Jij, twoJ, i,
        //        j, a,  ket_list,  recouple_list );
        Z3.GetKetIndex_withRecoupling(Jij, twoJ, i, j, a, ket_list,
                                      recouple_list);
        for (size_t ilist = 0; ilist < ket_list.size(); ilist++) {
          auto iter_find = kept_lookup.find(ket_list[ilist]);
          if (iter_find == kept_lookup.end()) continue;
    ...
    // kept_lookup is a map   Full index => Kept index, so iter_bra.first gives
    // the full index, and iter_bra.second is the index for the 3-body state we
    // keep in this commutator
    for (auto& iter_bra : kept_lookup) {
      for (auto& iter_ket : kept_lookup) {
        //        if ( X3NORM > 1e-6)
        if (x3_allocated)
          X3MAT(iter_bra.second, iter_ket.second) =
              X3.GetME_pn_ch(ch3, ch3, iter_bra.first, iter_ket.first);
        //        if ( Y3NORM > 1e-6)
        if (y3_allocated)
          Y3MAT(iter_bra.second, iter_ket.second) =
              Y3.GetME_pn_ch(ch3, ch3, iter_bra.first, iter_ket.first);
      }
    }

    // Do the matrix multiplication
    //    Z3MAT = X1MAT*Y3MAT - Y1MAT*X3MAT +  hermX*hermY * (
    //    X3MAT.t()*Y1MAT.t() - Y3MAT.t()*X1MAT.t() );
    ...
    for (auto& iter_bra : kept_lookup) {
      for (auto& iter_ket : kept_lookup) {
        ...
        Z3.AddToME_pn_ch(ch3, ch3, iter_bra.first, iter_ket.first,
                         Z3MAT(iter_bra.second, iter_ket.second));
      }
    }
...
//  #pragma omp parallel for schedule(dynamic,1)
  #pragma omp parallel for schedule(dynamic,1) if (not
Z.modelspace->scalar3b_transform_first_pass) for (size_t ch3=0; ch3<=nch3;
ch3++)
  {
    auto& Tbc = Z.modelspace->GetThreeBodyChannel(ch3);
    int twoJ = Tbc.twoJ;
...
//
//  i|  j|  k|   Uncoupled expression:
//   |~X~|   |      Z_ijklmn  =  P(ij/k)P(lm/n) sum_a  (X_ijla * Y_akmn - Y_ijla
//   * X_akmn) |   |a  | |   |~Y~|   Coupled expression:
//  l|  m|  n|     Z_{ijklmn}^{J1,J2,J}  = Z1 + Z2 + Z3 + Z4 + Z5 + Z6 + Z7 + Z8
//  + Z9
//                 where each of those terms corresponds to a permutation from
...
void comm223ss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  int hY = Y.IsHermitian() ? 1 : -1;
  int hZ = Z.IsHermitian() ? 1 : -1;
  if ((std::abs(X2.Norm() * Y2.Norm()) < 1e-6) and
      not Z.modelspace->scalar3b_transform_first_pass)
    return;
  ...
  // create a function object for hashing 3 size_t (unsigned long int) to a
  // single size_t for lookup in an unordered_map the 3 integers correspond to
  // J, parity,Tz
  ...
  std::vector<int> channels_pph(
      ch_pph_hash(max_twoJph, 1, 3) + 1,
      -1);  // map twoJ_ph, parity_ph, twoTz_ph  -> channel index
  ...
  // It would be great to figure out a dense mapping so that this can be used to
  // access a vector rather than as a key for a map (i.e. hash table).
  auto hash_key_ijnJ = [](size_t i, size_t j, size_t n, size_t Jij) {
    return (i + (j << 12) + (n << 24) + (Jij << 36));
  };

  //  0xF = 15 = 1111 (4bits), so 0xFFF is 12 bits of 1's. 0xFFFL makes it a
  //  long int
  auto unhash_key_ijnJ = [](size_t& i, size_t& j, size_t& n, size_t& Jij,
                            size_t key) {
    i = (key & 0xFFFL);
    j = ((key >> 12) & 0xFFFL);
    n = ((key >> 24) & 0xFFFL);
  ...
  // perfect hash ijn -> index,  then this points to a vector of indices given
  // by a loop over Jij,Jtot
  auto hash_ijn_alt = [norbs](size_t i, size_t j, size_t n) {
    return i + j * (j + 1) / 2 + norbs * (norbs + 3) / 2 * n;
  };
  std::vector<std::vector<size_t>> ket_lookup_pph_alt;
  ket_lookup_pph_alt.resize(hash_ijn_alt(norbs, norbs, norbs));
  std::cout << "Made vector ket_lookup_pph_alt, with size = "
            << ket_lookup_pph_alt.size() << std::endl;

  std::vector<float> Zbar;  // store the transformed Z in a 1D vector.
  std::vector<size_t>
      Zbar_start_pointers;  // ch -> index in Zbar where that channel starts

  // Different approach: store by ijklmn index, then for each ijklmn combo, loop
  // over Jij,Jlm,twoJ In this case, we need a map {i,j,k,l,m,n} => size_t,
  // where the size_t points to the start locating in Zbar

  //  std::cout << "Made it as far as line " << __LINE__ << std::endl;

  // we're looking at pph type states. We don't make a cut on the occupations in
  // this channel but if the first two orbits ij can't possibly make it past the
  // occnat cut later on, we don't bother including them To figure out if
  // they'll make it, we want to know the biggest contribution that can possibly
  // be made by a third orbit.
  double occnat_factor_max = 0;
  for (auto i : Z.modelspace->all_orbits) {
    double occnat_i = Z.modelspace->GetOrbit(i).occ_nat;
    occnat_factor_max = std::max(occnat_factor_max, occnat_i * (1 - occnat_i));
  ...
  // not the same as the standard 3-body channels since we aren't enforcing
  // antisymmetry with the 3rd particle
  for (auto& iter_obc : Z.modelspace->OneBodyChannels) {
    ...
    //     std::cout << "Made it as far as line " << __LINE__ << "   " <<
    //     parity_ph << " " << twoJph << " " << twoTz_ph << std::endl;
    ...
    for (size_t chij = 0; chij < nch2; chij++) {
      ...
      int Tz_ij = tbc_ij.Tz;
      //     std::cout << "Made it as far as line " << __LINE__ << "   " << Jij
      //     << " " << parity_ij << " " << Tz_ij << std::endl;
      ...
      for (size_t iket_ij = 0; iket_ij < nkets_ij; iket_ij++) {
        ...
        // if i and j cant make it past the OccNat and dE3max cuts, don't bother
        // including it
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) *
             occnat_factor_max) < Z.modelspace->GetOccNat3Cut())
          continue;
        ...
        if (perturbative_triples and
            ((ket_ij.op->cvq == 0 and ket_ij.oq->cvq != 0) or
             (ket_ij.op->cvq != 0 and ket_ij.oq->cvq == 0)))
          continue;
      ...
      for (auto& ij : good_ij) {
        for (size_t n : good_n) {
          if (perturbative_triples)  // we just want <ppp|hhh> terms in the end,
                                     // so for ijn we want pph or hhp.
          {
            Orbit& oi = Z.modelspace->GetOrbit(ij[0]);
            Orbit& oj = Z.modelspace->GetOrbit(ij[1]);
            Orbit& on = Z.modelspace->GetOrbit(n);
            if ((oi.cvq == 0 and oj.cvq == 0 and on.cvq == 0) or
                (oi.cvq != 0 and oj.cvq != 0 and on.cvq != 0))
              continue;
          }
      ...
      for (size_t n : Z.modelspace->all_orbits) {
        ...
        for (int Jij = Jij_min; Jij <= Jij_max; Jij++) {
          ...
          for (int twoJph = twoJph_min; twoJph <= twoJph_max; twoJph += 2) {
            ...
            //              size_t index_ijn =
            //              ket_lookup_pph[ch_pph].at(key_ijnJ );
            size_t index_ijn = iter_ijn->second;

            ket_lookup_pph_alt[ptr_ijn].push_back(index_ijn);
            //              std::cout << "    Jij twoJph = " << Jij << " " <<
            //              twoJph << "   index = " <<
            //              ket_lookup_pph_alt[ptr_ijn].size()-1 << "   ->  " <<
            //              index_ijn << std::endl;
          }
        }
      }
  ...
  std::cout << __func__ << "  allocating Zbar   " << Zbar_n_elements
            << " elements  ~ "
            << Zbar_n_elements * sizeof(float) / (1024. * 1024 * 1024) << " GB"
            << std::endl;
  ...
  // I don't think this specific loop has any thread safety issues, so we don't
  // need to check if this is the first pass
#pragma omp parallel for schedule( \
    dynamic, 1)  //  if (not Z.modelspace->scalar3b_transform_first_pass)
  for (size_t ch_pph = 0; ch_pph < nch_pph; ch_pph++) {
    ...
    int l_a = ((twoJph + 1) / 2) % 2 == parity_ph ? (twoJph + 1) / 2
                                                  : (twoJph - 1) / 2;

    //   auto& a_set = Z.modelspace->OneBodyChannels.at({l_a,twoJph,twoTz_ph});
    //   // list of one-body orbits in this channel
    std::vector<size_t> a_list;
    for (auto a : Z.modelspace->OneBodyChannels.at({l_a, twoJph, twoTz_ph}))
      a_list.push_back(a);
    ...
    //   std::cout << "a: " ;
    //   for (size_t index_a=0; index_a<number_a; index_a++ ) std::cout << "  "
    //   << a_list[index_a]; std::cout << std::endl;
    ...
    // we  use row-major ordering so that the inner loop runs over adjacent
    // elements
  ...
  // Now we transform back from <ijn|Z|lmk>  to <ijk|Z|lmn> (plus all
  // permutations between ijk and lmn)
  size_t nch3 = Z.modelspace->GetNumberThreeBodyChannels();
#pragma omp parallel for schedule( \
    dynamic, 1)  // if (not Z.modelspace->scalar3b_transform_first_pass)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    int twoJ = Tbc.twoJ;
    double Jtot = 0.5 * twoJ;
    //    std::cout << "   ch3 = " << ch3 << "  nkets3 = " << nkets3 <<
    //    std::endl;
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_k *
        ...
        continue;
      if (perturbative_triples and
          not((oi.cvq + oj.cvq + ok.cvq) == 0 or
              (oi.cvq > 0 and oj.cvq > 0 and ok.cvq > 0)))
        continue;
      ...
      std::vector<int> J1p_min = {
          J1, std::max(std::abs(ok.j2 - oj.j2), std::abs(twoJ - oi.j2)) / 2,
          std::max(std::abs(oi.j2 - ok.j2), std::abs(twoJ - oj.j2)) / 2};
      std::vector<int> J1p_max = {J1, std::min(ok.j2 + oj.j2, twoJ + oi.j2) / 2,
                                  std::min(oi.j2 + ok.j2, twoJ + oj.j2) / 2};
      std::vector<std::vector<double>> recouple_ijk = {{1}, {}, {}};
      std::vector<std::vector<bool>> skipJ1p = {{false}, {}, {}};

      // the SixJs called here may not have already been precomputed, so this
      // may cause thread issues on first pass
      for (int J1p = J1p_min[1]; J1p <= J1p_max[1]; J1p++) {
        recouple_ijk[1].push_back(
            sqrt((2 * J1 + 1) * (2 * J1p + 1)) *
            Z.modelspace->GetSixJ(ji, jj, J1, jk, Jtot, J1p));
        skipJ1p[1].push_back((k == j and J1p % 2 > 0) or
                             std::abs(recouple_ijk[1].back()) < 1e-7);
      }

      for (int J1p = J1p_min[2]; J1p <= J1p_max[2]; J1p++) {
        recouple_ijk[2].push_back(
            -Z.modelspace->phase((oj.j2 + ok.j2) / 2 + J1 + J1p) *
            sqrt((2 * J1 + 1) * (2 * J1p + 1)) *
            Z.modelspace->GetSixJ(jj, ji, J1, jk, Jtot, J1p));
        skipJ1p[2].push_back((k == i and J1p % 2 > 0) or
                             std::abs(recouple_ijk[2].back()) < 1e-7);
      }
      ...
      for (size_t iket = ibra; iket < nkets3; iket++) {
        ...
        if ((occnat_l * (1 - occnat_l) * occnat_m * (1 - occnat_m) * occnat_n *
             (1 - occnat_n)) < Z.modelspace->GetOccNat3Cut())
          continue;
        if (perturbative_triples and
            not((ol.cvq + om.cvq + on.cvq) == 0 or
                (ol.cvq > 0 and om.cvq > 0 and on.cvq > 0)))
          continue;
        if (perturbative_triples and
            ((oi.cvq == 0 and ol.cvq == 0) or (oi.cvq != 0 and ol.cvq != 0)))
          continue;
        if (imsrg3_no_qqq and (ol.cvq + om.cvq + on.cvq) > 5) continue;
        if ((d_ei + d_ej + d_ek + d_el + d_em + d_en) > imsrg3_dE6max) continue;
        //        if ( imsrg3_no_qqq and  (oi.cvq + oj.cvq + ok.cvq + ol.cvq +
        //        om.cvq + on.cvq)>11 ) continue;
        int J2 = ket.Jpq;
        ...
        std::vector<int> J2p_min = {
            J2, std::max(std::abs(on.j2 - om.j2), std::abs(twoJ - ol.j2)) / 2,
            std::max(std::abs(ol.j2 - on.j2), std::abs(twoJ - om.j2)) / 2};
        std::vector<int> J2p_max = {J2,
                                    std::min(on.j2 + om.j2, twoJ + ol.j2) / 2,
                                    std::min(ol.j2 + on.j2, twoJ + om.j2) / 2};
        std::vector<std::vector<double>> recouple_lmn = {{1}, {}, {}};
        std::vector<std::vector<bool>> skipJ2p = {{false}, {}, {}};

        // the SixJs called here may not have already been precomputed, so this
        // may cause thread issues on first pass it looks like these SixJs get
        // computed with PrecomputeSixJs
        for (int J2p = J2p_min[1]; J2p <= J2p_max[1]; J2p++) {
          recouple_lmn[1].push_back(
              sqrt((2 * J2 + 1) * (2 * J2p + 1)) *
              Z.modelspace->GetSixJ(jl, jm, J2, jn, Jtot, J2p));
          skipJ2p[1].push_back((n == m and J2p % 2 > 0) or
                               std::abs(recouple_lmn[1].back()) < 1e-7);
        }

        for (int J2p = J2p_min[2]; J2p <= J2p_max[2]; J2p++) {
          recouple_lmn[2].push_back(
              -Z.modelspace->phase((om.j2 + on.j2) / 2 + J2 + J2p) *
              sqrt((2 * J2 + 1) * (2 * J2p + 1)) *
              Z.modelspace->GetSixJ(jm, jl, J2, jn, Jtot, J2p));
          skipJ2p[2].push_back((n == l and J2p % 2 > 0) or
                               std::abs(recouple_lmn[2].back()) < 1e-7);
        }

        double zijklmn = 0;
        /// BEGIN THE SLOW BIT...

        //  #pragma omp parallel for schedule(dynamic,1) reduction( + : zijklmn)
        //  if ( Z.modelspace->scalar3b_transform_first_pass)
        for (int perm_ijk = 0; perm_ijk < 3; perm_ijk++) {
          ...
          //          for (int J1p=J1p_min[perm_ijk]; J1p<=J1p_max[perm_ijk];
          //          J1p++)
          //          {
          //            if ((I1==I2) and J1p%2!=0 ) continue;
          //            double rec_ijk =
          //            recouple_ijk[perm_ijk].at(J1p-J1p_min[perm_ijk]);
          ...
          for (int perm_lmn = 0; perm_lmn < 3; perm_lmn++) {
            ...
            //              size_t ptr_126  =
            //              hash_ijn_alt(std::min(I1,I2),std::max(I1,I2),I6);
            //              auto& ket_lookup_126 = ket_lookup_pph_alt[ptr_126];
            //              size_t ptr_453  =
            //              hash_ijn_alt(std::min(I4,I5),std::max(I4,I5),I3);
            //              auto& ket_lookup_453 = ket_lookup_pph_alt[ptr_453];

            int JJcounter_126 = 0;

            for (int J1p = std::abs(o1.j2 - o2.j2) / 2;
                 J1p <= J1p_max[perm_ijk]; J1p++) {
              int twoJph_min_126 = std::abs(2 * J1p - o6.j2);
              int twoJph_max_126 = 2 * J1p + o6.j2;
              //                if (((I1==I2) and J1p%2!=0) or
              //                (J1p<J1p_min[perm_ijk]) )
              if ((J1p < J1p_min[perm_ijk]) or
                  skipJ1p[perm_ijk][J1p - J1p_min[perm_ijk]]) {
              ...
              int JJcounter_453 = 0;
              for (int J2p = std::abs(o4.j2 - o5.j2) / 2;
                   J2p <= J2p_max[perm_lmn]; J2p++) {
                int twoJph_min_453 = std::abs(2 * J2p - o3.j2);
                int twoJph_max_453 = 2 * J2p + o3.j2;
                //                if (((I4==I5) and J2p%2!=0) or
                //                (J2p<J2p_min[perm_lmn]) )
                if (J2p < J2p_min[perm_lmn] or
                    skipJ2p[perm_lmn][J2p - J2p_min[perm_lmn]]) {
                  JJcounter_453 += (twoJph_max_453 - twoJph_min_453) / 2 + 1;
                  continue;
                }

                double rec_lmn =
                    recouple_lmn[perm_lmn].at(J2p - J2p_min[perm_lmn]);

                int phase_12 =
                    I1 > I2 ? -Z.modelspace->phase((o1.j2 + o2.j2) / 2 - J1p)
                            : 1;
                int phase_45 =
                    I4 > I5 ? -Z.modelspace->phase((o4.j2 + o5.j2) / 2 - J2p)
                            : 1;
                ...
                int twoJph_min = std::max(std::abs(2 * J1p - o6.j2),
                                          std::abs(2 * J2p - o3.j2));
                int twoJph_max = std::min(2 * J1p + o6.j2, 2 * J2p + o3.j2);

                // This inner loop is slow
                for (int twoJph = twoJph_min; twoJph <= twoJph_max;
                     twoJph += 2)  // <J1p,j6|Jph> and <J2p,j3|Jph>
                {
                  double zbar_126453 = 0;
                  double Jph = 0.5 * twoJph;

                  int ch_pph =
                      channels_pph[ch_pph_hash(twoJph, parity_ph, twoTz_ph)];
                  if (ch_pph < 0) continue;

                  //                   continue;
                  // Now this lookup seems to be the biggest time sink.
                  // We get a lot of zeros, but they satisfy the triangle
                  // contition, so not obvious how to avoid that. This probably
                  // means we need a faster lookup?

                  double sixj1 =
                      Jtot < Jph
                          ? Z.modelspace->GetSixJ(j3, Jtot, J1p, j6, Jph, J2p)
                          : Z.modelspace->GetSixJ(j6, Jph, J1p, j3, Jtot, J2p);

                  if (std::abs(sixj1) < 1e-6) continue;

                  // These don't need to be looked up in the inner loop here,
                  // but in tests moving them to an outer loop makes things
                  // marginally slower (???)
                  size_t ptr_126 =
                      hash_ijn_alt(std::min(I1, I2), std::max(I1, I2), I6);
                  size_t ptr_453 =
                  ...
                  //                   size_t index_126 =
                  //                   ket_lookup_126[JJcounter_126 +
                  //                   (twoJph-twoJph_min_126)/2]; size_t
                  //                   index_453 = ket_lookup_453[JJcounter_453
                  //                   + (twoJph-twoJph_min_453)/2];
                  size_t index_126 =
                      ket_lookup_pph_alt[ptr_126]
                                        [JJcounter_126 +
                                         (twoJph - twoJph_min_126) / 2];
                  size_t index_453 =
                      ket_lookup_pph_alt[ptr_453]
                                        [JJcounter_453 +
                                         (twoJph - twoJph_min_453) / 2];
                  ...
                  if (index_126 <= index_453) {
                    size_t zbar_index =
                        start_ptr +
                        (2 * ngood_ijn - index_126 - 1) * index_126 / 2 +
                        index_453;
                    zbar_126453 = Zbar[zbar_index] * phase_12 * phase_45;
                  } else {
                    size_t zbar_index =
                        start_ptr +
                        (2 * ngood_ijn - index_453 - 1) * index_453 / 2 +
                        index_126;
  ...
  //  std::cout << " Used " << used_sum << "  out of  " << Zbar.size() << "
  //  elements of Zbar " << std::endl;
}
...
void comm223ss( const Operator& X, const Operator& Y, Operator& Z )
{
  ...
  int hY = Y.IsHermitian() ? 1 : -1;
//  int hZ = Z.IsHermitian() ? 1 : -1;
  if ( (std::abs( X2.Norm() * Y2.Norm() ) < 1e-6 ) and not
Z.modelspace->scalar3b_transform_first_pass) return;

  std::map<int,double> e_fermi = Z.modelspace->GetEFermi();

  // Now we transform back from <ijn|Z|lmk>  to <ijk|Z|lmn> (plus all
permutations between ijk and lmn) size_t nch3 =
Z.modelspace->GetNumberThreeBodyChannels(); #pragma omp parallel for
schedule(dynamic,1) // if (not Z.modelspace->scalar3b_transform_first_pass) for
(size_t ch3=0; ch3<nch3; ch3++)
  {
    auto& Tbc = Z.modelspace->GetThreeBodyChannel(ch3);
    size_t nkets3 = Tbc.GetNumberKets();
    int twoJ = Tbc.twoJ;
...
//    std::cout << "   ch3 = " << ch3 << "  nkets3 = " << nkets3 << std::endl;
    for (size_t ibra=0; ibra<nkets3; ibra++)
    {
      ...
      if ( (d_ei + d_ej + d_ek) > Z.modelspace->dE3max ) continue;
      if ( (occnat_i*(1-occnat_i) * occnat_j*(1-occnat_j) *
occnat_k*(1-occnat_k) ) < Z.modelspace->GetOccNat3Cut() ) continue; if (
perturbative_triples and  not ( (oi.cvq + oj.cvq + ok.cvq)==0 or (oi.cvq>0 and
oj.cvq>0 and ok.cvq>0)) ) continue; if ( imsrg3_no_qqq and (oi.cvq + oj.cvq +
ok.cvq)>5 ) continue; // Need at least one core or valence particle

      int J1 = bra.Jpq;

      // Set up the permutation stuff for ijk
      std::vector<std::array<size_t,3>> ijk = { {i,j,k}, {k,j,i}, {i,k,j} };
      std::vector<int> J1p_min = {J1,
std::max(std::abs(ok.j2-oj.j2),std::abs(twoJ-oi.j2) )/2,
std::max(std::abs(oi.j2-ok.j2), std::abs(twoJ-oj.j2) )/2 }; std::vector<int>
J1p_max = {J1,  std::min(ok.j2+oj.j2, twoJ+oi.j2)/2 , std::min(oi.j2+ok.j2,
twoJ+oj.j2)/2 }; std::vector<std::vector<double>> recouple_ijk = {{1},{},{} };

      // the SixJs called here may not have already been precomputed, so this
may cause thread issues on first pass for (int J1p=J1p_min[1]; J1p<=J1p_max[1];
J1p++) recouple_ijk[1].push_back( sqrt( (2*J1+1)*(2*J1p+1)) *
Z.modelspace->GetSixJ(ji,jj,J1,jk,Jtot,J1p) );

      for (int J1p=J1p_min[2]; J1p<=J1p_max[2]; J1p++)
           recouple_ijk[2].push_back(
-Z.modelspace->phase((oj.j2+ok.j2)/2+J1+J1p)*sqrt( (2*J1+1)*(2*J1p+1)) *
Z.modelspace->GetSixJ(jj,ji,J1,jk,Jtot,J1p) );
      ...
      for (size_t iket=ibra; iket<nkets3; iket++)
      {
        ...
        double occnat_n = on.occ_nat;
        if ( (d_el + d_em + d_en) > Z.modelspace->dE3max ) continue;
        if ( (occnat_l*(1-occnat_l) * occnat_m*(1-occnat_m) *
occnat_n*(1-occnat_n) ) < Z.modelspace->GetOccNat3Cut() ) continue; if (
perturbative_triples and  not ( (ol.cvq + om.cvq + on.cvq)==0 or (ol.cvq>0 and
om.cvq>0 and on.cvq>0)) ) continue; if ( perturbative_triples and (  (oi.cvq==0
and ol.cvq==0) or (oi.cvq!=0 and ol.cvq!=0) ) ) continue; if ( imsrg3_no_qqq and
(ol.cvq + om.cvq + on.cvq)>5 ) continue; int J2 = ket.Jpq;

        // Set up the permutation stuff for lmn
        std::vector<std::array<size_t,3>> lmn = { {l,m,n}, {n,m,l}, {l,n,m} };
        std::vector<int> J2p_min = {J2,  std::max(std::abs(on.j2-om.j2),
std::abs(twoJ-ol.j2) )/2,   std::max( std::abs(ol.j2-on.j2),
std::abs(twoJ-om.j2) )/2 }; std::vector<int> J2p_max = {J2,
std::min(on.j2+om.j2, twoJ+ol.j2)/2 , std::min(ol.j2+on.j2,twoJ+om.j2)/2 };
        std::vector<std::vector<double>> recouple_lmn = {{1},{},{} };
...
may cause thread issues on first pass
      // it looks like these SixJs get computed with PrecomputeSixJs
        for (int J2p=J2p_min[1]; J2p<=J2p_max[1]; J2p++)
           recouple_lmn[1].push_back( sqrt( (2*J2+1)*(2*J2p+1)) *
Z.modelspace->GetSixJ(jl,jm,J2,jn,Jtot,J2p) );

        for (int J2p=J2p_min[2]; J2p<=J2p_max[2]; J2p++)
             recouple_lmn[2].push_back(
-Z.modelspace->phase((om.j2+on.j2)/2+J2+J2p)*sqrt( (2*J2+1)*(2*J2p+1)) *
Z.modelspace->GetSixJ(jm,jl,J2,jn,Jtot,J2p) );
        ...
        for ( int perm_ijk=0; perm_ijk<3; perm_ijk++ )
        {
          ...
          double j3 = 0.5*o3.j2;
              ...
              for (int J1p=J1p_min[perm_ijk]; J1p<=J1p_max[perm_ijk]; J1p++)
              {
                ...
                double rec_ijk =
recouple_ijk[perm_ijk].at(J1p-J1p_min[perm_ijk]); int ch1 =
Z.modelspace->GetTwoBodyChannelIndex(J1p, (o1.l+o2.l)%2, (o1.tz2+o2.tz2)/2 );
                ...
                size_t ind_12 =
tbc1.GetLocalIndex(std::min(I1,I2),std::max(I1,I2)); if (ind_12>nkets_1)
continue; double phase_12 = I1>I2 ?  -Z.modelspace->phase((o1.j2+o2.j2-2*J1p)/2)
: 1; if (I1==I2) phase_12 *= PhysConst::SQRT2;
            ...
            for ( int perm_lmn=0; perm_lmn<3; perm_lmn++ )
            {
              ...
              int tz2a = o1.tz2+o2.tz2-o6.tz2; // TODO: Need to fix this for
beta decay if ( std::abs(tz2a)!=1 ) continue; int parity_a = (o1.l+o2.l+o6.l)%2;

               // Maybe better to reorder the loops so that we first loop over
a,
               // then Jp1 and Jp2 don't need to be nested.
                ...
                double rec_lmn =
recouple_lmn[perm_lmn].at(J2p-J2p_min[perm_lmn]); int ch2 =
Z.modelspace->GetTwoBodyChannelIndex(J2p, (o4.l+o5.l)%2, (o4.tz2+o5.tz2)/2 );
                ...
                size_t ind_45 =
tbc2.GetLocalIndex(std::min(I4,I5),std::max(I4,I5)); if (ind_45>nkets_2)
continue; double phase_45 = I4>I5 ?  -Z.modelspace->phase((o4.j2+o5.j2-2*J2p)/2)
: 1; if (I4==I5) phase_45 *= PhysConst::SQRT2;


                double hat_factor = sqrt( (2*J1p+1)*(2*J2p+1) );

                int j2a_min = std::max( std::abs(o6.j2-2*J1p),
std::abs(o3.j2-2*J2p)); int j2a_max = std::min( o6.j2+2*J1p,  o3.j2+2*J2p);

//                std::cout << "j2a min,max = " << j2a_min << " " << j2a_max <<
std::endl;

                for (int j2a=j2a_min;j2a<=j2a_max; j2a+=2)
                {
                  double ja = 0.5 * j2a;
//                  std::cout << "   j2a = " << j2a << "  parity_a = " <<
parity_a  << std::endl; int la = (j2a-1)/2 + ((j2a-1)/2+parity_a)%2; if (la >
Z.modelspace->GetEmax()) continue;

                  double sixj = j2a<twoJ  ?  Z.modelspace->GetSixJ(j6,ja,J1p,
j3, 0.5*twoJ, J2p) :  Z.modelspace->GetSixJ(j6,0.5*twoJ,J2p, j3, ja, J1p) ; if
(std::abs(sixj)<1e-7) continue;
//                  double sixj = Z.modelspace->GetSixJ(j6,ja,J1p, j3, 0.5*twoJ,
J2p);
                  // l = j+-1/2.     l = j-1/2 + (j-1/2+parity)%2.   e.g 7/2-:
3 + (3+1)%2 = 3 if ( la%2 != parity_a ) std::cout << "OOPS parity_a = " <<
parity_a << " and la= " <<la << std::endl;
//                  for ( auto a : Z.modelspace->all_orbits )
//                  std::cout << " looking up a in " << la << " " << j2a << " "
<< tz2a << std::endl;
...
//                     if (std::abs(x_126a)<1e-7 and std::abs(y_126a)<1e-7)
continue;

/// PUT THESE 4 LINES BACK!!
                      size_t ind_6a =
tbc1.GetLocalIndex(std::min(I6,a),std::max(I6,a)); if (ind_6a>nkets_1) continue;
                      double phase_6a = I6>a ?
-Z.modelspace->phase((o6.j2+j2a-2*J1p)/2)  : 1; if (I6==a) phase_6a *=
PhysConst::SQRT2;

                      size_t ind_3a =
tbc2.GetLocalIndex(std::min(I3,a),std::max(I3,a)); if (ind_3a>nkets_2) continue;
                      double phase_3a = I3>a ?
-Z.modelspace->phase((o3.j2+j2a-2*J2p)/2)  : 1; if (I3==a) phase_3a *=
PhysConst::SQRT2;

//                     std::cout << "I1,I2,I6,a = " << I1 << " " << I2 << " " <<
I6 << " " << a << "  ind_12, ind_6a = " << ind_12 << " " << ind_6a << std::endl;
//                     double x_126a = phase_12 * phase_6a * X2.GetTBME_norm(
ch1,ch1, ind_12,ind_6a);
//                     double y_126a = phase_12 * phase_6a * Y2.GetTBME_norm(
ch1,ch1, ind_12,ind_6a);
//                     double x_3a45 = phase_45 * phase_3a * X2.GetTBME_norm(
ch2,ch2, ind_3a,ind_45);
//                     double y_3a45 = phase_45 * phase_3a * Y2.GetTBME_norm(
ch2,ch2, ind_3a,ind_45);
//
// The fastest one
//                     double x_126a = phase_12  * 0;
//                     double y_126a = phase_12  * 0;
                     double x_126a = phase_12 * phase_6a * XMAT1(
ind_12,ind_6a); double y_126a = phase_12 * phase_6a * YMAT1( ind_12,ind_6a);
                     double x_3a45 = phase_3a * phase_45 * XMAT2(
ind_3a,ind_45); double y_3a45 = phase_3a * phase_45 * YMAT2( ind_3a,ind_45);
...
//                     double sixj = Z.modelspace->GetSixJ(j6,ja,J1p, j3,
0.5*twoJ, J2p); zijklmn += rec_ijk * rec_lmn * sixj*hat_factor * (x_126a *
y_3a45 - y_126a * x_3a45);
...
void comm223ss_new(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  // create a function object for hashing 4 size_t (unsigned long int) to a
  // single size_t for lookup in an unordered_map
  int max_twoJph = 6 * Z.modelspace->GetEmax() + 3;
  auto ch_pph_hash = [](int twoJ_ph, int parity, int twoTz_ph) {
    return 8 * (twoJ_ph / 2) + parity + (twoTz_ph + 3);
  };
  ...
  std::vector<std::array<int, 3>>
      channel_list_pph;  // vector  channel_index -> twoJ-ph, parity_ph,
                         // twoTz_ph.
  ...
  std::vector<std::unordered_map<size_t, size_t>>
      ket_lookup_pph;  // in a given channel, map  i,j,n,Jij -> matrix index

  std::vector<float> Zbar;  // store the transformed Z in a 1D vector.
  std::vector<size_t>
      Zbar_start_pointers;  // ch -> index in Zbar where that channel starts
  std::unordered_map<size_t, size_t>
      Zbar_start_pointers_ijklmn;  // ijklmn => index in Zbar where that channel
                                   // starts
  ...
  // This is the hash {i,j,k,l,m,n} => size_t, for use in Zbar_start_pointers,
  // or maybe a different name?
  auto hash_key_ijklmn = [](size_t i, size_t j, size_t k, size_t l, size_t m,
                            size_t n) {
  ...
  //  looping over ijklmn, we keep i<=j,  l<=m,  i<=l, and if i==l then j<=m,
  //  and if i==l,j==m, then k<=n.
  // for for a given ijklmn,
  // twoJmax = min( ji+jj+jk,  jl+jm+jn)  and twoJmin = max(twoJmin_ijk,
  // twoJmin_lmn)
  ...
  for (size_t i = 0; i < norbits; i++) {
    ...
    for (size_t j = i; j < norbits; j++) {
      ...
      if ((ei + ej) > Z.modelspace->GetE3max()) continue;
      if (perturbative_triples and
          ((oi.cvq == 0 and oj.cvq != 0) or (oi.cvq != 0 and oj.cvq == 0)))
        continue;
      ...
      for (size_t k = 0; k < norbits; k++) {
        ...
        if (perturbative_triples and
            ((oi.cvq == 0 and oj.cvq == 0 and ok.cvq == 0) or
             (oi.cvq != 0 and oj.cvq != 0 and ok.cvq != 0)))
          continue;

        int twoJmin_ijk = std::max(
            {std::abs(oi.j2 - oj.j2) - ok.j2, ok.j2 - oi.j2 - oj.j2, 1});
        ...
        for (size_t l = i; l < norbits; l++) {
          ...
          for (size_t m = ((i == l) ? j : l); m < norbits; m++) {
            ...
            if (perturbative_triples and ((ol.cvq == 0 and om.cvq != 0) or
                                          (ol.cvq != 0 and om.cvq == 0)))
              continue;
            ...
            for (size_t n = ((i == l and j == m) ? k : 0); n < norbits; n++) {
              ...
              if ((occnat_l * (1 - occnat_l) * occnat_m * (1 - occnat_m) *
                   occnat_n * (1 - occnat_n)) < Z.modelspace->GetOccNat3Cut())
                continue;
              if ((d_el + d_em + d_en) > Z.modelspace->GetdE3max()) continue;
              if ((el + em + en) > Z.modelspace->GetE3max()) continue;
              if (perturbative_triples and
                  ((ol.cvq == 0 and om.cvq == 0 and on.cvq == 0) or
                   (ol.cvq != 0 and om.cvq != 0 and on.cvq != 0)))
                continue;

              int twoJmin_lmn = std::max(
                  {std::abs(ol.j2 - om.j2) - on.j2, on.j2 - ol.j2 - om.j2, 1});
              ...
              if (twoJmax >= twoJmin) {
                ...
                //                 std::cout << " HASHING " << i << " " << j <<
                //                 " " << k << " " << l << " " << m << " " << n
                //                 << "   to " << key << "   and pointing to "
                //                 << ngood_ijklmn << std::endl;
              }
              for (int twoJ = twoJmin; twoJ <= twoJmax; twoJ += 2) {
                int parity_ph = (oi.l + oj.l + ok.l) % 2;
                if ((twoJ == max_single_j2) and
                    (parity_ph != Z.modelspace->GetEmax() % 2))
                  continue;
                int twoTz_ph = (oi.tz2 + oj.tz2 - ok.tz2);
                int ch_pph =
                    channels_pph[ch_pph_hash(twoJ, parity_ph, twoTz_ph)];
                int Jij_min =
                    std::max(std::abs(oi.j2 - oj.j2), std::abs(twoJ - ok.j2)) /
                    2;
                int Jij_max = std::min(oi.j2 + oj.j2, twoJ + ok.j2) / 2;
                int Jlm_min =
                    std::max(std::abs(ol.j2 - om.j2), std::abs(twoJ - on.j2)) /
                    2;
                int Jlm_max = std::min(ol.j2 + om.j2, twoJ + on.j2) / 2;
                for (int Jij = Jij_min; Jij <= Jij_max; Jij++) {
                  if (i == j and Jij % 2 > 0) continue;
                  //                    size_t key_ijk =
                  //                    hash_key_ijnJ(std::min(i,j),std::max(i,j),k,Jij);
                  if (ch_pph == -1 or ch_pph >= (int)ket_lookup_pph.size()) {
                    std::cout << "TROUBLE line " << __LINE__ << " ijk = " << i
                              << " " << j << " " << k << "   " << Jij
                              << "   twoJ parity twoTz " << twoJ << " "
                              << parity_ph << " " << twoTz_ph
                              << "  ch = " << ch_pph << std::endl;
                  }
                  //                     size_t index_ijk =
                  //                     ket_lookup_pph[ch_pph].at(key_ijk );
                  ...
                  for (int Jlm = Jlm_min; Jlm <= Jlm_max; Jlm++) {
                    ...
                    //                    size_t key_lmn =
                    //                    hash_key_ijnJ(std::min(l,m),std::max(l,m),n,Jlm);
                    //                     size_t index_lmn =
                    //                     ket_lookup_pph[ch_pph].at(key_lmn );
...
#pragma omp parallel for schedule( \
  ...
  for (size_t ch_pph = 0; ch_pph < nch_pph; ch_pph++) {
    ...
    for (auto& iter_ijn : good_kets_ijn) {
      ...
      int twoJmin_ijn =
          std::max({std::abs(oi.j2 - oj.j2) - on.j2, on.j2 - oi.j2 - oj.j2, 1});
      ...
      for (auto& iter_lmk : good_kets_ijn) {
        ...
        if (Zbar_start_pointers_ijklmn.find(key_ijnlmk) ==
            Zbar_start_pointers_ijklmn.end()) {
          std::cout << "OOPS " << __LINE__ << " II: " << Iijn << " " << Ilmk
                    << "  ijnlmk " << i << " " << j << " " << n << " " << l
                    << " " << m << " " << k
                    << " parity: " << (oi.l + oj.l + on.l) % 2 << " vs "
                    << (ol.l + om.l + ok.l) % 2 << "  Tz "
                    << (oi.tz2 + oj.tz2 - on.tz2) << " vs "
                    << (ol.tz2 + om.tz2 - ok.tz2) << std::endl;
        }
        size_t start_ptr = Zbar_start_pointers_ijklmn[key_ijnlmk];

        int twoJmin_lmk = std::max(
            {std::abs(ol.j2 - om.j2) - on.j2, on.j2 - ol.j2 - om.j2, 1});
        ...
        for (int twoJ = twoJmin; twoJ <= twoJmax; twoJ += 2) {
          ...
          if ((twoJ == max_single_j2) and
            ...
            continue;
          int Jij_min =
              std::max(std::abs(oi.j2 - oj.j2), std::abs(twoJ - on.j2)) / 2;
          int Jij_max = std::min(oi.j2 + oj.j2, twoJ + ok.j2) / 2;
          int Jlm_min =
              std::max(std::abs(ol.j2 - om.j2), std::abs(twoJ - ok.j2)) / 2;
          int Jlm_max = std::min(ol.j2 + om.j2, twoJ + on.j2) / 2;
          for (int Jij = Jij_min; Jij <= Jij_max; Jij++) {
            if (i == j and Jij % 2 > 0) continue;
            ...
            for (int Jlm = Jlm_min; Jlm <= Jlm_max; Jlm++) {
              if (l == m and Jlm % 2 > 0) continue;
              if (i == l and j == m and k == n and Jlm < Jij) continue;
              if ((twoJ == twoJph) and (Jij == Jij_this) and
                  (Jlm == Jlm_this)) {
                if ((start_ptr + counter) >= Zbar.size()) {
                  std::cout << "TROUBLE " << __LINE__ << "  ijnlmk " << i << " "
                            << j << " " << n << " " << l << " " << m << " " << k
                            << std::endl;
                }
    ...
    //   // Fold the full (anti-)symmetric matrix into a more compact storage
    //   // we  use row-major ordering so that the inner loop runs over adjacent
    //   elements size_t start_ptr = Zbar_start_pointers[ch_pph]; for (size_t
    //   II=0; II<ngood_ijn; II++)
    //   {
    //     size_t row_index = start_ptr + ( 2*ngood_ijn - II - 1) * II / 2 ;
    //     for (size_t JJ=II; JJ<ngood_ijn; JJ++)
    //     {
      ...
      for (size_t iket = ibra; iket < nkets3; iket++) {
        ...
        for (int perm_ijk = 0; perm_ijk < 3; perm_ijk++) {
          ...
          for (int perm_lmn = 0; perm_lmn < 3; perm_lmn++) {
            ...
            int twoJmin_126 = std::max(
                {std::abs(o1.j2 - o2.j2) - o6.j2, o6.j2 - o1.j2 - o2.j2, 1});
            int twoJmax_126 = o1.j2 + o2.j2 + o6.j2;
            int twoJmin_453 = std::max(
                {std::abs(o4.j2 - o5.j2) - o3.j2, on.j2 - ol.j2 - om.j2, 1});
            int twoJmax_453 = o4.j2 + o5.j2 + o3.j2;
            int twoJph_min = std::max(twoJmin_126, twoJmin_453);
            int twoJph_max =
                std::min({twoJmax_126, twoJmax_453, max_single_j2});

            size_t key_126453;
            int phase_126453 = 1;
            if ((I4 < I1) or (I4 == I1 and I5 < I2) or
                (I4 == I1 and I5 == I2 and I3 < I6)) {
              key_126453 =
                  hash_key_ijklmn(std::min(I4, I5), std::max(I4, I5), I3,
                                  std::min(I1, I2), std::max(I1, I2), I6);
              phase_126453 *= hZ;
            } else {
              key_126453 =
                  hash_key_ijklmn(std::min(I1, I2), std::max(I1, I2), I6,
                                  std::min(I4, I5), std::max(I4, I5), I3);
            }
            ...
            for (int twoJph = twoJph_min; twoJph <= twoJph_max; twoJph++) {
              double zbar_126453 = 0;
              double Jph = 0.5 * twoJph;
              if ((twoJph == max_single_j2) and
                  (parity_ph != Z.modelspace->GetEmax() % 2))
                continue;

              int J12_min =
                  std::max(std::abs(o1.j2 - o2.j2), std::abs(twoJph - o6.j2)) /
                  2;
              int J12_max = std::min(o1.j2 + o2.j2, twoJph + o6.j2) / 2;
              int J45_min =
                  std::max(std::abs(o4.j2 - o5.j2), std::abs(twoJph - o3.j2)) /
                  2;
              ...
              for (int J12 = J12_min; J12 <= J12_max; J12++) {
                ...
                int phase_12 =
                    I1 > I2 ? -Z.modelspace->phase((o1.j2 + o2.j2) / 2 - J12)
                            : 1;

                double rec_ijk =
                    recouple_ijk[perm_ijk].at(J12 - J1p_min[perm_ijk]);
                for (int J45 = J45_min; J45 <= J45_max; J45++) {
                  if (I4 == I5 and J45 % 2 > 0) continue;
                  //                       if ( I1==I4 and I2==I5 and I6==I3 and
                  //                       J45<J12) continue; // maybe we don't
                  //                       want this...
                  if (AngMom::Triangle(J12, j3, Jtot) and
                      AngMom::Triangle(J12, j6, Jph) and
                      AngMom::Triangle(J45, j6, Jtot) and
                      AngMom::Triangle(J45, j3, Jph)) {
                    double rec_lmn =
                        recouple_lmn[perm_lmn].at(J45 - J2p_min[perm_lmn]);
                    int phase_45 =
                        I4 > I5
                            ? -Z.modelspace->phase((o4.j2 + o5.j2) / 2 - J45)
                            : 1;

                    zbar_126453 = Zbar[start_ptr + counter] * phase_126453 *
                                  phase_12 * phase_45;

                    double hat_factor = sqrt((2 * J12 + 1.) * (2 * J45 + 1));
                    double sixj1 =
                        Z.modelspace->GetSixJ(J12, j3, Jtot, J45, j6, Jph);
                    //                       z_123456 +=   sixj1 * zbar_126453 ;
        ...
        //
        //              size_t key_126 =
        //              hash_key_ijnJ(std::min(I1,I2),std::max(I1,I2),I6,J1p);
        //
        //
        //              for (int J2p=J2p_min[perm_lmn]; J2p<=J2p_max[perm_lmn];
        //              J2p++)
        //              {
        ...
        //                recouple_lmn[perm_lmn].at(J2p-J2p_min[perm_lmn]);
        //
        //                int phase_12 = I1>I2 ? -Z.modelspace->phase(
        //                (o1.j2+o2.j2)/2 - J1p) : 1; int phase_45 = I4>I5 ?
        //                -Z.modelspace->phase( (o4.j2+o5.j2)/2 - J2p) : 1;
        //                double hat_factor = sqrt( (2*J1p+1)*(2*J2p+1) );
        //                double z_123456 = 0;
        //
        //                size_t key_453 =
        //                hash_key_ijnJ(std::min(I4,I5),std::max(I4,I5),I3,J2p);
        //
        //                int parity_ph = (o1.l+o2.l+o6.l)%2;
        //                int twoTz_ph = (o1.tz2+o2.tz2-o6.tz2);
        //                int twoJph_min = std::max( std::abs(2*J1p-o6.j2),
        ...
        ///);
        //
        //                int twoJph_max = std::min( { 2*J1p+o6.j2 ,
        //                2*J2p+o3.j2, max_single_j2 } ); if (
        //                std::abs(twoTz_ph)>1 ) continue;
        //
        //// This inner loop is slow
        ////                for ( int twoJph=twoJph_min; twoJph<=twoJph_max;
        ///twoJph++) /                { /                   double
        ///zbar_126453=0; /                   double Jph = 0.5 * twoJph;
        //
        //
        //                   // This lookup is expensive. Replace map with
        //                   vector makes things better int ch_pph =
        //                   channels_pph[
        //                   ch_pph_hash(twoJph,parity_ph,twoTz_ph)]; if (
        //                   ch_pph<0 ) continue;
        //
        //
        //                   size_t index_126 =
        //                   ket_lookup_pph[ch_pph].at(key_126 ); size_t
        //                   index_453 = ket_lookup_pph[ch_pph].at(key_453 );
        //
        //                   size_t start_ptr = Zbar_start_pointers[ch_pph];
        //                   size_t ngood_ijn = ket_lookup_pph[ch_pph].size();
        //                   if ( index_126<= index_453 )
        //                   {
        //                     size_t zbar_index = start_ptr + ( 2*ngood_ijn -
        //                     index_126 - 1) * index_126 / 2  + index_453 ;
        //                     zbar_126453 = Zbar[ zbar_index ]  * phase_12 *
        //                     phase_45;
        //                   }
        //                   else
        //                   {
        //                     size_t zbar_index = start_ptr + ( 2*ngood_ijn -
        //                     index_453 - 1) * index_453 / 2  + index_126 ;
        //                     zbar_126453 = hZ * Zbar[ zbar_index ]  * phase_12
        //                     * phase_45;
        //                   }
        //
        //                   double sixj1 = Z.modelspace->GetSixJ(J1p,j3,Jtot,
        //                   J2p,j6,Jph); z_123456 +=   sixj1 * zbar_126453  ;
        //
        //                }
        //
        //                zijklmn += hat_factor * rec_ijk * rec_lmn * z_123456;
...
bool check_2b_channel_Tz_parity(const Operator& Op, Orbit& o1, Orbit& o2,
                                Orbit& o3, Orbit& o4) {
  return (((o1.l + o2.l + o3.l + o4.l) % 2 == Op.parity) and
          (std::abs(o1.tz2 + o2.tz2 - o3.tz2 - o4.tz2) == 2 * Op.rank_T));
}
...
Z.modelspace->scalar3b_transform_first_pass) return;
  ...
  std::map<int,double> e_fermi = Z.modelspace->GetEFermi();
  // we loop over i, j<=i, k<=j    l<=i, m<=l, n<=m, and Jij, Jmn free unless
ijk == lmn, then we take Jmn <= Jij.

//  std::vector< std::map<std::array<size_t,2>,size_t>::iterator>
channel_iterators; std::vector< std::pair<const std::array<size_t,2>,size_t>>
channel_iterators; for (auto iter : Z.ThreeBody.ch_start )
channel_iterators.push_back(iter); size_t nch3 =
Z.modelspace->GetNumberThreeBodyChannels(); size_t n_iter =
channel_iterators.size();
//  for (size_t ch3=0; ch3<nch3; ch3++ )
  #pragma omp parallel for schedule(dynamic,1) if (not
Z.modelspace->scalar3b_transform_first_pass) for (size_t iiter=0; iiter<n_iter;
iiter++ )
  {
...
//      Ket3& bra = Tbc.GetKet(ibra);
      ...
      double occnat_j = oj.occ_nat;
      double occnat_k = ok.occ_nat;
      if ( (std::abs(ei-e_fermi[oi.tz2]) + std::abs(ej-e_fermi[oj.tz2]) +
std::abs(ek-e_fermi[ok.tz2])) > Z.modelspace->GetdE3max() ) continue; if (
(occnat_i*(1-occnat_i) * occnat_j*(1-occnat_j) * occnat_k*(1-occnat_k) ) <
Z.modelspace->GetOccNat3Cut() ) continue; double ji = 0.5*oi.j2; double jj =
0.5*oj.j2; double jk = 0.5*ok.j2; int J1 = bra.Jpq;
          ...
          {
             double recouple_coeff = (2*Jx+1) * Z.modelspace->GetSixJ(ji,jj,J1,
jk,Jtot,Jx); if (std::abs( recouple_coeff ) <1e-7) continue; J_Pik.push_back( Jx
          ...
          for (int Jx=Jx_min; Jx<=Jx_max; Jx++)
          {
             double recouple_coeff = (2*Jx+1) * Z.modelspace->GetSixJ(jj,ji,J1,
jk,Jtot,Jx); if (std::abs( recouple_coeff ) <1e-7) continue; J_Pjk.push_back( Jx
...
//        Ket3& ket = Tbc.GetKet(iket);
        ...
        double occnat_m = om.occ_nat;
        double occnat_n = on.occ_nat;
        if ( (std::abs(el-e_fermi[ol.tz2]) + std::abs(em-e_fermi[om.tz2]) +
std::abs(en-e_fermi[on.tz2])) > Z.modelspace->GetdE3max() ) continue; if (
(occnat_l*(1-occnat_l) * occnat_m*(1-occnat_m) * occnat_n*(1-occnat_n) ) <
Z.modelspace->GetOccNat3Cut() ) continue; double jl = 0.5*ol.j2; double jm =
0.5*om.j2; double jn = 0.5*on.j2; int J2 = ket.Jpq;
        ...
        double zijklmn = 0;
          ...
          for (int Jx=Jx_min; Jx<=Jx_max; Jx++)
          {
             double recouple_coeff = (2*Jx+1) * Z.modelspace->GetSixJ(jl,jm,J2,
jn,Jtot,Jx); if (std::abs( recouple_coeff ) <1e-7) continue; J_Pln.push_back( Jx
          ...
          for (int Jx=Jx_min; Jx<=Jx_max; Jx++)
          {
             double recouple_coeff = (2*Jx+1) * Z.modelspace->GetSixJ(jm,jl,J2,
jn,Jtot,Jx); if (std::abs( recouple_coeff ) <1e-7) continue; J_Pmn.push_back( Jx
); recouple_Pmn.push_back(recouple_coeff);
          }


//         for (int a=0; a<norbs; a++) // TODO: this can maybe be made more
efficient by first looping through ja and computing the 6js? for ( auto OBC_a :
Z.OneBodyChannels )
         {
...
//          double ja = 0.5*oa.j2;
          ...
          // TODO: recast these 9 terms into a loop as done in the other []->3
commutators

          //Z5   remapped to ZZ1
          if (   (check_2b_channel_Tz_parity(X,oi,oj,on,oa1) and
check_2b_channel_Tz_parity(Y,ok,oa1,om,ol)) or
(check_2b_channel_Tz_parity(Y,oi,oj,on,oa1) and
check_2b_channel_Tz_parity(X,ok,oa1,om,ol)) )
          {
          ...
          if (  ( (check_2b_channel_Tz_parity(X,ok,oj,on,oa1) and
check_2b_channel_Tz_parity(Y,oa1,oi,om,ol)) or
(check_2b_channel_Tz_parity(Y,ok,oj,on,oa1) and
check_2b_channel_Tz_parity(X,oa1,oi,om,ol)) ) and (
(std::abs(oa1.j2-oi.j2)<=2*J2 ) and (oa1.j2+oi.j2>=2*J2)  )   )
          {
            ...
            for (auto a : OBC_a.second )
            {
             ...
             for ( size_t i_Pik=0; i_Pik<J_Pik.size(); i_Pik++)
             {
               ...
               zijklmn += recouple_ik * phase * sixj2 *
(xkjna*yaiml-ykjna*xaiml);
              }
            }
          }
          ...
          if ( (  (check_2b_channel_Tz_parity(X,oi,ok,on,oa1) and
check_2b_channel_Tz_parity(Y,oa1,oj,om,ol)) or
(check_2b_channel_Tz_parity(Y,oi,ok,on,oa1) and
check_2b_channel_Tz_parity(X,oa1,oj,om,ol)) ) and (
(std::abs(oa1.j2-oj.j2)<=2*J2 ) and (oa1.j2+oj.j2>=2*J2)  )  )
          {
            ...
            for (auto a : OBC_a.second )
            {
             ...
             for ( size_t i_Pjk=0; i_Pjk<J_Pjk.size(); i_Pjk++)
             {
              ...
              double phase = Z.modelspace->phase( (oa1.j2+ok.j2+ol.j2+om.j2)/2
-J1-Jx); double sixj2 = Z.modelspace->GetSixJ(jj,ja,J2, jn,Jtot,Jx); if
(std::abs(sixj2)<1e-7) continue; double xikna = X2.GetTBME_J(Jx,i,k,n,a); double
yikna = Y2.GetTBME_J(Jx,i,k,n,a); zijklmn +=  phase * recouple_jk * sixj2 *
(xikna*yajml-yikna*xajml);
             }
            }
          }
          ...
          if (  ( (check_2b_channel_Tz_parity(X,oi,oj,om,oa1) and
check_2b_channel_Tz_parity(Y,oa1,ok,ol,on)) or
(check_2b_channel_Tz_parity(Y,oi,oj,om,oa1) and
check_2b_channel_Tz_parity(X,oa1,ok,ol,on)) ) and (
(std::abs(om.j2-oa1.j2)<=2*J1 ) and (om.j2+oa1.j2>=2*J1)  ) )
          {
            Jx_min = std::max(std::abs(oa1.j2-ok.j2), std::abs(ol.j2-on.j2) )/2;
            Jx_max = std::min((oa1.j2+ok.j2), (ol.j2+on.j2) )/2;
            double phase = -Z.modelspace->phase(
(om.j2-on.j2-oa1.j2-ok.j2)/2-J2); for (auto a : OBC_a.second )
            {
              ...
              for ( size_t i_Pmn=0; i_Pmn<J_Pmn.size(); i_Pmn++)
              {
                ...
                zijklmn +=  phase * recouple_mn * sixj2 *
(xijma*yakln-yijma*xakln);
              }
            }
          }
          ...
          if (  (   (check_2b_channel_Tz_parity(X,oi,oj,ol,oa1) and
check_2b_channel_Tz_parity(Y,oa1,ok,om,on)) or
(check_2b_channel_Tz_parity(Y,oi,oj,ol,oa1) and
check_2b_channel_Tz_parity(X,oa1,ok,om,on)) ) and (std::abs(ol.j2-oa1.j2)<=2*J1)
and ((ol.j2+oa1.j2)>=2*J1) )
          {
            ...
            for (auto a : OBC_a.second )
            {
              ...
              for ( size_t i_Pln=0; i_Pln<J_Pln.size(); i_Pln++)
              {
                ...
                zijklmn +=  phase * recouple_ln * sixj2 *
(xijla*yakmn-yijla*xakmn);
              }
            }
          }
          ...
          if (   (check_2b_channel_Tz_parity(X,ok,oj,ol,oa1) and
check_2b_channel_Tz_parity(Y,oa1,oi,om,on)) or
(check_2b_channel_Tz_parity(Y,ok,oj,ol,oa1) and
check_2b_channel_Tz_parity(X,oa1,oi,om,on)) )
          {
            ...
            for ( size_t i_Pik=0; i_Pik<J_Pik.size(); i_Pik++)
            {
              ...
              for (auto a : OBC_a.second )
              {
                ...
                for ( size_t i_Pln=0; i_Pln<J_Pln.size(); i_Pln++)
                {
                 ...
                 zijklmn +=  phase * recouple_ik  * recouple_ln *sixj3 *
(xkjla*yaimn -ykjla*xaimn);
                }
              }
            }
          }
          ...
          if (   (check_2b_channel_Tz_parity(X,oi,ok,ol,oa1) and
check_2b_channel_Tz_parity(Y,oa1,oj,om,on)) or
(check_2b_channel_Tz_parity(Y,oi,ok,ol,oa1) and
check_2b_channel_Tz_parity(X,oa1,oj,om,on)) )
          {
            ...
            for ( size_t i_Pjk=0; i_Pjk<J_Pjk.size(); i_Pjk++)
            {
              ...
              double recouple_jk = recouple_Pjk[i_Pjk];
              double phase =-Z.modelspace->phase(
(ok.j2+om.j2+on.j2-oa1.j2)/2-J1-Jx); for (auto a : OBC_a.second )
               {
                 ...
                 for ( size_t i_Pln=0; i_Pln<J_Pln.size(); i_Pln++)
                 {
                  ...
                  zijklmn += phase * recouple_jk * recouple_ln *sixj3 *
(xikla*yajmn-yikla*xajmn);
                 }
              }
            }
          }
          ...
          if (   (check_2b_channel_Tz_parity(X,ok,oj,om,oa1) and
check_2b_channel_Tz_parity(Y,oa1,oi,ol,on)) or
(check_2b_channel_Tz_parity(Y,ok,oj,om,oa1) and
check_2b_channel_Tz_parity(X,oa1,oi,ol,on)) )
          {
            ...
            double phase =-Z.modelspace->phase(
(on.j2-om.j2-oi.j2-oa1.j2)/2-J2); for ( size_t i_Pik=0; i_Pik<J_Pik.size();
i_Pik++)
            {
              int Jx = J_Pik[i_Pik];
              if ( Jx<Jx_min or Jx>Jx_max) continue;
              ...
              for (auto a : OBC_a.second )
              {
                ...
                for ( size_t i_Pmn=0; i_Pmn<J_Pmn.size(); i_Pmn++)
                {
                  ...
                  zijklmn +=  phase * recouple_ik * recouple_mn * sixj3 *
(xkjma*yailn-ykjma*xailn);
                }
             }
            }
          }
          ...
          if (   (check_2b_channel_Tz_parity(X,oi,ok,om,oa1) and
check_2b_channel_Tz_parity(Y,oa1,oj,ol,on)) or
(check_2b_channel_Tz_parity(Y,oi,ok,om,oa1) and
check_2b_channel_Tz_parity(X,oa1,oj,ol,on)) )
          {
            ...
            for ( size_t i_Pjk=0; i_Pjk<J_Pjk.size(); i_Pjk++)
            {
              ...
              double recouple_jk = recouple_Pjk[i_Pjk];
              double phase =-Z.modelspace->phase(
(ok.j2+om.j2+on.j2-oa1.j2)/2-J1-J2-Jx); for (auto a : OBC_a.second )
              {
                ...
                for ( size_t i_Pmn=0; i_Pmn<J_Pmn.size(); i_Pmn++)
                {
                  ...
                  zijklmn +=  phase * recouple_jk * recouple_mn * sixj3 *
(xikma*yajln-yikma*xajln);
                }
              }
            }
          }
...
//      Ket3& bra = Tbc.GetKet(ibra);
      ...
      int ek = 2*ok.n + ok.l;
      if ( (std::abs(ei-e_fermi[oi.tz2]) + std::abs(ej-e_fermi[oj.tz2]) +
std::abs(ek-e_fermi[ok.tz2])) > Z.modelspace->GetdE3max() ) continue; double ji
= 0.5*oi.j2; double jj = 0.5*oj.j2; double jk = 0.5*ok.j2; int J1 = bra.Jpq;
      size_t ket_min = (ch_bra==ch_ket) ? ibra : 0;
//      for (size_t iket=ibra; iket<nkets3; iket++)
      for (size_t iket=ket_min; iket<nkets3; iket++)
      {
//        Ket3& ket = Tbc.GetKet(iket);
        ...
        int en = 2*on.n + on.l;
        if ( (std::abs(el-e_fermi[ol.tz2]) + std::abs(em-e_fermi[om.tz2]) +
std::abs(en-e_fermi[on.tz2])) > Z.modelspace->GetdE3max() ) continue; double jl
= 0.5*ol.j2; double jm = 0.5*om.j2; double jn = 0.5*on.j2; int J2 = ket.Jpq;

        double zijklmn = 0;

         for (int a=0; a<norbs; a++) // TODO: this can maybe be made more
efficient by first looping through ja and computing the 6js?
         {
          Orbit& oa = Z.modelspace->GetOrbit(a);
          ...
          //Z1
          if (  (   (check_2b_channel_Tz_parity(X,oi,oj,ol,oa) and
check_2b_channel_Tz_parity(Y,oa,ok,om,on)) or
(check_2b_channel_Tz_parity(Y,oi,oj,ol,oa) and
check_2b_channel_Tz_parity(X,oa,ok,om,on)) ) and (std::abs(ol.j2-oa.j2)<=2*J1)
and ((ol.j2+oa.j2)>=2*J1) )
          {
          ...
          //Z2
          if (   (check_2b_channel_Tz_parity(X,ok,oj,ol,oa) and
check_2b_channel_Tz_parity(Y,oa,oi,om,on)) or
(check_2b_channel_Tz_parity(Y,ok,oj,ol,oa) and
check_2b_channel_Tz_parity(X,oa,oi,om,on)) )
          {
...
); double sixj2 = Z.modelspace->GetSixJ(jl,jm,J2, jn,Jtot,Jy); double sixj3 =
Z.modelspace->GetSixJ(ji,ja,Jy, jl,Jtot,Jx); double xaimn =
X2.GetTBME_J(Jy,a,i,m,n); double yaimn = Y2.GetTBME_J(Jy,a,i,m,n); zijklmn +=
hats * phase * sixj1*sixj2*sixj3 * (xkjla*yaimn -ykjla*xaimn);
          ...
          //Z3
          if (   (check_2b_channel_Tz_parity(X,oi,ok,ol,oa) and
check_2b_channel_Tz_parity(Y,oa,oj,om,on)) or
(check_2b_channel_Tz_parity(Y,oi,ok,ol,oa) and
check_2b_channel_Tz_parity(X,oa,oj,om,on)) )
          {
            ...
            {
              double phase =-Z.modelspace->phase(
(ok.j2+om.j2+on.j2-oa.j2)/2-J1-Jx); double sixj1 =
Z.modelspace->GetSixJ(jj,ji,J1, jk,Jtot,Jx); double xikla =
X2.GetTBME_J(Jx,i,k,l,a); double yikla = Y2.GetTBME_J(Jx,i,k,l,a); for ( int
Jy=Jy_min; Jy<=Jy_max; Jy++)
              {
               double hats  = (2*Jx+1) * (2*Jy+1) * sqrt( (2*J1+1.)*(2*J2+1.) );
               ...
               double xajmn = X2.GetTBME_J(Jy,a,j,m,n);
               double yajmn = Y2.GetTBME_J(Jy,a,j,m,n);
               zijklmn += hats * phase * sixj1*sixj2*sixj3 *
(xikla*yajmn-yikla*xajmn);
              }
            }
          }
          ...
          //Z4
          if (  ( (check_2b_channel_Tz_parity(X,oi,oj,om,oa) and
check_2b_channel_Tz_parity(Y,oa,ok,ol,on)) or
(check_2b_channel_Tz_parity(Y,oi,oj,om,oa) and
check_2b_channel_Tz_parity(X,oa,ok,ol,on)) ) and ( (std::abs(om.j2-oa.j2)<=2*J1
) and (om.j2+oa.j2>=2*J1)  ) )
          {
            Jx_min = std::max(std::abs(oa.j2-ok.j2), std::abs(ol.j2-on.j2) )/2;
            Jx_max = std::min((oa.j2+ok.j2), (ol.j2+on.j2) )/2;
            double phase = -Z.modelspace->phase(
(om.j2-on.j2-oa.j2-ok.j2)/2-J2); double xijma = X2.GetTBME_J(J1,i,j,m,a); double
yijma = Y2.GetTBME_J(J1,i,j,m,a); for ( int Jx=Jx_min; Jx<=Jx_max; Jx++)
            {
              double hats  =  (2*Jx+1) * sqrt( (2*J1+1.)*(2*J2+1.) );
          ...
          //Z5
          if (   (check_2b_channel_Tz_parity(X,oi,oj,on,oa) and
check_2b_channel_Tz_parity(Y,ok,oa,om,ol)) or
(check_2b_channel_Tz_parity(Y,oi,oj,on,oa) and
check_2b_channel_Tz_parity(X,ok,oa,om,ol)) )
          {
          ...
          //Z6
          if (   (check_2b_channel_Tz_parity(X,ok,oj,om,oa) and
check_2b_channel_Tz_parity(Y,oa,oi,ol,on)) or
(check_2b_channel_Tz_parity(Y,ok,oj,om,oa) and
check_2b_channel_Tz_parity(X,oa,oi,ol,on)) )
          {
...
(xkjma*yailn-ykjma*xailn);
          ...
          //Z7
          if (  ( (check_2b_channel_Tz_parity(X,ok,oj,on,oa) and
check_2b_channel_Tz_parity(Y,oa,oi,om,ol)) or
(check_2b_channel_Tz_parity(Y,ok,oj,on,oa) and
check_2b_channel_Tz_parity(X,oa,oi,om,ol)) ) and ( (std::abs(oa.j2-oi.j2)<=2*J2
) and (oa.j2+oi.j2>=2*J2)  )   )
          {
          ...
          //Z8
          if (   (check_2b_channel_Tz_parity(X,oi,ok,om,oa) and
check_2b_channel_Tz_parity(Y,oa,oj,ol,on)) or
(check_2b_channel_Tz_parity(Y,oi,ok,om,oa) and
check_2b_channel_Tz_parity(X,oa,oj,ol,on)) )
          {
            ...
            {
              double phase =-Z.modelspace->phase(
(ok.j2+om.j2+on.j2-oa.j2)/2-J1-J2-Jx); double sixj1 =
Z.modelspace->GetSixJ(jj,ji,J1, jk,Jtot,Jx); double xikma =
X2.GetTBME_J(Jx,i,k,m,a); double yikma = Y2.GetTBME_J(Jx,i,k,m,a); for ( int
Jy=Jy_min; Jy<=Jy_max; Jy++)
             {
              double hats  =  (2*Jx+1) * (2*Jy+1) * sqrt( (2*J1+1.)*(2*J2+1.) );
              double sixj2 = Z.modelspace->GetSixJ(jm,jl,J2, jn,Jtot,Jy);
...
(xikma*yajln-yikma*xajln);
          ...
          //Z9
          if ( (  (check_2b_channel_Tz_parity(X,oi,ok,on,oa) and
check_2b_channel_Tz_parity(Y,oa,oj,om,ol)) or
(check_2b_channel_Tz_parity(Y,oi,ok,on,oa) and
check_2b_channel_Tz_parity(X,oa,oj,om,ol)) ) and ( (std::abs(oa.j2-oj.j2)<=2*J2
) and (oa.j2+oj.j2>=2*J2)  )  )
          {
...
-J1-Jx); double hats  =  (2*Jx+1) * sqrt( (2*J1+1.)*(2*J2+1.) ); double sixj1 =
Z.modelspace->GetSixJ(jj,ji,J1, jk,Jtot,Jx); double sixj2 =
Z.modelspace->GetSixJ(jj,ja,J2, jn,Jtot,Jx); double xikna =
X2.GetTBME_J(Jx,i,k,n,a); double yikna = Y2.GetTBME_J(Jx,i,k,n,a); zijklmn +=
hats * phase * sixj1*sixj2 * (xikna*yajml-yikna*xajml);
            }
          }
...
//
//  |     |    |     Uncoupled expression:
// i|    j|   k|       Z_ijklmn = 1/2 sum_{ab} (n`an`b - nanb) P(ij/k) X_{ijab}
// Y_{abklmn} - P(lm/n) Y_{ijkabn} X_{ablm}
//  *~~X~~*    |
// a|    b|    |
//  *~~~~[Y]~~~*     Coupled expression:
// l|    m|   n|     Z_{ijklmn}^{J1,J2,J} = 1/2 sum_{ab} (n`an`b - nanb) (
// P(ij/k)^{J1,J} X_{ijab}^{J1} Y_{abklmn}^{J1,J2,J}
//  |     |    |                                                         -
  ...
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t index_bra = 0; index_bra < nkets_kept; index_bra++) {
      ...
      for (size_t ch_ab = 0; ch_ab < nch2; ch_ab++) {
        ...
        if (((2 * tbc_ab.Tz + ok.tz2) == Tbc.twoTz) and
            ((tbc_ab.parity + ok.l + Tbc.parity) % 2 == 0) and Jab == Jij) {
          for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
            ...
            double d_ea = std::abs(2 * ket_ab.op->n + ket_ab.op->l -
                                   e_fermi[ket_ab.op->tz2]);
            double d_eb = std::abs(2 * ket_ab.op->n + ket_ab.op->l -
                                   e_fermi[ket_ab.op->tz2]);
            double occnat_a = ket_ab.op->occ_nat;
            double occnat_b = ket_ab.oq->occ_nat;
            ...
            //              size_t ch_check = Z3.GetKetIndex_withRecoupling(
            //              Jab, twoJ, a, b, k,  ket_list,  recouple_list );
            Z3.GetKetIndex_withRecoupling(Jab, twoJ, a, b, k, ket_list,
                                          recouple_list);
            for (size_t ilist = 0; ilist < ket_list.size(); ilist++) {
              auto iter_find = kept_lookup.find(ket_list[ilist]);
              if (iter_find == kept_lookup.end()) continue;
              size_t index_ket = iter_find->second;
              double recouple = recouple_list[ilist];
              X2MAT(index_bra, index_ket) +=
                  0.5 * symm_factor * occfactor * recouple * xijab;
              Y2MAT(index_bra, index_ket) +=
                  0.5 * symm_factor * occfactor * recouple * yijab;
            }  // for ilist
        ...
        // Permute  Pik
        if (((2 * tbc_ab.Tz + oi.tz2) == Tbc.twoTz) and
            ((tbc_ab.parity + oi.l + Tbc.parity) % 2 == 0) and
            (std::abs(2 * Jab - oi.j2) <= twoJ) and
            ((2 * Jab + oi.j2) >= twoJ) and
            (std::abs(2 * Jab - oj.j2) <= ok.j2) and
            ((2 * Jab + oj.j2) >= ok.j2)) {
          ...
          if (std::abs(sixj) > 1e-6) {
            ...
            for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
              ...
              //              std::cout << " line " << __LINE__ << "  calling
              //              GetKetIndex_withRecoupling " << Jab << " " << twoJ
              //              << " " << a << " " << b << " " << i << std::endl;
              //              size_t ch_check = Z3.GetKetIndex_withRecoupling(
              //              Jab, twoJ, a, b, i,  ket_list,  recouple_list );
              Z3.GetKetIndex_withRecoupling(Jab, twoJ, a, b, i, ket_list,
                                            recouple_list);
              //              std::cout << "    size of lists " <<
              //              ket_list.size() << " " << recouple_list.size() <<
              //              std::endl;
              for (size_t ilist = 0; ilist < ket_list.size(); ilist++) {
                auto iter_find = kept_lookup.find(ket_list[ilist]);
                if (iter_find == kept_lookup.end()) continue;
                size_t index_ket = iter_find->second;
                double recouple = recouple_list[ilist];
                X2MAT(index_bra, index_ket) += 0.5 * hats * sixj * symm_factor *
                                               occfactor * recouple * xkjab;
                Y2MAT(index_bra, index_ket) += 0.5 * hats * sixj * symm_factor *
                                               occfactor * recouple * ykjab;
              }  // for ilist
                 //              std::cout << "  ok. line " << __LINE__ <<
                 //              std::endl;
        ...
        // Permute  Pjk
        if (((2 * tbc_ab.Tz + oj.tz2) == Tbc.twoTz) and
            ((tbc_ab.parity + oj.l + Tbc.parity) % 2 == 0) and
            (std::abs(2 * Jab - oj.j2) <= twoJ) and
            ((2 * Jab + oj.j2) >= twoJ) and
            (std::abs(2 * Jab - oi.j2) <= ok.j2) and
            ((2 * Jab + oi.j2) >= ok.j2)) {
          ...
          if (std::abs(sixj) > 1e-6) {
            ...
            for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
              ...
              //              size_t ch_check = Z3.GetKetIndex_withRecoupling(
              //              Jab, twoJ, a, b, j,  ket_list,  recouple_list );
              Z3.GetKetIndex_withRecoupling(Jab, twoJ, a, b, j, ket_list,
                                            recouple_list);
              for (size_t ilist = 0; ilist < ket_list.size(); ilist++) {
                auto iter_find = kept_lookup.find(ket_list[ilist]);
                if (iter_find == kept_lookup.end()) continue;
                size_t index_ket = iter_find->second;
                double recouple = recouple_list[ilist];
                X2MAT(index_bra, index_ket) -= 0.5 * phase * hats * sixj *
                                               symm_factor * occfactor *
                                               recouple * xikab;
                Y2MAT(index_bra, index_ket) -= 0.5 * phase * hats * sixj *
                                               symm_factor * occfactor *
                                               recouple * yikab;
    ...
    // Do the matrix multiplication
    //    Z3MAT = X2MAT*Y3MAT - Y2MAT*X3MAT  +  hermX*hermY * (
    //    X3MAT.t()*Y2MAT.t() - Y3MAT.t()*X2MAT.t() );
...
void comm233_pp_hhss_debug( const Operator& X, const Operator& Y, Operator& Z )
{
  ...
  size_t nch2 = Z.modelspace->GetNumberTwoBodyChannels();
  #pragma omp parallel for schedule(dynamic,1) if (not
Z.modelspace->scalar3b_transform_first_pass) for (size_t ch=0; ch<=nch3; ch++)
  {
    ...
    for (size_t ibra=0; ibra<nket3; ibra++)
    {
      ...
      for (size_t iket=0; iket<=ibra; iket++)
      {
        ...
        for (size_t ch_ab=0; ch_ab<nch2; ch_ab++)
        {
          ...
          if ( ((2*tbc_ab.Tz + ok.tz2) == Tbc.twoTz)  and ( (tbc_ab.parity +
ok.l + Tbc.parity)%2==0) and Jab == J1)
          {
            for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
            {
              auto& ket_ab = tbc_ab.GetKet(iket_ab);
              ...
              size_t b = ket_ab.q;
              double d_ea = std::abs( 2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2] ); double d_eb = std::abs( 2*ket_ab.op->n + ket_ab.op->l
- e_fermi[ket_ab.op->tz2] ); if ( (d_ea + d_eb + d_ek) > Z.modelspace->dE3max )
continue; double occfactor = 1-ket_ab.op->occ-ket_ab.oq->occ; if
(std::abs(occfactor)<1e-6) continue; double symm_factor =  (a==b) ? 1.0 : 2.0;
              double xijab = X2.GetTBME_J(Jab,Jab,i,j,a,b);
              double yijab = Y2.GetTBME_J(Jab,Jab,i,j,a,b);
              double xabklmn = X3.GetME_pn(J1,J2,twoJ,a,b,k,l,m,n);
              double yabklmn = Y3.GetME_pn(J1,J2,twoJ,a,b,k,l,m,n);
              z_ijklmn += 0.5 * symm_factor * occfactor * (xijab * yabklmn -
yijab * xabklmn);
//              if (ch==0 and ibra==4 and iket==4)
//              {
//                 std::cout << "  iket_ab = " << iket_ab << "   adding to z 0.5
* " << symm_factor << " * " << occfactor << " * " << xijab << " " << yabklmn <<
"  =>  " << z_ijklmn << std::endl;;
//              }
            }// for iket_ab
          } // if J, Tz and parity check for term 1
          ...
          // Permute  Pik
          if ( ((2*tbc_ab.Tz + oi.tz2) == Tbc.twoTz)  and ( (tbc_ab.parity +
oi.l + Tbc.parity)%2==0) and (std::abs(2*Jab-oi.j2)<=twoJ) and
((2*Jab+oi.j2)>=twoJ) )
          {
             ...
             if (std::abs(sixj)>1e-6)
             {
              ...
              for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
              {
                ...
                double d_ea = std::abs( 2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2] ); double d_eb = std::abs( 2*ket_ab.op->n + ket_ab.op->l
- e_fermi[ket_ab.op->tz2] ); if ( (d_ea + d_eb + d_ei) > Z.modelspace->dE3max )
continue; double occfactor = 1-ket_ab.op->occ-ket_ab.oq->occ; if
(std::abs(occfactor)<1e-6) continue; double symm_factor =  (a==b) ? 1.0 : 2.0;
                double xkjab = X2.GetTBME_J(Jab,Jab,k,j,a,b);
                double ykjab = Y2.GetTBME_J(Jab,Jab,k,j,a,b);
                ...
                z_ijklmn += 0.5  * symm_factor * occfactor * hats * sixj *
(xkjab * yabilmn - ykjab * xabilmn);
//              if (ch==0 and ibra==4 and iket==4)
//              {
//                 std::cout << "Pik  iket_ab = " << iket_ab << "   adding to z
0.5 * " << symm_factor << " * " << occfactor << " * " << hats
//                           << " * " << sixj << " * " << xkjab << " " <<
yabilmn << "  =>  " << z_ijklmn << std::endl;;
//              }
              }// for iket_ab
            }// if sixj nonzero
          } // if J, Tz and parity check for term 2
          ...
          // Permute  Pjk
          if ( ((2*tbc_ab.Tz + oj.tz2) == Tbc.twoTz)  and ( (tbc_ab.parity +
oj.l + Tbc.parity)%2==0) and (std::abs(2*Jab-oj.j2)<=twoJ) and
((2*Jab+oj.j2)>=twoJ) )
          {
             ...
             if (std::abs(sixj)>1e-6)
             {
               ...
               for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
               {
                 ...
                 double d_ea = std::abs( 2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2] ); double d_eb = std::abs( 2*ket_ab.op->n + ket_ab.op->l
- e_fermi[ket_ab.op->tz2] ); if ( (d_ea + d_eb + d_ej) > Z.modelspace->dE3max )
continue; double occfactor = 1-ket_ab.op->occ-ket_ab.oq->occ; if
(std::abs(occfactor)<1e-6) continue; double symm_factor =  (a==b) ? 1.0 : 2.0;
                 double xikab = X2.GetTBME_J(Jab,Jab,i,k,a,b);
                 double yikab = Y2.GetTBME_J(Jab,Jab,i,k,a,b);
                 ...
                 z_ijklmn -= 0.5 * symm_factor  * occfactor * phase * hats *
sixj * (xikab * yabjlmn - yikab * xabjlmn);
//              if (ch==0 and ibra==4 and iket==4)
//              {
//                 std::cout << "Pjk  iket_ab = " << iket_ab << "   adding to z
0.5 * " << symm_factor << " * " << occfactor << " * "
//                           << phase << " * " << hats << " * " << sixj << " * "
<< xikab << " " << yabjlmn << "  =>  " << z_ijklmn << std::endl;
//              }
               }// for iket_ab
            }// if sixj nonzero
          } // if  J, Tz and parity check for term 3
          ...
          // direct Y3 X2 term
          if ( ((2*tbc_ab.Tz + on.tz2) == Tbc.twoTz)  and ( (tbc_ab.parity +
on.l + Tbc.parity)%2==0) and Jab == J2 )
          {
            for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
            {
              auto& ket_ab = tbc_ab.GetKet(iket_ab);
              ...
              double d_ea = std::abs( 2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2] ); double d_eb = std::abs( 2*ket_ab.op->n + ket_ab.op->l
- e_fermi[ket_ab.op->tz2] ); if ( (d_ea + d_eb + d_en) > Z.modelspace->dE3max )
continue; double occfactor = 1-ket_ab.op->occ-ket_ab.oq->occ; if
(std::abs(occfactor)<1e-6) continue; double symm_factor =  (a==b) ? 1.0 : 2.0;
              ...
              z_ijklmn -= 0.5 * symm_factor  * occfactor * (xablm * yijkabn -
yablm * xijkabn);
            }// for iket_ab
          ...
          // Permute  Pln
          if ( ((2*tbc_ab.Tz + ol.tz2) == Tbc.twoTz)  and ( (tbc_ab.parity +
ol.l + Tbc.parity)%2==0) and (std::abs(2*Jab-ol.j2)<=twoJ) and
((2*Jab+ol.j2)>=twoJ) )
          {
             ...
             if (std::abs(sixj)>1e-6)
             {
               ...
               for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
               {
                 ...
                 double d_ea = std::abs( 2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2] ); double d_eb = std::abs( 2*ket_ab.op->n + ket_ab.op->l
- e_fermi[ket_ab.op->tz2] ); if ( (d_ea + d_eb + d_el) > Z.modelspace->dE3max )
continue; double occfactor = 1-ket_ab.op->occ-ket_ab.oq->occ; if
(std::abs(occfactor)<1e-6) continue; double symm_factor =  (a==b) ? 1.0 : 2.0;
...
(xabnm * yijkabl - yabnm * xijkabl);
          ...
          // Permute  Pmn
          if ( ((2*tbc_ab.Tz + om.tz2) == Tbc.twoTz)  and ( (tbc_ab.parity +
om.l + Tbc.parity)%2==0) and (std::abs(2*Jab-om.j2)<=twoJ) and
((2*Jab+om.j2)>=twoJ) )
          {
             ...
             if (std::abs(sixj)>1e-6)
             {
              ...
              for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
              {
                ...
                double d_ea = std::abs( 2*ket_ab.op->n + ket_ab.op->l -
e_fermi[ket_ab.op->tz2] ); double d_eb = std::abs( 2*ket_ab.op->n + ket_ab.op->l
- e_fermi[ket_ab.op->tz2] ); if ( (d_ea + d_eb + d_em) > Z.modelspace->dE3max )
continue; double occfactor = 1-ket_ab.op->occ-ket_ab.oq->occ; if
(std::abs(occfactor)<1e-6) continue; double symm_factor =  (a==b) ? 1.0 : 2.0;
                ...
                z_ijklmn += 0.5 * symm_factor * phase  * occfactor * hats * sixj
* (xabln * yijkabm - yabln * xijkabm);
              }// for iket_ab
            }// if sixj nonzero
...
//        if (ch==0 and ibra<5 and iket<5)
//        {
//          std::cout << __func__ << "   ch = " << ch << "  ibra =  " <<  ibra
<< "  iket = " << iket
//                    << "   ijklmn " << bra.p << " " << bra.q << " " << bra.r
<< " " << ket.p << " " << ket.q << " " << ket.r
//                    << "  Jij Jlm twoJ " << bra.Jpq << " " << ket.Jpq << "  "
<< twoJ
//                    << "  zijklmn = " << z_ijklmn << std::endl;
//        }
        // no normalization to be done. we store un-normalized 3body matrix
elements. if (std::abs(z_ijklmn)>1e-6)
        {
...
//
// i|   j|        k|  Uncoupled expression:
//  |    |         |    C_ijklmn = sum_{ab} (n`anb -nan`b) P(ij/k)P(lm/n)
//  Y_{ijalmb} X_{bkan} |    |   *~~X~~| |    | a/ \b   | |    |  \ /    |
//  |~~~[Y]~~*     |  Coupled expression:
//  |    |         |  C_{ijklmn}^{J1,J2,J} =  P^{J1,J}(ij/k)P^{J2,j}_{lm/n)
//  sum_{ab} (n`anb -nan`b)
// l|   m|        n|                          * sum_{J',J3} (2J'+1)(2J3+1)
// (-1)^{2J+k-n+J1-J2}
//  |    |         |                          { J   J2  n  }
//                                          * { J1  J'  a  } *
//                                          Y_{ijalmb}^{J1,J2,J'} X_{bkan}^{J3}
//                                            { k   b   J3 }
//
//      Checked with UnitTest and passed
//
...
void comm233_phss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  size_t nch2 =
      Z.modelspace
          ->GetNumberTwoBodyChannels();  // number of regular 2b channels
  size_t nch2_CC = Z.modelspace->TwoBodyChannels_CC
                       .size();  // number of particle hole channels
  std::deque<std::vector<size_t>> ph_kets_cc(
      nch2_CC);  // list of ph 2b kets for each CC channel
  std::deque<std::map<size_t, size_t>> ph_kets_cc_lookup(
      nch2_CC);  // map i j1 to  matrix index
  std::deque<std::vector<double>> occfactors(
      nch2_CC);  // occupation factors for ab`

  auto hash_key_lmij = [](size_t l, size_t m, size_t Jlm, size_t i, size_t j,
                          size_t Jij) {
    return (l + (m << 12) + (i << 24) + (j << 36) + (Jlm << 48) + (Jij << 56));
  };
  auto unhash_key_lmij = [](size_t& l, size_t& m, size_t& Jlm, size_t& i,
                            size_t& j, size_t& Jij, size_t key) {
    l = (key & 0xFFFL);
    m = ((key >> 12) & 0xFFFL);
    i = ((key >> 24) & 0xFFFL);
    j = ((key >> 36) & 0xFFFL);
    ...
    Jij = ((key >> 56) & 0xFFL);
      ...
      //  long
  std::deque<std::unordered_map<size_t, size_t>> ph_kets3_lookup(
      nch2_CC);  // map  lm Jlm  (ij Jij)` to matrix index
  //  std::deque<std::map<std::array<int,6>,size_t>> ph_kets3_lookup(nch2_CC);
  //  // map  lm Jlm  (ij Jij)` to matrix index

  std::deque<arma::mat> Z3_ph(
      nch2_CC);  // matrices for ph transformed Z3. This will be the big one.
  ...
  for (size_t ch_cc = 0; ch_cc < nch2_CC; ch_cc++) {
    ...
    for (size_t ch_lm = 0; ch_lm < nch2; ch_lm++) {
      ...
      for (size_t ch_ij = 0; ch_ij < nch2; ch_ij++) {
        ...
        if ((std::abs(tbc_lm.J - tbc_ij.J) > tbc_CC.J) or
            (tbc_lm.J + tbc_ij.J) < tbc_CC.J)
          continue;
        ...
        for (size_t iket_lm = 0; iket_lm < nkets_lm; iket_lm++) {
          ...
          for (size_t iket_ij = 0; iket_ij < nkets_ij; iket_ij++) {
            ...
            size_t j = ket_ij.q;
            int Jij = tbc_ij.J;
            // Only compute one ordering of   lmij  vs ijlm  since we can get
            // the other ordering from symmetry
            if ((std::min(i, j) > std::min(l, m)) or
                ((std::min(i, j) == std::min(l, m)) and
                 (std::max(i, j) > std::max(l, m))) or
                ((std::min(i, j) == std::min(l, m)) and
                 (std::max(i, j) == std::max(l, m)) and (Jij > Jlm)))
              continue;
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch_cc = 0; ch_cc < nch2_CC; ch_cc++) {
    ...
    arma::mat X2_ph(2 * nkets_CC, 2 * nkets_ph,
                    arma::fill::zeros);  // we need to store both permutations
                                         // ij ji because the ph transformed
                                         // 2bmes don't have as nice a symmetry
    ...
    for (size_t ibra_ij = 0; ibra_ij < nkets_CC; ibra_ij++) {
      ...
      for (size_t iket_ab = 0; iket_ab < nkets_ph; iket_ab++) {
        ...
        double yjiab = 0;
        int Jprime_min =
            std::max(std::abs(oi.j2 - ob.j2), std::abs(oa.j2 - oj.j2)) / 2;
        ...
        for (int Jprime = Jprime_min; Jprime <= Jprime_max; Jprime++) {
          ...
          xijab -=
              (2 * Jprime + 1) * sixj_ij * X2.GetTBME_J(Jprime, i, b, a, j);
          yijab -=
              (2 * Jprime + 1) * sixj_ij * Y2.GetTBME_J(Jprime, i, b, a, j);
        }
        Jprime_min =
            std::max(std::abs(oj.j2 - ob.j2), std::abs(oa.j2 - oi.j2)) / 2;
        Jprime_max = std::min(oj.j2 + ob.j2, oa.j2 + oi.j2) / 2;
        for (int Jprime = Jprime_min; Jprime <= Jprime_max; Jprime++) {
          double sixj_ji = Z.modelspace->GetSixJ(jj, ji, Jph, ja, jb, Jprime);
          xjiab -=
              (2 * Jprime + 1) * sixj_ji * X2.GetTBME_J(Jprime, j, b, a, i);
          yjiab -=
              (2 * Jprime + 1) * sixj_ji * Y2.GetTBME_J(Jprime, j, b, a, i);
        }
        int phase_X =
            hX * Z.modelspace->phase((oi.j2 + oj.j2 + oa.j2 + ob.j2) / 2);
        int phase_Y =
            hY * Z.modelspace->phase((oi.j2 + oj.j2 + oa.j2 + ob.j2) / 2);
        ...
        X2_ph(ibra_ij + nkets_CC, iket_ab + nkets_ph) =
            phase_X * xijab * (-occ_factor);
        Y2_ph(ibra_ij + nkets_CC, iket_ab + nkets_ph) =
            phase_Y * yijab * (-occ_factor);
    ...
    for (auto iter_lmij : ph_kets3) {
      ...
      for (size_t iket_ab = 0; iket_ab < 2 * nkets_ph; iket_ab++) {
        ...
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_a *
             (1 - occnat_a)) < Z.modelspace->GetOccNat3Cut())
          continue;
        if ((occnat_l * (1 - occnat_l) * occnat_m * (1 - occnat_m) * occnat_b *
             (1 - occnat_b)) < Z.modelspace->GetOccNat3Cut())
          continue;
        ...
        int twoJp_min =
            std::max(std::abs(oa.j2 - 2 * Jij), std::abs(ob.j2 - 2 * Jlm));
        ...
        for (int twoJp = twoJp_min; twoJp <= twoJp_max; twoJp += 2) {
          ...
          double xablmij = 0;
          double yablmij = 0;
          // get the X3 and Y3 elements in one go (to avoid doing the recoupling
          // twice)
          if (x3_allocated and y3_allocated) {
            auto xandy =
                Y3.GetME_pn_TwoOps(Jij, Jlm, twoJp, i, j, a, l, m, b, X3, Y3);
            xablmij = xandy[0];
            yablmij = xandy[1];
          ...
          double sixjy =
              Z.modelspace->GetSixJ(Jij, Jlm, Jph, jb, ja, 0.5 * twoJp);
        ...
        for (int perm_ijk = 0; perm_ijk < 3; perm_ijk++) {
          ...
          for (int J1p = J1p_min[perm_ijk]; J1p <= J1p_max[perm_ijk]; J1p++) {
            ...
            for (int perm_lmn = 0; perm_lmn < 3; perm_lmn++) {
              ...
              //              double d_e4 = std::abs( 2*o4.n + o4.l -
              //              e_fermi[o4.tz2]); double d_e5 = std::abs( 2*o5.n +
              //              o5.l - e_fermi[o5.tz2]);
              double j6 = 0.5 * o6.j2;
              for (int J2p = J2p_min[perm_lmn]; J2p <= J2p_max[perm_lmn];
                   J2p++) {
                ...
                double z_123456 = 0;
                int Jph_min = std::max(std::abs(2 * J1p - 2 * J2p),
                                       std::abs(o3.j2 - o6.j2)) /
                              2;
                ...
                for (int Jph = Jph_min; Jph <= Jph_max; Jph++) {
                  size_t ch_ph = Z.modelspace->GetTwoBodyChannelIndex(
                      Jph, parity_ph, Tz_ph);
                  TwoBodyChannel_CC& tbc_CC =
                      Z.modelspace->GetTwoBodyChannel_CC(ch_ph);
                  //                   size_t nkets_CC = tbc_CC.GetNumberKets();

                  // we only compute one ordering of the right side of Zbar, but
                  // we can get the other ordering by symmetry so we check here
                  // which ordering we need, and pick up the corresponding phase
                  // factor if needed.
                  size_t index_36 = tbc_CC.GetLocalIndex(I3, I6);
                  size_t key_4512 = hash_key_lmij(
                      std::min(I4, I5), std::max(I4, I5), (size_t)J2p,
                      std::min(I1, I2), std::max(I1, I2), (size_t)J1p);
                  int phase_45 =
                      (I4 > I5)
                          ? -Z.modelspace->phase((o4.j2 + o5.j2) / 2 - J2p)
                          : 1;
                  int phase_12 =
                      (I1 > I2)
                          ? -Z.modelspace->phase((o1.j2 + o2.j2) / 2 - J1p)
                          : 1;
                  int flip_phase = 1;

                  // if any of these are true, we're looking for the not-stored
                  // ordering
                  if ((std::min(I1, I2) > std::min(I4, I5)) or
                      ((std::min(I1, I2) == std::min(I4, I5)) and
                       (std::max(I1, I2) > std::max(I4, I5))) or
                      ((std::min(I1, I2) == std::min(I4, I5)) and
                       (std::max(I1, I2) == std::max(I4, I5)) and
                       (J1p > J2p))) {
                    index_36 = tbc_CC.GetLocalIndex(I6, I3);
                    key_4512 = hash_key_lmij(std::min(I1, I2), std::max(I1, I2),
                                             (size_t)J1p, std::min(I4, I5),
                                             std::max(I4, I5), (size_t)J2p);
                    flip_phase = Z.modelspace->phase((o3.j2 - o6.j2) / 2);
                  }
                  size_t index_4512 = ph_kets3_lookup[ch_ph].at(key_4512);

                  // < 36` Jph | Z | 45 J2p  (12 J1p)` ; Jph >
                  double z_364512 = flip_phase * phase_45 * phase_12 *
                                    Z3_ph[ch_ph](index_36, index_4512);

                  // Swapped the ordering here to match with what's done in
                  // PreCalculateSixJ()
                  //                   double sixj_ph =
                  //                   Z.modelspace->GetSixJ(J1p,j3,Jtot,
                  //                   j6,J2p,Jph);
                  double sixj_ph =
                      Z.modelspace->GetSixJ(J1p, J2p, Jph, j6, j3, Jtot);
                  int phase_ph = -Z.modelspace->phase((twoJ + o3.j2) / 2);
                  z_123456 -= phase_ph * (2 * Jph + 1) * sixj_ph * z_364512;
                }  // for Jph

                // include the recoupling coefficients needed for the
                // permutations
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      for (size_t iket = ibra; iket < nkets3; iket++) {
        ...
        for (auto& a : Z.modelspace->all_orbits) {
          ...
          double d_ea = std::abs(2 * oa.n + oa.l - e_fermi[oa.tz2]);
          bool keep_ija =
              ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) *
                occnat_a * (1 - occnat_a)) >= Z.modelspace->GetOccNat3Cut());
          bool keep_kja =
              ((occnat_k * (1 - occnat_k) * occnat_j * (1 - occnat_j) *
                occnat_a * (1 - occnat_a)) >= Z.modelspace->GetOccNat3Cut());
          bool keep_ika =
              ((occnat_i * (1 - occnat_i) * occnat_k * (1 - occnat_k) *
                occnat_a * (1 - occnat_a)) >= Z.modelspace->GetOccNat3Cut());
          ...
          for (auto& b : Z.modelspace->all_orbits) {
            ...
            bool keep_lmb =
                ((occnat_l * (1 - occnat_l) * occnat_m * (1 - occnat_m) *
                  occnat_b * (1 - occnat_b)) >= Z.modelspace->GetOccNat3Cut());
            bool keep_lnb =
                ((occnat_l * (1 - occnat_l) * occnat_n * (1 - occnat_n) *
                  occnat_b * (1 - occnat_b)) >= Z.modelspace->GetOccNat3Cut());
            bool keep_nmb =
                ((occnat_n * (1 - occnat_n) * occnat_m * (1 - occnat_m) *
                  occnat_b * (1 - occnat_b)) >= Z.modelspace->GetOccNat3Cut());
            keep_lmb =
                keep_lmb and (d_el + d_em + d_eb <= Z.modelspace->dE3max);
            keep_lnb =
                keep_lnb and (d_el + d_en + d_eb <= Z.modelspace->dE3max);
            keep_nmb =
                keep_nmb and (d_en + d_em + d_eb <= Z.modelspace->dE3max);
            if (not(keep_lmb or keep_lnb or keep_nmb)) continue;

            // Direct term i.e. Z1
            if ((ob.l + ok.l + oa.l + on.l) % 2 == 0 and
                ((ob.tz2 + ok.tz2) == (oa.tz2 + on.tz2)) and keep_ija and
                keep_lmb) {
              int Jx_min =
                  std::max(std::abs(ob.j2 - ok.j2), std::abs(oa.j2 - on.j2)) /
                  2;
              int Jx_max = std::min(ob.j2 + ok.j2, oa.j2 + on.j2) / 2;
              int twoJJ_min =
                  std::max(std::abs(oa.j2 - 2 * J1), std::abs(ob.j2 - 2 * J2));
              ...
              for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                ...
                for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                  ...
                  double ninej = Z.modelspace->GetNineJ(jb, JJtot, J2, jk, J1,
                                                        Jtot, Jx, ja, jn);
                  z_ijklmn -= occ_factor * hats * phase * ninej *
                              (xbkan * yijalmb - ybkan * xijalmb);
                }  // for Jx
              }    // for twoJ
            }      // Z1 block

            // Z2  ~ Pik Z1     Xbian Ykjalmb
            if ((ob.l + oi.l + oa.l + on.l) % 2 == 0 and
                ((ob.tz2 + oi.tz2) == (oa.tz2 + on.tz2)) and keep_kja and
                keep_lmb) {
              int J1p_min = std::abs(ok.j2 - oj.j2) / 2;
              int J1p_max = (ok.j2 + oj.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - oi.j2), std::abs(oa.j2 - on.j2)) /
                  2;
              ...
              for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                ...
                int twoJJ_min = std::max(std::abs(ob.j2 - 2 * J2),
                                         std::abs(oa.j2 - 2 * J1p));
                int twoJJ_max = std::min(ob.j2 + 2 * J2, oa.j2 + 2 * J1p);
                int phase = Z.modelspace->phase((oi.j2 + on.j2) / 2 + J1p + J2);
                for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                  double xkjalmb =
                      X3.GetME_pn(J1p, J2, twoJJ, k, j, a, l, m, b);
                  double ykjalmb =
                      Y3.GetME_pn(J1p, J2, twoJJ, k, j, a, l, m, b);
                  double JJtot = 0.5 * twoJJ;
                  for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                    ...
                    double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                  sqrt((2 * J1 + 1) * (2 * J1p + 1));
                    double ninej = Z.modelspace->GetNineJ(
                        jb, JJtot, J2, ji, J1p, Jtot, Jx, ja, jn);
                    z_ijklmn += occ_factor * hats * phase * sixj * ninej *
                                (xbian * ykjalmb - ybian * xkjalmb);
                  }  // for Jx
                }    // for twoJ
              }      // for J1p
            }        // Z2 block
            ...
            if ((ob.l + oj.l + oa.l + on.l) % 2 == 0 and
                ((ob.tz2 + oj.tz2) == (oa.tz2 + on.tz2)) and keep_ika and
                keep_lmb) {
              int J1p_min = std::abs(oi.j2 - ok.j2) / 2;
              int J1p_max = (oi.j2 + ok.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - oj.j2), std::abs(oa.j2 - on.j2)) /
                  2;
              ...
              for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                ...
                for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                  double xikalmb =
                      X3.GetME_pn(J1p, J2, twoJJ, i, k, a, l, m, b);
                  double yikalmb =
                      Y3.GetME_pn(J1p, J2, twoJJ, i, k, a, l, m, b);
                  double JJtot = 0.5 * twoJJ;
                  for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                    ...
                    double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                  sqrt((2 * J1 + 1) * (2 * J1p + 1));
                    double ninej = Z.modelspace->GetNineJ(
                        jb, JJtot, J2, jj, J1p, Jtot, Jx, ja, jn);
                    z_ijklmn += occ_factor * hats * phase * sixj * ninej *
                                (xbjan * yikalmb - ybjan * xikalmb);
                  }  // for Jx
                }    // for twoJ
              }      // for J1p
            }        // Z3 block
            ...
            if ((ob.l + ok.l + oa.l + ol.l) % 2 == 0 and
                ((ob.tz2 + ok.tz2) == (oa.tz2 + ol.tz2)) and keep_ija and
                keep_nmb) {
              int J2p_min = std::abs(on.j2 - om.j2) / 2;
              int J2p_max = (on.j2 + om.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - ok.j2), std::abs(oa.j2 - ol.j2)) /
                  2;
              ...
              for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                ...
                if (std::abs(sixj) < 1e-6) continue;
                int twoJJ_min = std::max(std::abs(ob.j2 - 2 * J2p),
                                         std::abs(oa.j2 - 2 * J1));
                int twoJJ_max = std::min(ob.j2 + 2 * J2p, oa.j2 + 2 * J1);
                int phase = Z.modelspace->phase((ok.j2 + ol.j2) / 2 + J1 + J2p);
                for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                  double xijanmb =
                      X3.GetME_pn(J1, J2p, twoJJ, i, j, a, n, m, b);
                  double yijanmb =
                      Y3.GetME_pn(J1, J2p, twoJJ, i, j, a, n, m, b);
                  double JJtot = 0.5 * twoJJ;
                  for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                    ...
                    double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                  sqrt((2 * J2 + 1) * (2 * J2p + 1));
                    double ninej = Z.modelspace->GetNineJ(jb, JJtot, J2p, jk,
                                                          J1, Jtot, Jx, ja, jl);
                    z_ijklmn += occ_factor * hats * phase * sixj * ninej *
                                (xbkal * yijanmb - ybkal * xijanmb);
                  }  // for Jx
                }    // for twoJ
              }      // for J1p
            }        // Z4 block
            ...
            if ((ob.l + ok.l + oa.l + om.l) % 2 == 0 and
                ((ob.tz2 + ok.tz2) == (oa.tz2 + om.tz2)) and keep_ija and
                keep_lnb) {
              int J2p_min = std::abs(ol.j2 - on.j2) / 2;
              int J2p_max = (ol.j2 + on.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - ok.j2), std::abs(oa.j2 - om.j2)) /
                  2;
              ...
              for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                ...
                for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                  double xijalnb =
                      X3.GetME_pn(J1, J2p, twoJJ, i, j, a, l, n, b);
                  double yijalnb =
                      Y3.GetME_pn(J1, J2p, twoJJ, i, j, a, l, n, b);
                  double JJtot = 0.5 * twoJJ;
                  for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
            ...
            if ((ob.l + oi.l + oa.l + ol.l) % 2 == 0 and
                ((ob.tz2 + oi.tz2) == (oa.tz2 + ol.tz2)) and keep_kja and
                keep_nmb) {
              int J1p_min = std::abs(ok.j2 - oj.j2) / 2;
              int J1p_max = (ok.j2 + oj.j2) / 2;
              int J2p_min = std::abs(on.j2 - om.j2) / 2;
              int J2p_max = (on.j2 + om.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - oi.j2), std::abs(oa.j2 - ol.j2)) /
                  2;
              ...
              for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                ...
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  int phase =
                      -Z.modelspace->phase((oi.j2 + ol.j2) / 2 + J1p + J2p);
                  for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                    double xkjanmb =
                        X3.GetME_pn(J1p, J2p, twoJJ, k, j, a, n, m, b);
                    double ykjanmb =
                        Y3.GetME_pn(J1p, J2p, twoJJ, k, j, a, n, m, b);
                    double JJtot = 0.5 * twoJJ;
                    for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                      ...
                      double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                    sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                         (2 * J2 + 1) * (2 * J2p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jb, JJtot, J2p, ji, J1p, Jtot, Jx, ja, jl);
                      z_ijklmn -= occ_factor * hats * phase * sixj1 * sixj2 *
                                  ninej * (xbial * ykjanmb - ybial * xkjanmb);
                    }  // for Jx
                  }    // for twoJ
                }      // for J2p
              }        // for J1p
            ...
            if ((ob.l + oj.l + oa.l + ol.l) % 2 == 0 and
                ((ob.tz2 + oj.tz2) == (oa.tz2 + ol.tz2)) and keep_ika and
                keep_nmb) {
              int J1p_min = std::abs(oi.j2 - ok.j2) / 2;
              int J1p_max = (oi.j2 + ok.j2) / 2;
              int J2p_min = std::abs(on.j2 - om.j2) / 2;
              int J2p_max = (on.j2 + om.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - oj.j2), std::abs(oa.j2 - ol.j2)) /
                  2;
              ...
              for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                ...
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  int twoJJ_max = std::min(ob.j2 + 2 * J2p, oa.j2 + 2 * J1p);
                  int phase =
                      -Z.modelspace->phase((ok.j2 + ol.j2) / 2 + J1 + J2p);
                  for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                    double xikanmb =
                        X3.GetME_pn(J1p, J2p, twoJJ, i, k, a, n, m, b);
                    double yikanmb =
                        Y3.GetME_pn(J1p, J2p, twoJJ, i, k, a, n, m, b);
                    double JJtot = 0.5 * twoJJ;
                    for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                      ...
                      double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                    sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                         (2 * J2 + 1) * (2 * J2p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jb, JJtot, J2p, jj, J1p, Jtot, Jx, ja, jl);
                      z_ijklmn -= occ_factor * hats * phase * sixj1 * sixj2 *
                                  ninej * (xbjal * yikanmb - ybjal * xikanmb);
                    }  // for Jx
                  }    // for twoJ
                }      // for J2p
              }        // for J1p
            ...
            if ((ob.l + oi.l + oa.l + om.l) % 2 == 0 and
                ((ob.tz2 + oi.tz2) == (oa.tz2 + om.tz2)) and keep_kja and
                keep_lnb) {
              int J1p_min = std::abs(ok.j2 - oj.j2) / 2;
              int J1p_max = (ok.j2 + oj.j2) / 2;
              int J2p_min = std::abs(ol.j2 - on.j2) / 2;
              int J2p_max = (ol.j2 + on.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - oi.j2), std::abs(oa.j2 - om.j2)) /
                  2;
              ...
              for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                ...
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  int twoJJ_max = std::min(ob.j2 + 2 * J2p, oa.j2 + 2 * J1p);
                  int phase =
                      -Z.modelspace->phase((oi.j2 + on.j2) / 2 + J1p + J2);
                  for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                    double xkjalnb =
                        X3.GetME_pn(J1p, J2p, twoJJ, k, j, a, l, n, b);
                    double ykjalnb =
                        Y3.GetME_pn(J1p, J2p, twoJJ, k, j, a, l, n, b);
                    double JJtot = 0.5 * twoJJ;
                    for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                      ...
                      double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                    sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                         (2 * J2 + 1) * (2 * J2p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jb, JJtot, J2p, ji, J1p, Jtot, Jx, ja, jm);
                      z_ijklmn -= occ_factor * hats * phase * sixj1 * sixj2 *
                                  ninej * (xbiam * ykjalnb - ybiam * xkjalnb);
                    }  // for Jx
                  }    // for twoJ
                }      // for J2p
              }        // for J1p
            ...
            if ((ob.l + oj.l + oa.l + om.l) % 2 == 0 and
                ((ob.tz2 + oj.tz2) == (oa.tz2 + om.tz2)) and keep_ika and
                keep_lnb) {
              int J1p_min = std::abs(oi.j2 - ok.j2) / 2;
              int J1p_max = (oi.j2 + ok.j2) / 2;
              int J2p_min = std::abs(ol.j2 - on.j2) / 2;
              int J2p_max = (ol.j2 + on.j2) / 2;
              int Jx_min =
                  std::max(std::abs(ob.j2 - oj.j2), std::abs(oa.j2 - om.j2)) /
                  2;
              ...
              for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                ...
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  int twoJJ_max = std::min(ob.j2 + 2 * J2p, oa.j2 + 2 * J1p);
                  int phase =
                      -Z.modelspace->phase((ok.j2 + on.j2) / 2 + J1 + J2);
                  for (int twoJJ = twoJJ_min; twoJJ <= twoJJ_max; twoJJ += 2) {
                    double xikalnb =
                        X3.GetME_pn(J1p, J2p, twoJJ, i, k, a, l, n, b);
                    double yikalnb =
                        Y3.GetME_pn(J1p, J2p, twoJJ, i, k, a, l, n, b);
                    double JJtot = 0.5 * twoJJ;
                    for (int Jx = Jx_min; Jx <= Jx_max; Jx++) {
                      ...
                      double hats = (twoJJ + 1) * (2 * Jx + 1) *
                                    sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                         (2 * J2 + 1) * (2 * J2p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jb, JJtot, J2p, jj, J1p, Jtot, Jx, ja, jm);
                      z_ijklmn -= occ_factor * hats * phase * sixj1 * sixj2 *
                                  ninej * (xbjam * yikalnb - ybjam * xikalnb);
                    }  // for Jx
                  }    // for twoJ
                }      // for J2p
              }        // for J1p
...
//
//  |    |    |     Uncoupled expression:
// i|   j|   k|       Z_ijklmn = 1/6 sum_{abc} (nanbnc + n`an`bn`c) ( X_{ijkabc}
// Y_{abclmn} - Y_{ijkabc} X_{abclmn}
//  *~~~[X]~~~*
// a|   b|   c|
//  *~~~[Y]~~~*     Coupled expression:
// l|   m|   n|     Z_{ijklmn}^{J1,J2,J} = 1/6 sum_{abc} sum_{J'} (nanbnc +
// n`an`bn`c) ( X_{ijkabc}^{J1,J',J} Y_{abclmn}^{J',J2,J} - X<->Y )
//  |    |    |
//
//  Checked with UnitTest and passed
//
...
//
//  |i  |j      k/     Uncoupled expression:
//  |   |       /      Z_ijklmn =-1/2 P(ij/k)P(lm/n) sum_{abc} (nanbn`c +
//  na`n`bnc) ( X_{ijcabn} Y_{abklmc} - Y_{ijcabn} X_{abklmc} )
//  *~~[X]~*   /
//  |   |  |\ /        Coupled expression:
// a|  b| c| /         Z_{ijklmn}^{J1,J2,J} = +1/2 P^{J1,J}(ij/k)P^{J2,j}_{lm/n)
// sum_{abc} (nanbn`c+n`an`bnc) sum_{J',J",J3} (2J'+1)(2J"+1)
//  |   |  |/ \                                  { k   J1  J  }
//  *~~[Y]~*   \                               * { J3  J'  n  } * (
//  X_{ijcabn}^{J1,J3,J'} Y_{abklmc}^{J3,J2,J"} - X<->Y ) |   | \ { J"  c   J2 }
//  |l  |m       \ n
//
//
//  Checked with UnitTest and passed
...
void comm333_pph_hhpss(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  std::map<std::array<int, 3>, size_t>
      channels_pph;  // map twoJ_ph, parity_ph, twoTz_ph  -> channel index
  std::vector<std::array<int, 3>>
      channel_list_pph;  // vector  channel_index -> twoJ-ph, parity_ph,
                         // twoTz_ph.

  //  std::vector< std::map<std::array<int,4>, size_t>> ket_lookup_pph; // in a
  //  given channel, map  i,j,k,Jij -> matrix index
      ...
      for (int twoTz_ph = -3; twoTz_ph <= 3; twoTz_ph += 2) {
        ...
        for (size_t chij = 0; chij < nch2; chij++) {
          ...
          for (int n : Z.modelspace->all_orbits) {
            ...
            if ((std::abs(2 * Jij - on.j2) > twoJph) or
                ((2 * Jij + on.j2) < twoJph))
              continue;
            ...
            for (size_t iket_ij = 0; iket_ij < nkets_ij; iket_ij++) {
              ...
              double occnat_i = ket_ij.op->occ_nat;
              double occnat_j = ket_ij.oq->occ_nat;
              double d_ei = std::abs(2 * ket_ij.op->n + ket_ij.op->l -
                                     e_fermi[ket_ij.op->tz2]);
              double d_ej = std::abs(2 * ket_ij.oq->n + ket_ij.oq->l -
                                     e_fermi[ket_ij.oq->tz2]);
              // if i and j cant make it past the OccNat and dE3max cuts, don't
              // bother including it
              if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) *
                   occnat_factor_max) < Z.modelspace->GetOccNat3Cut())
                continue;
              ...
              ngood_ijn++;
              double occ_factor =
                  ket_ij.op->occ * ket_ij.oq->occ * (1 - on.occ) +
                  (1 - ket_ij.op->occ) * (1 - ket_ij.oq->occ) * on.occ;
  ...
  // Now that we've made our lists and allocated the matrices, we fill the
  // matrices inside the parallel block.
  // #pragma omp parallel for schedule(dynamic,1) if (not
  // Z.modelspace->scalar3b_transform_first_pass)
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch_pph = 0; ch_pph < nch_pph; ch_pph++) {
    ...
    for (auto& iter_ijn : good_kets_ijn) {
      ...
      for (size_t Iabc = 0; Iabc < ngood_abc; Iabc++) {
        ...
        if ((occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) * occnat_n *
             (1 - occnat_n)) < Z.modelspace->GetOccNat3Cut())
          continue;
        if ((occnat_i * (1 - occnat_i) * occnat_j * (1 - occnat_j) * occnat_c *
             (1 - occnat_c)) < Z.modelspace->GetOccNat3Cut())
          continue;
        ...
        double ybar = 0;
        int twoJprime_min =
            std::max(std::abs(2 * Jij - oc.j2), std::abs(2 * Jab - on.j2));
        int twoJprime_max = std::min((2 * Jij + oc.j2), (2 * Jab + on.j2));
        for (int twoJprime = twoJprime_min; twoJprime <= twoJprime_max;
             twoJprime += 2) {
          // We use this if/else construct because the precomputed SixJ symbols
          // have the largest half-integer on the bottom row.
          //         double sixj_ijn = Z.modelspace->GetSixJ( Jij, jn, Jph, Jab,
          //         jc, 0.5*twoJprime);
          double sixj_ijn = (2 * Jph < twoJprime)
                                ? Z.modelspace->GetSixJ(jn, Jph, Jij, jc,
                                                        0.5 * twoJprime, Jab)
                                : Z.modelspace->GetSixJ(jn, 0.5 * twoJprime,
                                                        Jab, jc, Jph, Jij);

          // TODO This comes up often enough that we should just make
          // ThreeBodyMEpn do this.
          std::vector<double> xandy =
              Y3.GetME_pn_TwoOps(Jij, Jab, twoJprime, i, j, c, a, b, n, X3, Y3);
  ...
  Z.profiler.timer["_comm333_pph_fill_matrices"] +=
      omp_get_wtime() - t_internal;
  t_internal = omp_get_wtime();

  // Now that we've computed Zbar (i.e. the pph transformed commutator), we need
  // to transform that back to Z, and do all the permutations to ensure
  // antisymmetry
  //  #pragma omp parallel for schedule(dynamic,1) if (not
  //  Z.modelspace->scalar3b_transform_first_pass)
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      std::vector<int> J1p_max = {J1, std::min(ok.j2 + oj.j2, twoJ + oi.j2) / 2,
                                  std::min(oi.j2 + ok.j2, twoJ + oj.j2) / 2};
      //      std::vector<int> J1p_min = {J1,  std::abs(ok.j2-oj.j2)/2,
      //      std::abs(oi.j2-ok.j2)/2 }; std::vector<int> J1p_max = {J1,
      //      (ok.j2+oj.j2)/2 , (oi.j2+ok.j2)/2 };
      std::vector<std::vector<double>> recouple_ijk = {{1}, {}, {}};
      for (int J1p = J1p_min[1]; J1p <= J1p_max[1]; J1p++)
        recouple_ijk[1].push_back(
            sqrt((2 * J1 + 1) * (2 * J1p + 1)) *
      ...
      for (size_t iket = 0; iket <= ibra; iket++) {
        ...
        std::vector<int> J2p_max = {J2,
                                    std::min(on.j2 + om.j2, twoJ + ol.j2) / 2,
                                    std::min(ol.j2 + on.j2, twoJ + om.j2) / 2};
        //        std::vector<int> J2p_min = {J2,  std::abs(on.j2-om.j2)/2,
        //        std::abs(ol.j2-on.j2)/2 }; std::vector<int> J2p_max = {J2,
        //        (on.j2+om.j2)/2 , (ol.j2+on.j2)/2 };
        std::vector<std::vector<double>> recouple_lmn = {{1}, {}, {}};
          ...
          for (int J1p = J1p_min[perm_ijk]; J1p <= J1p_max[perm_ijk]; J1p++) {
            ...
            for (int perm_lmn = 0; perm_lmn < 3; perm_lmn++) {
              ...
              for (int J2p = J2p_min[perm_lmn]; J2p <= J2p_max[perm_lmn];
                ...
                //                std::vector<double> Zbar_126453 (
                //                (twoJph_max-twoJph_min)/2+1, 0. );
                int phase_12 =
                    I1 > I2 ? -Z.modelspace->phase((o1.j2 + o2.j2) / 2 - J1p)
                            : 1;
                int phase_45 =
                ...
                for (int twoJph = twoJph_min; twoJph <= twoJph_max;
                     twoJph += 2) {
                  auto iter_ch =
                      channels_pph.find({twoJph, parity_ph, twoTz_ph});
                  ...
                  //                  size_t index_126 =
                  //                  ket_lookup_pph[ch_pph].at({
                  //                  std::min(I1,I2),std::max(I1,I2),I6,J1p} );
                  //                  size_t index_453 =
                  //                  ket_lookup_pph[ch_pph].at({
                  //                  std::min(I4,I5),std::max(I4,I5),I3,J2p} );
                  double zbar =
                      phase_12 * phase_45 * Zbar[ch_pph](index_126, index_453);

                  double Jph = 0.5 * twoJph;
                  double sixj_ph =
                      (Jph > Jtot)
                          ? Z.modelspace->GetSixJ(j3, Jtot, J1p, j6, Jph, J2p)
                          : Z.modelspace->GetSixJ(j3, Jph, J2p, j6, Jtot, J1p);
                  //                  double sixj_ph = Z.modelspace->GetSixJ(
                  //                  J1p, j3, Jtot,  J2p, j6, Jph ); double
                  //                  sixj_ph; if (Jph>Jtot)   sixj_ph =
                  //                  Z.modelspace->GetSixJ(  j3, Jtot,J1p , j6,
                  //                  Jph,J2p ); else    sixj_ph =
                  //                  Z.modelspace->GetSixJ(  j3,Jph,J2p,  j6  ,
                  //                  Jtot,J1p ); double sixj_ph =
                  //                  Z.modelspace->GetSixJ(  j3, Jtot,J1p , j6,
                  //                  Jph,J2p );
...
/*
//
// The old slow way, just to make sure we didn't screw anything up by going
faster
//
...
)
{
  ...
  #pragma omp parallel for schedule(dynamic,1) if (not
Z.modelspace->scalar3b_transform_first_pass) for (size_t ch3=0; ch3<nch3; ch3++)
  {
    auto& Tbc = Z.modelspace->GetThreeBodyChannel(ch3);
    size_t nkets3 = Tbc.GetNumberKets();
    int twoJ = Tbc.twoJ;
    ...
    for (size_t ibra=0; ibra<nkets3; ibra++)
    {
      ...
      if ( (d_ei + d_ej + d_ek) > Z.modelspace->dE3max ) continue;
      if ( (occnat_i*(1-occnat_i) * occnat_j*(1-occnat_j) *
occnat_k*(1-occnat_k) ) < Z.modelspace->GetOccNat3Cut() ) continue; int J1 =
bra.Jpq;
      ...
      std::vector<std::array<size_t,3>> ijk = { {i,j,k}, {k,j,i}, {i,k,j} };
      std::vector<int> J1p_min = {J1,  std::abs(ok.j2-oj.j2)/2,
std::abs(oi.j2-ok.j2)/2 }; std::vector<int> J1p_max = {J1,  (ok.j2+oj.j2)/2 ,
(oi.j2+ok.j2)/2 }; std::vector<std::vector<double>> recouple_ijk = {{1},{},{} };
      for (int J1p=J1p_min[1]; J1p<=J1p_max[1]; J1p++)
           recouple_ijk[1].push_back( sqrt( (2*J1+1)*(2*J1p+1)) *
Z.modelspace->GetSixJ(ji,jj,J1,jk,Jtot,J1p) );
...
//      for (size_t iket=ibra; iket<nkets3; iket++)
      for (size_t iket=0; iket<=ibra; iket++)
      {
        ...
        if ( (d_el + d_em + d_en) > Z.modelspace->dE3max ) continue;
        if ( (occnat_l*(1-occnat_l) * occnat_m*(1-occnat_m) *
occnat_n*(1-occnat_n) ) < Z.modelspace->GetOccNat3Cut() ) continue; int J2 =
ket.Jpq;


        std::vector<std::array<size_t,3>> lmn = { {l,m,n}, {n,m,l}, {l,n,m} };
        std::vector<int> J2p_min = {J2,  std::abs(on.j2-om.j2)/2,
std::abs(ol.j2-on.j2)/2 }; std::vector<int> J2p_max = {J2,  (on.j2+om.j2)/2 ,
(ol.j2+on.j2)/2 }; std::vector<std::vector<double>> recouple_lmn = {{1},{},{} };
...
recouple_lmn[perm_lmn].at(J2p-J2p_min[perm_lmn]);
                ...
                for (size_t ch2=0; ch2<nch2; ch2++)
                {
                  ...
                  if ( std::abs(Tbc.twoTz-2*tbc_ab.Tz)==5) continue; // TODO
there are probably other checks at the channel level... size_t nkets_ab =
tbc_ab.GetNumberKets(); int Jab = tbc_ab.J;
                  ...
                  for (size_t iket_ab=0; iket_ab<nkets_ab; iket_ab++)
                  {
                    ...
                    Orbit& oa = Z.modelspace->GetOrbit(a);
                    Orbit& ob = Z.modelspace->GetOrbit(b);
                    if (std::abs(oa.occ * ob.occ)<1e-6 and std::abs(
(1-oa.occ)*(1-ob.occ))<1e-6) continue;

                    double occnat_a = oa.occ_nat;
                    double occnat_b = ob.occ_nat;
                    double d_ea = std::abs( 2*oa.n + oa.l - e_fermi[oa.tz2]);
                    double d_eb = std::abs( 2*ob.n + ob.l - e_fermi[ob.tz2]);
                    bool keep_ab3 = (occnat_a*(1-occnat_a) *
occnat_b*(1-occnat_b) * occnat_3*(1-occnat_3) ) >=
Z.modelspace->GetOccNat3Cut(); bool keep_ab6 = (occnat_a*(1-occnat_a) *
occnat_b*(1-occnat_b) * occnat_6*(1-occnat_6) ) >=
Z.modelspace->GetOccNat3Cut(); keep_ab3 = keep_ab3 and (d_ea+d_eb+d_e3 <=
Z.modelspace->dE3max ); keep_ab6 = keep_ab6 and (d_ea+d_eb+d_e6 <=
Z.modelspace->dE3max );
                    ...
                    {
                      Orbit& oc = Z.modelspace->GetOrbit(c);
                      double occ_factor = oa.occ * ob.occ * (1-oc.occ) +
(1-oa.occ)*(1-ob.occ)*oc.occ; if (std::abs(occ_factor)<1e-6) continue; if (a==b)
occ_factor *=0.5; // because we only sum b<a double occnat_c = oc.occ_nat;
                      double d_ec = std::abs( 2*oc.n + oc.l - e_fermi[oc.tz2]);
                      double jc = 0.5 * oc.j2;
                      bool keep_12c = (occnat_1*(1-occnat_1) *
occnat_2*(1-occnat_2) * occnat_c*(1-occnat_c) ) >=
Z.modelspace->GetOccNat3Cut(); bool keep_45c = (occnat_4*(1-occnat_4) *
occnat_5*(1-occnat_5) * occnat_c*(1-occnat_c) ) >=
Z.modelspace->GetOccNat3Cut(); keep_12c = keep_12c and (d_e1+d_e2+d_ec <=
Z.modelspace->dE3max ); keep_45c = keep_45c and (d_e4+d_e5+d_ec <=
Z.modelspace->dE3max );


                      if ( ((oa.l+ob.l+o3.l+o4.l+o5.l+oc.l)%2==0) and
((o1.l+o2.l+oc.l+oa.l+ob.l+o6.l)%2==0) and  (
(oa.tz2+ob.tz2+o3.tz2)==(o4.tz2+o5.tz2+oc.tz2) ) and (
(o1.tz2+o2.tz2+oc.tz2)==(oa.tz2+ob.tz2+o6.tz2)) and keep_ab3 and keep_45c and
keep_12c and keep_ab6)
//                        )//and keep_ab3 and keep_45c and keep_12c and
keep_ab6)
                      {

                        int twoJx_min = std::max( std::abs(2*Jab - o3.j2),
std::abs(2*J2p - oc.j2) ); int twoJx_max = std::min( 2*Jab+o3.j2 , 2*J2p + oc.j2
                        ...
                        {
                          std::vector<double> xab345c(
(twoJx_max-twoJx_min)/2+1, 0); std::vector<double> yab345c(
(twoJx_max-twoJx_min)/2+1, 0); std::vector<double> x12cab6(
(twoJy_max-twoJy_min)/2+1, 0); std::vector<double> y12cab6(
(twoJy_max-twoJy_min)/2+1, 0); for (int twoJx=twoJx_min; twoJx<=twoJx_max;
twoJx+=2)
                          {
                            size_t iJx = (twoJx-twoJx_min)/2;
                            xab345c[iJx] = X3.GetME_pn(Jab,J2p,twoJx,
a,b,I3,I4,I5,c); yab345c[iJx] = Y3.GetME_pn(Jab,J2p,twoJx, a,b,I3,I4,I5,c);
                          }
                          for (int twoJy=twoJy_min; twoJy<=twoJy_max; twoJy+=2)
                          {
                             size_t iJy = (twoJy-twoJy_min)/2;
                             x12cab6[iJy] = X3.GetME_pn(J1p,Jab,twoJy,
I1,I2,c,a,b,I6); y12cab6[iJy] = Y3.GetME_pn(J1p,Jab,twoJy, I1,I2,c,a,b,I6);
                          }
                          for (int twoJx=twoJx_min; twoJx<=twoJx_max; twoJx+=2)
                          {
                            double JJx = 0.5 * twoJx;
                            size_t iJx = (twoJx-twoJx_min)/2;
                            for (int twoJy=twoJy_min; twoJy<=twoJy_max;
twoJy+=2)
                            {
                               ...
                               double ninej = Z.modelspace->GetNineJ(
j3,Jab,JJx, J1p,JJy,jc, Jtot,j6,J2p); z_ijklmn +=  rec_ijk * rec_lmn *
occ_factor * hats * ninej * ( xab345c[iJx]*y12cab6[iJy] -
yab345c[iJx]*x12cab6[iJy] );
                            }// for twoJy
                          }// for twoJx
                        }
                      }// Z1 block
...
// void comm333_pph_hhpss( const Operator& X, const Operator& Y, Operator& Z )
void comm333_pph_hhpss_debug(const Operator& X, const Operator& Y,
                             Operator& Z) {
  double tstart = omp_get_wtime();
  ...
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets3; ibra++) {
      ...
      for (size_t iket = 0; iket <= ibra; iket++) {
        ...
        //              if ( not ((i==2 and j==4 and k==5 and l==4 and m==4 and
        //              n==5) if ( not ((i==4 and j==4 and k==5 and l==2 and
        //              m==4 and n==5)
        //               or (i==5 and j==4 and k==2 and l==5 and m==4 and n==4))
        //               ) continue;
        double z_ijklmn = 0;
        ...
        for (size_t ch2 = 0; ch2 < nch2; ch2++) {
          ...
          if (std::abs(Tbc.twoTz - 2 * tbc_ab.Tz) == 5)
            continue;  // TODO there are probably other checks at the channel
                       // level...
          size_t nkets_ab = tbc_ab.GetNumberKets();
          int Jab = tbc_ab.J;
          ...
          for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++) {
            ...
            Orbit& oa = Z.modelspace->GetOrbit(a);
            Orbit& ob = Z.modelspace->GetOrbit(b);
            if (std::abs(oa.occ * ob.occ) < 1e-6 and
                std::abs((1 - oa.occ) * (1 - ob.occ)) < 1e-6)
              continue;
            ...
            bool keep_abm =
                (occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                 occnat_m * (1 - occnat_m)) >= Z.modelspace->GetOccNat3Cut();
            bool keep_abn =
                (occnat_a * (1 - occnat_a) * occnat_b * (1 - occnat_b) *
                 occnat_n * (1 - occnat_n)) >= Z.modelspace->GetOccNat3Cut();
            keep_abi =
            ...
            keep_abl =
                keep_abl and (d_ea + d_eb + d_el <= Z.modelspace->dE3max);
            keep_abm =
                keep_abm and (d_ea + d_eb + d_em <= Z.modelspace->dE3max);
            keep_abn =
                keep_abn and (d_ea + d_eb + d_en <= Z.modelspace->dE3max);
            ...
            for (auto c : Z.modelspace->all_orbits) {
              ...
              bool keep_lnc =
                  (occnat_l * (1 - occnat_l) * occnat_n * (1 - occnat_n) *
                   occnat_c * (1 - occnat_c)) >= Z.modelspace->GetOccNat3Cut();
              keep_ijc =
                  keep_ijc and (d_ei + d_ej + d_ec <= Z.modelspace->dE3max);
              keep_kjc =
                  keep_kjc and (d_ek + d_ej + d_ec <= Z.modelspace->dE3max);
              keep_ikc =
                  keep_ikc and (d_ei + d_ek + d_ec <= Z.modelspace->dE3max);
              keep_lmc =
                  keep_lmc and (d_el + d_em + d_ec <= Z.modelspace->dE3max);
              keep_nmc =
                  keep_nmc and (d_en + d_em + d_ec <= Z.modelspace->dE3max);
              keep_lnc =
                  keep_lnc and (d_el + d_en + d_ec <= Z.modelspace->dE3max);

              // Direct Z1 term
              if (((oa.l + ob.l + ok.l + ol.l + om.l + oc.l) % 2 == 0) and
                  ((oi.l + oj.l + oc.l + oa.l + ob.l + on.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + ok.tz2) == (ol.tz2 + om.tz2 + oc.tz2)) and
                  ((oi.tz2 + oj.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + on.tz2)) and
                  keep_abk and keep_lmc and keep_ijc and keep_abn) {
                int twoJx_min = std::max(std::abs(2 * Jab - ok.j2),
                                         std::abs(2 * J2 - oc.j2));
                int twoJx_max = std::min(2 * Jab + ok.j2, 2 * J2 + oc.j2);
                int twoJy_min = std::max(std::abs(2 * J1 - oc.j2),
                                         std::abs(2 * Jab - on.j2));
                int twoJy_max = std::min(2 * J1 + oc.j2, 2 * Jab + on.j2);
                if (twoJx_min <= twoJx_max and twoJy_min <= twoJy_max) {
                  std::vector<double> xabklmc((twoJx_max - twoJx_min) / 2 + 1,
                                              0);
                  std::vector<double> yabklmc((twoJx_max - twoJx_min) / 2 + 1,
                                              0);
                  std::vector<double> xijcabn((twoJy_max - twoJy_min) / 2 + 1,
                                              0);
                  std::vector<double> yijcabn((twoJy_max - twoJy_min) / 2 + 1,
                                              0);
                  for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                    //                  double JJx = 0.5 * twoJx;
                    size_t iJx = (twoJx - twoJx_min) / 2;
                    xabklmc[iJx] =
                        X3.GetME_pn(Jab, J2, twoJx, a, b, k, l, m, c);
                    yabklmc[iJx] =
                        Y3.GetME_pn(Jab, J2, twoJx, a, b, k, l, m, c);
                    //                  double xabklmc =
                    //                  X3.GetME_pn(Jab,J2,twoJx, a,b,k,l,m,c);
                    //                  double yabklmc =
                    //                  Y3.GetME_pn(Jab,J2,twoJx, a,b,k,l,m,c);
                  ...
                  for (int twoJy = twoJy_min; twoJy <= twoJy_max; twoJy += 2) {
                    size_t iJy = (twoJy - twoJy_min) / 2;
                    xijcabn[iJy] =
                        X3.GetME_pn(J1, Jab, twoJy, i, j, c, a, b, n);
                    yijcabn[iJy] =
                        Y3.GetME_pn(J1, Jab, twoJy, i, j, c, a, b, n);
                  }
                  for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                    double JJx = 0.5 * twoJx;
                    size_t iJx = (twoJx - twoJx_min) / 2;
                    for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                      ...
                      double hats = (twoJx + 1) * (twoJy + 1);
                      double ninej = Z.modelspace->GetNineJ(
                          jk, Jab, JJx, J1, JJy, jc, Jtot, jn, J2);
                      //                     double xijcabn =
                      //                     X3.GetME_pn(J1,Jab,twoJy,
                      //                     i,j,c,a,b,n); double yijcabn =
                      //                     Y3.GetME_pn(J1,Jab,twoJy,
                      //                     i,j,c,a,b,n); z_ijklmn +=
                      //                     occ_factor * hats * ninej * (
                      //                     xabklmc*yijcabn - yabklmc*xijcabn
                      //                     );
                      z_ijklmn += occ_factor * hats * ninej *
                                  (xabklmc[iJx] * yijcabn[iJy] -
                                   yabklmc[iJx] * xijcabn[iJy]);
                    }  // for twoJy
                  }    // for twoJx
                }
              }  // Z1 block
                 //              std::cout << "  after Z1  " << z_ijklmn <<
                 //              std::endl;

              // Z2 term  Pik    Xabilmc Ykjcabn
              if (((oa.l + ob.l + oi.l + ol.l + om.l + oc.l) % 2 == 0) and
                  ((ok.l + oj.l + oc.l + oa.l + ob.l + on.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + oi.tz2) == (ol.tz2 + om.tz2 + oc.tz2)) and
                  ((ok.tz2 + oj.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + on.tz2)) and
                  keep_abi and keep_lmc and keep_kjc and keep_abn) {
                int twoJx_min = std::max(std::abs(2 * Jab - oi.j2),
                                         std::abs(2 * J2 - oc.j2));
                ...
                for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                  double JJx = 0.5 * twoJx;
                  double xabilmc =
                      X3.GetME_pn(Jab, J2, twoJx, a, b, i, l, m, c);
                  double yabilmc =
                      Y3.GetME_pn(Jab, J2, twoJx, a, b, i, l, m, c);

                  for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                    int twoJy_min = std::max(std::abs(2 * J1p - oc.j2),
                                             std::abs(2 * Jab - on.j2));
                    ...
                    for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                      ...
                      double hats = (twoJx + 1) * (twoJy + 1) *
                                    sqrt((2 * J1 + 1) * (2 * J1p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          ji, Jab, JJx, J1p, JJy, jc, Jtot, jn, J2);
                      double xkjcabn =
                          X3.GetME_pn(J1p, Jab, twoJy, k, j, c, a, b, n);
                      double ykjcabn =
                          Y3.GetME_pn(J1p, Jab, twoJy, k, j, c, a, b, n);
                      z_ijklmn -= occ_factor * phase * hats * sixj * ninej *
                                  (xabilmc * ykjcabn - yabilmc * xkjcabn);
                    }  // for twoJy
                  }    // for J1p
                }      // for twoJx
              }        // Z2 block
              //              std::cout << "  after Z2  " << z_ijklmn <<
              //              std::endl;

              // Z3 term  Pjk    Xabjlmc Yikcabn
              if (((oa.l + ob.l + oj.l + ol.l + om.l + oc.l) % 2 == 0) and
                  ((oi.l + ok.l + oc.l + oa.l + ob.l + on.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + oj.tz2) == (ol.tz2 + om.tz2 + oc.tz2)) and
                  ((oi.tz2 + ok.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + on.tz2)) and
                  keep_abj and keep_lmc and keep_ikc and keep_abn) {
                int twoJx_min = std::max(std::abs(2 * Jab - oj.j2),
                                         std::abs(2 * J2 - oc.j2));
                ...
                for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                  double JJx = 0.5 * twoJx;
                  double xabjlmc =
                      X3.GetME_pn(Jab, J2, twoJx, a, b, j, l, m, c);
                  double yabjlmc =
                      Y3.GetME_pn(Jab, J2, twoJx, a, b, j, l, m, c);
                  ...
                  for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                    ...
                    for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                      ...
                      double hats = (twoJx + 1) * (twoJy + 1) *
                                    sqrt((2 * J1 + 1) * (2 * J1p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jj, Jab, JJx, J1p, JJy, jc, Jtot, jn, J2);
                      double xikcabn =
                          X3.GetME_pn(J1p, Jab, twoJy, i, k, c, a, b, n);
                      double yikcabn =
                          Y3.GetME_pn(J1p, Jab, twoJy, i, k, c, a, b, n);
                      z_ijklmn -= occ_factor * phase * hats * sixj * ninej *
                                  (xabjlmc * yikcabn - yabjlmc * xikcabn);
                    }  // for twoJy
                  }    // for J1p
                }      // for twoJx
              }        // Z3 block
              //              std::cout << "  after Z3  " << z_ijklmn <<
              //              std::endl;

              // Z4 term  Pln    Xabknmc Yijcabl
              if (((oa.l + ob.l + ok.l + on.l + om.l + oc.l) % 2 == 0) and
                  ((oi.l + oj.l + oc.l + oa.l + ob.l + ol.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + ok.tz2) == (on.tz2 + om.tz2 + oc.tz2)) and
                  ((oi.tz2 + oj.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + ol.tz2)) and
                  keep_abk and keep_nmc and keep_ijc and keep_abl) {
                int twoJy_min = std::max(std::abs(2 * J1 - oc.j2),
                                         std::abs(2 * Jab - ol.j2));
                ...
                for (int twoJy = twoJy_min; twoJy <= twoJy_max; twoJy += 2) {
                  double xijcabl =
                      X3.GetME_pn(J1, Jab, twoJy, i, j, c, a, b, l);
                  double yijcabl =
                      Y3.GetME_pn(J1, Jab, twoJy, i, j, c, a, b, l);
                  double JJy = 0.5 * twoJy;
                  for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                    double sixj =
                        Z.modelspace->GetSixJ(jl, jm, J2, jn, Jtot, J2p);
                    ...
                    for (int twoJx = twoJx_min; twoJx <= twoJx_max;
                         twoJx += 2) {
                      double JJx = 0.5 * twoJx;
                      double xabknmc =
                          X3.GetME_pn(Jab, J2p, twoJx, a, b, k, n, m, c);
                      double yabknmc =
                          Y3.GetME_pn(Jab, J2p, twoJx, a, b, k, n, m, c);
                      double hats = (twoJx + 1) * (twoJy + 1) *
                                    sqrt((2 * J2 + 1) * (2 * J2p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jk, Jab, JJx, J1, JJy, jc, Jtot, jl, J2p);
                      z_ijklmn -= occ_factor * phase * hats * sixj * ninej *
                                  (xabknmc * yijcabl - yabknmc * xijcabl);
                    }  // for twoJx
                  }    // for J2p
                }      // for twoJy
              }        // Z4 block
              //              std::cout << "  after Z4  " << z_ijklmn <<
              //              std::endl;

              // Z5 term  Pmn    Xabklnc Yijcabm
              if (((oa.l + ob.l + ok.l + ol.l + on.l + oc.l) % 2 == 0) and
                  ((oi.l + oj.l + oc.l + oa.l + ob.l + om.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + ok.tz2) == (ol.tz2 + on.tz2 + oc.tz2)) and
                  ((oi.tz2 + oj.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + om.tz2)) and
                  keep_abk and keep_lnc and keep_ijc and keep_abm) {
                int twoJy_min = std::max(std::abs(2 * J1 - oc.j2),
                                         std::abs(2 * Jab - om.j2));
                ...
                for (int twoJy = twoJy_min; twoJy <= twoJy_max; twoJy += 2) {
                  double xijcabm =
                      X3.GetME_pn(J1, Jab, twoJy, i, j, c, a, b, m);
                  double yijcabm =
                      Y3.GetME_pn(J1, Jab, twoJy, i, j, c, a, b, m);
                  double JJy = 0.5 * twoJy;
                  for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                    double sixj =
                        Z.modelspace->GetSixJ(jm, jl, J2, jn, Jtot, J2p);
                    ...
                    for (int twoJx = twoJx_min; twoJx <= twoJx_max;
                      ...
                      double JJx = 0.5 * twoJx;
                      double xabklnc =
                          X3.GetME_pn(Jab, J2p, twoJx, a, b, k, l, n, c);
                      double yabklnc =
                          Y3.GetME_pn(Jab, J2p, twoJx, a, b, k, l, n, c);
                      double hats = (twoJx + 1) * (twoJy + 1) *
                                    sqrt((2 * J2 + 1) * (2 * J2p + 1));
                      double ninej = Z.modelspace->GetNineJ(
                          jk, Jab, JJx, J1, JJy, jc, Jtot, jm, J2p);
                      z_ijklmn -= occ_factor * phase * hats * sixj * ninej *
                                  (xabklnc * yijcabm - yabklnc * xijcabm);
                    }  // for twoJx
                  }    // for J2p
                }      // for twoJy
              }        // Z5 block
              //              std::cout << "  after Z5  " << z_ijklmn <<
              //              std::endl;

              // Z6 term  Pik Pln    Xabinmc Ykjcabl
              if (((oa.l + ob.l + oi.l + on.l + om.l + oc.l) % 2 == 0) and
                  ((ok.l + oj.l + oc.l + oa.l + ob.l + ol.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + oi.tz2) == (on.tz2 + om.tz2 + oc.tz2)) and
                  ((ok.tz2 + oj.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + ol.tz2)) and
                  keep_abi and keep_nmc and keep_kjc and keep_abl) {
                int J1p_min = std::abs(ok.j2 - oj.j2) / 2;
                int J1p_max = (ok.j2 + oj.j2) / 2;
                int J2p_min = std::abs(on.j2 - om.j2) / 2;
                int J2p_max = (on.j2 + om.j2) / 2;
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                    double JJx = 0.5 * twoJx;
                    double xabinmc =
                        X3.GetME_pn(Jab, J2p, twoJx, a, b, i, n, m, c);
                    double yabinmc =
                        Y3.GetME_pn(Jab, J2p, twoJx, a, b, i, n, m, c);

                    for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                      int twoJy_min = std::max(std::abs(2 * J1p - oc.j2),
                                               std::abs(2 * Jab - ol.j2));
                      int twoJy_max =
                          std::min(2 * J1p + oc.j2, 2 * Jab + ol.j2);
                      double sixj1 =
                          Z.modelspace->GetSixJ(ji, jj, J1, jk, Jtot, J1p);
                      if (std::abs(sixj1) < 1e-6) continue;
                      int phase = 1;
                      for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                        ...
                        double hats = (twoJx + 1) * (twoJy + 1) *
                                      sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                           (2 * J2 + 1) * (2 * J2p + 1));
                        double ninej = Z.modelspace->GetNineJ(
                            ji, Jab, JJx, J1p, JJy, jc, Jtot, jl, J2p);
                        double xkjcabl =
                            X3.GetME_pn(J1p, Jab, twoJy, k, j, c, a, b, l);
                        double ykjcabl =
                            Y3.GetME_pn(J1p, Jab, twoJy, k, j, c, a, b, l);
                        z_ijklmn += occ_factor * phase * hats * sixj1 * sixj2 *
                                    ninej *
                                    (xabinmc * ykjcabl - yabinmc * xkjcabl);
                      }  // for twoJy
                    }    // for J1p
                  }      // for twoJx
                }        // for J2p
              }          // Z6 block
              //              std::cout << "  after Z6  " << z_ijklmn <<
              //              std::endl;

              // Z7 term  Pjk Pln    Xabjnmc Yikcabl
              if (((oa.l + ob.l + oj.l + on.l + om.l + oc.l) % 2 == 0) and
                  ((oi.l + ok.l + oc.l + oa.l + ob.l + ol.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + oj.tz2) == (on.tz2 + om.tz2 + oc.tz2)) and
                  ((oi.tz2 + ok.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + ol.tz2)) and
                  keep_abj and keep_nmc and keep_ikc and keep_abl) {
                int J1p_min = std::abs(oi.j2 - ok.j2) / 2;
                int J1p_max = (oi.j2 + ok.j2) / 2;
                int J2p_min = std::abs(on.j2 - om.j2) / 2;
                int J2p_max = (on.j2 + om.j2) / 2;
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                    double JJx = 0.5 * twoJx;
                    double xabjnmc =
                        X3.GetME_pn(Jab, J2p, twoJx, a, b, j, n, m, c);
                    double yabjnmc =
                        Y3.GetME_pn(Jab, J2p, twoJx, a, b, j, n, m, c);
                      ...
                      for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                        ...
                        double hats = (twoJx + 1) * (twoJy + 1) *
                                      sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                           (2 * J2 + 1) * (2 * J2p + 1));
                        double ninej = Z.modelspace->GetNineJ(
                            jj, Jab, JJx, J1p, JJy, jc, Jtot, jl, J2p);
                        double xikcabl =
                            X3.GetME_pn(J1p, Jab, twoJy, i, k, c, a, b, l);
                        double yikcabl =
                            Y3.GetME_pn(J1p, Jab, twoJy, i, k, c, a, b, l);
                        z_ijklmn += occ_factor * phase * hats * sixj1 * sixj2 *
                                    ninej *
                                    (xabjnmc * yikcabl - yabjnmc * xikcabl);
                      }  // for twoJy
                    }    // for J1p
                  }      // for twoJx
                }        // for J2p
              }          // Z7 block
              //              std::cout << "  after Z7  " << z_ijklmn <<
              //              std::endl;

              // Z8 term  Pik Pmn    Xabilnc Ykjcabm
              if (((oa.l + ob.l + oi.l + ol.l + on.l + oc.l) % 2 == 0) and
                  ((ok.l + oj.l + oc.l + oa.l + ob.l + om.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + oi.tz2) == (ol.tz2 + on.tz2 + oc.tz2)) and
                  ((ok.tz2 + oj.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + om.tz2)) and
                  keep_abi and keep_lnc and keep_kjc and keep_abm) {
                int J1p_min = std::abs(ok.j2 - oj.j2) / 2;
                int J1p_max = (ok.j2 + oj.j2) / 2;
                int J2p_min = std::abs(ol.j2 - on.j2) / 2;
                int J2p_max = (ol.j2 + on.j2) / 2;
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                    double JJx = 0.5 * twoJx;
                    double xabilnc =
                        X3.GetME_pn(Jab, J2p, twoJx, a, b, i, l, n, c);
                    double yabilnc =
                        Y3.GetME_pn(Jab, J2p, twoJx, a, b, i, l, n, c);

                    for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                      int twoJy_min = std::max(std::abs(2 * J1p - oc.j2),
                                               std::abs(2 * Jab - om.j2));
                      int twoJy_max =
                          std::min(2 * J1p + oc.j2, 2 * Jab + om.j2);
                      double sixj1 =
                          Z.modelspace->GetSixJ(ji, jj, J1, jk, Jtot, J1p);
                      if (std::abs(sixj1) < 1e-6) continue;
                      int phase =
                      ...
                      for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                        ...
                        double hats = (twoJx + 1) * (twoJy + 1) *
                                      sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                           (2 * J2 + 1) * (2 * J2p + 1));
                        double ninej = Z.modelspace->GetNineJ(
                            ji, Jab, JJx, J1p, JJy, jc, Jtot, jm, J2p);
                        double xkjcabm =
                            X3.GetME_pn(J1p, Jab, twoJy, k, j, c, a, b, m);
                        double ykjcabm =
                            Y3.GetME_pn(J1p, Jab, twoJy, k, j, c, a, b, m);
                        z_ijklmn += occ_factor * phase * hats * sixj1 * sixj2 *
                                    ninej *
                                    (xabilnc * ykjcabm - yabilnc * xkjcabm);
                      }  // for twoJy
                    }    // for J1p
                  }      // for twoJx
                }        // for J2p
              }          // Z8 block
              //              std::cout << "  after Z8  " << z_ijklmn <<
              //              std::endl;

              // Z9 term  Pjk Pmn    Xabjlnc Yikcabm
              if (((oa.l + ob.l + oj.l + ol.l + on.l + oc.l) % 2 == 0) and
                  ((oi.l + ok.l + oc.l + oa.l + ob.l + om.l) % 2 == 0) and
                  ((oa.tz2 + ob.tz2 + oj.tz2) == (ol.tz2 + on.tz2 + oc.tz2)) and
                  ((oi.tz2 + ok.tz2 + oc.tz2) == (oa.tz2 + ob.tz2 + om.tz2)) and
                  keep_abj and keep_lnc and keep_ikc and keep_abm) {
                int J1p_min = std::abs(oi.j2 - ok.j2) / 2;
                int J1p_max = (oi.j2 + ok.j2) / 2;
                int J2p_min = std::abs(ol.j2 - on.j2) / 2;
                int J2p_max = (ol.j2 + on.j2) / 2;
                for (int J2p = J2p_min; J2p <= J2p_max; J2p++) {
                  ...
                  for (int twoJx = twoJx_min; twoJx <= twoJx_max; twoJx += 2) {
                    double JJx = 0.5 * twoJx;
                    double xabjlnc =
                        X3.GetME_pn(Jab, J2p, twoJx, a, b, j, l, n, c);
                    double yabjlnc =
                        Y3.GetME_pn(Jab, J2p, twoJx, a, b, j, l, n, c);
                    ...
                    for (int J1p = J1p_min; J1p <= J1p_max; J1p++) {
                      ...
                      double sixj1 =
                          Z.modelspace->GetSixJ(jj, ji, J1, jk, Jtot, J1p);
                      if (std::abs(sixj1) < 1e-6) continue;
                      int phase = Z.modelspace->phase(
                          (oj.j2 + ok.j2 + om.j2 + on.j2) / 2 + J1 + J1p + J2 +
                          J2p);
                      for (int twoJy = twoJy_min; twoJy <= twoJy_max;
                           twoJy += 2) {
                        double JJy = 0.5 * twoJy;
                        double hats = (twoJx + 1) * (twoJy + 1) *
                                      sqrt((2 * J1 + 1) * (2 * J1p + 1) *
                                           (2 * J2 + 1) * (2 * J2p + 1));
                        double ninej = Z.modelspace->GetNineJ(
                            jj, Jab, JJx, J1p, JJy, jc, Jtot, jm, J2p);
                        double xikcabm =
                            X3.GetME_pn(J1p, Jab, twoJy, i, k, c, a, b, m);
                        double yikcabm =
                            Y3.GetME_pn(J1p, Jab, twoJy, i, k, c, a, b, m);
                        z_ijklmn += occ_factor * phase * hats * sixj1 * sixj2 *
                                    ninej *
                                    (xabjlnc * yikcabm - yabjlnc * xikcabm);
                      }  // for twoJy
                    }    // for J1p
                  }      // for twoJx
                }        // for J2p
              }          // Z9 block
              //              std::cout << "  after Z9  " << z_ijklmn <<
              //              std::endl;
        ...
        //        std::cout << "  " << i << " " << j << " " << k << " " << l <<
        //        " " << m << " " << n << "  J1 J2 two J " << J1 << " " << J2 <<
        //        " " << twoJ << "   Z = " << z_ijklmn << std::endl;
        Z3.AddToME_pn_ch(ch3, ch3, ibra, iket, z_ijklmn);
      }  // for iket
    }    // for ibra
  }      // for ch3
...
#pragma omp parallel for schedule( \
    dynamic,                       \
    1) if (not Z.modelspace->tensor_transform_first_pass.at(Z.rank_J))
  for (int indexi = 0; indexi < norbits; ++indexi)
  //   for (int i=0;i<norbits;++i)
  {
    //      auto i = Z.modelspace->all_orbits[indexi];
    ...
    for (auto j : Z.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
      ...
      for (auto a : Z.modelspace->holes)  // C++11 syntax
      {
        ...
        for (auto b : X.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
          ...
          for (int J1 = J1min; J1 <= J1max; ++J1) {
            ...
            for (int J2 = J2min; J2 <= J2max; ++J2) {
              ...
              double prefactor =
                  nanb * phasefactor * sqrt((2 * J1 + 1) * (2 * J2 + 1)) *
                  Z.modelspace->GetSixJ(J1, J2, Lambda, jj, ji, ja);
              Zij +=
                  prefactor *
                  (X.OneBody(a, b) * Y.TwoBody.GetTBME_J(J1, J2, b, i, a, j) -
                   X.OneBody(b, a) * Y.TwoBody.GetTBME_J(J1, J2, a, i, b, j));
            }
          }
        }
        ...
        for (auto& b : Y.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
          ...
          double zij = 0;
          for (int J1 = J1min; J1 <= J1max; ++J1) {
            zij -= Z.modelspace->phase(ji + jb + J1) * (2 * J1 + 1) *
                   Z.modelspace->GetSixJ(ja, jb, Lambda, ji, jj, J1) *
                   X.TwoBody.GetTBME_J(J1, J1, b, i, a, j);
          }
          ...
          for (int J1 = J1min; J1 <= J1max; ++J1) {
            zij += Z.modelspace->phase(ji + jb + J1) * (2 * J1 + 1) *
                   Z.modelspace->GetSixJ(jb, ja, Lambda, ji, jj, J1) *
                   X.TwoBody.GetTBME_J(J1, J1, a, i, b, j);
          }
  ...
  for (int ii = 0; ii < nmat; ++ii) {
    ...
    for (int ibra = 0; ibra < nbras; ++ibra) {
      ...
      for (int iket = 0; iket < nkets; ++iket) {
        ...
        if (i == j) {
          c2 = c1;  // there should be a phase here, but if the ket exists, it'd
                    // better be +1.
        } else {
          for (int a : X.OneBodyChannels.at({oj.l, oj.j2, oj.tz2})) {
            c2 +=
                X.OneBody(j, a) * Y.TwoBody.GetTBME(ch_bra, ch_ket, i, a, k, l);
          }
        }
        ...
        if (k == l) {
          ...
          for (int a : X.OneBodyChannels.at({ol.l, ol.j2, ol.tz2})) {
            c4 +=
                X.OneBody(a, l) * Y.TwoBody.GetTBME(ch_bra, ch_ket, i, j, k, a);
          }
        }
        ...
        for (int a : Y.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
          ...
          c1 -= Z.modelspace->GetSixJ(J2, J1, Lambda, ji, ja, jj) *
                Y.OneBody(i, a) * X.TwoBody.GetTBME(ch_ket, ch_ket, a, j, k, l);
        }
        if (i == j) {
          c2 = -c1;
        } else {
          for (int a : Y.OneBodyChannels.at({oj.l, oj.j2, oj.tz2})) {
            double ja = Z.modelspace->GetOrbit(a).j2 * 0.5;
            c2 += Z.modelspace->GetSixJ(J2, J1, Lambda, jj, ja, ji) *
                  Y.OneBody(j, a) *
                  X.TwoBody.GetTBME(ch_ket, ch_ket, a, i, k, l);
          }
        }
        for (int a : Y.OneBodyChannels.at({ok.l, ok.j2, ok.tz2})) {
          double ja = Z.modelspace->GetOrbit(a).j2 * 0.5;
          c3 -= Z.modelspace->GetSixJ(J1, J2, Lambda, jk, ja, jl) *
                Y.OneBody(a, k) * X.TwoBody.GetTBME(ch_bra, ch_bra, i, j, l, a);
        }
        if (k == l) {
          c4 = -c3;
        } else {
          for (int a : Y.OneBodyChannels.at({ol.l, ol.j2, ol.tz2})) {
            double ja = Z.modelspace->GetOrbit(a).j2 * 0.5;
            c4 += Z.modelspace->GetSixJ(J1, J2, Lambda, jl, ja, jk) *
                  Y.OneBody(a, l) *
                  X.TwoBody.GetTBME(ch_bra, ch_bra, i, j, k, a);
          }
        }
        cijkl +=
            hatfactor * (phase1 * c1 + phase2 * c2 + phase3 * c3 + phase4 * c4);
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t i = 0; i < nchan; ++i) {
    ...
    // the complicated-looking construct after the % signs just multiply the
    // matrix elements by the proper occupation numbers (nanb, etc.)

    arma::mat MLeft = join_horiz(LHS1.cols(bras_hh), -RHS.cols(kets_hh));
    arma::mat MRight = join_vert(
        RHS.rows(bras_hh) %
            tbc_bra.Ket_occ_hh.cols(arma::uvec(RHS.n_cols, arma::fill::zeros)),
        LHS2.rows(kets_hh) % tbc_ket.Ket_occ_hh.cols(
                                 arma::uvec(LHS2.n_cols, arma::fill::zeros)));

    Matrixhh = MLeft * MRight;

    MLeft =
        join_horiz(LHS1.cols(join_vert(bras_pp, join_vert(bras_hh, bras_ph))),
                   -RHS.cols(join_vert(kets_pp, join_vert(kets_hh, kets_ph))));
    MRight = join_vert(
        join_vert(
            RHS.rows(bras_pp),
            join_vert(RHS.rows(bras_hh) % tbc_bra.Ket_unocc_hh.cols(arma::uvec(
                                              RHS.n_cols, arma::fill::zeros)),
                      RHS.rows(bras_ph) % tbc_bra.Ket_unocc_ph.cols(arma::uvec(
                                              RHS.n_cols, arma::fill::zeros)))),
        join_vert(
            LHS2.rows(kets_pp),
            join_vert(
                LHS2.rows(kets_hh) % tbc_ket.Ket_unocc_hh.cols(arma::uvec(
                                         LHS2.n_cols, arma::fill::zeros)),
                LHS2.rows(kets_ph) % tbc_ket.Ket_unocc_ph.cols(arma::uvec(
                                         LHS2.n_cols, arma::fill::zeros)))));
      ...
      for (auto& c : Z.modelspace->holes) {
        ...
        for (int J1 = j1min; J1 <= j1max; ++J1) {
          ...
          for (int J2 = j2min; J2 <= j2max; ++J2) {
            ...
            cijJ += hatfactor * sixj *
                    Z.modelspace->phase(jj + jc + J1 + Lambda) *
                    (oc.occ * Mpp.GetTBME_J(J1, J2, c, i, c, j) +
                     (1 - oc.occ) * Mhh.GetTBME_J(J1, J2, c, i, c, j));
          }
        }
...
// void Operator::DoTensorPandyaTransformation(vector<arma::mat>& TwoBody_CC_hp,
// vector<arma::mat>& TwoBody_CC_ph) void
// Operator::DoTensorPandyaTransformation(map<array<int,2>,arma::mat>&
// TwoBody_CC_hp, map<array<int,2>,arma::mat>& TwoBody_CC_ph) const
void DoTensorPandyaTransformation(
    const Operator& Z,
    std::map<std::array<index_t, 2>, arma::mat>& TwoBody_CC_ph) {
  int Lambda = Z.rank_J;
  // loop over cross-coupled channels
  ...
  for (index_t ch_bra_cc : Z.modelspace->SortedTwoBodyChannels_CC) {
    ...
    arma::uvec bras_ph = arma::join_cols(tbc_bra_cc.GetKetIndex_hh(),
                                         tbc_bra_cc.GetKetIndex_ph());
    index_t nph_bras = bras_ph.n_rows;
    for (index_t ch_ket_cc : Z.modelspace->SortedTwoBodyChannels_CC) {
      TwoBodyChannel& tbc_ket_cc =
          Z.modelspace->GetTwoBodyChannel_CC(ch_ket_cc);
      index_t nKets_cc = tbc_ket_cc.GetNumberKets();

      TwoBody_CC_ph[{ch_bra_cc, ch_ket_cc}] =
          arma::mat(2 * nph_bras, nKets_cc, arma::fill::zeros);
    }
  }

#pragma omp parallel for schedule( \
    dynamic, 1) if (not Z.modelspace->tensor_transform_first_pass.at(Lambda))
  for (index_t ich = 0; ich < nch; ++ich) {
    ...
    for (index_t ch_ket_cc : Z.modelspace->SortedTwoBodyChannels_CC) {
      ...
      if ((Jbra_cc + Jket_cc < Z.GetJRank()) or
          std::abs(Jbra_cc - Jket_cc) > Z.GetJRank())
        continue;
      ...
      for (index_t ibra = 0; ibra < nph_bras; ++ibra) {
        ...
        for (index_t iket_cc = 0; iket_cc < iket_max; ++iket_cc) {
          ...
          for (int J1 = j1min; J1 <= j1max; ++J1) {
            ...
            for (int J2 = j2min; J2 <= j2max; ++J2) {
              //                  double ninej =
              //                  Z.modelspace->GetNineJ(ja,jd,J1,jb,jc,J2,Jbra_cc,Jket_cc,Lambda);
              double ninej = 0;
              if (Lambda == 0) {
                ninej = AngMom::phase(jb + jd + J1 + Jbra_cc) *
                        Z.modelspace->GetSixJ(ja, jb, Jbra_cc, jc, jd, J1) /
                        sqrt((2 * J2 + 1) * (2 * Jbra_cc + 1));
              } else {
                ninej = Z.modelspace->GetNineJ(ja, jd, J1, jb, jc, J2, Jbra_cc,
                                               Jket_cc, Lambda);
              }

              if (std::abs(ninej) < 1e-8) continue;
              double hatfactor = sqrt((2 * J1 + 1) * (2 * J2 + 1) *
                                      (2 * Jbra_cc + 1) * (2 * Jket_cc + 1));
              double tbme = Z.TwoBody.GetTBME_J(J1, J2, a, d, c, b);
              sm -= hatfactor * Z.modelspace->phase(jb + jd + Jket_cc + J2) *
                    ninej * tbme;
            }
          }
          ...
          for (int J1 = j1min; J1 <= j1max; ++J1) {
            ...
            for (int J2 = j2min; J2 <= j2max; ++J2) {
              //                  double ninej =
              //                  Z.modelspace->GetNineJ(jb,jd,J1,ja,jc,J2,Jbra_cc,Jket_cc,Lambda);
              double ninej = 0;
              if (Lambda == 0) {
                ninej = AngMom::phase(ja + jd + J1 + Jbra_cc) *
                        Z.modelspace->GetSixJ(jb, ja, Jbra_cc, jc, jd, J1) /
                        sqrt((2 * J2 + 1) * (2 * Jbra_cc + 1));
              } else {
                ninej = Z.modelspace->GetNineJ(jb, jd, J1, ja, jc, J2, Jbra_cc,
                                               Jket_cc, Lambda);
              }
              ...
              sm -= hatfactor * Z.modelspace->phase(ja + jd + Jket_cc + J2) *
                    ninej * tbme;
            }
          }
...
// This happens inside an OMP loop, and so everything here needs to be thread
// safe
//
// void Operator::DoTensorPandyaTransformation_SingleChannel( arma::mat&
// TwoBody_CC_ph, int ch_bra_cc, int ch_ket_cc) const void
// Operator::DoTensorPandyaTransformation_SingleChannel( arma::mat& MatCC_ph,
// int ch_bra_cc, int ch_ket_cc) const
void DoTensorPandyaTransformation_SingleChannel(const Operator& Z,
                                                arma::mat& MatCC_ph,
                                                int ch_bra_cc, int ch_ket_cc) {
  int Lambda = Z.rank_J;

  TwoBodyChannel& tbc_bra_cc = Z.modelspace->GetTwoBodyChannel_CC(ch_bra_cc);
  arma::uvec bras_ph =
      arma::join_cols(tbc_bra_cc.GetKetIndex_hh(), tbc_bra_cc.GetKetIndex_ph());
  ...
  // The Pandya-transformed (formerly cross-coupled) particle-hole type matrix
  // elements (this is the output of this method)
...
//// Take Pandya-transformed matrix Zbar for a single channel, invert the Pandya
///transformation and add the result to the current operator. /void
///Operator::AddInverseTensorPandyaTransformation_SingleChannel(arma::mat& Zbar,
///int ch_bra_cc, int ch_ket_cc) /void
///AddInverseTensorPandyaTransformation_SingleChannel(Operator& Z, arma::mat&
///Zbar, int ch_bra_cc, int ch_ket_cc)
// void AddInverseTensorPandyaTransformation_SingleChannel(Operator& Z,
// arma::mat& Zbar, size_t ch_bra_cc, size_t ch_ket_cc)
//{
//     // Do the inverse Pandya transform
//    if (ch_bra_cc > ch_ket_cc)  // hopefully this won't happen
//    {
//       std::cout << "WARNING: Called
//       Operator::AddInverseTensorPandyaTransformation_SingleChannel with
//       ch_bra_cc > ch_ket_cc : " << ch_bra_cc << " > " << ch_ket_cc <<
//       std::endl; std::cout << " Skipping this channel." << std::endl; return;
//    }
////   int n_channels_kept = 0;
//   const auto& pandya_lookup = Z.modelspace->GetPandyaLookup(Z.GetJRank(),
//   Z.GetTRank(),Z.GetParity())[{ch_bra_cc,ch_ket_cc}]; int Lambda = Z.rank_J;
//   int hZ = Z.IsHermitian() ? 1 : -1;
//   TwoBodyChannel_CC& tbc_bra_cc =
//   Z.modelspace->GetTwoBodyChannel_CC(ch_bra_cc); TwoBodyChannel_CC&
//   tbc_ket_cc = Z.modelspace->GetTwoBodyChannel_CC(ch_ket_cc); int J3 =
//   tbc_bra_cc.J; int J4 = tbc_ket_cc.J; int Tz_bra_cc = tbc_bra_cc.Tz; int
//   Tz_ket_cc = tbc_ket_cc.Tz; int parity_bra_cc = tbc_bra_cc.parity; int
//   parity_ket_cc = tbc_ket_cc.parity; int nkets_cc =
//   tbc_ket_cc.GetNumberKets();
//
...
//            int j4max = int(jk+jj);
//
//            if (   (oi.l+ol.l)%2==parity_bra_cc             and
//            (ok.l+oj.l)%2==parity_ket_cc
////                      and std::abs(oi.tz2+ol.tz2)==2*Tz_bra_cc   and
///std::abs(ok.tz2+oj.tz2)==2*Tz_ket_cc
//                      and std::abs(oi.tz2-ol.tz2)==2*Tz_bra_cc   and
//                      std::abs(ok.tz2-oj.tz2)==2*Tz_ket_cc and j3min<=J3 and
//                      J3<=j3max           and j4min<=J4 and J4<=j4max )
//            {
//               int indx_il = tbc_bra_cc.GetLocalIndex( std::min(i,l),
//               std::max(i,l) ); int indx_kj = tbc_ket_cc.GetLocalIndex(
//               std::min(j,k), std::max(j,k) );
//
////               double ninej =
///Z.modelspace->GetNineJ(ji,jl,J3,jj,jk,J4,J1,J2,Lambda);
//               double ninej = 0;
//               if ( Lambda==0 )
//               {
//                 ninej = AngMom::phase( jj + jl + J1 + J3 ) *
//                 Z.modelspace->GetSixJ( ji, jj, J1, jk, jl, J3 ) /
//                 sqrt((2*J2+1)*(2*J4+1) );
//               }
//               else
//               {
...
//               else      tbme = Zbar( indx_il ,indx_kj+(k>j?0:nkets_cc) ) * hZ
//               * Z.modelspace->phase(J3-J4 + ji+jj+jk+jl); commij += hatfactor
//               * Z.modelspace->phase(jj+jl+J2+J4) * ninej * tbme ;
////               inner_loop = true;
//            }
//
//            if (  (ch_bra_cc != ch_ket_cc)  and ((oi.l+ol.l)%2==parity_ket_cc)
//            and ((ok.l+oj.l)%2==parity_bra_cc)
////                                            and
///(std::abs(oi.tz2+ol.tz2)==2*Tz_ket_cc)   and
///(std::abs(ok.tz2+oj.tz2)==2*Tz_bra_cc)
//                                            and
//                                            (std::abs(oi.tz2-ol.tz2)==2*Tz_ket_cc)
//                                            and
//                                            (std::abs(ok.tz2-oj.tz2)==2*Tz_bra_cc)
//                                            and (j3min<=J4) and (J4<=j3max)
//                                            and  (j4min<=J3) and (J3<=j4max )
//                                            )
//              {
//                 int indx_kj = tbc_bra_cc.GetLocalIndex( std::min(j,k),
//                 std::max(j,k)); int indx_il = tbc_ket_cc.GetLocalIndex(
//                 std::min(i,l), std::max(i,l));
//
////                 double ninej =
///Z.modelspace->GetNineJ(ji,jl,J4,jj,jk,J3,J1,J2,Lambda);
//                 double ninej = 0;
//                 if ( Lambda==0 )
//                 {
//                   ninej = AngMom::phase( jj + jl + J1 + J4 ) *
//                   Z.modelspace->GetSixJ( ji, jj, J1, jk, jl, J4 ) /
//                   sqrt((2*J2+1)*(2*J4+1) );
//                 }
//                 else
//                 {
...
//                 double tbme = 0;
//
//                 if(k<=j) tbme = Zbar(indx_kj, indx_il+(i>l?nkets_cc:0) ) * hZ
//                 * Z.modelspace->phase(J3-J4); // Z_ilkj = Z_kjil * (phase)
//                 else     tbme = Zbar(indx_kj, indx_il+(i>l?0:nkets_cc) ) *
//                 Z.modelspace->phase( ji+jj+jk+jl) ; // Z_ilkj = Z_kjil *
//                 (phase)
//
//                 commij += hatfactor * Z.modelspace->phase(jj+jl+J2+J3) *
//                 ninej * tbme ;
////               inner_loop = true;
//              }
//
...
//              j4max = int(jk+ji);
//              if (   ((oj.l+ol.l)%2==parity_bra_cc) and
//              ((ok.l+oi.l)%2==parity_ket_cc)
////                      and (std::abs(oj.tz2+ol.tz2)==2*Tz_bra_cc) and
///(std::abs(ok.tz2+oi.tz2)==2*Tz_ket_cc)
//                      and (std::abs(oj.tz2-ol.tz2)==2*Tz_bra_cc) and
//                      (std::abs(ok.tz2-oi.tz2)==2*Tz_ket_cc) and (J3>=j3min)
//                      and (J3<=j3max) and (J4>=j4min) and (J4<=j4max)   )
//              {
//                // Transform Z_jlki
//                int indx_jl = tbc_bra_cc.GetLocalIndex( std::min(j,l),
//                std::max(j,l) ); int indx_ki = tbc_ket_cc.GetLocalIndex(
//                std::min(k,i), std::max(k,i) );
//
////                double ninej =
///Z.modelspace->GetNineJ(jj,jl,J3,ji,jk,J4,J1,J2,Lambda);
//                double ninej = 0;
//                if ( Lambda==0 )
//                {
//                  ninej = AngMom::phase( ji + jl + J1 + J3 ) *
//                  Z.modelspace->GetSixJ( jj, ji, J1, jk, jl, J3 ) /
//                  sqrt((2*J2+1)*(2*J4+1) );
//                }
//                else
//                {
...
//                else      tbme = Zbar( indx_jl ,indx_ki+(k>i?0:nkets_cc) ) *
//                hZ * Z.modelspace->phase(J3-J4 + ji+jj+jk+jl); commji +=
//                hatfactor * Z.modelspace->phase(ji+jl+J2+J4) * ninej * tbme ;
////               inner_loop = true;
//              }
//              if ( (ch_bra_cc!=ch_ket_cc) and ((oj.l+ol.l)%2==parity_ket_cc)
//              and ((ok.l+oi.l)%2==parity_bra_cc)
////                                       and
///(std::abs(oj.tz2+ol.tz2)==2*Tz_ket_cc) and
///(std::abs(ok.tz2+oi.tz2)==2*Tz_bra_cc)
//                                       and
//                                       (std::abs(oj.tz2-ol.tz2)==2*Tz_ket_cc)
//                                       and
//                                       (std::abs(ok.tz2-oi.tz2)==2*Tz_bra_cc)
//                                       and (J4>=j3min) and (J4<=j3max) and
//                                       (J3>=j4min) and (J3<=j4max)   )
//              {
//                int indx_jl = tbc_ket_cc.GetLocalIndex( std::min(j,l),
//                std::max(j,l)); int indx_ki = tbc_bra_cc.GetLocalIndex(
//                std::min(k,i), std::max(k,i));
////                double ninej =
///Z.modelspace->GetNineJ(jj,jl,J4,ji,jk,J3,J1,J2,Lambda);
//                double ninej = 0;
//                if ( Lambda==0 )
//                {
//                  ninej = AngMom::phase( ji + jl + J1 + J4 ) *
//                  Z.modelspace->GetSixJ( jj, ji, J1, jk, jl, J4 ) /
//                  sqrt((2*J2+1)*(2*J4+1) );
//                }
//                else
//                {
...
//                double tbme = 0;
//
//                if(k<=i) tbme = Zbar(indx_ki, indx_jl+(j>l?nkets_cc:0) ) * hZ
//                * Z.modelspace->phase(J3-J4); // Z_ilkj = Z_kjil * (phase)
//                else     tbme = Zbar(indx_ki, indx_jl+(j>l?0:nkets_cc) ) *
//                Z.modelspace->phase( ji+jj+jk+jl) ; // Z_ilkj = Z_kjil *
//                (phase) commji += hatfactor * Z.modelspace->phase(ji+jl+J2+J3)
//                * ninej * tbme ;
////               inner_loop = true;
//              }
//            }
...
//
//            double norm = bra.delta_pq()==ket.delta_pq() ? 1+bra.delta_pq() :
//            PhysConst::SQRT2; #pragma omp atomic Zijkl(ibra,iket) +=  (commij
//            - Z.modelspace->phase(ji+jj-J1)*commji) / norm; if
//            (ch_bra==ch_ket)
//            {
...
// void Operator::AddInverseTensorPandyaTransformation( const
// std::map<std::array<index_t,2>,arma::mat>&  Zbar )
void AddInverseTensorPandyaTransformation(
    Operator& Z, const std::map<std::array<index_t, 2>, arma::mat>& Zbar) {
  // Do the inverse Pandya transform
  int Lambda = Z.rank_J;
  //   std::vector<std::map<std::array<int,2>,arma::mat>::iterator>
  //   iteratorlist;
  std::vector<std::map<std::array<size_t, 2>, arma::mat>::iterator>
      iteratorlist;
  //   for (std::map<std::array<int,2>,arma::mat>::iterator iter=
  //   Z.TwoBody.MatEl.begin(); iter!= Z.TwoBody.MatEl.end(); ++iter)
  //   iteratorlist.push_back(iter);
  for (auto iter = Z.TwoBody.MatEl.begin(); iter != Z.TwoBody.MatEl.end();
       ++iter)
    iteratorlist.push_back(iter);
  int niter = iteratorlist.size();
  int hZ = Z.IsHermitian() ? 1 : -1;

  // Only go parallel if we've previously calculated the SixJs/NineJs.
    ...
    for (index_t ibra = 0; ibra < nBras; ++ibra) {
      ...
      for (index_t iket = ketmin; iket < nKets; ++iket) {
        ...
        for (int J3 = j3min; J3 <= j3max; ++J3) {
          index_t ch_bra_cc = Z.modelspace->GetTwoBodyChannelIndex(
              J3, parity_bra_cc, Tz_bra_cc);
          const TwoBodyChannel_CC& tbc_bra_cc =
              Z.modelspace->GetTwoBodyChannel_CC(ch_bra_cc);
          index_t nbras = tbc_bra_cc.GetNumberKets();
          index_t indx_il =
              tbc_bra_cc.GetLocalIndex(std::min(i, l), std::max(i, l));
          int j4min = std::max(std::abs(int(jk - jj)), std::abs(J3 - Lambda));
          int j4max = std::min(int(jk + jj), J3 + Lambda);
          for (int J4 = j4min; J4 <= j4max; ++J4) {
            index_t ch_ket_cc = Z.modelspace->GetTwoBodyChannelIndex(
                J4, parity_ket_cc, Tz_ket_cc);
            const TwoBodyChannel_CC& tbc_ket_cc =
                Z.modelspace->GetTwoBodyChannel_CC(ch_ket_cc);
            index_t nkets = tbc_ket_cc.GetNumberKets();
            index_t indx_kj =
                tbc_ket_cc.GetLocalIndex(std::min(j, k), std::max(j, k));

            //                  double ninej =
            //                  Z.modelspace->GetNineJ(ji,jl,J3,jj,jk,J4,J1,J2,Lambda);
            double ninej = 0;
            double hatfactor =
                sqrt((2 * J1 + 1) * (2 * J2 + 1) * (2 * J3 + 1) * (2 * J4 + 1));
            double tbme = 0;
            ...
            if (Lambda == 0) {
              ...
              ninej = Z.modelspace->GetNineJ(ji, jl, J3, jj, jk, J4, J1, J2,
                                             Lambda);
            }
            if (std::abs(ninej) < 1e-8) continue;
            ...
            if (ch_bra_cc <= ch_ket_cc) {
              if (i <= l)
                ...
                tbme = Zmat(indx_il, indx_kj + (k > j ? 0 : nkets)) * hZ *
                       Z.modelspace->phase(J3 - J4 + ji + jj + jk + jl);
            } else {
              if (k <= j)
                tbme =
                    Zmat(indx_kj, indx_il + (i > l ? nbras : 0)) * hZ *
                    Z.modelspace->phase(J3 - J4);  // Z_ilkj = Z_kjil * (phase)
              else
                tbme = Zmat(indx_kj, indx_il + (i > l ? 0 : nbras)) *
                       Z.modelspace->phase(ji + jj + jk +
                                           jl);  // Z_ilkj = Z_kjil * (phase)
            }

            /*
             */
            commij += hatfactor * Z.modelspace->phase(jj + jl + J2 + J4) *
                      ninej * tbme;
          }
        }
        ...
        if (i == j) {
          ...
          for (int J3 = j3min; J3 <= j3max; ++J3) {
            int ch_bra_cc = Z.modelspace->GetTwoBodyChannelIndex(
                J3, parity_bra_cc, Tz_bra_cc);
            const TwoBodyChannel_CC& tbc_bra_cc =
                Z.modelspace->GetTwoBodyChannel_CC(ch_bra_cc);
            int nbras = tbc_bra_cc.GetNumberKets();
            int indx_jl =
                tbc_bra_cc.GetLocalIndex(std::min(j, l), std::max(j, l));
            int j4min = std::max(std::abs(int(jk - ji)), std::abs(J3 - Lambda));
            int j4max = std::min(int(jk + ji), J3 + Lambda);
            for (int J4 = j4min; J4 <= j4max; ++J4) {
              int ch_ket_cc = Z.modelspace->GetTwoBodyChannelIndex(
                  J4, parity_ket_cc, Tz_ket_cc);
              const TwoBodyChannel_CC& tbc_ket_cc =
                  Z.modelspace->GetTwoBodyChannel_CC(ch_ket_cc);
              int nkets = tbc_ket_cc.GetNumberKets();
              int indx_ki =
                  tbc_ket_cc.GetLocalIndex(std::min(k, i), std::max(k, i));
              ...
              if (Lambda == 0) {
                ...
                ninej = Z.modelspace->GetNineJ(jj, jl, J3, ji, jk, J4, J1, J2,
                                               Lambda);
              }
              ...
              if (ch_bra_cc <= ch_ket_cc) {
                if (j <= l)
                  ...
                  tbme = Zmat(indx_jl, indx_ki + (k > i ? 0 : nkets)) * hZ *
                         Z.modelspace->phase(J3 - J4 + ji + jj + jk + jl);
              } else {
                if (k <= i)
                  tbme = Zmat(indx_ki, indx_jl + (j > l ? nbras : 0)) * hZ *
                         Z.modelspace->phase(J3 -
                                             J4);  // Z_ilkj = Z_kjil * (phase)
                else
                  tbme = Zmat(indx_ki, indx_jl + (j > l ? 0 : nbras)) *
                         Z.modelspace->phase(ji + jj + jk +
                                             jl);  // Z_ilkj = Z_kjil * (phase)
              }
              /*
              ...
              commji += hatfactor * Z.modelspace->phase(ji + jl + J2 + J4) *
                        ninej * tbme;
            }
          }
        }
        ...
        double norm = bra.delta_pq() == ket.delta_pq() ? 1 + bra.delta_pq()
                                                       : PhysConst::SQRT2;
        Zijkl(ibra, iket) +=
            (commij - Z.modelspace->phase(ji + jj - J1) * commji) / norm;
...
// -- This appears to agree with Nathan's results
//
/// Calculates the part of \f$ [X_{(2)},\mathbb{Y}^{\Lambda}_{(2)}]_{ijkl} \f$
/// which involves ph intermediate states, here indicated by \f$
/// \mathbb{Z}^{J_1J_2\Lambda}_{ijkl} \f$ \f[ \mathbb{Z}^{J_1J_2\Lambda}_{ijkl}
/// = \sum_{abJ_3J_4}(n_a-n_b) \hat{J_1}\hat{J_2}\hat{J_3}\hat{J_4} \left[
///  \left\{ \begin{array}{lll}
///  j_i  &  j_l  &  J_3 \\
///  j_j  &  j_k  &  J_4 \\
///  J_1  &  J_2  &  \Lambda \\
///  \end{array} \right\}
/// \left(
/// \bar{X}^{J3}_{i\bar{l}a\bar{b}}\bar{\mathbb{Y}}^{J_3J_4\Lambda}_{a\bar{b}k\bar{j}}
/// -
///   \bar{\mathbb{Y}}^{J_3J_4\Lambda}_{i\bar{l}a\bar{b}}\bar{X}^{J_4}_{a\bar{b}k\bar{j}}
///   \right)
///  -(-1)^{j_i+j_j-J_1}
///  \left\{ \begin{array}{lll}
///  j_j  &  j_l  &  J_3 \\
...
///  \end{array} \right\}
/// \left(
/// \bar{X}^{J_3}_{i\bar{l}a\bar{b}}\bar{\mathbb{Y}}^{J_3J_4\Lambda}_{a\bar{b}k\bar{j}}
/// -
///   \bar{\mathbb{Y}}^{J_3J_4\Lambda}_{i\bar{l}a\bar{b}}\bar{X}^{J_4}_{a\bar{b}k\bar{j}}
///   \right)
/// \right]
...
/// This is implemented by defining an intermediate matrix
/// \f[
/// \bar{\mathbb{Z}}^{J_3J_4\Lambda}_{i\bar{l}k\bar{j}} \equiv
/// \sum_{ab}(n_a\bar{n}_b) \left[ \left(
/// \bar{X}^{J3}_{i\bar{l}a\bar{b}}\bar{\mathbb{Y}}^{J_3J_4\Lambda}_{a\bar{b}k\bar{j}}
/// -
///   \bar{\mathbb{Y}}^{J_3J_4\Lambda}_{i\bar{l}a\bar{b}}\bar{X}^{J_4}_{a\bar{b}k\bar{j}}
///   \right)
/// -\left(
/// \bar{X}^{J_3}_{i\bar{l}b\bar{a}}\bar{\mathbb{Y}}^{J_3J_4\Lambda}_{b\bar{a}k\bar{j}}
/// -
///    \bar{\mathbb{Y}}^{J_3J_4\Lambda}_{i\bar{l}b\bar{a}}\bar{X}^{J_4}_{b\bar{a}k\bar{j}}
///    \right)\right]
/// \f]
/// The Pandya-transformed matrix elements are obtained with
/// DoTensorPandyaTransformation(). The matrices \f$
/// \bar{X}^{J_3}_{i\bar{l}a\bar{b}}\bar{\mathbb{Y}}^{J_3J_4\Lambda}_{a\bar{b}k\bar{j}}
/// \f$ and \f$
/// \bar{\mathbb{Y}}^{J_4J_3\Lambda}_{i\bar{l}a\bar{b}}\bar{X}^{J_3}_{a\bar{b}k\bar{j}}
/// \f$ are related by a Hermitian conjugation, which saves two matrix
/// multiplications, provided we take into account the phase \f$ (-1)^{J_3-J_4}
/// \f$ from conjugating the spherical tensor. The commutator is then given by
/// \f[
/// \mathbb{Z}^{J_1J_2\Lambda}_{ijkl} = \sum_{J_3J_4}
/// \hat{J_1}\hat{J_2}\hat{J_3}\hat{J_4} \left[
///  \left\{ \begin{array}{lll}
///  j_i  &  j_l  &  J_3 \\
///  j_j  &  j_k  &  J_4 \\
///  J_1  &  J_2  &  \Lambda \\
...
void comm222_phst(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  double t_start = omp_get_wtime();
  // We reuse Xt_bar multiple times, so it makes sense to calculate them once
  // and store them in a deque.
  std::deque<arma::mat> Xt_bar_ph = InitializePandya(
      Z, Z.nChannels,
      "transpose");  // We re-use the scalar part multiple times, so there's a
                     // significant speed gain for saving it
  std::map<std::array<index_t, 2>, arma::mat> Y_bar_ph;
  DoPandyaTransformation(X, Xt_bar_ph, "transpose");
  X.profiler.timer["DoTensorPandyaTransformationX"] +=
      omp_get_wtime() - t_start;

  t_start = omp_get_wtime();
  // Construct the intermediate matrix Z_bar
  // First, we initialize the map Z_bar with empty matrices
  // to avoid problems in the parallel loop -- (do we even want a parallel loop
  // here?)
  std::map<std::array<index_t, 2>, arma::mat> Z_bar;

  t_start = omp_get_wtime();
  const auto& pandya_lookup =
      Z.modelspace->GetPandyaLookup(Z.GetJRank(), Z.GetTRank(), Z.GetParity());
  ...
  for (auto ich_bra : pandya_lookup) {
    ...
    //     int n_rows =
    //     Z.modelspace->GetTwoBodyChannel_CC(ich_bra).GetNumberKets();
    ...
    for (auto ich_ket : pandya_lookup) {
      ...
      //       if (pandya_lookup.at({(int)ich_bra,(int)ich_ket})[0].size()<1)
      //       continue; if (pandya_lookup.at({ich_bra,ich_ket})[0].size()<1)
      //       continue;
      auto tbc_ket_cc = Z.modelspace->GetTwoBodyChannel_CC(ich_ket);
      int n_cols = 2 * tbc_ket_cc.GetNumberKets();
      //         int n_cols =
      //         2*Z.modelspace->GetTwoBodyChannel_CC(ich_ket).GetNumberKets();
      if (n_cols < 1) continue;
      if ((tbc_bra_cc.parity + tbc_ket_cc.parity + Z.parity) % 2 > 0) continue;
      if ((tbc_bra_cc.J + tbc_ket_cc.J < Z.GetJRank()) or
          (std::abs(tbc_bra_cc.J - tbc_ket_cc.J) > Z.GetJRank()))
        continue;
      // Important to remember. For CC channels, Tz is the magnitude of the
      // difference of the isospin | tz1 - tz2| For RankT=0, we can have
      // <pn|pn>, <pp|nn>, <pp|pp>, <nn|nn>, (Tz_bra,Tz_ket) =>  (1,1) , (0,0)
      // For RankT=1, we can have <pn|pp>, <pn|nn>  (Tz_bra,Tz_ket) => (0,1) ,
      // (1,0) For RankT=2, we can have <pn|pn>   (Tz_bra,Tz_ket) => (1,1)
      if (not((tbc_bra_cc.Tz + tbc_ket_cc.Tz == Z.GetTRank()) or
              (std::abs(tbc_bra_cc.Tz - tbc_ket_cc.Tz) == Z.GetTRank())))
        continue;
  ...
  // BEGIN OLD WAY
  if (Z.GetJRank() > 0) {
    //      std::cout << "  in  " << __func__ << "  doing it the old way.
    //      Counter = " << counter << std::endl;
#ifndef OPENBLAS_NOUSEOMP
#pragma omp parallel for schedule( \
    dynamic,                       \
    1) if (not Z.modelspace->tensor_transform_first_pass.at(Z.GetJRank()))
#endif
    for (int i = 0; i < counter; ++i) {
      ...
      ////      const auto plookup =
      ///pandya_lookup.find({(int)ch_bra_cc,(int)ch_ket_cc});
      ...
      arma::uvec kets_ph = arma::join_cols(tbc_ket_cc.GetKetIndex_hh(),
                                           tbc_ket_cc.GetKetIndex_ph());
      arma::uvec bras_ph = arma::join_cols(tbc_bra_cc.GetKetIndex_hh(),
                                           tbc_bra_cc.GetKetIndex_ph());

      DoTensorPandyaTransformation_SingleChannel(Y, YJ1J2, ch_bra_cc,
                                                 ch_ket_cc);
      if (ch_bra_cc == ch_ket_cc) {
        YJ2J1 = YJ1J2;
      } else {
        DoTensorPandyaTransformation_SingleChannel(Y, YJ2J1, ch_ket_cc,
                                                   ch_bra_cc);
      }

      int flipphaseY = hY * Z.modelspace->phase(Jbra - Jket);
      // construct a matrix of phases (-1)^{k+j+p+h} used below to generate
      // X_phkj for k>j
      arma::mat PhaseMatXJ2(tbc_ket_cc.GetNumberKets(), kets_ph.size(),
                            arma::fill::ones);
      arma::mat PhaseMatYJ1J2(bras_ph.size(), tbc_ket_cc.GetNumberKets(),
                              arma::fill::ones);
      for (index_t iket = 0; iket < (index_t)tbc_ket_cc.GetNumberKets();
           iket++) {
      ...
      //      J1   [       |       ]       J1   [           |          ] [hp |ph
      //      ]
      //     i<=j  [  Zbar | Zbar  ]  =   i<=j  [   Xbar    | -Ybar    ]   * J1
      //     [   Ybar   |   Ybar'  ]
      //           [       |       ]            [           |          ] [ph |hp
      //           ]      where Ybar'_phkj = Ybar_hpkj *
      //           (-1)^{p+h+k+j}*(-1)^{J1-J2}*hY
      //                                                                         [----------|----------]       and
      //                                                                     J2
      //                                                                     [hp
      //                                                                     |ph
      ...
      arma::mat Mright = join_vert(
          join_horiz(YJ1J2,
                     join_vert(YJ1J2.tail_rows(halfnry12) % PhaseMatYJ1J2,
      ...
      Zmat = Mleft * Mright;
      //         std::cout << "   ... line " << __LINE__ << "  " << ch_bra_cc <<
      //         " " << ch_ket_cc << "  |Z| = " << arma::norm(Zmat,"fro") <<
      //         std::endl;
    }

  } else  // faster, more memory hungry way
  {
    //      std::cout << "  in  " << __func__ << "  doing it the new way" <<
    //      std::endl;
...
#pragma omp parallel for schedule( \
    ...
    X.profiler.timer["DoTensorPandyaTransformationY"] +=
        omp_get_wtime() - t_start;
...
#pragma omp parallel for schedule( \
    ...
    for (int i = 0; i < counter; ++i) {
      ...
      arma::mat& YJ1J2 = YJ1J2_list[i];
      arma::mat& YJ2J1 =
          (ch_bra_cc == ch_ket_cc) ? YJ1J2_list[i] : YJ2J1_list[i];
  ...
  X.profiler.timer["InverseTensorPandyaTransformation"] +=
      omp_get_wtime() - t_start;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////// Begin Scalar-Dagger Commutator
///Terms////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////

///  In the below formulas, the number in parentheses indicates the number of
///  external legs of a diagram, rather than the usual particle rank. In this
///  notation, a one-body operator has 2 external legs and so would be indicated
///  with a (2). The (1) then means a single creation/annihilation operator, and
///  (3) means two creators and one annihilator.
...
// Adapted from
//    void Operator::comm111ss( const Operator & X, const Operator& Y)
// This could be modified to be more efficent since we only need one column from
// Y to go to one column in Z. But this is so far from being the bottleneck that
// it makes more sense to be clear.
    ...
    for (auto& a : Z.modelspace->holes)  // C++11 syntax
    {
      ...
      for (auto b : X.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
        ...
        double nanb =
            oa.occ *
            (1 -
             ob.occ);  // despite what the name suggests, nanb is n_a * (1-n_b).
        if (std::abs(nanb) < ModelSpace::OCC_CUT) continue;
        ...
        for (int J = Jmin; J <= Jmax; J++) {
          Ymon_bia +=
              (2 * J + 1.0) / (oQ.j2 + 1.0) * Y.ThreeLeg.GetME_J(J, b, i, a);
          Ymon_aib +=
              (2 * J + 1.0) / (oQ.j2 + 1.0) * Y.ThreeLeg.GetME_J(J, a, i, b);
        }
        //                Z.OneBody(i,Q) += nanb * X.OneBody(a,b) * Ymon_bia -
        //                X.OneBody(b,a) * Ymon_aib;
        Z.OneBody(i, 0) +=
            nanb * X.OneBody(a, b) * Ymon_bia - X.OneBody(b, a) * Ymon_aib;
        //                  Z.OneBody(i,Q) += (ob.j2+1) * nanb *  X.OneBody(a,b)
        //                  * Y.TwoBody.GetTBMEmonopole(b,i,a,Q) ;  // Is this
        //                  still the right way to do this? Do we need to worry
        //                  about normalization? (It looks ok). Z.OneBody(i,Q)
        //                  -= (oa.j2+1) * nanb *  X.OneBody(b,a) *
        //                  Y.TwoBody.GetTBMEmonopole(a,i,b,Q) ;  //
        //                  GetTBMEmonopole returns unnormalized TBME summed
        //                  over J times (2J+1)/((2ji+1)*(2jj+1))
      }
    }
  }
...
//
//  I adapted this, but then gave up and brute forced it because the
//  intermediate matrix stuff was too obfuscated, and I'm the one who wrote
//  it... -SRS 14/04/2018
// Adapted from
//  void Operator::comm122ss( const Operator& X, const Operator& Y )
  ...
  for (int ich = 0; ich < n_nonzero; ++ich) {
    ...
    for (int indx_ij = 0; indx_ij < npq; ++indx_ij) {
      ...
      for (auto k : Z.modelspace->all_orbits) {
        ...
        //           if ( ( (ok.tz2+oQ.tz2)!=2*tbc.Tz ) or (
        //           (ok.l+oQ.l)%2!=tbc.parity ) or ( (ok.j2+oQ.j2)<2*tbc.J) or
        //           ( std::abs(ok.j2-oQ.j2)>2*tbc.J) ) continue;
        if (((ok.tz2 - oQ.tz2) != 2 * tbc.Tz) or
            ((ok.l + oQ.l) % 2 != tbc.parity) or
            ((ok.j2 + oQ.j2) < 2 * tbc.J) or
            (std::abs(ok.j2 - oQ.j2) > 2 * tbc.J))
          continue;
        ...
        //           std::cout << "INNER LOOPs ijk = " << i << " " << j << " "
        //           << k << " Z has " << Z.GetNumberLegs() << " legs " << " and
        //           there are " << Z.ThreeLeg.MatEl.size() << " 3 leg channels
        //           " << std::endl;
        // The first three loops are the [2,3]->3 bit
        for (int a : X.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
          //             cijk += X1(i,a) *
          //             Y.TwoBody.GetTBME_norm(ch,ch,a,j,k,Qorbit); cijk +=
          //             X1(i,a) * Y.TwoBody.GetTBME(ch,ch,a,j,k,Qorbit);
          cijk += X1(i, a) * Y.ThreeLeg.GetME(ch, a, j, k);
        }
        //           std::cout << "check 1. size of X.OneBodyChannels is " <<
        //           X.OneBodyChannels.Size() << std::endl;
        for (int a : X.OneBodyChannels.at({oj.l, oj.j2, oj.tz2})) {
          //             cijk += X1(j,a) *
          //             Y.TwoBody.GetTBME_norm(ch,ch,i,a,k,Qorbit); cijk +=
          //             X1(j,a) * Y.TwoBody.GetTBME(ch,ch,i,a,k,Qorbit);
          cijk += X1(j, a) * Y.ThreeLeg.GetME(ch, i, a, k);
        }
        //           std::cout << "check 2" << std::endl;
        for (int a : X.OneBodyChannels.at({ok.l, ok.j2, ok.tz2})) {
          //             cijk += X1(a,k) *
          //             Y.TwoBody.GetTBME_norm(ch,ch,i,j,a,Qorbit); // should
          //             this have a minus sign? cijk -= X1(a,k) *
          //             Y.TwoBody.GetTBME_norm(ch,ch,i,j,a,Qorbit); cijk -=
          //             X1(a,k) * Y.TwoBody.GetTBME(ch,ch,i,j,a,Qorbit);
        ...
        for (int a : Y.OneBodyChannels.at({oQ.l, oQ.j2, oQ.tz2})) {
          //             cijk += Y1(a,Qorbit) *
          //             X.TwoBody.GetTBME_norm(ch,ch,i,j,k,a);   // This
          //             determines the normalization for the (adagger adagger
          //             a) term. cijk +=  X.TwoBody.GetTBME(ch,ch,i,j,k,a) *
          //             Y1(a,Qorbit) ;   // This determines the normalization
          //             for the (adagger adagger a) term.
          cijk += X.TwoBody.GetTBME(ch, ch, i, j, k, a) *
                  Y1(a, 0);  // This determines the normalization for the
                             // (adagger adagger a) term.
                             //             if (i==0 and j==8 and k==0)
                             //             {
          //                std::cout << "   " << __func__ << "  a = " << a << "
          //                Xijka, Ya = " << X.TwoBody.GetTBME(ch,ch,i,j,k,a) <<
          //                " , " << Y1(a,Qorbit) << "  cijk = " << cijk <<
          //                std::endl;
          //             }
        }

        cijk *= (norm_ij *
                 norm_kQ);  // We normalize like a scalar TBME so that the
                            // setter/getters make sense. We'll fix the wrong
                            // |kQ> normalization when writing to file.

        Z.ThreeLeg.AddToME(
            ch, i, j, k,
            cijk);  // AddToME  assumes a normalized matrix element.
      }
    }
  }
  Z.profiler.timer[__func__] += omp_get_wtime() - t_start;
...
void ConstructDaggerMpp_Mhh(const Operator& X, const Operator& Y,
                            const Operator& Z, ThreeLegME& Mpp,
                            ThreeLegME& Mhh) {
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  for (int ich = 0; ich < nch; ++ich) {
    ...
    if (kets_hh.size() > 0)
      Matrixpp +=
          LHS.cols(kets_hh) * arma::diagmat(nbarnbar_hh) * RHS.rows(kets_hh);
    if (kets_ph.size() > 0)
      Matrixpp +=
          LHS.cols(kets_ph) * arma::diagmat(nbarnbar_ph) * RHS.rows(kets_ph);
...
/// the intermediate matrices \f$\mathcal{M}_{pp} \f$ and \f$ \mathcal{M}_{hh}
/// \f$, we can combine them and only calculate the intermediates once. We do
/// the same thing in the standard scalar-scalar commutators.
void comm433_pp_hh_431sd(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  std::vector<index_t> ilist(
      Z.OneBodyChannels.at({oQ.l, oQ.j2, oQ.tz2}).begin(),
      Z.OneBodyChannels.at({oQ.l, oQ.j2, oQ.tz2}).end());
  int ni = ilist.size();
#pragma omp parallel for schedule(dynamic, 1)
  for (int i_ind = 0; i_ind < ni; ++i_ind) {
    ...
    for (int ch = 0; ch < Z.nChannels; ++ch) {
      ...
      for (auto& c : Z.modelspace->holes) {
        Orbit& oc = Z.modelspace->GetOrbit(c);
        //            cijJ += Jfactor * oc.occ * Mpp.GetTBME(ch,c,i,c,Q);     //
        //            We use the GetTBME, which returns an unnormalized matrix
        //            element, as required. cijJ += Jfactor * (1-oc.occ) *
        //            Mhh.GetTBME(ch,c,i,c,Q);
        cijJ += Jfactor * oc.occ *
                Mpp.GetME(ch, c, i,
                          c);  // We use the GetTBME, which returns an
                               // unnormalized matrix element, as required.
    ...
    Z.OneBody(i, 0) +=
        cijJ / (oi.j2 + 1.0);  // The factor of 1/2 in the formula is absorbed
                               // by the fact that the mat-mult only sums a<=b.
    //      Z.OneBody(i,Q) += cijJ /(oi.j2+1.0);   // The factor of 1/2 in the
    //      formula is absorbed by the fact that the mat-mult only sums a<=b.
  }  // for i
...
//  faster mat-mult implementation.  The two implementations agree as of Nov 2,
//  2018 - SRS Benchmarked against the m-scheme version in UnitTest.cc, and it
//  agrees (although some changes were required compared to the impoementation
//  tested in Nov 2018). So this again agrees with the more efficient way, and
//  with the m-scheme verion as of Sep 30 2019 - SRS
void comm433sd_ph_dumbway(const Operator& X, const Operator& Y, Operator& Z) {
  ...
  for (auto khh :
       Z.modelspace->KetIndex_hh)  // ph kets needs to include hh list in case
                                   // there are fractionally occupied states
  {
  ...
  for (int ich = 0; ich < nch; ++ich) {
    ...
    for (size_t ibra = 0; ibra < nkets; ibra++) {
      ...
      std::vector<int> ijsign_cases = {
          +1, bra.Phase(
                  J)};  // This accounts for the 1 - (-1)^(ji +jj-J) Pij factor.
      ...
      for (auto k : Z.modelspace->all_orbits) {
        ...
        //            if ( ( (ok.tz2+oQ.tz2)!=2*tbc.Tz) or (
        //            (ok.l+oQ.l)%2!=tbc.parity) or ( (ok.j2+oQ.j2)<2*tbc.J) or
        //            ( std::abs(ok.j2-oQ.j2)>2*tbc.J) ) continue; // if |kQ>
        //            doesn't live in this channel, move along.
        if (((ok.tz2 - oQ.tz2) != 2 * tbc.Tz) or
            ((ok.l + oQ.l) % 2 != tbc.parity) or
            ((ok.j2 + oQ.j2) < 2 * tbc.J) or
            (std::abs(ok.j2 - oQ.j2) > 2 * tbc.J))
        ...
        for (int ijcase = 0; ijcase <= 1; ijcase++) {
          ...
          for (int Jprime = Jprime_min; Jprime <= Jprime_max; ++Jprime) {
            ...
            for (auto& iketab : kets_ph) {
              ...
              if ((ketab.op->j2 + ketab.oq->j2) < 2 * Jprime or
                  std::abs(ketab.op->j2 - ketab.oq->j2) > 2 * Jprime)
                continue;
              ...
              for (int abcase = 0; abcase <= 1; abcase++) {
                ...
                //                  if ( not (a==0 and b==10) or (a==10 and
                //                  b==0) ) continue; if ( not ((a==1 and b==8)
                //                  or (a==8 and b==1)) ) continue;
                ...
                for (int JA = JA_min; JA <= JA_max; ++JA) {
                  double sixjA =
                      Z.modelspace->GetSixJ(ja, jb, Jprime, jk, jj, JA);
                  matelX -=
                      (2 * JA + 1) * sixjA *
                      X.TwoBody.GetTBME_J(JA, a, j, k,
                                          b);  // GetTBME_J returns an
                                               // un-normalized matrix element
                }
                for (int JB = JB_min; JB <= JB_max; ++JB) {
                  double sixjB =
                      Z.modelspace->GetSixJ(ja, jb, Jprime, ji, jQ, JB);
                  //                    matelY -= (2*JB+1) * sixjB *
                  //                    Y.TwoBody.GetTBME_J(JB,i,b,a,Q);   //
                  //                    GetTBME_J returns an un-normalized
                  //                    matrix element
                  matelY -= (2 * JB + 1) * sixjB *
                            Y.ThreeLeg.GetME_J(
                                JB, i, b, a);  // GetTBME_J returns an
                                               // un-normalized matrix element
                }
                XYprod += nanb * matelX * matelY;
                //                  if ( ( (i==0 and j==1) or (i==1 and j==0))
                //                  and k==1  and std::abs(nanb)>0) if ( ( (i==0
                //                  and j==1) or (i==1 and j==0)) and k==1 and
                //                  ((a==0 and b==10) or (b==0 and a==10)) ) if
                //                  ( ( (i==0 and j==1) or (i==1 and j==0)) and
                //                  k==1 and ((a==1 and b==8) or (b==1 and
                //                  a==8)) )
                //                  {
                //                    std::cout << "   J = " << J << "  a,b = "
                //                    << a << " " << b << "  Jprime = " <<
            ...
            //                  if (( (i==0 and j==1) or (i==1 and j==0)) and
            //                  k==1)
            //                  {
            //                    std::cout << "i,j = " << i << " " << j << "
            //                    ijsign(J=" << J << ") = " << ijsign << "
            //                    2Jprime+1 " << 2*Jprime+1 << "  sixjprime " <<
            //                    sixjprime << "  XYprod " << XYprod << "  =>
            //                    zijk " << zijk
            //                               <<  std::endl;
            //                  }
          }  // loop over ab kets
          ...
          //                  k==1)
          //                  {
          //                    std::cout << " zijk = " << zijk <<  " norm_ij
          //                    norm_kQ = " << norm_ij << " " << norm_kQ << "
          //                    before setting, the ME is " <<
          //                    Z.ThreeLeg.GetME_J(J,i,j,k) << std::endl <<
          //                    std::endl;
          //                  }

        }  // loop over ij cases

        //            Z.TwoBody.AddToTBME_J(J,i,j,k,Q,  zijk * norm_ij*norm_kQ);
        //            // AddToTBME_J assumes a normalized matrix element.
        Z.ThreeLeg.AddToME_J(
            J, bra.p, bra.q, k,
            zijk * norm_ij *
                norm_kQ);  // AddToTBME_J assumes a normalized matrix element.
      }                    // loop over k
    }                      // for ibra
  }                        // for ich
  Z.profiler.timer[__func__] += omp_get_wtime() - t_start;
...
//   |                   |
//
// For formula and details of implementation, see Operator::comm222_phss. The
// only change made here to accommodate a dagger operator is to replace XY - YX
// with -YX. This ensures that we don't include contributions from the Qspace
// orbit to X. Adapted from
//    void Operator::comm222_phss( const Operator& X, const Operator& Y )

void comm433sd_ph(const Operator& X, const Operator& Y, Operator& Z) {
  double t_start = omp_get_wtime();

  int hx = X.IsHermitian()
               ? 1
               : -1;  // assuming X is either hermitian or antihermitian. I'm
                      // sure this will come back to bite me some day.

  // Construct the intermediate matrix Z_bar and fill it with zeros.
  //   const auto& pandya_lookup = Z.modelspace->GetPandyaLookup( Z.GetJRank(),
  //   Z.GetTRank(), Z.GetParity() );
  size_t nch = Z.modelspace->SortedTwoBodyChannels_CC.size();
  size_t norb = Z.modelspace->GetNumberOrbits();
  std::deque<arma::mat> Z_bar(Z.nChannels);
  std::vector<bool> lookup_empty(Z.nChannels, true);
  for (size_t ich = 0; ich < nch; ++ich) {
    ...
    Z_bar[ch].zeros(norb, 2 * nKets_cc);  // Z_iQ`kj`   we want both orderings
                                          // of k and j, but Q is fixed.
    //      if ( pandya_lookup.at({ch,ch})[0].size()>0 ) lookup_empty[ich] =
    //      false;
...
#pragma omp parallel for schedule( \
  ...
  for (size_t ich = 0; ich < nch; ++ich) {
    ...
    DoPandyaTransformation_SingleChannel_Dagger(
        Y, Y_bar_ph,
        ch);  // Generate  YbarJ'_iQ`ab` * (na-nb)      for a<=b and a>b
    DoPandyaTransformation_SingleChannel(
        X, X_bar_ph, ch, "normal");  // Generate XbarJ'_ab`kj` for a<=b and a>b
    auto& Zbar_ch =
        Z_bar.at(ch);  // Marginally useful aliasing to avoid lookups...

    // Zbar will be
    // ZbarJ'_iQ`kj` = Sum_ab   (na-nb) <iQ`|YbarJ'|ab`> <ab`|XbarJ'|kj`>
    // So then we can perform an inverse Pandya transform (which is really just
    // another Pandya transform) to get ZJ_ijkQ = - Sum_J' (2J'+1) { ji  jj  J
    // }  ZbarJ'_iQ`kj`
    //                            { jk  jQ  J' }

    // The shape of Xt_bar_ph is  ( rows, columns) = ( 2*nph_kets,  nKets_cc) =>
    // [     Xbar    ]  |  ab`, element of nph_kets, which has ph` and hp`
    //                                                                               [             ]  v
    //                                                                                    ->
    //                                                                                    kj` is an element of kets_cc
    //
    // The shape of Y_bar_ph is   (rows, columns) = (norb, 2*nph_kets) =>    [
    // Ybar    ]  | iQ`  runs over all orbits i, with Q fixed.
    //                                                                               [             ]  v
    //                                                                                    ->
    //                                                                                    ab` is an element of nph_kets, here we have ph` and hp`
    //
    // So we should multiply Ybar * Xbar to sum over ab` and get out Zbar => [
    // Zbar    ]  |  iQ`
    //                                                                                 [             ]  v
    //                                                                                      ->
    //                                                                                      kj`
    ...
    if (Y_bar_ph.size() < 1 or
        X_bar_ph.size() < 1)  // for an armadillo matrix, .size()  gives the
                              // total number of elements, i.e. n_rows * n_cols
    {
      //        Zbar_ch = arma::zeros( norb, 2*nKets_cc );  // This seems
      //        unnecessary since we initialized things earlier... try getting
      //        rid of it and see if things break.
      continue;
    }
    ...
    //                                           [      |     ]
    //     create full Y matrix from the half:   [  Yhp | Y'ph]   where the
    //     prime indicates multiplication by (-1)^(i+j+k+l) h_x
    //                                           [      |     ]   Flipping hp
    //                                           <-> ph and multiplying by the
    //                                           phase is equivalent to [  Yph |
    //                                           Y'hp]   having kets |kj> with
    ...
    Zbar_ch = Y_bar_ph *
              join_horiz(X_bar_ph,
                         join_vert(X_bar_ph.tail_rows(nph_kets) % PhaseMatX,
  ...
  //   Z.modelspace->scalar_transform_first_pass = false;
  Z.profiler.timer["InversePandyaTransformation_Dagger"] +=
      omp_get_wtime() - t_start;
...
/// \f]
/// where the overbar indicates time-reversed orbits.
/// Since we use this for the commutator expression, the k and j orbits will be
/// a and b, i.e. the orbits we sum over with a factor (na-nb). We include the
/// (na-nb) factor in the output matrix.
void DoPandyaTransformation_SingleChannel_Dagger(const Operator& Z,
                                                 arma::mat& TwoBody_CC_ph,
                                                 int ch_cc) {
  TwoBodyChannel& tbc_cc = Z.modelspace->GetTwoBodyChannel_CC(ch_cc);
  //   int nKets_cc = tbc_cc.GetNumberKets();
  size_t norb = Z.modelspace->GetNumberOrbits();
  ...
  TwoBody_CC_ph.zeros(
      norb,
      2 * nph_kets);  // factor 2 because we want both orderings ab and ba.
  ...
  for (int iket = 0; iket < nph_kets; ++iket) {
    ...
    // Here we make some lists so that we don't need to repeat code when
    // switching (a,b) -> (b,a). Instead we can just iterate over the two cases.
    std::vector<int> ab = {a, b};
    std::vector<double> jab = {ja, jb};
    std::vector<double> nanb = {(oa.occ - ob.occ), (ob.occ - oa.occ)};

    // we loop over both orderings, a<b and a>b. Here, this is done by
    // exchanging a<->b and taking a minus sign due to the (na-nb) factor.
    for (int ab_case = 0; ab_case <= 1; ab_case++) {
      ...
      for (size_t i = 0; i < norb; i++) {
        ...
        if (not tbc_cc.CheckChannel_ket(&oi, &oQ))
          continue;  //  <iQ|  isn't in this channel, so move along. (Note, for
                     //  this check, the ordering of i,Q doesn't matter).
        size_t indx_iQ =
            i;  // since Q is fixed, we can label the bra <iQ| by the index i.
        double ji = oi.j2 * 0.5;
...
// Same idea as the scalar commutator version, with enough modifications to make
// it not wrong (I hope). One big difference is that the Pandya-transformed
// operator Zbar has its bra indices numbered by orbit index rather than ket
// index, because it's   <iQ|Zbar|kj>  and Q is fixed. Otherwise, it should look
// pretty similar.
void AddInversePandyaTransformation_Dagger(const std::deque<arma::mat>& Zbar,
                                           Operator& Z) {
  // Do the inverse Pandya transform
  int n_nonzeroChannels = Z.modelspace->SortedTwoBodyChannels.size();
  size_t norb = Z.modelspace->GetNumberOrbits();
  ...
  for (int ich = 0; ich < n_nonzeroChannels; ++ich) {
    ...
    for (size_t ibra = 0; ibra < nKets; ++ibra) {
      ...
      // Two-element lists for use below, because we need to antisymmetrize with
      // 1-Pij, including a phase factor (-1)^(ji+jj-J) The code would look like
      // the exact same thing copy-pasted twice, with i and j exchanged in the
      // second copy.
      ...
      for (size_t k = 0; k < norb; k++) {
        Orbit& ok = Z.modelspace->GetOrbit(k);
        //          if ( not tbc.CheckChannel_ket(&ok, &oQ) ) continue; // if
        //          |kQ> doesn't live in this channel, move along. if ( (
        //          (ok.tz2+oQ.tz2)!=2*tbc.Tz) or ( (ok.l+oQ.l)%2!=tbc.parity)
        //          or ( (ok.j2+oQ.j2)<2*tbc.J) or (
        //          std::abs(ok.j2-oQ.j2)>2*tbc.J) ) continue; // if |kQ>
        //          doesn't live in this channel, move along.
        if (((ok.tz2 - oQ.tz2) != 2 * tbc.Tz) or
            ((ok.l + oQ.l) % 2 != tbc.parity) or
        ...
        for (int ij_case = 0; ij_case <= 1;
             ij_case++)  // loop over the two cases corresponding to 1-Pij (with
                         // the appropriate phase on the exchange term)
        {
          ...
          for (int Jprime = Jmin; Jprime <= Jmax; ++Jprime) {
            ...
            size_t indx_iQ = i;
            size_t indx_kj =
                tbc_cc.GetLocalIndex(std::min(j, k), std::max(j, k)) +
                (k > j ? nkets_cc : 0);
        ...
        //            Z.TwoBody.AddToTBME_J(J,i,j,k,Q,  commijk );   //
        //            AddToTBME_J assumes a normalized matrix element.
        //            Z.ThreeLeg.AddToME_J(J,i,j,k,  commijk );   // AddToTBME_J
        //            assumes a normalized matrix element.
        Z.ThreeLeg.AddToME_J(
            J, bra.p, bra.q, k,
            commijk);  // AddToTBME_J assumes a normalized matrix element.
      }                // for k
    }                  // for ibra, which is <ij|
  }                    // for ich, which is J etc
}

/// Product of two operators. Similar to commutator, and hopefully useful for
/// some debugging
//
//
void prod110ss(const Operator& X, const Operator& Y, Operator& Z) {
  //     if (X.IsHermitian() and Y.IsHermitian()) return ; // I think this is
  //     the case if (X.IsAntiHermitian() and Y.IsAntiHermitian()) return ; // I
