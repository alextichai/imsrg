...
IMSRGSolver::~IMSRGSolver() { CleanupScratch(); }
...
IMSRGSolver::IMSRGSolver()
    ...
    : s(0),
      ...
      flowfile(""),
      n_omega_written(0),
      max_omega_written(500),
      magnus_adaptive(true),
      hunter_gatherer(false),
      perturbative_triples(false),
      pert_triples_this_omega(0),
      pert_triples_sum(0),
      ode_monitor(*this),
      ode_mode("H"),
      ode_e_abs(1e-6),
...
IMSRGSolver::IMSRGSolver(Operator& H_in)
    //   : modelspace(H_in.GetModelSpace()),rw(NULL), H_0(&H_in),
    //   FlowingOps(1,H_in), Eta(H_in),
    : modelspace(H_in.GetModelSpace()),
      ...
      ode_e_abs(1e-6),
      ode_e_rel(1e-6) {
  //   std::cout << "  " << __FILE__ << " line " << __LINE__ << "  nops = " <<
  //   Eta.profiler.counter["N_Operators"] << std::endl; std::cout << "   sizes
  //   of operator vectors:  FlowingOps " << FlowingOps.size() << " Omega " <<
  //   Omega.size() << std::endl;
  Eta.Erase();
  ...
  //   std::cout << "  " << __FILE__ << " line " << __LINE__ << "  nops = " <<
  //   Eta.profiler.counter["N_Operators"] << std::endl;
}

void IMSRGSolver::NewOmega() {
  H_saved = FlowingOps[0];
  std::cout << "pushing back another Omega. Omega.size = " << Omega.size()
            << " , operator size = " << Omega.front().Size() / 1024. / 1024.
            << " MB"
            << ",  memory usage = "
            << profiler.CheckMem()["RSS"] / 1024. / 1024. << " GB";
  if (perturbative_triples)
    std::cout << "  pert. triples = " << pert_triples_this_omega
              << "   sum = " << pert_triples_sum;
  std::cout << std::endl;
  //  scratchdir = rw->GetScratchDir();
  //  if ((rw != NULL) and (rw->GetScratchDir() !=""))
  if (scratchdir != "") {
    //    char tmp[512];
    //    sprintf(tmp,"%s/OMEGA_%06d_%03d",rw->GetScratchDir().c_str(),
    //    getpid(), n_omega_written); std::string fname(tmp); std::ofstream
    //    ofs(fname, std::ios::binary); if (
    //    rw->GetScratchDir().find("/dev/null") == std::string::npos )
    if (scratchdir.find("/dev/null") == std::string::npos) {
      std::ostringstream filename;
      //      filename << rw->GetScratchDir().c_str() << "/OMEGA_" <<
      //      std::setw(6) << std::setfill('0') << getpid() << std::setw(3) <<
      //      std::setfill('0') << n_omega_written;
      filename << scratchdir.c_str() << "/OMEGA_" << std::setw(6)
               << std::setfill('0') << getpid() << std::setw(3)
               << std::setfill('0') << n_omega_written;
      std::ofstream ofs(filename.str(), std::ios::binary);
      Omega.back().WriteBinary(ofs);
      //    std::cout << "Omega written to file " << fname << "  written " <<
      //    n_omega_written << " so far." << std::endl;
      std::cout << "Omega written to file " << filename.str() << "  written "
                << n_omega_written << " so far." << std::endl;
      if (n_omega_written > max_omega_written) {
        std::cout << "n_omega_written > max_omega_written.  ("
                  << n_omega_written << " > " << max_omega_written
                  << " ) deleting OMEGA files and calling terminate."
                  << std::endl;
        CleanupScratch();
        std::terminate();
      }
...
// We then clear out the hunter, and update H(s) according to the gathered
// Omega. And we continue hunting, using H(s) = exp[Omega_g] H(0) exp[-Omega_g]
// as our starting point. This is essentially a compromise between the "split"
// and "no split" approaches, combining the advantages of both. As we hunt, we
// have a relatively small Omega, so we don't need to evaluate as many nested
// commutators, but in the end we have just one Omega so that if we want to
// transform some operator we don't need to do a bunch of transformations.
void IMSRGSolver::GatherOmega() {
  ...
  // the last omega in the list is the hunter. the one just preceeding it is the
  // gatherer.
...
void IMSRGSolver::Solve() {
  if (s < 1e-4) WriteFlowStatusHeader(std::cout);
  ...
  if (method == "magnus_euler" or method == "magnus")
    ...
    FlowingOps.emplace_back(
        Operator(*(FlowingOps[0].GetModelSpace()), 0, 0, 0, 1));
    Solve_ode_adaptive();
  } else
...
void IMSRGSolver::Solve_magnus_euler() {
  ...
  //   if ( generator.GetType() == "rspace" ) { generator.modelspace =
  //   (Eta.modelspace); generator.SetRegulatorLength(800005.0); };
  ...
  for (istep = 1; s < smax; ++istep) {
    ...
    if (magnus_adaptive)
      ds = std::min(
          std::min(std::min(norm_domega / norm_eta,
                            norm_domega / norm_eta / (norm_omega + 1.0e-9)),
                   omega_norm_max / norm_eta),
          ds_max);
    ...
    //      << MAT(2,0) << "  " << MAT(1,1) << " " << MAT(2,1) << "  " <<
    //      MAT(2,2) << std::endl; std::cout << " ______>  " << dMAT(0,0) << "
    //      " << dMAT(1,0) << "   " << dMAT(2,0) << "  " << dMAT(1,1) << " " <<
    //      dMAT(2,1) << "  " << dMAT(2,2) << std::endl;
  }
}
...
void IMSRGSolver::Solve_magnus_modified_euler() {
  ...
  for (istep = 1; s < smax; ++istep) {
    ...
    // ds should never be more than 1, as this is over-rotating
    ds = std::min(std::min(norm_domega / norm_eta,
                           norm_domega / norm_eta / (norm_omega + 1.0e-9)),
                  ds_max);
...
// vector<Operator> operator/ (const vector<Operator>& num, const
// vector<Operator>& denom)
std::deque<Operator> operator/(const std::deque<Operator>& num,
                               const std::deque<Operator>& denom) {
  ...
  for (size_t i = 0; i < num.size(); ++i) {
    ...
    for (auto& itmat : quotient[i].TwoBody.MatEl)
      itmat.second /=
          denom[i].TwoBody.GetMatrix(itmat.first[0], itmat.first[1]);
  }
  return quotient;
}
...
// vector<Operator> operator+ ( const vector<Operator>& X, const
// vector<Operator>& Y)
std::deque<Operator> operator+(const std::deque<Operator>& X,
                               const std::deque<Operator>& Y) {
  //  vector<Operator> Z = X;
...
std::deque<Operator> operator+(const double a, const std::deque<Operator>& X) {
  ...
  for (auto& y : Y) {
    ...
    for (auto& itmat : y.TwoBody.MatEl) itmat.second += a;
...
void IMSRGSolver::Solve_ode() {
  ...
  //   runge_kutta4< vector<Operator>, double, vector<Operator>, double,
  //   vector_space_algebra> stepper;
  odeint::runge_kutta4<std::deque<Operator>, double, std::deque<Operator>,
                       double, odeint::vector_space_algebra>
      stepper;
  auto system = *this;
  auto monitor = ode_monitor;
  //   size_t steps = integrate_const(stepper, system, FlowingOps, s, smax, ds,
  //   monitor);
...
void IMSRGSolver::Solve_ode_adaptive() {
  ...
  namespace odeint = boost::numeric::odeint;
  auto system = *this;
  //   typedef runge_kutta_dopri5< vector<Operator> , double , vector<Operator>
  //   ,double , vector_space_algebra > stepper;
  typedef odeint::runge_kutta_dopri5<std::deque<Operator>, double,
                                     std::deque<Operator>, double,
                                     odeint::vector_space_algebra>
      stepper;
  //   typedef adams_bashforth_moulton< 4, vector<Operator> , double ,
  //   vector<Operator> ,double , vector_space_algebra > stepper;
  auto monitor = ode_monitor;
  //   size_t steps =
  //   integrate_adaptive(make_controlled<stepper>(ode_e_abs,ode_e_rel), system,
  //   FlowingOps, s, smax, ds, monitor);
  odeint::integrate_adaptive(
      odeint::make_controlled<stepper>(ode_e_abs, ode_e_rel), system,
      FlowingOps, s, smax, ds, monitor);
  monitor.report();
}

// Evaluate dx/dt for boost ode
// void IMSRGSolver::operator()( const Operator& x, Operator& dxdt, const double
// t) void IMSRGSolver::operator()( const vector<Operator>& x, vector<Operator>&
// dxdt, const double t)
void IMSRGSolver::operator()(const std::deque<Operator>& x,
                             std::deque<Operator>& dxdt, const double t) {
  s = t;
  if (ode_mode == "H") {
    FlowingOps[0] = x[0];
    ...
    double norm_omega = Omega.back().Norm();
    if (norm_omega >
        omega_norm_max)  // This doesn't seem to works so well just yet...
    {
      NewOmega();
      norm_omega = 0;
    }
...
void IMSRGSolver::Solve_ode_magnus() {
  ...
  auto system = *this;
  auto monitor = ode_monitor;
  //   size_t steps = integrate_const(stepper, system, Omega, s, smax, ds,
  //   monitor);
...
Operator IMSRGSolver::Transform_Partial(Operator&& OpIn, int n) {
  ...
  if (scratchdir != "") {
    ...
    for (int i = n; i < n_omega_written; i++) {
      //     sprintf(tmp,"%s/OMEGA_%06d_%03d",rw->GetScratchDir().c_str(),
      //     getpid(), i); std::string fname(tmp); Operator omega(OpIn);
      Operator omega(Eta);
      std::ostringstream filename;
      //     filename << rw->GetScratchDir().c_str() << "/OMEGA_" <<
      //     std::setw(6) << std::setfill('0') << getpid() << std::setw(3) <<
...
void IMSRGSolver::FlushOmegaToScratch() {
  if (scratchdir.find("/dev/null") != std::string::npos)
    return;  // if we're writing to /dev/null, we'd better not try to erase
             // things
  if (scratchdir == "") return;
  for (size_t i = 0; i < Omega.size(); i++) {
    std::stringstream filename;
    filename << scratchdir << "/OMEGA_" << std::setw(6) << std::setfill('0')
             << getpid() << std::setw(3) << std::setfill('0')
             << i + n_omega_written;
    std::ofstream ofs(filename.str(), std::ios::binary);
    Omega[i].WriteBinary(ofs);
    n_omega_written++;
    std::cout << "Omega written to file " << filename.str() << "  written "
              << i + n_omega_written << " so far." << std::endl;
  }
}

//// It's important that we don't refer to rw in this routine, since we call on
///destruction / and it may be that rw gets destroyed before this instance,
///making the pointer to rw invalid.
void IMSRGSolver::CleanupScratch() {
  if (n_omega_written <= 0) return;
  //  if ( rw->GetScratchDir().find("/dev/null") != std::string::npos ) return;
  //  // if we're writing to /dev/null, we'd better not try to erase things
  if (scratchdir.find("/dev/null") != std::string::npos)
    return;  // if we're writing to /dev/null, we'd better not try to erase
             // things
  ...
  for (int i = 0; i < n_omega_written; i++) {
    std::ostringstream filename;
    //    filename << rw->GetScratchDir() << "/OMEGA_" << std::setw(6) <<
    //    std::setfill('0') << getpid() << std::setw(3) << std::setfill('0') <<
    //    i;
    filename << scratchdir << "/OMEGA_" << std::setw(6) << std::setfill('0')
             << getpid() << std::setw(3) << std::setfill('0') << i;
    //    filename << rw->GetScratchDir().c_str() << "/OMEGA_" << std::setw(6)
    //    << std::setfill('0') << getpid() << std::setw(3) << std::setfill('0')
    //    << i; std::cout << "  scratchdir:  " << rw->GetScratchDir() << "  , "
    //    << rw->GetScratchDir().c_str() << std::endl; std::cout << "
    //    scratchdir:  " << scratchdir << "  ,   " << scratchdir.c_str() <<
    //    std::endl; std::cout << "  pid :  " << std::setw(6) <<
    //    std::setfill('0') << getpid() << std::endl; std::cout << "    i :  "
    //    << std::setw(3) << std::setfill('0') << i << std::endl;
    //    sprintf(tmp,"%s/OMEGA_%06d_%03d",rw->GetScratchDir().c_str(),
    //    getpid(), i); std::string fname(tmp); if ( remove(tmp) !=0 )
    if (remove(filename.str().c_str()) != 0) {
      std::cout << "Error when attempting to delete " << filename.str()
                << std::endl;
    }
  }
}
...
double IMSRGSolver::EstimateStepError() {
  ...
  if (std::abs(s) > 1e-9 and gtype.find("qtransfer") != std::string::npos) {
    ...
    //    double suppression = std::min( (modelspace->GetTargetMass()-2.)/3.0,
    //    pow( (kF*R), 3) ); // Not sure if this is actually legit
    double suppression = pow((kF * R), 3);
    err = std::abs(dEds) * suppression;
    //    std::cout << std::endl << "Made it inside, suppression = " <<
    //    suppression << "  dEds = " << dEds << " ->  "
    //              << Elast << " " << Ecurrent << "  err = " << err <<
    //              std::endl;
  }
  //  Elast = Ecurrent;
  return err;
...
double IMSRGSolver::GetPerturbativeTriples()
{
  ...
  pert_triples_sum += pert_triples_this_omega;
  std::cout << "Adding " << pert_triples_this_omega << "  =>  " <<
pert_triples_sum << std::endl;
...
double IMSRGSolver::GetPerturbativeTriples() {
  ...
  std::cout << "I GOT pert_triples = " << std::setw(14) << std::setprecision(8)
            << pert_triples << "  size of omega is " << Omega.size()
            << std::endl;
  //  pert_triples_this_omega = Wbar.GetMP2_3BEnergy();
  //  pert_triples_sum += pert_triples_this_omega;
  //  std::cout << "Adding " << pert_triples_this_omega << "  =>  " <<
...
void IMSRGSolver::WriteFlowStatus(std::ostream& f) {
  if (f.good()) {
    ...
    f << std::fixed << std::setw(5) << istep << std::setw(12)
      << std::setprecision(5) << s << std::setw(fwidth)
      << std::setprecision(fprecision) << H_s.ZeroBody << std::setw(fwidth)
      << std::setprecision(fprecision)
      << H_s.Norm()
      //        << std::setw(fwidth) << std::setprecision(fprecision) <<
      //        H_s.Trace( modelspace->GetAref(), modelspace->GetZref() )
      << std::setw(fwidth) << std::setprecision(fprecision)
      << cumulative_error
      //        << std::setw(fwidth) << std::setprecision(fprecision) <<
      //        H_s.OneBodyNorm()
      ...
      //        Omega.Norm()
      << std::setw(fwidth) << std::setprecision(fprecision)
      << Omega.back().OneBodyNorm() << std::setw(fwidth)
      << std::setprecision(fprecision) << Omega.back().TwoBodyNorm()
      << std::setw(fwidth) << std::setprecision(fprecision)
      << Omega.back().ThreeBodyNorm() << std::setw(fwidth)
      << std::setprecision(fprecision) << Eta.OneBodyNorm() << std::setw(fwidth)
      << std::setprecision(fprecision) << Eta.TwoBodyNorm() << std::setw(fwidth)
      << std::setprecision(fprecision) << Eta.ThreeBodyNorm() << std::setw(7)
      << std::setprecision(0)
      << profiler.counter["N_ScalarCommutators"] +
             profiler.counter["N_TensorCommutators"]
      << std::setw(fwidth) << std::setprecision(fprecision)
      << H_s.GetMP2_Energy() << std::setw(7) << std::setprecision(0)
      << profiler.counter["N_Operators"] << std::setprecision(fprecision)
      << std::setw(12) << std::setprecision(3) << profiler.GetTimes()["real"]
      << std::setw(12) << std::setprecision(3)
      << profiler.CheckMem()["RSS"] / 1024. << " / " << std::skipws
      << profiler.MaxMemUsage() / 1024. << std::fixed << std::endl;
  }
}
...
void IMSRGSolver::WriteFlowStatusHeader(std::ostream& f) {
  if (f.good()) {
    ...
    int fprecision = 9;
    f.setf(std::ios::fixed);
    f << std::fixed << std::setw(5) << "i" << std::setw(12)
      << std::setprecision(5) << "s" << std::setw(fwidth)
      << std::setprecision(fprecision)
      << "E0"
      //        << std::setw(fwidth) << std::setprecision(fprecision) <<
