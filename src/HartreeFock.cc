...
HartreeFock::HartreeFock(Operator& hbare)
    : Hbare(hbare),
      modelspace(hbare.GetModelSpace()),
      KE(Hbare.OneBody),
      energies(Hbare.OneBody.diag()),
      tolerance(1e-8),
      convergence_ediff(7, 0),
      convergence_EHF(7, 0),
  ...
  for (int Tz = -1; Tz <= 1; ++Tz) {
    for (int parity = 0; parity <= 1; ++parity) {
      ...
      Vmon_exch[Tz + 1][parity] =
          arma::mat(nKetsMon, nKetsMon, arma::fill::zeros);
    }
  }
  prev_energies = arma::vec(norbits, arma::fill::zeros);
  std::vector<double> occvec;
  for (auto& h : modelspace->holes)
    occvec.push_back(modelspace->GetOrbit(h).occ);
  holeorbs = arma::uvec(
      std::vector<index_t>(modelspace->holes.begin(), modelspace->holes.end()));
...
void HartreeFock::Solve() {
  ...
  for (iterations = 0; iterations < maxiter; ++iterations) {
    ...
    ReorderCoefficients();  // Reorder columns of C so we can properly identify
                            // the hole orbits.

    // This bit is occationally the source of non-convergence of the iterations
    if (iterations > 10 and (iterations % fill_modulus) == 0 and
        not freeze_occupations) {
      FillLowestOrbits();  // if we don't freeze the occupations, then calculate
                           // the new ones.
      fill_modulus = fill_modulus % 5;
      fill_modulus +=
          1;  // to avoid cycles, we keep increasing the interval between
              // checking if we should change the occupations.
    }

    if (iterations == 500) {
      density_mixing_factor = 0.7;
      field_mixing_factor = 0.5;
      std::cout << "Still not converged after 500 iterations. Setting "
                   "density_mixing_factor => "
                << density_mixing_factor << " field_mixing_factor => "
                << field_mixing_factor << std::endl;
    }
    if (iterations > 600 and
        iterations % 10 ==
            3)  // if we've made it to 600, really put on the brakes with a big
                // mixing factor, with random noise
    {
      field_mixing_factor = 1 - 0.005 * (std::rand() % 100);
      density_mixing_factor = 1 - 0.005 * (std::rand() % 100);
    }
    if (iterations ==
        800)  // Things are really not healthy at this point. Most likely we're
              // stuck in a occupation changing circle.
    {
      std::cout << "Not converged after 800 iterations. That's it, I'm "
                   "freezing the occupations."
                << std::endl;
      freeze_occupations = true;
    }

    if (iterations > 100 and
        (DIIS_error_mats.size() < 1 or
         arma::norm(DIIS_error_mats.back(), "fro") > 0.01)) {
      if (iterations == 100) {
        std::cout
            << "Still not converged after 100 iterations. Switching to DIIS."
            << std::endl;
      ...
      if (arma::norm(DIIS_error_mats.back(), "fro") < 0.01) {
        std::cout << "DIIS error matrix below 0.01, switching back to simpler "
                     "SCF algorithm."
                  << std::endl;
      }
    } else {
      arma::mat last_rho = rho;
      UpdateDensityMatrix();  // Update the 1 body density matrix, used in
                              // UpdateF()
      rho = (1.0 - density_mixing_factor) * rho +
            density_mixing_factor * last_rho;
    }
    arma::mat last_F = F;
  ...
  if (iterations < maxiter) {
    std::cout << "HF converged after " << iterations << " iterations. "
              << std::endl;
  } else {
    std::cout << "!!!! Warning: Hartree-Fock calculation didn't converge after "
              << iterations << " iterations." << std::endl;
    std::cout << "!!!! Last " << convergence_ediff.size()
              << " points in convergence check:";
...
///                    V_{\alpha\beta\alpha\beta}
///                    + \frac{1}{6}\sum_{\alpha\beta\gamma}
///                    V_{\alpha\beta\gamma\alpha\beta\gamma} \\
///    &=& \sum_{ij} (2j_i+1) \rho_{ij} ( t_{ij}
///    +\frac{1}{2}\tilde{V}^{(2)}_{ij} + \frac{1}{6}\tilde{V}^{(3)}_{ij} )
/// \f}
/// Where the matrices \f{eqnarray*}
///  \tilde{V}^{(2)}_{ij} &=& \sum_{ab} \rho_{ab}\bar{V}^{(2)}_{iajb} \\
///  \tilde{V}^{(3)}_{ij} &=& \sum_{abcd} \rho_{ab}\rho_{cd}
///  \bar{V}^{(3)}_{iacjbd} \\
/// \f}
...
//**************************************************************************************
/// Print out the Hartree Fock energy, and the 1-, 2-, and 3-body contributions
/// to it.
//**************************************************************************************
...
/// for convergence.
/// Submatrices corresponding to different channels are diagonalized
/// independently. This guarantees that J,Tz, and \f$ \pi \f$ remain good.
//*********************************************************************
void HartreeFock::Diagonalize() {
  //  std::cout << __func__ << " begin " << std::endl;
  prev_energies = energies;
  for (auto& it : Hbare.OneBodyChannels) {
    arma::uvec orbvec(std::vector<index_t>(
        it.second.begin(),
        it.second.end()));  // convert std::set to std::vector, and convert that
                            // to arma::uvec...
    ...
    while (not success) {
      ...
      if (diag_tries > 5) {
        std::cout << "Hartree-Fock: Failed to diagonalize the submatrix "
                  ...
                  << ". The submatrix looks like:" << std::endl;
        F_ch.print();
        break;
      }
    }
...
///   = \sqrt{(1+\delta_{ab})(1+\delta_{cd})} \sum_{J} (2J+1) \langle ab |
///   V^{(2)} | cd \rangle_{J} \f]
    ...
    for (int parity = 0; parity <= 1; ++parity) {
      ...
      for (auto& itbra : modelspace->MonopoleKets[Tz + 1][parity]) {
        ...
        for (auto& itket : modelspace->MonopoleKets[Tz + 1][parity]) {
          ...
          int c = ket.p;
          int d = ket.q;
          Vmon[Tz + 1][parity](itbra.second, itket.second) =
              Hbare.TwoBody.GetTBMEmonopole(a, b, c, d) * norm;
          Vmon_exch[Tz + 1][parity](itbra.second, itket.second) =
              Hbare.TwoBody.GetTBMEmonopole(a, b, d, c) * norm;
        }
      }
...
#pragma omp atomic write
  ...
  std::cout << "HartreeFock::BuildMonopoleV3  storing " << Vmon3.size()
            << " doubles for Vmon3 and " << Vmon3_keys.size()
            << " uint64's for Vmon3_keys." << std::endl;

  profiler.timer[std::string(__func__)] += omp_get_wtime() - start_time;
}

//*********************************************************************
/// Construct the V3 monopole matrix elements directly from the jacobi matrix
/// elements. So far this is completely untested and certainly wrong.
//*********************************************************************
  ...
  for (auto& obc_a : Hbare.OneBodyChannels) {
    ...
    for (auto& obc_b : Hbare.OneBodyChannels) {
      ...
      for (auto& obc_c : Hbare.OneBodyChannels) {
        ...
        // The T coefficents in this sub-block can be labeled by: na,nb,nc, Jab,
        // twoJ, E12, alpha, Ncm, Lcm
        std::unordered_map<size_t, double> Ttable;
        // We'll need one loop to allocate (not thread-safe), one loop to
        // compute the Tcoefficients (can be parallel), and one to compute V3mon
        // (also parallel). first, allocate
                ...
                for (auto c : obc_c.second) {
                  ...
                  for (int Lcm = 0; Lcm <= Lcm_max; Lcm++) {
                    for (int Ncm = 0; 2 * Ncm <= Eabc - Lcm; Ncm++) {
                      ...
                      int twoT = 1;
                      int alpha_dim =
                          jacobi3bme->GetDimensionNAS(twoT, twoJ, E12 % 2, E12);
                      for (int alpha = 0; alpha < alpha_dim; alpha++) {
                        auto key = Vmon3JacobiHash(oa.n, ob.n, oc.n, Jab, twoJ,
                                                   E12, alpha, Ncm, Lcm);
                        Ttable[key] = 0;  // make sure there's somewhere to put
                                          // the value when we calculate it.
                      }                   // for alpha
                    }                     // for Ncm
                  }                       // for Lcm
                }                         // for c
        ...
        for (size_t ikey = 0; ikey < keyvector.size(); ikey++) {
          ...
          jacobi3bme->GetJacobiStates(twoT, twoJ, E12 % 2, E12, alpha, jac1,
                                      jac2);
          Orbit& oa = modelspace->GetOrbit(modelspace->GetOrbitIndex(
              na, obc_a.first[0], obc_a.first[1], obc_a.first[2]));
          Orbit& ob = modelspace->GetOrbit(modelspace->GetOrbitIndex(
              nb, obc_b.first[0], obc_b.first[1], obc_b.first[2]));
          Orbit& oc = modelspace->GetOrbit(modelspace->GetOrbitIndex(
              nc, obc_c.first[0], obc_c.first[1], obc_c.first[2]));
          Ket3 ket3(oa, ob, oc);
          double Tcoef = jacobi3bme->Tcoeff_wrapper(ket3, Jab, twoJ, jac1, jac2,
                                                    twoJ, Ncm, Lcm);
          Ttable[key] = Tcoef;
        }  // for ikey
        // and now that we have all the relevant T coefficients computed, let's
        // get all the 3-body monopole terms we can again parallelize here
        //         size_t asize = obc_a.second.size();
        //         for ( auto a : obc_a.second )
        //         #pragma omp parallel for schedule(dynamic,1)
        //         for ( size_t ia=0; ia<asize; ia++)  // do it this way because
        //         omp doesn't play nice with range-based for loops
        for (auto a : obc_a.second) {
                ...
                for (auto e : obc_b.second) {
                  ...
                  for (auto f : obc_c.second) {
                    ...
                    for (int Jab = Jab_min; Jab <= Jab_max; Jab++) {
                      ...
                      int twoJ_max = (2 * Jab - j2c);
                      for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2) {
                        for (int Ncm = 0; 2 * Ncm <= std::min(Eabc, Edef);
                             Ncm++) {
                          for (int Lcm = 0;
                               Lcm <= std::min(Eabc, Edef) - 2 * Ncm;
                               Lcm += 2) {
                            int E12_abc = Eabc - 2 * Ncm - Lcm;
                            int E12_def = Edef - 2 * Ncm - Lcm;
                            if ((E12_abc + Eabc + Lcm) % 2 > 0)
                              continue;  // check parity
                            int twoT = 1;
                            size_t alpha_dim_abc = jacobi3bme->GetDimensionNAS(
                                twoT, twoJ, E12_abc % 2, E12_abc);
                            size_t alpha_dim_def = jacobi3bme->GetDimensionNAS(
                                twoT, twoJ, E12_def % 2, E12_def);
                            for (size_t alpha_abc = 0;
                                 alpha_abc < alpha_dim_abc; alpha_abc++) {
                              auto key_abc =
                                  Vmon3JacobiHash(oa.n, ob.n, oc.n, Jab, twoJ,
                                                  E12_abc, alpha_abc, Ncm, Lcm);
                              double T_abc = Ttable[key_abc];
                              for (size_t alpha_def = 0;
                                   alpha_def < alpha_dim_def; alpha_def++) {
                                auto key_def = Vmon3JacobiHash(
                                    od.n, oe.n, of.n, Jab, twoJ, E12_def,
                                    alpha_def, Ncm, Lcm);
                                double T_def = Ttable[key_def];
                                double me_jacobi_NAS = jacobi3bme->GetMatElNAS(
                                    alpha_abc, alpha_def, E12_abc, E12_def,
                                    twoJ, twoT, E12_abc % 2);
                                vmon3 +=
                                    (twoJ + 1) * T_abc * me_jacobi_NAS * T_def;
                              }
                            }
                          }
                        }
                    ...
                    // Probably should think of a more clever way to do the
                    // lookup here
                    auto key = Vmon3Hash(a, b, c, d, e, f);
                    auto ptr =
                        std::find(Vmon3_keys.begin(), Vmon3_keys.end(), key);
...
//*********************************************************************
/// Hashing function for rolling six orbit indices into a single long unsigned
/// int. Each orbit gets 10 bits.
//*********************************************************************
uint64_t HartreeFock::Vmon3Hash(uint64_t a, uint64_t b, uint64_t c, uint64_t d,
...
//*********************************************************************
// void HartreeFock::ParseVmon3HashKey(uint64_t key, int& a, int& b, int& c,
// int& d, int& e, int& f)
void HartreeFock::Vmon3UnHash(uint64_t key, int& a, int& b, int& c, int& d,
                              int& e, int& f) {
  a = (key)&0x3FFL;  // The L means long, and 0x3FF is 1's in the lowest 10
                     // bits.
  b = (key >> 10) & 0x3FFL;
  c = (key >> 20) & 0x3FFL;
  d = (key >> 30) & 0x3FFL;
...
/// Each n value get 5 bits, which means they should be less than 32 (emax=64,
/// probably ok for the near future) Jab gets 6 bits, twoJ gets 7 bits, Lcm gets
/// 7 bits, Ncm gets 6 bits, E12 gets 7 bits leaving 16 bits for alpha, which is
/// hopefully (?) enough.
//*********************************************************************
uint64_t HartreeFock::Vmon3JacobiHash(uint64_t na, uint64_t nb, uint64_t nc,
                                      uint64_t Jab, uint64_t twoJ, uint64_t E12,
                                      uint64_t alpha, uint64_t Ncm,
                                      uint64_t Lcm) {
  return (alpha + (na << 16) + (nb << 21) + (nc << 26) + (Jab << 31) +
          ((twoJ / 2) << 37) + (E12 << 44) + (Ncm << 51) + (Lcm << 57));
}

//*********************************************************************
/// Take a hashed key and extract the six orbit indices that went into it.
//*********************************************************************
void HartreeFock::Vmon3JacobiUnHash(uint64_t key, uint64_t& na, uint64_t& nb,
                                    uint64_t& nc, uint64_t& Jab, uint64_t& twoJ,
                                    uint64_t& E12, uint64_t& alpha,
                                    uint64_t& Ncm, uint64_t& Lcm) {
...
// See P Pulay J. Comp. Chem. 3(4) 556 (1982), and  Garza & Scuseria J. Chem.
// Phys. 137 054110 (2012)
void HartreeFock::UpdateDensityMatrix_DIIS() {
  size_t N_MATS_STORED =
      5;  // How many past density matrices and error matrices to store
  ...
  if (DIIS_density_mats.size() > N_MATS_STORED)
    DIIS_density_mats.pop_front();   // out with the old
  ...
  if (nsave < N_MATS_STORED)
    return;  // check that have enough previous rhos and errors stored to do
             // this
  ...
  for (size_t i = 0; i < nsave; i++) {
    for (size_t j = 0; j < nsave; j++) {
      Bij(i, j) =
          arma::norm(DIIS_error_mats[i] * DIIS_error_mats[j].t(), "fro");
    }
  }
...
void HartreeFock::FillLowestOrbits() {
  ...
  for (auto i : sorted_indices) {
    ...
    if (oi.tz2 < 0 and (placedZ < refereceZ)) {
      ...
      hole_occ_tmp.push_back(
          std::min(1.0, double(refereceZ - placedZ) / (oi.j2 + 1.)));
      placedZ = std::min(placedZ + oi.j2 + 1., refereceZ);
    } else if (oi.tz2 > 0 and (placedN < refereceN)) {
      holeorbs_tmp.push_back(i);
      hole_occ_tmp.push_back(
          std::min(1.0, double(refereceN - placedN) / (oi.j2 + 1.)));
...
//*********************************************************************
///  [See Suhonen eq 4.85]
/// \f[ F_{ij} = t_{ij}  +  \frac{1}{2j_i+1}\sum_{ab} \rho_{ab}
/// \bar{V}^{(2)}_{iajb}
///               + \frac{1}{2(2j_i+1)}\sum_{abcd}\rho_{ab} \rho_{cd}
///               \bar{V}^{(3)}_{iacjbd}  \f]
...
/// * \f$ \bar{V}^{(2)} \f$ is the monopole component of the 2-body interaction
/// defined in BuildMonopoleV().
/// * \f$ \bar{V}^{(3)} \f$ is the monopole component of the 3-body interaction
/// devined in BuildMonopoleV3().
//*********************************************************************
void HartreeFock::UpdateF() {
  ...
  // This loop isn't thread safe for some reason. Regardless, parallelizing it
  // makes it slower.
...
#pragma omp parallel for schedule(dynamic, 1)
      ...
      for (auto j : chvec) {
        ...
        for (auto a : modelspace->all_orbits) {
          ...
          for (auto b : Hbare.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
            ...
            // 2body term <ai|V|bj>
            if ((a > i) xor (b > j))  // code needed some obfuscation, so threw
                                      // an xor in there...
            {
              Vij(i, j) +=
                  rho(a, b) *
                  Vmon_exch[Tz + 1][parity](
                      local_bra, local_ket);  // <a|rho|b> * <ai|Vmon|jb>
            } else {
              Vij(i, j) +=
                  rho(a, b) *
                  Vmon[Tz + 1][parity](local_bra,
                                       local_ket);  // <a|rho|b> * <ai|Vmon|bj>
            }
          }
        }
      }
  ...
  if (Hbare.GetParticleRank() >= 3) {
    // it's just a one-body matrix, so we can store different copy for each
    // thread.
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    V3vec.clear();  // free up the memory (is this at all necessary, since it
                    // goes out of scope?)
  }
...
/// between iterations.
/// Converged when
/// \f[ \delta_{e} \equiv \sqrt{ \sum_{i}(e_{i}^{(n)}-e_{i}^{(n-1)})^2} <
/// \textrm{tolerance} \f] where \f$ e_{i}^{(n)} \f$ is the \f$ i \f$th
/// eigenvalue of the Fock matrix after \f$ n \f$ iterations.
///
//********************************************************
...
/// the l,j,tz sub-blockes of the matrix C to be energy-ordered and positive
/// along the diagonal. For a 3x3 matrix this would be something like (this
/// needs to be updated) \f[ \left( \begin{array}{rrr}
///   -0.8 & 0.2 & -0.6 \\
///   -0.3 & 0.3 &  0.9 \\
///    0.2 & 0.9 & -0.4 \\
/// \end{array}\right)
...
void HartreeFock::ReorderCoefficients() {
  for (auto& it : Hbare.OneBodyChannels) {
    arma::uvec orbvec(std::vector<index_t>(
        it.second.begin(),
        it.second.end()));  // convert from std::set to std::vector, and then to
                            // arma::uvec
    arma::vec E_ch = energies(orbvec);
    int nswaps = 10;  // keep track of the number of swaps we had to do, iterate
                      // until nswaps==0
    ...
    for (index_t i = 0; i < C.n_cols;
         ++i)  // loop through original basis states
    {
      if (C(i, i) < 0)
        C.col(i) *= -1;  // C is <HO|HF>, armadillow ordering is C(row,col)  so
                         // column j is the jth HF state
    }
  }
}
...
/// \f[ D_{ab\alpha\beta} \equiv \sqrt{ \frac{1+\delta_{ab}}
/// {1+\delta_{\alpha\beta}} }  C_{a\alpha} C_{b\beta} \f] The factor in the
/// square root is due to the fact that we're using normalized TBME's. Since
/// only kets with \f$ a\leq b\f$ are stored, we can use the antisymmetry of the
/// TBME's and define \f[ D(J)_{ab\alpha\beta} \equiv \sqrt{
/// \frac{1+\delta_{ab}} {1+\delta_{\alpha\beta}} }
///      \left( C_{a\alpha} C_{b\beta} -(1-\delta_{ab})(-1)^{j_a+j_b-J}
///      C_{b\alpha}C_{a\beta}\right) \f]
///
//**************************************************************************
Operator HartreeFock::TransformToHFBasis(Operator& OpHO) {
  ...
  if (OpHF.legs % 2 == 0 and OpHO.legs > 3) {
    for (auto& it : OpHO.TwoBody.MatEl) {
      ...
      // loop over all possible original basis configurations <pq| in this
      // J,p,Tz channel. and all possible HF configurations |p'q'> in this
      // J,p,Tz channel bra is in the original basis, ket is in the HF basis i
      // and j are the indices of the matrix D for this channel
      for (int i = 0; i < nkets; ++i) {
        ...
        for (int j = 0; j < nkets; ++j) {
          ...
          if (ket_ho.p != ket_ho.q) {
            Dket(i, j) += C(ket_ho.q, ket_hf.p) * C(ket_ho.p, ket_hf.q) *
        ...
        for (int i = 0; i < nbras; ++i) {
          ...
          for (int j = 0; j < nbras; ++j) {
            ...
            if (bra_ho.p != bra_ho.q) {
              Dbra(i, j) += C(bra_ho.q, bra_hf.p) * C(bra_ho.p, bra_hf.q) *
...
//**************************************************************************
/// If the lowest orbits are different from our previous guess, we should update
/// the reference.
//**************************************************************************
void HartreeFock::UpdateReference() {
  ...
  for (index_t i = 0; i < holeorbs.size(); ++i) {
    ...
    if (std::abs(modelspace->GetOrbit(holeorbs[i]).occ - hole_occ[i]) > 1e-3) {
      ...
      std::cout << "After HF, occupation of orbit " << holeorbs[i]
                << " has changed. Modelspace will be updated." << std::endl;
    }
  }
...
Operator HartreeFock::GetNormalOrderedH(arma::mat& Cin, int particle_rank) {
  C = Cin;
  //  ReorderCoefficients();  // Reorder columns of C so we can properly
  //  identify the hole orbits.
...
//**************************************************************************
/// Returns the normal-ordered Hamiltonian in the Hartree-Fock basis, neglecting
/// the residual 3-body piece. \f[ E_0 = E_{HF} \f] \f[ f = C^{\dagger} F C \f]
/// \f[ \Gamma = D^{\dagger} \left(V^{(2)}+V^{(3\rightarrow 2)} \right) D \f]
/// \f[ V^{(2\rightarrow 3)J}_{ijkl} \equiv
/// \frac{1}{\sqrt{(1+\delta_{ij})(1+\delta_{kl})}}\sum_{ab}\sum_{J_3}(2J_{3}+1)\rho_{ab}V^{JJJ_{3}}_{ijaklb}
/// \f] Where \f$ F\f$ is the Fock matrix obtained in UpdateF() and the matrix
/// \f$ D\f$ is the same as the one defined in TransformToHFBasis().
///
//**************************************************************************
...
#pragma omp parallel for schedule( \
    dynamic, 1)  // confirmed that this improves performance
    for (int i = 0; i < npq; ++i) {
      ...
      for (int j = 0; j < npq; ++j) {
        ...
        for (auto a : modelspace->all_orbits) {
          ...
          for (int b : Hbare.OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
            ...
            if (std::abs(rho(a, b)) < 1e-8)
              continue;  // Turns out this helps a bit (factor of 5 speed up in
                         // tests)
            V3NO(i, j) += rho(a, b) * Hbare.ThreeBody.GetME_pn_no2b(
                                          bra.p, bra.q, a, ket.p, ket.q, b, J);
          }
        }
        V3NO(i, j) /= (2 * J + 1);
    ...
    // This option is just for running diagnostics. No reason to use it for
    // production runs
    if (discard_NO2B_from_3N) {
      std::cout << " ====  Discarding NO2B contribution from 3N. Is that what "
                   "you want? ===== "
                << std::endl;
      V3NO *= 0;
    }
...
//**************************************************************************
/// Get the one-body generator corresponding to the transformation to the HF
/// basis. Since the unitary transformation for HF is given by the \f$ U_{HF} =
/// C^{\dagger} \f$ matrix, we have \f$ e^{-\Omega} = C \Rightarrow \Omega =
/// -\log(C) \f$. The log is evaluated by diagonalizing the one-body submatrix
/// and taking the log of the diagonal entries. This is much slower than the
/// other methods, but it might be useful for debugging/benchmarking.
//**************************************************************************
Operator HartreeFock::GetOmega() {
  ...
  for (auto& it : Hbare.OneBodyChannels) {
    ...
    Omega.OneBody.submat(orbvec, orbvec) =
        -arma::real(eigvect * arma::diagmat(arma::log(eigval)) * eigvect.t());
  }
  return Omega;
}
...
void HartreeFock::PrintSPE() {
  ...
  for (auto i : modelspace->all_orbits) {
    Orbit& oi = modelspace->GetOrbit(i);
    std::cout << std::fixed << std::setw(3) << oi.n << " " << std::setw(3)
              << oi.l << " " << std::setw(3) << oi.j2 << " " << std::setw(3)
              << oi.tz2 << "   " << std::setw(10) << F_hfbasis(i, i)
              << std::endl;
  }
}
...
void HartreeFock::PrintSPEandWF() {
  ...
  for (auto i : modelspace->all_orbits) {
    Orbit& oi = modelspace->GetOrbit(i);
    std::cout << std::fixed << std::setw(3) << i << ": " << std::setw(3) << oi.n
              << " " << std::setw(3) << oi.l << " " << std::setw(3) << oi.j2
              << " " << std::setw(3) << oi.tz2 << "   " << std::setw(12)
              << std::setprecision(6) << F_hfbasis(i, i) << " " << std::setw(12)
              << oi.occ << "   | ";
    for (int j : Hbare.OneBodyChannels.at(
             {oi.l, oi.j2, oi.tz2}))  // j loops over harmonic oscillator states
    {
...
void HartreeFock::GetRadialWF(index_t index, std::vector<double>& R,
...
double HartreeFock::GetRadialWF_r(index_t index, double R) {
  ...
  for (index_t j : Hbare.OneBodyChannels.at({orb.l, orb.j2, orb.tz2})) {
    Orbit& oj = modelspace->GetOrbit(j);
    double Norm =
        2 * sqrt(AngMom::factorial(oj.n) * pow(2, oj.n + oj.l) / SQRTPI /
                 AngMom::double_fact(2 * oj.n + 2 * oj.l + 1) * pow(b, -3.0));
    psi += C(index, j) * Norm * pow(x, oj.l) * exp(-x * x * 0.5) *
...
double HartreeFock::GetHFPotential(size_t i, double r, double rprime) {
  ...
  for (auto j : Hbare.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
    for (auto k : Hbare.OneBodyChannels.at({oi.l, oi.j2, oi.tz2})) {
      Uhf += (Vij(j, k) + 0.5 * V3ij(j, k)) * GetRadialWF_r(j, r) *
             GetRadialWF_r(k, rprime);
    }
  }
  return Uhf;
}
...
double HartreeFock::GetAverageHFPotential(double r, double rprime) {
  ...
  Uhf /= 2 * (2 * modelspace->GetEmax() +
              1);  // 2 for protons/neutrons, and 2*emax+1 is number of
                   // different l,j combinations
  return Uhf;
}
...
ThreeBodyME HartreeFock::GetTransformed3B(Operator& OpIn) {
  ...
  ThreeBodyME hf3bme(modelspace, OpIn.GetJRank(), OpIn.GetTRank(),
                     OpIn.GetParity());
  hf3bme.SwitchToPN_and_discard();
  //  std::cout << __func__ << "  Norm of input 3bme in oscillator basis = " <<
  //  OpIn.ThreeBodyNorm() << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ich = 0; ich < nch3; ich++) {
    ...
    for (size_t ibra = 0; ibra < nbras; ibra++) {
      ...
      int tz2j = bra.oq->tz2;
      int tz2k = bra.oR->tz2;
      if ((std::abs(ei - e_fermi[tz2i]) + std::abs(ej - e_fermi[tz2j]) +
           std::abs(ek - e_fermi[tz2k])) > modelspace->GetdE3max())
        continue;
    ...
    // Potentially worth exploring if any improvement is made by using sparse
    // matrices
      ...
      for (size_t indxHO = 0; indxHO < nbras_kept; indxHO++) {
        ...
        for (auto perm3b : OpIn.ThreeBody.UniquePermutations(iHO, jHO, kHO)) {
          ...
          if (std::abs(overlap) > 1e-8) {
            double phase = OpIn.ThreeBody.PermutationPhase(
                perm3b);  // the fermionic sign from the permutation
            double recouple = OpIn.ThreeBody.RecouplingCoefficient(
                perm3b, ji, jj, jk, JijHF, JijHO,
                twoJ);  // the angular momentum recoupling coefficient
    ...
    for (size_t indx_bra = 0; indx_bra < nbras_kept; indx_bra++) {
      ...
      for (size_t indx_ket = 0; indx_ket < nkets_kept; indx_ket++) {
        ...
        Vho(indx_bra, indx_ket) = OpIn.ThreeBody.GetME_pn(
            JijHO, JlmHO, twoJ, iHO, jHO, kHO, lHO, mHO, nHO);
      }
    }
   ...
   //        if (ich < 3)
   //        {
   //        double VHO = OpIn.ThreeBody.GetME_pn( Jij,  Jlm, twoJ,
   i,j,k,l,m,n);
   //          std::cout << " ich = " << ich << " ibra,iket " << ibra << " " <<
   iket << "   VHO VHF = " << VHO << " " << VHF
   //                    << "  ijklmn = " << i << " " << j << " " << k << " " <<
   l << " " << m << " " << n << "   "
   //                    << "Jij Jlm twoJ = " << Jij << " " << Jlm << " " <<
   twoJ << std::endl;
   //        }
  ...
  //  std::cout << __func__ << "   After transforming norm is " << hf3bme.Norm()
  //  << std::endl;
...
ThreeBodyME HartreeFock::GetValence3B(Operator& OpIn, int emax, int E3max) {
  ...
  for (size_t ch3 = 0; ch3 < nch3; ch3++) {
    ...
    for (size_t ibra = 0; ibra < nkets; ibra++) {
      ...
      if ((bra.op->cvq != 1) or (bra.oq->cvq != 1) or (bra.oR->cvq != 1))
        continue;
      size_t a = bra.p;
      size_t b = bra.q;
      size_t c = bra.r;
      ...
      for (size_t iket = 0; iket < nkets; iket++) {
        ...
        if ((ket.op->cvq != 1) or (ket.oq->cvq != 1) or (ket.oR->cvq != 1))
          continue;
        size_t d = ket.p;
        size_t e = ket.q;
        size_t f = ket.r;
  ...
  IMSRGProfiler::timer["HartreeFock::GetValence3B"] +=
...
double HartreeFock::GetTransformed3bme(Operator& OpIn, int Jab, int Jde, int J2,
  ...
  for (auto alpha : modelspace->OneBodyChannels.at({oa.l, oa.j2, oa.tz2})) {
    ...
    for (auto beta : modelspace->OneBodyChannels.at({ob.l, ob.j2, ob.tz2})) {
      ...
      for (auto gamma : modelspace->OneBodyChannels.at({oc.l, oc.j2, oc.tz2})) {
        ...
        for (auto delta :
             modelspace->OneBodyChannels.at({od.l, od.j2, od.tz2})) {
          if (std::abs(C(delta, d)) < 1e-8) continue;
          for (auto epsilon :
               modelspace->OneBodyChannels.at({oe.l, oe.j2, oe.tz2})) {
            if (std::abs(C(epsilon, e)) < 1e-8) continue;
            for (auto phi :
                 modelspace->OneBodyChannels.at({of.l, of.j2, of.tz2})) {
              double V_ho = OpIn.ThreeBody.GetME_pn(Jab, Jde, J2, alpha, beta,
                                                    gamma, delta, epsilon, phi);
              V_hf += V_ho * C(alpha, a) * C(beta, b) * C(gamma, c) *
                      C(delta, d) * C(epsilon, e) * C(phi, f);
