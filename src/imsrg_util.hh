...
//#define M_ELECTRON 0.5109989461 // I take all my physical constants from
//Wikipedia.
//
//#ifndef ISQRT2
...
// DaggerOperator DaggerOperatorFromString(ModelSpace& modelspace, std::string
// str);
...
//////////////////
Operator M0nu_TBME_Op(
    ModelSpace& modelspace, int Nquad,
    std::string src);  // put in by CP, it is still in development
double CPrbmeGen(ModelSpace& modelspace, double rho, double x, int n, int l,
                 int np, int lp, int mm, double pp);  // testing...
// inline double cpNorm(int a, int b) {return a==b ? ISQRT2 : 1.0;}; // put in
// by CP, for anti-symmetrization normalization
inline double cpNorm(int a, int b) {
  return a == b ? INVSQRT2 : 1.0;
};  // put in by CP, for anti-symmetrization normalization
    // SRS - the following three don't seem to be used.
// double CPrbme(ModelSpace& modelspace, double x, int n, int l, int np); // put
// in by CP, function for M0nu_TBME_Op above Operator CPchecker_Op(ModelSpace&
// modelspace); // put in by CP Operator CharlieSdS_Op(ModelSpace& modelspace);
// // put in by CP Operator RagnarSdS_Op(ModelSpace& modelspace); // put in by
// CP
...
Operator SerberTypePotential(ModelSpace& modelspace, double V0, double mu,
                             ...
                             double D);  // not yet implemented
double SerberMatEl(ModelSpace& modelspace, Ket& bra, Ket& ket, int J,
                   const std::array<double, 6>& params);
Operator SurfaceDeltaInteraction(ModelSpace& modelspace, double V0,
                                 double R);  // not yet implemented

Operator BareDelta(ModelSpace& modelspace);

Operator Single_Ref_1B_Density_Matrix(
    ModelSpace& modelspace);                        // This doesn't work
...
std::vector<double> GetDensity(std::vector<double>& occ, std::vector<double>& R,
                               std::vector<int>& orbits,
                               ModelSpace& modelspace);

void WriteSPWaveFunctions(std::vector<std::string>& sporbits, HartreeFock& hf,
                          std::string intfile);
...
// Templated functions need to be defined in the header file (or else explicitly
// declared in the .cc file).
template <typename T>
...
template <typename T, typename... Args>
std::set<T> VectorUnion(const std::set<T>& s1, const std::set<T>& s2,
                        Args... args) {
  std::set<T> s3 = s1;
