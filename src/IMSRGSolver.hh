...
class IMSRGSolver {
 public:
  ...
  //  void SetReadWrite( ReadWrite& r){rw = &r; scratchdir =
  //  rw->GetScratchDir();};
  ...
  void SetDenominatorDeltaIndex(int i) {
    generator.SetDenominatorDeltaIndex(i);
  };
  void SetDenominatorDeltaOrbit(std::string o) {
    generator.SetDenominatorDeltaOrbit(o);
  };
  ...
  //  void operator()( const vector<Operator>& x, vector<Operator>& dxdt, const
  //  double t);
  void operator()(const std::deque<Operator>& x, std::deque<Operator>& dxdt,
                  const double t);
  void Solve_ode();
