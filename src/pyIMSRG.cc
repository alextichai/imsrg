...
//  self.GetOrbitIndex(s);};
TwoBodyChannel MS_GetTwoBodyChannel(ModelSpace& self, int ch) {
...
size_t TBCGetLocalIndex(TwoBodyChannel& self, int p, int q) {
  return self.GetLocalIndex(p, q);
};

//  void ArmaMatPrint( arma::mat& self){ self.print();};
void OpSetOneBodyME(Operator& self, int i, int j, double v) {
  self.OneBody(i, j) = v;
};

//  void MS_SetRef(ModelSpace& self, std::string str){ self.SetReference(
//  str);};
void MS_SetRef(ModelSpace& self, const std::set<index_t>& ref) {
  self.SetReference(ref);
};

//  Operator HF_GetNormalOrderedH(HartreeFock& self){ return
//  self.GetNormalOrderedH();};
Operator HF_GetNormalOrderedH(HartreeFock& self, int particle_rank = 2) {
  return self.GetNormalOrderedH(particle_rank);
};
...
PYBIND11_MODULE(pyIMSRG, m) {
  ...
  py::class_<Operator>(m, "Operator")
      ...
      .def(py::init<ModelSpace&, int, int, int, int>(), py::arg("modelspace"),
           py::arg("j_rank"), py::arg("parity"), py::arg("t_rank"),
  ...
  py::class_<IMSRGSolver>(m, "IMSRGSolver")
      ...
      .def("SetDenominatorPartitioning",
           &IMSRGSolver::SetDenominatorPartitioning)
      ...
      .def_readwrite(
          "n_omega_written",
          &IMSRGSolver::n_omega_written)  // I'm not sure I like just directly
                                          // exposing this...
      ;
  ...
  Commutator.def("Set_BCH_Transform_Threshold",
                 &Commutator::Set_BCH_Transform_Threshold);
  Commutator.def("Set_BCH_Product_Threshold",
                 &Commutator::Set_BCH_Product_Threshold);
