...
std::map<std::string, size_t> IMSRGProfiler::CheckMem() {
  ...
  std::ifstream ifs(
      "/proc/self/statm");  // this is a pseudo file that provides information
                            // on the current process (linux only)
  if (ifs.good()) {
    size_t npages;
    size_t page_size = sysconf(_SC_PAGESIZE);
    ifs >> npages >> npages;
    s["RSS"] =
        npages * page_size / 1024;  // divide by 1024 to convert to kilobytes
  }

  /*
  //#if defined(CHECKMEM) && ! defined(__APPLE__)
  //  sprintf(commandstr,"pmap -x %d | tail -1",getpid()); // TODO make this
  more portable. On OSX, use vmmap. no idea for Windows... std::ostringstream
  commandstr; commandstr << "pmap -x " << getpid() << " | tail -1";
  //#ifndef __APPLE__
  //  FILE* output = popen(commandstr,"r");
    FILE* output = popen(commandstr.str().c_str(),"r");
    if (output==NULL or fgets(outbuf,500,output) == NULL)
      std::cout << " <<< IMSRGProfiler::CheckMem():  Problem reading output of
  pmap (pid = " << getpid() << ")" << std::endl; else std::istringstream(outbuf)
  >> buf >> buf >> s["Kbytes"] >> s["RSS"] >> s["DIRTY"];
  //    std::istringstream(outbuf) >> commandstr >> buf >> s["Kbytes"] >>
  s["RSS"] >> s["DIRTY"];
  */
...
void IMSRGProfiler::PrintTimes() {
  ...
  for (auto it : timer) {
    ...
    std::cout << std::setw(40) << std::left << it.first + ":  " << std::setw(12)
              << std::setprecision(5) << std::right << it.second;
    std::cout << " (" << std::setw(4) << std::setprecision(1)
              << 100 * it.second / time_tot["real"] << "%) |";
  ...
  for (auto it : time_tot)
    std::cout << std::setw(40) << std::left << it.first + ":  " << std::setw(12)
              << std::setprecision(5) << std::right << it.second << std::endl;
}
...
void IMSRGProfiler::PrintCounters() {
  ...
  for (auto it : counter)
    std::cout << std::setw(40) << std::left << it.first + ":  " << std::setw(12)
              << std::setprecision(0) << std::right << it.second << std::endl;
}
...
void IMSRGProfiler::PrintMemory() {
  ...
  for (auto it : CheckMem())
    std::cout << std::fixed << std::setw(40) << std::left << it.first + ":  "
              << std::setw(12) << std::setprecision(3) << std::right
              << it.second / 1024. << std::endl;

  std::cout << std::fixed << std::setw(40) << std::left
            << "Max Used:  " << std::setw(12) << std::setprecision(3)
            << std::right << MaxMemUsage() / 1024. << std::endl;
}
