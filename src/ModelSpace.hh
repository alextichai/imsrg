...
struct Orbit {
  ...
  double occ_nat;  // occupation in the natural orbital basis (if we use natural
                   // orbitals) particle=0, hole=1
  ...
  bool operator==(const Orbit& rhs) const {
    return (n == rhs.n and l == rhs.l and j2 == rhs.j2 and tz2 == rhs.tz2);
  };
};
...
struct Ket  //  | pq >
{
  ...
  // Methods
  Ket(){};
  Ket(Orbit& op_in, Orbit& oq_in)
      : op(&op_in),
        oq(&oq_in),
        p(op_in.index),
        q(oq_in.index),
        dpq(op_in.index == oq_in.index ? 1 : 0) {
...
struct Ket3  // | p q r >
{
  ...
  Orbit* oq;
  Orbit*
      oR;  // unfortunate that this convention collides with the "or" keyword.
  int Jpq;
  ...
  bool operator==(Ket3& rhs) {
    return (p == rhs.p and q == rhs.q and r == rhs.r and Jpq == rhs.Jpq);
  };
...
struct TwoBodyChannel {
  ...
  std::vector<int> KetList;  // eg [2, 4, 7, ...] Used for looping over all the
                             // kets in the channel
  std::vector<int> KetMap;   // eg [ -1, -1, 0, -1, 1, -1, -1, 2 ...] Used for
                             // asking what is the local index of this ket. -1
                            // means the ket doesn't participate in this channel
  ...
  size_t GetLocalIndex(int ketindex) const {
    return KetMap[ketindex];
  };  // modelspace ket index => local ket index
  size_t GetLocalIndex(int p, int q) const;
  size_t GetKetIndex(int i) const {
    return KetList[i];
  ...
  virtual bool CheckChannel_ket(Orbit* op, Orbit* oq)
      const;  // check if |pq> participates in this channel
  //   bool CheckChannel_ket(Orbit* op, Orbit* oq) const;  // check if |pq>
  //   participates in this channel
  bool CheckChannel_ket(Ket& ket) const {
    return CheckChannel_ket(ket.op, ket.oq);
...
class TwoBodyChannel_CC : public TwoBodyChannel {
 public:
  ...
  bool CheckChannel_ket(Orbit* op, Orbit* oq)
      const;  // check if |pq> participates in this channel
...
class ModelSpace {
 public:
  ...
  int EmaxUnocc;      // Separate emax cut for orbits with l,j,tz that aren't
                      // present in the HF reference

  double
      dE3max;  //  cut on three-body configurations which are considered in the
               //  IMSRG(3) commutators, taken relative to the fermi energy.
  double occnat3cut;  //  cut on three-body configurations which are considered
                      //  in the IMSRG(3) commutators, taken relative to the
                      //  fermi energy.
  std::map<int, double>
      e_fermi;  // The fermi energy, probably in oscillator units. It's
                // different for protons and neutrons, so index by tz

  int norbits;  // number of single-particle orbits (not counting the degenerate
                // m-projections)
  ...
  double Acore;  // Average particle number of the core of the valnce space
                 // (this should probably always be an integer)
  ...
  std::map<std::array<int, 3>, std::set<index_t>>
      OneBodyChannels;  // map that takes l,j,tz and gives a list of indicies of
                        // orbits with those quantum numbers

  int nTwoBodyChannels;  // number of two body channels J,parity,Tz
  std::vector<TwoBodyChannel>
      TwoBodyChannels;  // vector of TwoBodyChannel structs
  std::vector<TwoBodyChannel_CC>
      TwoBodyChannels_CC;  // Pandya transformed channels (the CC stands for
                           // cross-coupled)

  int nThreeBodyChannels;  // number of three body channels J,parity,Tz
  std::vector<ThreeBodyChannel>
      ThreeBodyChannels;  // vector of ThreeBodyChannel structs
  std::map<size_t, size_t> Ket3IndexLookup;
  std::map<size_t, size_t> ThreeBodyChannelLookup;

  //   std::map< std::array<int,3>, std::map<
  //   std::array<size_t,2>,std::array<std::vector<size_t>,2> > > PandyaLookup;
  //   // I'm not sure how necessary this is...
  std::map<std::array<int, 3>, std::vector<size_t>>
      PandyaLookup;  // I'm not sure how necessary this is...
  std::unordered_map<size_t, index_t>
      OrbitLookup;  // Map to help look up the orbit index based on its quantum
                    // numbers
  ...
  std::set<std::array<int, 2>>
      hole_quantum_numbers;  // For checking if an orbit could mix with the hole
                             // orbits
  ...
  std::array<std::array<std::unordered_map<index_t, index_t>, 3>, 3>
      MonopoleKets;  // List of kets of a given Tz,parity
  ...
  static std::unordered_map<uint64_t, double>
      T3bList;  // Tcoefficients for transforming Jacobi 3b to lab-frame 3b

  // Constructors
  ModelSpace();
  ModelSpace(const ModelSpace&);  // copy constructor
  ModelSpace(ModelSpace&&);       // move constructor
  ModelSpace(int emax, std::vector<std::string> hole_list,
  ...
  //   void Init(int emax, std::map<index_t,double> hole_list, std::string
  //   valence);
  void Init(int emax, std::map<std::array<int, 4>, double> hole_list,
            std::string valence);
  //   void Init(int emax, std::map<index_t,double> hole_list,
  //   std::vector<index_t> core_list, std::vector<index_t> valence_list);//
  //   keep this for backward-compatibility void Init(int emax,
  //   std::map<index_t,double> hole_list, std::set<index_t> core_list,
  //   std::set<index_t> valence_list);
  void Init(std::map<std::array<int, 4>, double> hole_list,
            std::set<std::array<int, 4>> core_list,
            std::set<std::array<int, 4>> valence_list);
  void Init(int emax, std::vector<std::string> hole_list,
            std::vector<std::string> core_list,
            std::vector<std::string> valence_list);  // backward-compatibility
  void Init_occ_from_file(int emax, std::string valence, std::string occ_file);
  void InitSingleSpecies(
      int emax, std::string reference,
      std::string valence);  // Work with just one type of fermion
  ...
  std::map<std::array<int, 4>, double> GetOrbitsAZ(
  ...
  void Get0hwSpace(int Aref, int Zref, std::set<std::array<int, 4>>& core_list,
                   std::set<std::array<int, 4>>& valence_list);
  //   void Get0hwSpace(int Aref, int Zref, std::set<index_t>& core_list,
  //   std::set<index_t>& valence_list);
  void ParseCommaSeparatedValenceSpace(
      std::string valence, std::set<std::array<int, 4>>& core_list,
      std::set<std::array<int, 4>>& valence_list);
  //   void ParseCommaSeparatedValenceSpace(std::string valence,
  //   std::set<index_t>& core_list, std::set<index_t>& valence_list);

  void SetupKets();
  void Setup3bKets();
  void AddOrbit(Orbit orb);
  void AddOrbit(int n, int l, int j2, int tz2, double occ,
                int cvq);  // cvq: 0=>core, 1=>valence, 2=>qspace
  ...
  size_t GetOrbitIndex(int n, int l, int j2, int tz2) const {
    return Index1(n, l, j2, tz2);
  };
  size_t GetKetIndex(int p, int q) const {
    return Index2(p, q);
  };  // convention is p<=q
  size_t GetKetIndex(Ket* ket) const {
    return Index2(ket->p, ket->q);
  };  // convention is p<=q
  size_t GetKet3Index(int p, int q, int r, int Jpq) {
    return Ket3IndexLookup.at(Ket3IndexHash(p, q, r, Jpq));
  };
  ...
  // Two helper functions for counting the number of particles in various sets
  // of orbits, e.g. protons in the core.
  std::set<index_t> IntersectionOfSets(const std::set<index_t>& set1,
                                       const std::set<index_t>& set2) const;
  ...
  size_t GetNumberTwoBodyChannels_CC() const {
    return TwoBodyChannels_CC.size();
  };
  size_t GetNumberThreeBodyChannels() const {
    return ThreeBodyChannels.size();
  };

  TwoBodyChannel& GetTwoBodyChannel(int ch) const {
    return (TwoBodyChannel&)TwoBodyChannels[ch];
  };
  TwoBodyChannel_CC& GetTwoBodyChannel_CC(int ch) const {
    return (TwoBodyChannel_CC&)TwoBodyChannels_CC[ch];
  };
  ...
  double GetOccNat3Cut() {
    return occnat3cut;
  };  // setting this to zero or less makes no cut, setting to 0.25 cuts out
      // everything
  void SetEFermi(double ef) {
    e_fermi[-1] = ef;
    e_fermi[+1] = ef;
  };
  std::map<int, double> GetEFermi() { return e_fermi; };
  void SetEFermi(double ef_proton, double ef_neutron) {
    e_fermi[-1] = ef_proton;
    e_fermi[1] = ef_neutron;
  };

  double GetSixJ(double j1, double j2, double j3, double J1, double J2,
                 double J3);
  double GetNineJ(double j1, double j2, double j3, double j4, double j5,
                  double j6, double j7, double j8, double j9);
  double GetMoshinsky(int N, int Lam, int n, int lam, int n1, int l1, int n2,
                      int l2, int L);  // Inconsistent notation. Not ideal.
  bool SixJ_is_empty() { return SixJList.empty(); };
  double GetT3b(index_t a, index_t b, index_t c, int Jab, int J, int N1, int L1,
                int S1, int J1, int N2, int L2, int J2, int J12, int Ncm,
                int Lcm);  // 15 index object. yikes.
  ...
  void ClearVectors();
  void ResetFirstPass();
  void CalculatePandyaLookup(int rank_J, int rank_T,
                             int parity);  // construct a lookup table for more
                                           // efficient pandya transformation
  //   std::map<std::array<size_t,2>,std::array<std::vector<size_t>,2>>&
  //   GetPandyaLookup(int rank_J, int rank_T, int parity);
  std::vector<size_t>& GetPandyaLookup(int rank_J, int rank_T, int parity);
  uint64_t SixJHash(double j1, double j2, double j3, double J1, double J2,
                    double J3);
  void SixJUnHash(uint64_t key, uint64_t& j1, uint64_t& j2, uint64_t& j3,
                  uint64_t& J1, uint64_t& J2, uint64_t& J3);

  uint64_t NineJHash(double j1, double j2, double J12, double j3, double j4,
                     double J34, double J13, double J24, double J);

  uint64_t MoshinskyHash(uint64_t N, uint64_t Lam, uint64_t n, uint64_t lam,
                         uint64_t n1, uint64_t l1, uint64_t n2, uint64_t l2,
                         uint64_t L);
  void MoshinskyUnHash(uint64_t key, uint64_t& N, uint64_t& Lam, uint64_t& n,
                       uint64_t& lam, uint64_t& n1, uint64_t& l1, uint64_t& n2,
                       uint64_t& l2, uint64_t& L);
};
