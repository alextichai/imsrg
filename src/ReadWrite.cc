...
void ReadWrite::Write_me2j_np( std::string outfilename, Operator& Hbare, int emax, int Emax, int lmax)
{
  std::ofstream outfile(outfilename);
  if ( !outfile.good() )
  {
  ...
  time_t time_now = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
//  outfile << "    generated by IMSRG code on " << ctime(&time_now)<< std::endl;
  outfile << "   me2j_np-f1 -- generated by IMSRG code on " << ctime(&time_now);
  int icount = 0;
    ...
    for(int nlj2=0; nlj2<=nlj1; ++nlj2)
    {
      ...
      for(int nlj3=0; nlj3<=nlj1; ++nlj3)
      {
        ...
        for(int nlj4=0; nlj4<=(nlj3==nlj1 ? nlj2 : nlj3); ++nlj4)
        {
          ...
          for (int J=Jmin; J<=Jmax; ++J)
          {
             // me2j_np format is unnormalized

             double tbme_npnp = Hbare.TwoBody.GetTBME(J,parity,0,a+1,b,c+1,d);        // unnormalized
             double tbme_pnpn = Hbare.TwoBody.GetTBME(J,parity,0,a,b+1,c,d+1);        // unnormalized
             double tbme_nnnn = Hbare.TwoBody.GetTBME(J,parity,1,a+1,b+1,c+1,d+1); // unnormalized
             double tbme_pnnp = Hbare.TwoBody.GetTBME(J,parity,0,a,b+1,c+1,d);        // unnormalized
             double tbme_nppn = Hbare.TwoBody.GetTBME(J,parity,0,a+1,b,c,d+1);        // unnormalized
             double tbme_pppp = Hbare.TwoBody.GetTBME(J,parity,-1,a,b,c,d); // unnormalized

              // ME order: npnp, pnpn, nnnn, pnnp, nppn, pppp
             std::vector<double> tbmes = {
               tbme_npnp,
               tbme_pnpn,
               tbme_nnnn,
               tbme_pnnp,
...
void ReadWrite::Write_nat_occs_to_me1j(std::string outfilename, Operator &Hbare, int emax,
                           int lmax) {
  std::ofstream outfile(outfilename);
  if (!outfile.good()) {
    std::cerr << "************************************" << std::endl
  ...
  outfile << std::setprecision(12) << std::setw(14) << 0.0 << "\n";
  int icount = 0;
...
void ReadWrite::Write_ref_occs_to_me1j(std::string outfilename, Operator &Hbare, int emax,
                           int lmax) {
  std::ofstream outfile(outfilename);
  if (!outfile.good()) {
    std::cerr << "************************************" << std::endl
