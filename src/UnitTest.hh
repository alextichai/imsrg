...
class UnitTest {
 ...
 public:
  ...
  Operator RandomOp(ModelSpace& modelspace, int jrank, int tz, int parity,
                    int particle_rank, int hermitian);

  Operator RandomDaggerOp(ModelSpace& modelspace, index_t Q);

  double GetMschemeMatrixElement_1b(const Operator& Op, int a, int ma, int b,
                                    int mb);
  double GetMschemeMatrixElement_2b(const Operator& Op, int a, int ma, int b,
                                    int mb, int c, int mc, int d, int md);
  double GetMschemeMatrixElement_3b(const Operator& Op, int a, int ma, int b,
                                    int mb, int c, int mc, int d, int md, int e,
                                    int me, int f, int mf);

  double GetMschemeMatrixElement_1leg(const Operator& Op, int a, int ma);
  double GetMschemeMatrixElement_3leg(const Operator& Op, int a, int ma, int b,
                                      int mb, int c, int mc);
  ...
  // test strategy: Fill two random operators, calculate a specific commutator
  // term using the J-coupled expression, and in m-scheme (where the formula is
  // simpler) and make sure that they give the same answer
  ...
  bool Test_comm332_pphhss(const Operator& X,
                           const Operator& Y);  // test in progress
  ...
  bool TestRPAEffectiveCharge(const Operator& H, const Operator& OpIn, size_t k,
                              size_t l);
