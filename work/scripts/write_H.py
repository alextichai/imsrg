  ...
  for reference in ['%s%d'%(ELEM[Z],A)]:
    ...
    for e in [10]:
      for hw in [16]:
        ...
        #ARGS['omefile'] = omega_dir + '/%s_hw%s_eMax%02d_E3Max%s_s%s' % (intlabel, ARGS['hw'], int(ARGS['emax']), ARGS['e3max'], ARGS['smax'])
