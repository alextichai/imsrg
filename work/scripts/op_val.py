...
##  operator.py
##########################################################################
##  Evaluate the expectation values of a list of operators at the HF and 
##  IMSRG level starting from the .me1j and .me2jp HF Hamiltonian and a
##  list of Magnus operators previously stored (using write_H.py).
##  Andrea, February 9th 2025
##########################################################################
  ...
  for reference in ['%s%d'%(ELEM[Z],A)]:
    ...
    for e in [10]:
      for hw in [16]:
        ...
        jobname  = '%s_opval_%s_e%s_E%s_s%s_hw%s' %(ARGS['valence_space'],intlabel,ARGS['emax'],ARGS['e3max'],ARGS['smax'],ARGS['hw'])
